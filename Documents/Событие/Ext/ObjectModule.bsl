...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ)
	...
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Помещение) и ВыборкаПоШапкеДокумента.ПроверяемоеЗначение <> "Можно" Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Помещение на указанный период времени ранее уже зарезервировано!';uk='Приміщення на зазначений період часу раніше вже зарезервовано!'"), Отказ);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.НачалоСобытия = ВыборкаПоШапкеДокумента.ОкончаниеСобытия
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не задана продолжительность встречи!';uk='Не задана тривалість зустрічі!'"), Отказ);
	ИначеЕсли ВыборкаПоШапкеДокумента.НачалоСобытия > ВыборкаПоШапкеДокумента.ОкончаниеСобытия Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Начало встречи не может быть позже её окончания!';uk='Початок зустрічі не може бути пізніше її закінчення!'"), Отказ);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДокОснование)
	...
	ИначеЕсли ТипЗнч(ДокОснование) = Тип("ДокументСсылка.РезервированиеТоваров") Тогда
		Если ДокОснование.Заказ <> Неопределено И НЕ ДокОснование.Заказ.Пустая()
		   И ТипЗнч(ДокОснование.Заказ) <> Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
			ВидОбъекта     = Перечисления.ВидыОбъектовСобытия.Контрагент;
	...
	ИначеЕсли ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ЭлектронноеПисьмо") Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Иначе
				...
				ВыбранныйЭлемент = СписокВыбора.ВыбратьЭлемент(НСтр("ru='Выберите объект события';uk=""Виберіть об'єкт події"""));
