...
Функция ПолучитьСоответствиеСчетов()
	...
	// остатки выбираем всегда, а закрываем по опции, либо НУ, либо НУ и БУ
	...
	Для каждого СчетФинРез Из МассивСчетовФинРез Цикл
		...
		Для каждого ВидСубконтоСтрока Из СчетФинРез.ВидыСубконто Цикл
			...
			Если НЕ ВидСубконтоСтрока.ТолькоОбороты Тогда
				...
				ОбщегоНазначения.ОшибкаПриПроведении(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На счетах финансовых результатов допускается аналитический учет только по оборотам (проверьте счет %1, вид субконто ""%2"")!';uk='На рахунках фінансових результатів допускається аналітичний облік тільки по оборотах (перевірте рахунок %1, вид субконто ""%2"")!'"), СчетФинРез, ВидСубконтоСтрока.ВидСубконто), Отказ, Заголовок);
...
Процедура ЗакрытиеДоходовИРасходов(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Пока ВыборкаОстатков.Следующий() Цикл
		...
		Если СчетФинансовыхРезультатов = Неопределено Тогда
			...
			ОбщегоНазначения.СообщитьИнформацию(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для счета %1 не определен счет финансовых результатов! Остаток на счете не закрыт.';uk='Для рахунку %1 не визначений рахунок фінансових результатів! Залишок на рахунку не закритий.'"), ВыборкаОстатков.КодСчета), Заголовок, СтатусСообщения.Важное);
			Продолжить;
		...
		Если ВыборкаОстатков.Счет = ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходы ИЛИ ВыборкаОстатков.Счет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходы) Тогда 
			...
			ЗакрыватьНУ = СтруктураШапкиДокумента.Дата < глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ");
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ)
	...
	УчетнаяПолитика = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитики(СтруктураШапкиДокумента.Дата, Отказ, СтруктураШапкиДокумента.Организация, "Бух");
