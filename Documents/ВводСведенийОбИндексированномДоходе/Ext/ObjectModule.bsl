...
Процедура ПроверитьЗаполнениеСтрокиРаботникиОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаДействия) Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата действия индексации!';uk='не зазначена дата дії індексації!'"), Отказ);
	КонецЕсли;
	
	// Индексация
	Если ВыборкаПоСтрокамДокумента.Индексация = 0 Тогда
		ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана сумма индексации!';uk='не зазначена сума індексації!'"), Отказ);
	КонецЕсли;
