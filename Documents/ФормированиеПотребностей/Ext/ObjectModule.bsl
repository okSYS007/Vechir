...
Функция ВыполнитьВыборкуДанных() Экспорт
	...
	Для каждого Строка из ПланыПродаж Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.Сценарий.Периодичность) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для сценария ""';uk='Для сценарію ""'") + СокрЛП(Строка.Сценарий.Наименование) + НСтр("ru='"" не указана периодичность.';uk='"" не зазначена періодичність.'"));
			Продолжить;
