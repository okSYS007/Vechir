...
Процедура ПроверитьЗаполнениеРеквизитовДокумента(ТаблицаДиапазонов, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ТаблицаДиапазонов.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнена шкала диапазонов.';uk='Не заповнена шкала діапазонів.'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Для Тмп = 0 По КоличествоСтрок Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ';uk='У рядку номер '") + СокрЛП(ТекущаяСтрокаТЗ.НомерСтроки);

		Если Тмп > 0 Тогда
			Если ТекущаяСтрокаТЗ.ВерхняяГраница = ПредыдущаяСтрокаТЗ.ВерхняяГраница Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru=' продублирована верхняя граница.';uk=' продубльована верхня межа.'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
