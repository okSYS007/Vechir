...
Процедура ПроверитьСоответствиеПериодичностиИПериода(Отказ, Заголовок)
	...
	Если ЗначениеЗаполнено(Периодичность) И ДатаОкончания <> '00010101' Тогда
		...
		Если РасчетнаяДатаКонца > КонецДня(ДатаОкончания) Тогда
			Отказ = Истина;
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Некорректная дата окончания, не будет закончен хотя бы один период с указанной периодичностью!';uk='Некоректна дата закінчення, не буде закінчений хоча б один період з вказаною періодичністю!'"), Отказ, Заголовок);
		КонецЕсли;
...
Процедура ПроверитьПересекающиесяДокументы(Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗапроса Цикл
		...
		Если НЕ ЗначениеЗаполнено(ДатаОкончания) Тогда // Без даты окончания
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДатаОкончания) Тогда
				...
				КонецЕсли; 
				СтрокаДокументовПересечений = СтрокаДокументовПересечений + Строка(СтрокаТаблицы.Регистратор) + НСтр("ru=' с ';uk=' з '") + Формат(СтрокаТаблицы.ДатаНачала, "ДФ=dd.MM.yyyy") + НСтр("ru=' по ';uk='до '") + ?(НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДатаОкончания), "...", Формат(СтрокаТаблицы.ДатаОкончания, "ДФ=dd.MM.yyyy"));
	...
	Если НЕ ПустаяСтрока(СтрокаДокументовПересечений) Тогда
		ОбщегоНазначения.СообщитьОбОшибке((НСтр("ru='Найдены условия поставок, которые уже действуют в выбранном периоде:';uk='Знайдені умови поставок, які вже діють в обраному періоді:'") + Символы.ПС + СтрокаДокументовПересечений), Отказ, Заголовок);
		Отказ = Истина;
	КонецЕсли; 
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДоговорКонтрагента) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не выбран договор контрагента в документе!';uk='Не вибраний договір контрагента в документі!'"), Отказ, Заголовок);
	Иначе
		Если ВыборкаПоШапкеДокумента.ВидУсловийДоговора <> Перечисления.ВидыУсловийДоговоровВзаиморасчетов.СДополнительнымиУсловиями Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В договоре взаиморасчетов не указан признак ведения договора с дополнительными условиями поставок !';uk='У договорі взаєморозрахунків не зазначена ознака ведення договору з додатковими умовами поставок !'"), Отказ, Заголовок);
		КонецЕсли; 
		Если ВыборкаПоШапкеДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.ПустаяСсылка()
		 ИЛИ ВыборкаПоШапкеДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.Прочее Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Вид договора взаиморасчетов может быть только ""С поставщиком"" или ""С покупателем"" !';uk='Вид договору взаєморозрахунків може бути тільки ""З постачальником"" або ""З покупцем"" !'"), Отказ, Заголовок);
		КонецЕсли; 
	КонецЕсли;
	
	// Контрагент
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Контрагент) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не выбран контрагент в документе!';uk='Не вибраний контрагент у документі!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// Дата начала
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаНачала) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана дата начала в документе!';uk='Не вказана дата початку в документі!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Периодичность) И НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончания) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана периодичность или дата окончания в документе!';uk='Не зазначена періодичність або дата закінчення в документі!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончания) И ВыборкаПоШапкеДокумента.ДатаОкончания < ВыборкаПоШапкеДокумента.ДатаНачала Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Дата окончания не может быть меньше даты начала!';uk='Дата закінчення не може бути менша дати початку!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// Периодичность
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.СуммаУсловийДоговора) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана сумма условий по договору в документе!';uk='Не зазначена сума умов по договору у документі!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНоменклатуры(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ""Товары"": ';uk='"" табл. частини ""Товари"": '");
	// Номенклатура
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Номенклатура) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана номенклатура!';uk='не обрана номенклатура!'"), Отказ, Заголовок);
	КонецЕсли;

	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Номенклатура) И ВыборкаПоСтрокамДокумента.Номенклатура.Набор Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='выбранная номенклатура не должна быть набором-пакетом!';uk='обрана номенклатура не повинна бути набором-пакетом!'"), Отказ, Заголовок);
	КонецЕсли;

	// Валюта
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВалютаЦены) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана валюта цены!';uk='не обрана валюта ціни!'"), Отказ, Заголовок);
	КонецЕсли;

	// ЕдиницаИзмерения
	Если НЕ ВыборкаПоСтрокамДокумента.Услуга И НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ЕдиницаИзмерения) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана единица!';uk='не обрана одиниця!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// Цена
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Цена) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана цена!';uk='не обрана ціна!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьДублиНоменклатуры(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.КоличествоДублей > 1 Тогда
		ОбщегоНазначения.СообщитьОбОшибке("""" + СокрЛП(ВыборкаПоСтрокамДокумента.Номенклатура) + ", " + СокрЛП(ВыборкаПоСтрокамДокумента.ХарактеристикаНоменклатуры) + НСтр("ru='"""
": дублей строк номенклатуры в этом документе быть не должно!';uk='"""
": дублів рядків номенклатури в цьому документі бути не повинно!'"), Отказ, Заголовок);
	КонецЕсли; 
...
Процедура ПроверитьСуммыТабличнойЧастиИШапки(Отказ, Заголовок)
	...
	Если СуммаПоТЧ > СуммаУсловийДоговора Тогда
		Отказ = Истина;
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Сумма по всем номенклатурным позициям табличной части, не может быть больше суммы условий по договору!';uk='Сума по всім номенклатурним позиціям табличної частини, не може бути більше суми умов за договором!'"), Отказ, Заголовок);
	КонецЕсли; 
