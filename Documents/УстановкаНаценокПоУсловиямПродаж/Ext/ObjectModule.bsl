...
Процедура ПроверитьДубли(ТаблицаПроверкиДублей, СтруктураПолей, Отказ, Заголовок)
	...
	Для Каждого СтрокаДублей Из ТаблицаПроверкиДублей Цикл
		Если СтрокаДублей.КоличествоДублей > 1 Тогда
			СтрокаСообщения = НСтр("ru='Дублируется строка: ';uk='Дублюється рядок: '");
...
Процедура ПроверитьПересекающиесяДокументы(Отказ, Заголовок)
	...
	Если НЕ ПустаяСтрока(СтрокаДокументовПересечений) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Найдены записи о наценках для групп по условиям продаж, которые уже действуют:';uk='Знайдені записи про націнки для груп за умовами продажів, які вже діють:'") + СтрокаДокументовПересечений, Отказ, Заголовок);
		Отказ = Истина;
	КонецЕсли;
