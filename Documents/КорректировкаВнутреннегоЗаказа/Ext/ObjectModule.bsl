...
Функция ПечатьДокумента()
	...
	Если НЕ ЗначениеЗаполнено(ВнутреннийЗаказ) Тогда
		Сообщить(НСтр("ru='Не указан внутренний заказ';uk='Не вказано внутрішнє замовлення'"));
		Возврат Неопределено;
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(ВнутреннийЗаказ.Организация) Тогда
		Сообщить(НСтр("ru='В документе основании не указана организация';uk='У документі підставі не вказана організація'"));
		Возврат Неопределено;
	КонецЕсли;
	...
	ОбластьМакета.Параметры.ТекстЗаголовка      = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Корректировка внутреннего заказа';uk='Коригування внутрішнього замовлення'",КодЯзыкаПечать), КодЯзыкаПечать);
	...
	ОбластьМакета.Параметры.Заполнить(Шапка);
	НазваниеДокумента = Локализация.ПолучитьЛокализованноеПредставление(Шапка.ВнутреннийЗаказ,КодЯзыкаПечать);
	ОбластьМакета.Параметры.ПредставлениеЗаказа = НазваниеДокумента;
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("КорректировкаЗаказа",НСтр("ru='Корректировка внутреннего заказа';uk='Коригування внутрішнього замовлення'"));
...
//
Процедура ЗаполнитьТабличныеЧастиПередПроведениемУпр(Параметры, Отказ) Экспорт
    Если НЕ ЗначениеЗаполнено(ВнутреннийЗаказ) Тогда
		Возврат;
	КонецЕсли; 
	Параметры.Вставить("Заказ",ВнутреннийЗаказ);
    Параметры.Вставить("РезервироватьПоСериям",ложь);
    УправлениеЗаказами.Заказ_ЗаполнитьТабличныеЧастиВозможнымРазмещением(Параметры, Отказ, Товары, ВозвратнаяТара);
