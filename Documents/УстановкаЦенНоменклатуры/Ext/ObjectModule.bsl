...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов = Новый Структура("ПереченьЦен",НСтр("ru='Перечень цен';uk='Перелік цін'"));
...
Функция ПолучитьТабличныеЧастиДляЗаполнения() Экспорт
	...
	ТабличныеЧасти.Добавить("ТаблицаЦен", НСтр("ru='Таблица цен';uk='Таблиця цін'"));
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(СтрокаТаблицы.ИндексСтрокиТаблицыЦен + 1) +
			                    НСтр("ru='"" табличной части для типа цен ""';uk='"" табличної частини для типу цін ""'") 
			                   + СокрЛП(СтрокаТаблицы.ТипЦен) + """ ";

		// Номенклатура.
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не заполнена Номенклатура.';uk='не заповнена Номенклатура.'"), Отказ, Заголовок);
		КонецЕсли;

		// Валюта.
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Валюта) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не заполнена Валюта.';uk='не заповнена Валюта.'"), Отказ, Заголовок);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
		   ...
		   И НЕ ЗначениеЗаполнено(СтрокаТаблицы.ЕдиницаИзмерения) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не заполнена единица измерения.';uk='не заповнена одиниця виміру.'"), Отказ, Заголовок);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СпособРасчетаЦены)
		 ...
		 Или ЗначениеЗаполнено(СтрокаТаблицы.БазовыйТипЦен)) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не заполнен способ расчета цены.';uk='не заповнений спосіб розрахунку ціни.'"), Отказ, Заголовок);
		КонецЕсли;
	КонецЦикла;
