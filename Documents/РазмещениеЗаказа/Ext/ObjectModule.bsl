...
Функция ПечатьРазмещениеЗаказа()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Размещение заказа';uk='Замовлення розміщення'",КодЯзыкаПечать),КодЯзыкаПечать);
	ТабДокумент.Вывести(ОбластьМакета);

	ОбластьМакета = Макет.ПолучитьОбласть("Заказ");
	ОбластьМакета.Параметры.ПредставлениеЗаказа = Локализация.ПолучитьЛокализованноеПредставление(Шапка.Заказ, КодЯзыкаПечать);
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаСтрокТовары.Номенклатура) Тогда
			ОбщегоНазначения.Сообщение(НСтр("ru='В одной из строк не заполнено значение номенклатуры - строка при печати пропущена.';uk='В одному з рядків не заповнене значення номенклатури - рядок під час друку буде пропущений.'"), СтатусСообщения.Важное);
			Продолжить;
		КонецЕсли;
		...
		ОбластьМакета.Параметры.ИсходноеРазмещение 			  = ?(ЗначениеЗаполнено(ВыборкаСтрокТовары.ИсходноеРазмещение), Локализация.ПолучитьЛокализованноеПредставление(ВыборкаСтрокТовары.ИсходноеРазмещение, КодЯзыкаПечать), "");
		ОбластьМакета.Параметры.ИсходноеРазмещениеРасшифровка = ВыборкаСтрокТовары.ИсходноеРазмещение;
		
		ОбластьМакета.Параметры.НовоеРазмещение    		   = ?(ЗначениеЗаполнено(ВыборкаСтрокТовары.НовоеРазмещение), Локализация.ПолучитьЛокализованноеПредставление(ВыборкаСтрокТовары.НовоеРазмещение, КодЯзыкаПечать), "");;
		ОбластьМакета.Параметры.НовоеРазмещениеРасшифровка = ВыборкаСтрокТовары.НовоеРазмещение;
	...
	Пока ВыборкаСтрокТара.Следующий() Цикл
		...
		ОбластьМакета.Параметры.НомерСтроки = Ном;
		ОбластьМакета.Параметры.Товар       = ВыборкаСтрокТара.Товар +  НСтр("ru=' (возвратная тара)';uk=' (зворотна тара)'",КодЯзыкаПечать);
	...
	ОбластьМакета.Параметры.ИтоговаяСтрока =НСтр("ru='Всего наименований ';uk='Всього найменувань '",КодЯзыкаПечать) + (ВыборкаСтрокТовары.Количество()+ВыборкаСтрокТара.Количество());
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов = Новый Структура("РазмещениеЗаказа", НСтр("ru='Размещение заказа';uk='Замовлення розміщення'"));
