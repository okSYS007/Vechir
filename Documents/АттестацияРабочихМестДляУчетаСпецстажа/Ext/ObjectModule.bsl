...
Процедура ПроверитьЗаполнениеСтрокиШтатнойЕдиницы(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Подразделение) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрано подразделение!';uk='не обраний підрозділ!'"), Отказ);
	ИначеЕсли ВыборкаПоСтрокамДокумента.Подразделение.Владелец <> ВыборкаПоШапкеДокумента.Организация Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='указано подразделение другой организации!';uk='зазначений інший підрозділ організації!'"), Отказ);
	КонецЕсли;

	// Должность
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Должность) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана должность!';uk='не обрана посада!'"), Отказ);
	КонецЕсли;
	
	// Вид стажа
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидСтажа) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбран вид стажа!';uk='не вибраний вид стажу!'"), Отказ);
	ИначеЕсли НЕ ВыборкаПоСтрокамДокумента.ВидСтажа.ЛьготныйСтаж   Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='указан не льготный вид стажа!';uk='зазначений не пільговий вид стажу!'"), Отказ);
	КонецЕсли;
	...
	Если  ВыборкаПоСтрокамДокумента.Актуальность Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ЕдиницаВремени) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана единица времени!';uk='не вказана одиниця часу!'"), Отказ);
		КонецЕсли;
		...
		Если (НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ГрафикНормы)) И (ВыборкаПоСтрокамДокумента.ЕдиницаВремени <> Перечисления.ЕдиницыВремениУчетаСпецстажа.КалендарныеДни)  Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан график нормы времени!';uk='не вказаний графік норми часу!'"), Отказ);
		КонецЕсли;
		...
		Если (ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончания)) И (ВыборкаПоСтрокамДокумента.ДатаОкончания <= ВыборкаПоШапкеДокумента.Дата)  Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата окончания меньше даты аттестации!';uk='дата закінчення менше дати атестації!'"), Отказ);
		КонецЕсли;
