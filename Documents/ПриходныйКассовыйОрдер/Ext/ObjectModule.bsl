...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, РежимПроведения, Отказ, Заголовок)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		...
		СуммаВозвратБезналичныхОплат = 0;
		Для каждого ТекСтрока Из ОплатаПлатежнымиКартами Цикл
		
			Если ТекСтрока.Сумма < 0 Тогда 
				СуммаВозвратБезналичныхОплат = СуммаВозвратБезналичныхОплат - ТекСтрока.Сумма;
			КонецЕсли;
		...
		Для Каждого Строка Из ПриемРозничнойВыручки Цикл
			...
			Если НЕ Строка.Возврат Тогда
				...
				Если НЕ ВыручкаСНТТ И СуммаБезналичныхОплат > 0 Тогда
					...
					СуммаПроводки = МАКС(0, Строка.Сумма - СуммаБезналичныхОплат-СуммаВозвратБезналичныхОплат);
				...
				ИначеЕсли НЕ ВыручкаСНТТ И СуммаБезналичныхОплат < 0 Тогда 
					
					СуммаПроводки = МАКС(0, Строка.Сумма - (СуммаВозвратБезналичныхОплат+СуммаБезналичныхОплат));
					СуммаБезналичныхОплат = СуммаБезналичныхОплат + Строка.Сумма;
			...
			Иначе
				...
				Если СуммаВозвратБезналичныхОплат > 0 Тогда
					
					СуммаПроводки = МАКС(0, Строка.Сумма - СуммаВозвратБезналичныхОплат);
					СуммаБезналичныхОплат = СуммаБезналичныхОплат + Мин(Строка.Сумма,СуммаВозвратБезналичныхОплат);
					СуммаВозвратБезналичныхОплат = Макс(0,СуммаВозвратБезналичныхОплат - Строка.Сумма);
...
Процедура ПроверитьЗаполнениеДокументаРегл(СтруктураШапкиДокумента,Отказ, Заголовок)
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка) Тогда
		...
		УправлениеДенежнымиСредствами.ПроверитьЗаполнениеТабличнойЧастиПлатежногоДокумента(ЭтотОбъект, "ПриемРозничнойВыручки", СтруктураПолей, Отказ, Заголовок);
...
"не збігаються сума документа та її розшифровка.'"));
	...
	Если  ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка
		...
		УправлениеДенежнымиСредствами.ПроверитьЗаполнениеТабличнойЧастиПлатежногоДокумента(ЭтотОбъект, "ОплатаПлатежнымиКартами", СтруктураОбязательныхПолейТЧ, Отказ, Заголовок);
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда  
			СтруктураОбязательныхПолейТЧ.Вставить("СчетУчетаРасчетовСКонтрагентом");
		КонецЕсли;
		УправлениеДенежнымиСредствами.ПроверитьЗаполнениеТабличнойЧастиПлатежногоДокумента(ЭтотОбъект, "ОплатаБанковскимиКредитами", СтруктураОбязательныхПолейТЧ, Отказ, Заголовок);
