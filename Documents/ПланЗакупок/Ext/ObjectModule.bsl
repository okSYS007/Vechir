...
Функция ПечатьПланаЗакупок()
	...
	|	ПланЗакупок.ВалютаДокумента.Наименование КАК ВалютаДокумента,
	...
	|	ВЫБОР КОГДА &ВедениеУчетаПоПроектам = Истина ТОГДА
	|		ПланЗакупок.Проект
	|	ИНАЧЕ
	|		NULL
	|	КОНЕЦ КАК Проект,
	|	ВЫБОР КОГДА &ВедениеУчетаПоПроектам = Истина ТОГДА
	|		ПРЕДСТАВЛЕНИЕ(ПланЗакупок.Проект)
	|	ИНАЧЕ
	|		NULL
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='План покупок';uk='План купівель'",КодЯзыкаПечать),КодЯзыкаПечать);
	ТабДокумент.Вывести(ОбластьМакета);

	ОбластьМакета = Макет.ПолучитьОбласть("Шапка");
	ОбластьМакета.Параметры.Заполнить(Шапка);
	
	Период  = Шапка.Периодичность;
	Периоды = Перечисления.Периодичность;
	ДатаНачала = Шапка.ДатаПланирования;
	ДатаОкончания = Шапка.ДатаПланирования;
	УправлениеПланированием.ВыровнятьПериод(ДатаНачала, ДатаОкончания, Период);	
	
	Если Период = Периоды.День тогда
		ПериодЛок = НСтр("ru='День';uk='День'",КодЯзыкаПечать);
	ИначеЕсли Период = Периоды.Неделя тогда
		ПериодЛок = НСтр("ru='Неделя';uk='Тиждень'",КодЯзыкаПечать);
	ИначеЕсли Период = Периоды.Месяц тогда
		ПериодЛок = НСтр("ru='Месяц';uk='Місяць'",КодЯзыкаПечать);
	ИначеЕсли Период = Периоды.Квартал тогда
		ПериодЛок = НСтр("ru='Квартал';uk='Квартал'",КодЯзыкаПечать);
	ИначеЕсли Период = Периоды.Год тогда
		ПериодЛок = НСтр("ru='Год';uk='Рік'",КодЯзыкаПечать);
	КонецЕсли;

	ОбластьМакета.Параметры.ПериодичностьПредставление  = ПериодЛок;
	ОбластьМакета.Параметры.ПериодПланирования = ПредставлениеПериода(ДатаНачала,ДатаОкончания, "ФП=Истина"+";Л="+ Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать));
	ТабДокумент.Вывести(ОбластьМакета);
	...
	Пока ВыборкаСоставПлана.Следующий() Цикл
		...
		Иначе
			ОбластьСтроки.Параметры.ЗаказПредставление = ВыборкаСоставПлана.ЗаказНомер + НСтр("ru=' от ';uk=' від '",КодЯзыкаПечать) + Формат(ВыборкаСоставПлана.ЗаказДата, "ДФ=dd.MM.yyyy");
		КонецЕсли;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Сценарий) Тогда
		Сообщить(НСтр("ru='Не указан сценарий планирования!';uk='Не зазначений сценарій планування!'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ПланЗакупок", НСтр("ru='План закупок';uk='План купівель'"));
...
Функция ПодготовитьТаблицуСоставПлана(РезультатЗапросаПоСоставПлана, ВыборкаПоШапкеДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаСоставПлана Цикл
		...
		Периодичность = ?(ПериодичностьДетализации.Пустая(), ?(Сценарий.Периодичность.Пустая(), Перечисления.Периодичность.День, Сценарий.Периодичность), ПериодичностьДетализации);
		НачалоПериода = СтрокаТаблицы.Период;
		УправлениеПланированием.ВыровнятьДатуПоНачалуПериода(НачалоПериода, Периодичность);
		СтрокаТаблицы.Период = ?((ДатаПланирования - НачалоПериода)/(24*3600) > 0, 
								 ДатаПланирования,
								 НачалоПериода);	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ДатаПланирования <= ГраницаФиксации Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Документ не может быть проведен, поскольку дата планирования находится в закрытом периоде (до ';uk='Документ не може бути проведений, оскільки дата планування знаходиться в закритому періоді (до '") + Формат(ГраницаФиксации, "ДФ=dd.MM.yyyy; ДЛФ=D") + ").", Отказ);
		Возврат;
	...
	// Получим необходимые данные для проведения и проверки заполенения данных по табличной части "СоставПлана".
	...
	РезультатЗапросаПоСоставПлана = ОбщегоНазначения.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "СоставПлана", СтруктураПолей);
