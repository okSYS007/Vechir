...
Процедура ЗаполнитьДокумент() Экспорт
	...
	Если Спецстаж.Количество() = 0 Тогда
		Сообщить(НСтр("ru='Нет информации для заполнения данных о стаже';uk='Немає інформації для заповнення даних про стажі'"));
	КонецЕсли;	
...
Процедура РассчитатьДокумент() Экспорт
	...
	Пока ВыборкаПоНачислениям.Следующий() Цикл
		...
		Если ТребуетсяРасчет И НЕ Отказ Тогда
			...
			Если ВыборкаПоНачислениям.ЕдиницаВремени = Перечисления.ЕдиницыВремениУчетаСпецстажа.Часы Тогда
				Движение.ВидРасчета					= ПланыВидовРасчета.Начисления.СтажВЧасах;
			Иначе	
				Движение.ВидРасчета					= ПланыВидовРасчета.Начисления.СтажВДнях;
			КонецЕсли;
	...
	Если ТребуетсяРасчет Тогда
		...
		ПроведениеРасчетов.РассчитатьЗаписиРегистраРасчета("Начисления", НаборОсновныеНачисления,Спецстаж,МассивИндексыСтрокНачисления,мСотрудники);
	...
	Если ТребуетсяРасчет Тогда
		...
		|		ГДЕ НАЧАЛОПЕРИОДА(ГрафикиРаботыПоВидамВремени.Дата,МЕСЯЦ) = &Месяц
...
Процедура ПроверитьЗаполнениеСтрокиСтажа(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ""Спецстаж"": ';uk='"" табл. частини ""Спецстаж"": '");
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидСтажа) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан вид стажа!';uk='не зазначений вид стажу!'"), Отказ, Заголовок);
	ИначеЕсли НЕ ВыборкаПоСтрокамДокумента.ВидСтажа.ЛьготныйСтаж Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='указан не льготный вид стажа!';uk='зазначений не пільговий вид стажу!'"), Отказ, Заголовок);	
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Приказ) и ПериодРегистрации >= ПроведениеРасчетов.ДатаВводаЕСВ() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан приказ об аттестации рабочего места!';uk='не зазначений наказ про атестацію робочого місця!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// Дата начала
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаНачала) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата начала!';uk='не вказана дата початку!'"), Отказ, Заголовок);
	ИначеЕсли НачалоМесяца(ВыборкаПоСтрокамДокумента.ДатаНачала) <> ВыборкаПоШапкеДокумента.ПериодРегистрации ТОГДА
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата начала не соответствует месяцу документа!';uk='дата початку не відповідає місяцю документа!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// Дата окончания
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончания) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата окончания!';uk='не вказана дата закінчення!'"), Отказ, Заголовок);
	ИначеЕсли НачалоМесяца(ВыборкаПоСтрокамДокумента.ДатаОкончания) <> ВыборкаПоШапкеДокумента.ПериодРегистрации ТОГДА
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата окончания не соответствует месяцу документа!';uk='дата закінчення не відповідає місяцю документа!'"), Отказ, Заголовок);	
	ИначеЕсли ВыборкаПоСтрокамДокумента.ДатаОкончания <= ВыборкаПоСтрокамДокумента.ДатаНачала ТОГДА
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата окончания должна быть больше даты начала!';uk='дата закінчення повинна бути більше дати початку!'"), Отказ, Заголовок);		
	КонецЕсли;
	
	// Единица времени
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ЕдиницаВремени) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана единица времени!';uk='не вказана одиниця часу!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если (ВыборкаПоСтрокамДокумента.ЕдиницаВремени <> Перечисления.ЕдиницыВремениУчетаСпецстажа.КалендарныеДни) И НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ГрафикНормы) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан график нормы времени!';uk='не вказаний графік норми часу!'"), Отказ, Заголовок);
	КонецЕсли;
