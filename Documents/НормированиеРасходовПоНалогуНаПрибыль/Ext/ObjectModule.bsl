...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоСтатьям, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоСтатьям Цикл
		...
		ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаТаблицы.Статья);
		ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СтрокаТаблицы.Статья);
	...
	Если Не Отказ Тогда
		...
		Для Уровень1 = 1 По КоличествоСтрокВТЧ Цикл
			...
			Если Статьи[Уровень1- 1].СуммаОборот < Статьи[Уровень1- 1].СуммаВПределелахНорм Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке ""%1"" сумма в пределах норм превышает сумму зарегистрированных затрат!';uk='У рядку ""%1"" сума у межах норми перевищує суму зареєстрованих витрат!'"), Уровень1), Отказ, Заголовок);		
			...
			Для Уровень2 = Уровень1 + 1 По КоличествоСтрокВТЧ Цикл
				...
				Если  Статьи[Уровень1- 1].Статья = Статьи[Уровень2- 1].Статья 
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строках ""%1"" и  ""%2"" указаны одинаковые статьи и налоговые назначения затрат!';uk='У рядках ""%1"" та  ""%2"" зазначені однакові статті та податкові призначення витрат!'"), Уровень1, Уровень2), Отказ, Заголовок);	
