	...
	Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
		Если ТипЗнч(СтрокаТЧ.НовоеРазмещение) = Тип("СправочникСсылка.Склады") Тогда
			Если СтрокаТЧ.НовоеРазмещение.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В строке № ';uk='У рядку № '") + СтрокаТЧ.НомерСтроки + НСтр("ru=' табличной части ""Товары"" выбрана НТТ. ';uk=' табличної частини ""Товари"" обрана НТТ. '") + Символы.ПС + НСтр("ru='Нельзя выбирать НТТ.';uk='Не можна вибирати НТТ.'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
		Если ТипЗнч(СтрокаТЧ.НовоеРазмещение) = Тип("СправочникСсылка.Склады")
			...
			Если СтрокаТЧ.НовоеРазмещение = Заказ.Заказчик Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В строке № ';uk='У рядку № '") + СтрокаТЧ.НомерСтроки + НСтр("ru=' табличной части ""Товары"" выбран склад, совпадающий со складом-заказчиком внутреннего заказа. ';uk=' табличної частини ""Товари"" обраний склад, що збігається зі складом-замовником внутрішнього замовлення. '") + Символы.ПС + НСтр("ru='Нельзя зарезервировать товар на складе-заказчике.';uk='Не можна зарезервувати товар на складі-замовнику.'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
