...
Функция ПечатьРКО()
	...
	|
	|ГДЕ
	|	УпрРасходныйКассовыйОрдер.Ссылка = &ТекущийДокумент";

	Шапка = Запрос.Выполнить().Выбрать();
	Шапка.Следующий();

	ТабДокумент   = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_УпрРасходныйКассовыйОрдер_КО2";
...
Процедура УстановитьНомерОрдера()
	//ДО_Привелигированный.УстановитьНомерОрдера(ЭтотОбъект,ЕстьАналитикаПоОбособленнымПодразделениям);
...
Если ВидОперации <> Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику 
	И ВидОперации <> Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
// к ЛИВ	
		...
		ЕстьРасчетыСКонтрагентами=УпрУправлениеДенежнымиСредствами.ЕстьРасчетыСКонтрагентами(ВидОперации);
		ЕстьРасчетыПоКредитам=УпрУправлениеДенежнымиСредствами.ЕстьРасчетыПоКредитам(ВидОперации);
		РасчетыВозврат = УпрУправлениеДенежнымиСредствами.НаправленияДвиженияДляДокументаДвиженияДенежныхСредствУпр(ВидОперации);
		...
		ТаблицаПлатежейУпр=УпрУправлениеДенежнымиСредствами.ПолучитьТаблицуПлатежейУпр(Дата,ВалютаДокумента,Ссылка, "УпрРасходныйКассовыйОрдер");
		...
		//+Никитин
		Если ПередатьВКассу  Тогда 
			Если не ЗначениеЗаполнено(КассаПолучатель) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбрана касса получатель",,,,Отказ);
			иначе
				Мас = Новый Массив;
				мас.Добавить(Ссылка);
				ДО_Привелигированный.СформиороватьПКО_из_РКО(мас,Отказ);
				//СформиороватьПКО_из_РКО(мас,Отказ);
		...
		Для Каждого Запись из Движения.Хозрасчетный Цикл 
					...
					Если Запись.СчетКт.Код = "301" Тогда 
						СтруктураЗаполнения = ПолучитьСтатьюИСчетДоходовЗатратДляСтатьиДДС(Запись.СубконтоКт[Запись.СчетКт.ВидыСубконто[0].ВидСубконто])
					ИначеЕсли Запись.СчетКт.Код = "311" Тогда 
						СтруктураЗаполнения = ПолучитьСтатьюИСчетДоходовЗатратДляСтатьиДДС(Запись.СубконтоКт[Запись.СчетКт.ВидыСубконто[1].ВидСубконто])
					Иначе
						Продолжить;	
					КонецЕсли;
					...
					КонецЕсли;
						...
						Если Запись.СчетДт.ВидыСубконто.Количество()>0 Тогда
							Запись.СубконтоДт.Удалить(Запись.СчетДт.ВидыСубконто[0].ВидСубконто);
						КонецЕсли;
						
						Если  Запись.СчетДт.ВидыСубконто.Количество()>1 Тогда
							Запись.СубконтоДт.Удалить(Запись.СчетДт.ВидыСубконто[1].ВидСубконто);
						КонецЕсли;
						
						Если  Запись.СчетДт.ВидыСубконто.Количество()>2 Тогда
							Запись.СубконтоДт.Удалить(Запись.СчетДт.ВидыСубконто[2].ВидСубконто);
						КонецЕсли;
						
						Если  Запись.СчетДт.ВидыСубконто.Количество()>3 Тогда
							Запись.СубконтоДт.Удалить(Запись.СчетДт.ВидыСубконто[3].ВидСубконто);
						КонецЕсли;
						
						Если  Запись.СчетДт.ВидыСубконто.Количество()>4 Тогда
							Запись.СубконтоДт.Удалить(Запись.СчетДт.ВидыСубконто[4].ВидСубконто);
						КонецЕсли;
						
						Запись.СчетДт = СтруктураЗаполнения.Счет;

						Если Запись.СчетДт.ВидыСубконто.Количество()>0 Тогда
							Если Запись.СчетДт.ВидыСубконто[0].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[0].ВидСубконто] = СтруктураЗаполнения.СтатьяДР;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[0].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.вчПодразделения Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[0].ВидСубконто] = Запись.Подразделение;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[0].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Проекты Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[0].ВидСубконто] = Запись.Проект;
							КонецЕсли;
						КонецЕсли;
						
						Если Запись.СчетДт.ВидыСубконто.Количество()>1 Тогда
							Если Запись.СчетДт.ВидыСубконто[1].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[1].ВидСубконто] = СтруктураЗаполнения.СтатьяДР;
							КонецЕсли;	
							Если Запись.СчетДт.ВидыСубконто[1].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.вчПодразделения Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[1].ВидСубконто] = Запись.Подразделение;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[1].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Проекты Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[1].ВидСубконто] = Запись.Проект;
							КонецЕсли;
						КонецЕсли;
						
						Если Запись.СчетДт.ВидыСубконто.Количество()>2 Тогда
							Если Запись.СчетДт.ВидыСубконто[2].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[2].ВидСубконто] = СтруктураЗаполнения.СтатьяДР;
							КонецЕсли;	
							Если Запись.СчетДт.ВидыСубконто[2].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.вчПодразделения Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[2].ВидСубконто] = Запись.Подразделение;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[2].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Проекты Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[2].ВидСубконто] = Запись.Проект;
							КонецЕсли;
						КонецЕсли;
						
						Если Запись.СчетДт.ВидыСубконто.Количество()>3 Тогда
							Если Запись.СчетДт.ВидыСубконто[3].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[3].ВидСубконто] = СтруктураЗаполнения.СтатьяДР;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[3].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.вчПодразделения Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[3].ВидСубконто] = Запись.Подразделение;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[3].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Проекты Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[3].ВидСубконто] = Запись.Проект;
							КонецЕсли;
						КонецЕсли;
						
						Если  Запись.СчетДт.ВидыСубконто.Количество()>4 Тогда
							Если Запись.СчетДт.ВидыСубконто[4].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[4].ВидСубконто] = СтруктураЗаполнения.СтатьяДР;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[4].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.вчПодразделения Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[4].ВидСубконто] = Запись.Подразделение;
							КонецЕсли;
							Если Запись.СчетДт.ВидыСубконто[4].ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Проекты Тогда 
								Запись.СубконтоДт[Запись.СчетДт.ВидыСубконто[4].ВидСубконто] = Запись.Проект;
							КонецЕсли;
						КонецЕсли;
...
// н ЛИВ
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
		...
		Если Не ЗначениеЗаполнено(Подразделение) И Не ЗначениеЗаполнено(ВедомостьКВыплате) Тогда
			Отказ = Истина;
			Сообщить("Не заповнено підрозділ");
			Возврат;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Сотрудник) Тогда
			...
			Если Не ЗначениеЗаполнено(ВедомостьКВыплате) Тогда
				Запись.Подразделение    	= Подразделение;
			Иначе
				Запись.Подразделение    	= ВедомостьКВыплате.Подразделение;
			КонецЕсли;
			Запись.СуммаВзаиморасчетов  = СуммаДокумента;
			Запись.Ведомость  			= ВедомостьКВыплате;
			Запись.КодОперации 			= Перечисления.КодыОперацийВзаиморасчетыСРаботникамиОрганизаций.Выплата;
...
// бух		
		Если Не Отказ Тогда
			...
			Проводка.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("6611");
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.КассаВНациональнойВалюте;
			ОбособленноеПодразделение = Касса.ОбособленноеПодразделение;
			СотрудникСсылка = Справочники.СотрудникиОрганизаций.ПустаяСсылка();
			//Если Сотрудник.СотрудникиОрганизаций.Количество() > 0 Тогда 
			//	СотрудникСсылка = Сотрудник.СотрудникиОрганизаций[0].Сотрудник;
			//КонецЕсли;
			//Проект = Справочники.Проекты.ПустаяСсылка();
			//Если РасшифровкаПлатежа.Количество() > 0 Тогда 
			//	Проект = РасшифровкаПлатежа[0].Проект;
			//КонецЕсли;
			
			//ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, Справочники.СотрудникиОрганизаций.ПустаяСсылка());
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, Подразделение);
			//ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, Подразделение.Проект);
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 4, ПериодРегистрации);
			
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"СтатьиДвиженияДенежныхСредств", СтатьяДвиженияДенежныхСредств);
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ОбособленныеПодразделенияБезОбразованияЮрЛица", ОбособленноеПодразделение);
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"вчПодразделения", Подразделение);
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Проекты", Подразделение.Проект);
...
// бух		
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		Если ВыплатаЗаработнойПлаты.Количество() > 0 Тогда
			...
			Для Каждого стр из ВыплатаЗаработнойПлаты Цикл
				масПодразделения.Добавить(стр.ВедомостьКВыплате.Подразделение);	
			КонецЦикла;
			...
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
			               |	СУММА(ВзаиморасчетыСРаботникамиУправленческийОстатки.СуммаВзаиморасчетовОстаток) КАК Остаток,
			               |	ВзаиморасчетыСРаботникамиУправленческийОстатки.Сотрудник,
			               |	ВзаиморасчетыСРаботникамиУправленческийОстатки.Подразделение
			               |ИЗ
			               |	РегистрНакопления.ВзаиморасчетыСРаботникамиУправленческий.Остатки(
			               |			,
			               |			ПериодВзаиморасчетов = &ПериодВзаиморасчетов
			               |				И Подразделение В (&масПодразделения)) КАК ВзаиморасчетыСРаботникамиУправленческийОстатки
			               |
			               |СГРУППИРОВАТЬ ПО
			               |	ВзаиморасчетыСРаботникамиУправленческийОстатки.Сотрудник,
			               |	ВзаиморасчетыСРаботникамиУправленческийОстатки.Подразделение";
			Запрос.УстановитьПараметр("ПериодВзаиморасчетов", ПериодРегистрации);
			Запрос.УстановитьПараметр("масПодразделения", масПодразделения);
			...
			Пока Выборка.Следующий() Цикл 
				...
				Запись.СуммаВзаиморасчетов  = Выборка.Остаток;
				Запись.Ведомость  			= ВедомостьКВыплате;
				Запись.КодОперации 			= Перечисления.КодыОперацийВзаиморасчетыСРаботникамиОрганизаций.Выплата;
				Запись.ВидДвижения			= ВидДвиженияНакопления.Расход;
				НаборЗаписей.Записать();	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее и ПередатьВКассу Тогда 
		Контрагент = КассаПолучатель;
	КонецЕсли;	
...
Процедура ЗаполнитьПоЗаявкеНаРасходованиеСредствУпр(Основание) 
	...
	УпрУправлениеДенежнымиСредствами.ОпределитьОперациюПоОснованиюУпр(ВидОперации,ВидОперацииЗаявка);
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		Если Не Отказ Тогда
			...
			Если Платеж.ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам Тогда
				...
				ТекстСделка=?(УпрУправлениеДенежнымиСредствами.ОпределитьПараметрыВыбораСделки(ВидОперации).ТипЗаказа="ЗаказПокупателя",НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"),НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"));
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, РежимПроведения, Отказ, Заголовок)
	...
	Если ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
		КассаВВалюте = Истина;
		Если мУчетДенежныхСредствПоОбособленнымПодразделениям Тогда
			УпрУправлениеДенежнымиСредствами.ПроверитьОстатокДенежныхСредств(СтруктураШапкиДокумента.СуммаДокумента, Организация, ВалютаДокумента, МоментДокумента, СтруктураШапкиДокумента.СчетКасса, СтруктураШапкиДокумента.Касса.ОбособленноеПодразделение, Заголовок);
		Иначе
			УпрУправлениеДенежнымиСредствами.ПроверитьОстатокДенежныхСредств(СтруктураШапкиДокумента.СуммаДокумента, Организация, ВалютаДокумента, МоментДокумента, СтруктураШапкиДокумента.СчетКасса, , Заголовок);
		КонецЕсли;
	Иначе
		Если мУчетДенежныхСредствПоОбособленнымПодразделениям Тогда
			УпрУправлениеДенежнымиСредствами.ПроверитьОстатокДенежныхСредств(СтруктураШапкиДокумента.СуммаДокумента, Организация, , МоментДокумента, СтруктураШапкиДокумента.СчетКасса,СтруктураШапкиДокумента.Касса.ОбособленноеПодразделение, Заголовок);
		Иначе
			УпрУправлениеДенежнымиСредствами.ПроверитьОстатокДенежныхСредств(СтруктураШапкиДокумента.СуммаДокумента, Организация, , МоментДокумента, СтруктураШапкиДокумента.СчетКасса, ,Заголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРКО.РасходДенежныхСредствПрочее
			...
			Если  ПередатьВКассу Тогда 
				...
				Если КассаПолучатель.ВалютаДенежныхСредств = Константы.ВалютаРегламентированногоУчета.Получить() Тогда 
					СчетУчетаРасчетовСКонтрагентом  = ПланыСчетов.Хозрасчетный.НайтиПоКоду("333");
					//СубконтоДт1Врем = СтатьяДвиженияДенежныхСредств; 
...
Процедура ПроверитьЗаполнениеДокументаУпр(Отказ, Режим, Заголовок)
	...
	Если (ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам) Тогда
		...
		Если Не Отказ Тогда
			УпрУправлениеДенежнымиСредствами.КонтрольОстатковПоТЧ(Дата, ТаблицаПлатежейУпр, Отказ, Заголовок,РасчетыВозврат);
		КонецЕсли;
	...
	Если ОтражатьВУправленческомУчете 
		...
		Если Не Оплачено Тогда
			...
			ЕстьРазрешение=УпрУправлениеДенежнымиСредствами.ЕстьРазрешениеПроводитьБезЗаявки();
			
			УпрУправлениеДенежнымиСредствами.ПроверитьОстаткиПоЗаявке(Дата,Отказ,Заголовок,
			...
			СвободныйОстаток = УпрУправлениеДенежнымиСредствами.ПолучитьСвободныйОстатокДС(Касса, Дата, ТаблицаПлатежейУпр.ВыгрузитьКолонку("ДокументПланированияПлатежа"));
...
"Можливий до використання залишок: '")+Формат(СвободныйОстаток,"ЧЦ=15; ЧДЦ=2")+" "+ВалютаДокумента+"
				...
				Если НЕ УпрУправлениеДенежнымиСредствами.ЕстьРазрешениеПревышатьСвободныйОстатокДС() Тогда
					Отказ = Истина;
				КонецЕсли;
...
Процедура ПроверитьЗаполнениеДокументаРегл(СтруктураШапкиДокумента,Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		|ИЗ
		|	Документ.УпрРасходныйКассовыйОрдер.ВыплатаЗаработнойПлаты КАК УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗарплатаКВыплатеОрганизаций.РаботникиОрганизации КАК ЗарплатаКВыплатеОрганизаций
		|		ПО УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ведомость = ЗарплатаКВыплатеОрганизаций.Ссылка
		|ГДЕ
		|	ЗарплатаКВыплатеОрганизаций.ВыплаченностьЗарплаты = &НеВыплачено
		|	И УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ссылка = &Ссылка";
		...
		|		ПО УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ведомость = ЗарплатаКВыплате.Ссылка
		|		И УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.СчетУчета = ЗарплатаКВыплате.СчетУчета
		|
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗарплатаКВыплатеОрганизаций.РаботникиОрганизации КАК РаботникиОрганизации
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОВыплатахРаботникамОрганизацийПоПлатежнымВедомостям КАК РанееВыплаченныеСуммы
		|		ПО УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ведомость = РанееВыплаченныеСуммы.Ведомость
		|			И ЗарплатаКВыплате.Сотрудник = РанееВыплаченныеСуммы.Сотрудник
		|			И РанееВыплаченныеСуммы.Регистратор <> &Ссылка
		|ГДЕ
		|	УпрРасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ссылка = &Ссылка
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
		...
		Запрос.УстановитьПараметр("парамПустойРКО", Документы.УпрРасходныйКассовыйОрдер.ПустаяСсылка());	
...
Процедура ПриЗаписи(Отказ)
	//Если ПередатьВКассу  Тогда 
	//	ДО_Привелигированный.СформиороватьПКО_из_РКО(ссылка);
	//КонецЕсли;	
КонецПроцедуры
...
Функция ПолучитьСтатьюИСчетДоходовЗатратДляСтатьиДДС(СтатьяДДС)
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	СоответствиеСтатейДДССтатьямЗатратИДоходов.Счет,
	               |	СоответствиеСтатейДДССтатьямЗатратИДоходов.СтатьяДР
	               |ИЗ
	               |	РегистрСведений.СоответствиеСтатейДДСиДР КАК СоответствиеСтатейДДССтатьямЗатратИДоходов
	               |ГДЕ
	               |	СоответствиеСтатейДДССтатьямЗатратИДоходов.СтатьяДДС = &СтатьяДДС";
