...
Процедура ПроверитьТабличнуюЧасть(Отказ)
	...
	Если СтрокиСПустымиЗначениями.Количество() > 0 Тогда
		...
		Для каждого СтрокаПустыхЗначений Из СтрокиСПустымиЗначениями Цикл
			СтрокаОшибок = СтрокаОшибок + ?(НЕ ПустаяСтрока(СтрокаОшибок), Символы.ПС, "") + НСтр("ru='В строке ';uk='У рядку '") + СтрокаПустыхЗначений.НомерСтроки + НСтр("ru=' не указаны стадия и класс контрагента!';uk=' не вказані стадія і клас контрагента!'");
		КонецЦикла;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(СписокКонтрагентов) Тогда
		Отказ = Истина;
		СтрокаОшибок = СтрокаОшибок + НСтр("ru='Найдены строки с повторяющимися контрагентами!';uk='Знайдені рядки з повторюваними контрагентами!'") + СписокКонтрагентов;
	КонецЕсли; 
