...
Процедура ПроверитьНаДубльДокументаВПериоде(Отказ,Заголовок)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В текущем месяце по данной организации коэффициенты уже были расчитаны другим документом!';uk='У поточному місяці по вказаній організації коефіцієнти вже були розраховані іншим документом!'")), Отказ);	
...
Функция ПодготовитьТаблицуСтатей(РезультатЗапросаПоСтатьям, СтруктураШапкиДокумента, ПогрешностиОкругления)
	...
	// добавим статьи по ОПЗ
	Если Дата >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
	
		СтатьиОПЗ = Справочники.СтатьиНалоговыхДеклараций.ВыбратьИерархически(Справочники.СтатьиНалоговыхДеклараций.НПНК_ЗВ_);
		
		Пока СтатьиОПЗ.Следующий() Цикл
			
			ТекСтатья = СтатьиОПЗ.Ссылка;
		...
		ТекСтатья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_ОС_ЗВ;
		Если ТаблицаСтатей.Найти(ТекСтатья, "Статья") = Неопределено Тогда
			НоваяСтрока = ТаблицаСтатей.Добавить();
			НоваяСтрока.Статья = ТекСтатья;
		КонецЕсли;
		
		ТекСтатья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_НА_ЗВ;
		Если ТаблицаСтатей.Найти(ТекСтатья, "Статья") = Неопределено Тогда
			НоваяСтрока = ТаблицаСтатей.Добавить();
			НоваяСтрока.Статья = ТекСтатья;
		КонецЕсли;
		
		ТекСтатья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_Ремонт_ЗВ;
		Если ТаблицаСтатей.Найти(ТекСтатья, "Статья") = Неопределено Тогда
			НоваяСтрока = ТаблицаСтатей.Добавить();
			НоваяСтрока.Статья = ТекСтатья;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиСтатьи(ТаблицаПоСтатьям, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если Не Отказ Тогда
		...
		Для Уровень1 = 1 По КоличествоСтрокВТЧ Цикл
			...
			ОшибкаПоВидуСтатьи = Ложь;
			ТекСтратья = Статьи[Уровень1- 1].Статья;
			
			Если (ТекСтратья = Справочники.СтатьиНалоговыхДеклараций.НПНК_ПЗ_Пф) Тогда
				ОшибкаПоВидуСтатьи = Истина;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ТекСтратья)  Тогда
				Если Дата >= глЗначениеПеременной("ДатаНачалаРаспределенияОПЗвНУ") Тогда
					Если НЕ ТекСтратья.ПринадлежитЭлементу(Справочники.СтатьиНалоговыхДеклараций.НПНК_ЗВ_)
					   И НЕ ТекСтратья.ПринадлежитЭлементу(Справочники.СтатьиНалоговыхДеклараций.НПНК_ПЗ_)
					   И НЕ ТекСтратья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_ОС_ЗВ
					   И НЕ ТекСтратья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_НА_ЗВ
					   И НЕ ТекСтратья = Справочники.СтатьиНалоговыхДеклараций.НПНК_АМ_Ремонт_ЗВ Тогда
						  
						  ОшибкаПоВидуСтатьи = Истина;
						  
					КонецЕсли;	  
				Иначе
					Если НЕ ТекСтратья.ПринадлежитЭлементу(Справочники.СтатьиНалоговыхДеклараций.НПНК_ПЗ_) Тогда
			...
			Если ОшибкаПоВидуСтатьи Тогда
			
				ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке ""%1"" указана статья  ""%2""! Выбирать данную статью в этом докумнте нельзя!';uk='У рядку ""%1""  зазначена статтья ""%2""! Не можна у цьому документі вказувати зазначену статтю!'"), Уровень1, Статьи[Уровень1- 1].Статья), Отказ, Заголовок);	
				Продолжить;	
			...
			Для Уровень2 = Уровень1 + 1 По КоличествоСтрокВТЧ Цикл
				...
				Если Статьи[Уровень1- 1].Статья = Статьи[Уровень2- 1].Статья Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строках ""%1"" и  ""%2"" указаны одинаковые статьи!';uk='У рядках ""%1"" та  ""%2"" зазначені однакові статті!'"), Уровень1, Уровень2), Отказ, Заголовок);	
	...
	Если  СуммаКоэффициентов <> 1 Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сумма коэффициентов на конец периода в строках не равна единице!';uk='Сума коефіцієнтів на кінець періода у рядках не дорівнюе  одиниці!'"), Уровень1, Уровень2), Отказ);		
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоСтатьям, Отказ, Заголовок) Экспорт
	...
	РезультатЗапросаПоСтатьям = ОбщегоНазначения.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "Статьи", СтруктураПолей, СтруктураПростыхПолей);
