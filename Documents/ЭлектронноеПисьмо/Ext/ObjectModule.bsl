...
Функция ЗахватитьТекст() Экспорт
	...
	ИмяФайла = НСтр("ru='Электронное письмо ';uk='Електронний лист '") + СокрЛП(Номер) + ?(ЗначениеЗаполнено(Тема), (" (" + СокрЛП(Тема) + ")"), "");
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Не удалось сохранить файл на диск!';uk='Не вдалося зберегти файл на диск!'"));
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОсвободитьТекст() Экспорт
	...
	Если ИмяКомпьютера() <> ИмяКомпьютераРедактированияХТМЛТекста Тогда
		Сообщить(НСтр("ru='Файл редактируется на копмьютере""';uk='Файл редагується на комп''ютері""'") + ИмяКомпьютераРедактированияХТМЛТекста + """" + НСтр("ru='. Редактирование на текущем комьютере невозможно.';uk="". Редагування на поточному комп'ютері неможливе."""));
		Возврат;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Файл редактирования не удалось открыть.';uk='Файл редагування не вдалося відкрити.'"));
		Возврат;
	КонецПопытки;
...
//
Процедура ОтправитьПисьмо(ФормаПисьма = Неопределено, ПользовательПочты = Неопределено) Экспорт
	...
	Если ПустаяСтрока(Кому) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан ни один получатель.';uk='Не вказано жодного отримувача.'"),, ШапкаСообщения);
		Возврат;
	КонецЕсли;
	
	Если ПометкаУдаления Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Нельзя отправлять помеченное на удаление письмо.';uk='Неможна відправляти відмічений для вилучення лист.'"),, ШапкаСообщения);
		Возврат;
	КонецЕсли;
	...
	//Дандали В. 20.05.2024 +
	ПользовательОтправки = ?(ЗначениеЗаполнено(ПользовательПочты),ПользовательПочты,глЗначениеПеременной("глТекущийПользователь"));
	//Дандали В. 20.05.2024 -
	
	Если УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(ПользовательОтправки).Запись.НайтиПоЗначению(УчетнаяЗапись) <> Неопределено Тогда
		УчетныеЗаписи = Новый Массив;
		УчетныеЗаписи.Добавить(УчетнаяЗапись);
		Письма = Новый Соответствие;
		Письма.Вставить(Ссылка, ЭтотОбъект);
		УправлениеЭлектроннойПочтой.ПолучениеОтправкаПисем(глЗначениеПеременной("глСоответствиеТекстовЭлектронныхПисем"), ПользовательОтправки, УчетныеЗаписи, Письма, Истина);
	КонецЕсли;
...
Процедура НапечататьПисьмо() Экспорт
	...
	ТекстОписанияЭлектронногоПисьма = НСтр("ru='Электронное письмо № ';uk='Електронний лист № '") + СокрЛП(Номер) + ", " + Дата + "<BR>" + ПредставлениеСтрокиДляHTML((СокрЛП(УчетнаяЗапись) + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + ">"));
	...
	ТекстВыходногоДокумента = ТекстВыходногоДокумента + СтрокаТаблицы(НСтр("ru='Дата создания:';uk='Дата створення:'"), Строка(Дата));
	ТекстВыходногоДокумента = ТекстВыходногоДокумента + СтрокаТаблицы(НСтр("ru='Дата отправления:';uk='Дата відправлення:'"), Строка(ДатаОтправления));
	...
	Если НЕ ОбменДанными.Загрузка Тогда
		...
		Если НЕ ЗначениеЗаполнено(УчетнаяЗапись) Тогда
			Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана учетная запись.';uk='Не вказаний оліковий запис.'"),, ШапкаСообщения);
			КонецЕсли; 
			Отказ = Истина;
		КонецЕсли;
		...
		ИначеЕсли СтатусПисьма = Перечисления.СтатусыПисем.Отправленное Тогда
			...
			Иначе
				Если Ответственный <> Ссылка.Ответственный Тогда
					Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
						ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Нельзя менять ответственного в отправленном электронном письме.';uk='Неможна змінювати відповідального у відправленому електронному листі.'"),, ШапкаСообщения);
					КонецЕсли; 
					Отказ = Истина;
				КонецЕсли; 
			КонецЕсли;
		...
		Если ГруппаУчетнойЗаписи = ГруппаУдаленные Тогда
			Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
				ОбщегоНазначения.СообщитьОбОшибке((НСтр("ru='Нельзя выбирать в качестве группы письма группу ""';uk='Неможна обирати як групу листа групу ""'") + ГруппаУдаленные + НСтр("ru='"", потому что она предназначена только для отображения удаленных писем.';uk='"", тому що вона призначена тільки для відображення вилучених листів'")),, ШапкаСообщения);
			КонецЕсли; 
			Отказ = Истина;
		КонецЕсли;
		...
		Если ГруппаУчетнойЗаписи = ГруппаЧерновики Тогда
			Если НЕ НеВыдаватьСообщенияПриЗаписиОбъекта Тогда
				ОбщегоНазначения.СообщитьОбОшибке((НСтр("ru='Нельзя выбирать в качестве группы письма группу ""';uk='Неможна обирати як групу листа групу ""'") + ГруппаЧерновики + НСтр("ru='"", потому что она предназначена только для отображения черновиков писем.';uk='"", тому що вона призначена тільки для відображення чернеток листів'")),, ШапкаСообщения);
			КонецЕсли; 
			Отказ = Истина;
		КонецЕсли; 
