...
Процедура ПроверитьПересекающиесяДокументы(Отказ, Заголовок)
	...
	Для Тмп = 0 По КоличествоСтрок Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЗ.ТипЦен) Тогда
			Для Тмп1 = 1 По КоличествоСтрок - Тмп Цикл
				СтрокаПоиска = ТЗ[КоличествоСтрок - Тмп - Тмп1];
	...
	Если НЕ ПустаяСтрока(СтрокаДокументовПересечений) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Найдены записи о типах цен для групп, которые уже действуют в выбранном периоде:';uk='Знайдені записи про типи цін для груп, які вже діють в обраному періоді:'") + СтрокаДокументовПересечений, Отказ, Заголовок);
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПроверитьДублиЦеновыхНоменклатурныхГрупп(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.КоличествоДублей > 1 Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке("""" + СокрЛП(ВыборкаПоСтрокамДокумента.НоменклатурнаяЦеноваяГруппа) + НСтр("ru='"""
": дублей строк ';uk='"""
": дублів рядків '") + НазваниеГрупп + НСтр("ru=' групп в этом документе быть не должно!';uk=' груп у цьому документі бути не повинно!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если РезультатЗапросаПоГруппам.Пустой() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнена табличная часть документа!';uk='Не заповнена таблична частина документа!'"), Отказ, Заголовок);
	КонецЕсли;
