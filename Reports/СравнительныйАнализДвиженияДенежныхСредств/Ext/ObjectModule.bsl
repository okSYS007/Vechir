...
Функция ТекстЗапросаПоСценарию(СтрокаСценарий,СтруктураПараметров,Номер,ПериодДанных, СтруктураГруппировки)
	...
	ИначеЕсли СтрокаСценарий.ВидДанных=Перечисления.ВидыДанныхДляПланФактногоАнализаДДС.ФактическиеДанные Тогда
			...
			|	ВЫБОР КОГДА ФактПоступления.ДокументПланированияПлатежа.ЦФО=&ПустойЦФО
			...
			|ВЫБРАТЬ
			|	ФактПлатежи.Период,
			|	ВЫБОР КОГДА ФактПлатежи.ДокументПланированияПлатежа.ЦФО=&ПустойЦФО
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей)
	...
	Для каждого СтрокаСценарий Из КопияТаблицаСценарии Цикл
		...
		СтрокаПериод=НСтр("ru='Период с ';uk='Період з '", КодЯзыкаПечать)+Формат(СтрокаСценарий.ДатаНач,"ДФ=dd.MM.yyyy")+НСтр("ru=' по ';uk='до '", КодЯзыкаПечать)+Формат(СтрокаСценарий.ДатаКон,"ДФ=dd.MM.yyyy");

		Если СтрокаСценарий.ВидДанных=Перечисления.ВидыДанныхДляПланФактногоАнализаДДС.ФактическиеДанные Тогда
			СтрокаСценарий.ТекстПараметра=?(Номер=0,НСтр("ru=' Эталонный период.';uk=' Еталонний період.'", КодЯзыкаПечать),НСтр("ru='Сравниваемый период ';uk='Період, що порівнюється '", КодЯзыкаПечать)+Номер+".")
			+Символы.ПС+НСтр("ru='Фактические данные.';uk='Фактичні дані.'", КодЯзыкаПечать)+" "+ПериодДанных+"."+Символы.ПС+СтрокаПериод;
		...
		ИначеЕсли СтрокаСценарий.ВидДанных=Перечисления.ВидыДанныхДляПланФактногоАнализаДДС.ФактическиеИПланируемые Тогда
			СтрокаСценарий.ТекстПараметра=?(Номер=0,НСтр("ru=' Эталонный период.';uk=' Еталонний період.'", КодЯзыкаПечать),НСтр("ru='Сравниваемый период ';uk='Період, що порівнюється '", КодЯзыкаПечать)+Номер+".")
			+Символы.ПС+НСтр("ru='Фактические данные с учетом заявок и планируемых поступлений.';uk='Фактичні дані з урахуванням заявок і планованих надходжень.'", КодЯзыкаПечать)+" "+ПериодДанных+"."+Символы.ПС+СтрокаПериод;
		КонецЕсли;
	...
	|"+УправлениеДенежнымиСредствами.СформироватьТекстЗапросаПоПериодам(ПериодДанных,"ДанныеСценариев.Период")+"
	...
	"Поступления в валюте упр. учета (" + Константы.ВалютаУправленческогоУчета.Получить()+")",
	"Платежи в валюте упр. учета (" + Константы.ВалютаУправленческогоУчета.Получить()+")",
	"Чистый денежный поток в валюте упр. учета (" + Константы.ВалютаУправленческогоУчета.Получить()+")",
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мНазваниеОтчета = НСтр("ru='Сравнительный анализ движения денежных средств';uk='Порівняльний аналіз руху грошових коштів'", КодЯзыкаПечать);
	мНаДату = Ложь;
	...
	Если Показатели.Количество()=0 Тогда
		...
		СтрПоказатели.Представление = НСтр("ru='Поступления в валюте операций';uk='Надходження у валюті операцій'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Ложь;
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя           = "СуммаРасходВал";
		СтрПоказатели.Представление = НСтр("ru='Платежи в валюте операций';uk='Платежі у валюті операцій'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Ложь;
		
		// Числовые показатели заносятся в таблицу этого отчета
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя           = "ДенежныйПотокВал";
		СтрПоказатели.Представление = НСтр("ru='Чистый денежный поток в валюте операций';uk='Чистий грошовий потік у валюті операцій'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Ложь;
		
		// Числовые показатели заносятся в таблицу этого отчета
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя           = "СуммаПриходУпр";
		СтрПоказатели.Представление = НСтр("ru='Поступления в валюте упр. учета';uk='Надходження у валюті упр. обліку'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Истина;
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя           = "СуммаРасходУпр";
		СтрПоказатели.Представление = НСтр("ru='Платежи в валюте упр. учета';uk='Платежі у валюті упр. обліку'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Истина;
		
		// Числовые показатели заносятся в таблицу этого отчета
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя           = "ДенежныйПотокУпр";
		СтрПоказатели.Представление = НСтр("ru='Чистый денежный поток в валюте упр. учета';uk='Чистий грошовий потік у валюті упр. обліку'", КодЯзыкаПечать);
		СтрПоказатели.Использование   = Истина;		
		...
		Если (Выборка.Группировка()<>"НомерСценария") и (Выборка.Группировка()<>"Период")  Тогда  // Не выводим группировку, разворачивающуюся по горизонтали.
			...
			ИначеЕсли ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
				...
				Если СтруктураВыводГруппировок.Свойство(ИмяГруппировки, МассивВыводГруппировок) Тогда
					...
					Для Каждого ВыводимаяГруппировка Из МассивВыводГруппировок Цикл
						...
						Если Найти(ВыводимаяГруппировка,"Период")>0 Тогда
							ЗначениеТекущейГруппировки=УправлениеДенежнымиСредствами.ПолучитьПериодСтрокой(Выборка[ВыводимаяГруппировка],СтрЗаменить(ВыводимаяГруппировка,"Период",""));
						Иначе
				...
				Если Найти(ИмяГруппировки,"Период")>0 Тогда
					ЗначениеТекущейГруппировки=УправлениеДенежнымиСредствами.ПолучитьПериодСтрокой(Выборка[ИмяГруппировки],СтрЗаменить(ИмяГруппировки,"Период",""));
				Иначе
					ЗначениеТекущейГруппировки = "" + Выборка[ИмяГруппировки];
				КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	Если ТаблицаСценарии.Количество()=0 Тогда
		Предупреждение(НСтр("ru='Не выбраны данные для сравнения!';uk='Не вибрані дані для порівняння!'"));
		Возврат;
	КонецЕсли;
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1  Цикл
		...
		Иначе
			Предупреждение(НСтр("ru='Повторяющаяся группировка ';uk='Повторюване групування '") + ПостроительОтчета.ИзмеренияСтроки[Инд].Представление +"."+ Символы.ПС+
			НСтр("ru='Нельзя использовать одинаковые поля группировки строк!';uk='Не можна використовувати однакові поля групування рядків!'"));
			ЕстьОшибки = Истина;
			Возврат;
	...
	Для Сч=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		...
		СписокИзмерений = СписокИзмерений +", "+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ИзмеренияСтроки[Сч].Представление)
		+" "+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.ИзмеренияСтроки[Сч].ТипИзмерения));
	...
	ЗаголовокКолонки = НСтр("ru='Общий итог';uk='Загальний підсумок'"); // этот заголовок выводится в случае отсутствия группировок
	...
	Для Сч = 1 По 3 Цикл
		...
		ИначеЕсли Префикс = "Середина" Тогда
			...
			ОбластьЗначение.Параметры.Фильтры = НСтр("ru='Отбор: ';uk='Відбір: '", КодЯзыкаПечать) + СписокОтбор;
			ДокументРезультат.Присоединить(ОбластьЗначение);
		...
		ИначеЕсли Префикс = "Низ" Тогда
			...
			ОбластьЗначение.Параметры.Группировки=НСтр("ru='Группировки: ';uk='Групування: '", КодЯзыкаПечать) +СписокИзмерений;
			ОбластьЗначение.Параметры.Показатели = НСтр("ru='Показатели: ';uk='Показники: '", КодЯзыкаПечать) + СписокПоказателей;
			ДокументРезультат.Присоединить(ОбластьЗначение);
...
Процедура ВывестиСтрокуШапки(ВыборкаГоризонтальная, СтруктураПараметров, ЗначениеВыборкиДанные,СтрокаДанныхОтчета)
	...
	ЗначениеГруппировкиПериоды=УправлениеДенежнымиСредствами.ПолучитьПериодСтрокой(ВыборкаГоризонтальная["Период"],ПериодДанных,КодЯзыкаПечать);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Группировка()="НомерСценария" Тогда
			...
			ОбластьЗначениеГруппировкиГор.Параметры.РасшифровкаДанные=Новый Структура("Сценарий,ДатаНач,ДатаКон",
																		...
																		СтрокаДанныхОтчета.ДатаНач,
																		СтрокаДанныхОтчета.ДатаКон);
			ОбластьЗначениеГруппировкиГор.Параметры.ПредставлениеГруппировкиПериоды=НСтр("ru='Итого';uk='Разом'", КодЯзыкаПечать);
...
Процедура РасшифровкаДвижений(Расшифровка, ЭтотОтчет, Выбор)
	...
	Иначе
		...
		Отчет.Сценарий = Расшифровка.Сценарий;
	...
	Для Каждого Элемент Из Расшифровка Цикл
		...
		Если Найти(Элемент.Ключ, "Период") > 0 Тогда
			...
			ДатаНач = УправлениеДенежнымиСредствами.ДатаНачалаПериода(Элемент.Значение, Периодичность);
			ДатаКон = УправлениеДенежнымиСредствами.ДатаКонцаПериода(Элемент.Значение, Периодичность);
...
Процедура ОбработкаРасшифровкиОтчетаБюджетирования(Расшифровка, СтандартнаяОбработка, ЭтотОтчет) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Выбор = СписокВыбора.ВыбратьЭлемент(НСтр("ru='Выберите действие';uk='Виберіть дію'"), СписокВыбора[СписокВыбора.Количество()-1]);
