...
Функция ОпределитьВозможностьПолученияОтчета()
	...
	Если Документ.Пустая() Тогда
		Предупреждение(НСтр("ru='Запишите и проведите документ, по которому Вы формируете отчет';uk='Запишіть і проведіть документ, за яким Ви формуєте звіт'"));
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	ОбластьЗаголовок.Параметры.Документ = ОбщегоНазначения.СформироватьЗаголовокДокумента(Документ,,КодЯзыкаПечать);
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		Состояние(НСтр("ru='Обрабатывается регистр (';uk='Обробляється регістр ('") + Цел(НомерРегистра/КоличествоРегистров * 100) + "%): " + СвойстваОбъекта.Синоним);
		...
		Если (ВидРегистра = "Сведений") 
			...
			Если (ВидРегистра = "Накопления")
				...
				СтруктураПредставлениеПолей.Вставить("ВидДвижения", НСтр("ru='Вид движения';uk='Вид руху'", КодЯзыкаПечать));
			...
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстИзмерения, НСтр("ru='Измерения';uk='Виміри'", КодЯзыкаПечать), 40));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРесурсы, НСтр("ru='Ресурсы';uk='Ресурси'", КодЯзыкаПечать), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРеквизиты, НСтр("ru='Реквизиты';uk='Реквізити'", КодЯзыкаПечать), 20));
		...
		ИначеЕсли ВидРегистра = "Бухгалтерии" Тогда
			...
			Для Каждого Ресурс из СвойстваОбъекта.Ресурсы Цикл
				...
				Если Ресурс.Балансовый Или Не КорреспонденцияОбъекта Тогда
					...
					СтруктураПредставлениеПолей.Вставить(Ресурс.Имя, ЛокализацияПовтИсп.ПолучитьПереводТекста(Ресурс.Синоним, КодЯзыкаПечать));
				Иначе
					ТекстРесурсыДт = ТекстРесурсыДт + ", "+ Ресурс.Имя +"Дт";
					СтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "Дт", ЛокализацияПовтИсп.ПолучитьПереводТекста(Ресурс.Синоним, КодЯзыкаПечать) + " Дт");
					
					ТекстРесурсыКт = ТекстРесурсыКт + ", "+ Ресурс.Имя +"Кт";
					СтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "Кт", ЛокализацияПовтИсп.ПолучитьПереводТекста(Ресурс.Синоним, КодЯзыкаПечать) + " Кт");
			...
			МаксКоличествоСубконто = ? (ВестиУчетПоДопРазрезамРегл, СвойстваОбъекта.ПланСчетов.МаксКоличествоСубконто, Мин(СвойстваОбъекта.ПланСчетов.МаксКоличествоСубконто, 3));
			
			Для Инд = 1 по МаксКоличествоСубконто Цикл
			...
			Если КорреспонденцияОбъекта Тогда
				...
				СтруктураПредставлениеПолей.Вставить("СчетДт", НСтр("ru='Счет Дт';uk='Рахунок Дт'", КодЯзыкаПечать));
				ТекстИзмеренияКт = ", СчетКт";
				СтруктураПредставлениеПолей.Вставить("СчетКт", НСтр("ru='Счет Кт';uk='Рахунок Кт'", КодЯзыкаПечать));
			Иначе
				ТекстИзмеренияДт = "";
				ТекстИзмеренияКт = "";
				ТекстИзмерения = ", Счет";
				СтруктураПредставлениеПолей.Вставить("Счет", НСтр("ru='Счет';uk='Рахунок'", КодЯзыкаПечать));
			КонецЕсли;
			...
			Для Каждого Измерение из СвойстваОбъекта.Измерения Цикл
				...
				Если Измерение.Балансовый Или Не КорреспонденцияОбъекта Тогда
					...
					СтруктураПредставлениеПолей.Вставить(Измерение.Имя, ЛокализацияПовтИсп.ПолучитьПереводТекста(Измерение.Синоним, КодЯзыкаПечать));
				Иначе
					ТекстИзмеренияДт = ТекстИзмеренияДт + ", " + Измерение.Имя + "Дт";
					СтруктураПредставлениеПолей.Вставить(Измерение.Имя + "Дт", ЛокализацияПовтИсп.ПолучитьПереводТекста(Измерение.Синоним, КодЯзыкаПечать) + " Дт");
					
					ТекстИзмеренияКт = ТекстИзмеренияКт + ", " + Измерение.Имя + "Кт";
					СтруктураПредставлениеПолей.Вставить(Измерение.Имя + "Кт", ЛокализацияПовтИсп.ПолучитьПереводТекста(Измерение.Синоним, КодЯзыкаПечать) + " Кт");
				КонецЕсли;
			...
			Иначе
				Попытка
					...
					Построитель.МакетШапкиТаблицы.КодЯзыкаМакета = КодЯзыкаПечать;
					Если (СвойстваОбъекта.Имя = "Хозрасчетный") И ВестиУчетПоДопРазрезамРегл Тогда
						Построитель.МакетДетальныхЗаписей = МакетНастройки.ПолучитьОбласть("Строка4Субконто");
					Иначе
						Построитель.МакетДетальныхЗаписей = МакетНастройки.ПолучитьОбласть("Строка");
					КонецЕсли;
				Исключение
		...
		ИначеЕсли ВидРегистра = "Расчета" Тогда
			...
			СтруктураПредставлениеПолей.Вставить("ВидРасчета", НСтр("ru='Вид расчета';uk='Вид розрахунку'", КодЯзыкаПечать));
			СтруктураПредставлениеПолей.Вставить("ПериодРегистрации", НСтр("ru='Период регистрации';uk='Період реєстрації'", КодЯзыкаПечать));
			...
			Если СвойстваОбъекта.БазовыйПериод Тогда
				...
				СтруктураПредставлениеПолей.Вставить("БазовыйПериодНачало", НСтр("ru='Начало базового периода';uk='Початок базового періоду'", КодЯзыкаПечать));
				СтруктураПредставлениеПолей.Вставить("БазовыйПериодКонец", НСтр("ru='Конец базового периода';uk='Кінець базового періоду'", КодЯзыкаПечать));
			КонецЕсли;
			...
			Если СвойстваОбъекта.ПериодДействия Тогда
				...
				СтруктураПредставлениеПолей.Вставить("ПериодДействияНачало", НСтр("ru='Начало периода действия';uk='Початок періоду дії'", КодЯзыкаПечать));
				СтруктураПредставлениеПолей.Вставить("ПериодДействияКонец", НСтр("ru='Конец периода действия';uk='Кінець періоду дії'", КодЯзыкаПечать));
			КонецЕсли;
			...
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", СпецифическиеДанныеРегистраРасчета, НСтр("ru='Данные расчета';uk='Дані розрахунку'", КодЯзыкаПечать), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстИзмерения, НСтр("ru='Измерения';uk='Виміри'", КодЯзыкаПечать), 40));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРесурсы, НСтр("ru='Ресурсы';uk='Ресурси'", КодЯзыкаПечать), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРеквизиты, НСтр("ru='Реквизиты';uk='Реквізити'", КодЯзыкаПечать), 20));
		...
		ОбластьЗаголовок.Параметры.ИмяРегистра = ЛокализацияПовтИсп.ПолучитьПереводТекста(СвойстваОбъекта.Синоним);
		ОбластьЗаголовок.Параметры.ВидРегистра = ЛокализацияПовтИсп.ПолучитьПереводТекста(НРег(ВидРегистра));
		ДокументРезультат.Вывести(ОбластьЗаголовок);
		...
		Если ВыводитьРасходПриход Тогда
			...
			Для Номерстроки = НомерВыводимойСтроки + 2 По НомерТекущейСтроки - 1 Цикл
				...
				Если ТекстСтрокиЯчейки = "Расход" Тогда
					ОбластьВывода.Текст = НСтр("ru='Расход';uk='Видаток'", КодЯзыкаПечать);
					ОбластьВывода.ЦветТекста = ЦветаСтиля.ЦветОтрицательногоЧисла;	
				ИначеЕсли ТекстСтрокиЯчейки = "Приход" Тогда
					ОбластьВывода.Текст = НСтр("ru='Приход';uk='Надходження'", КодЯзыкаПечать);
