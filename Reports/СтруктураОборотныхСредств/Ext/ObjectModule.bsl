...
Функция ПолучитьНаименованиеКолонки(Дата, НомерКолонки)
	...
	Если НомерКолонки = 1 Тогда
		ТекстВозврата = НСтр("ru='На начало периода';uk='На початок періоду'", КодЯзыкаПечать);
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда
		ТекстВозврата = НСтр("ru='Неделя с';uk='Тиждень з'", КодЯзыкаПечать) + Символы.ПС + Формат(Дата, "ДФ = ""дд ММММ гггг""");
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ЗаполнитьПоказатели("Показатели"     , НСтр("ru='Показатели';uk='Показники'", КодЯзыкаПечать),
	                    ЛОЖЬ,   "ЧЦ = 15; ЧДЦ = 2");
	ЗаполнитьПоказатели("КАбсЛиквидности", НСтр("ru='Коэффициент абсолютной ликвидности';uk='Коефіцієнт абсолютної ліквідності'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "ДС / КЗ");
	ЗаполнитьПоказатели("КСрЛиквидности" , НСтр("ru='Коэффициент срочной ликвидности';uk='Коефіцієнт термінової ліквідності'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "(ДС + ДЗ) / КЗ");
	ЗаполнитьПоказатели("КТекЛиквидности", НСтр("ru='Коэффициент текущей ликвидности';uk='Коефіцієнт поточної ліквідності'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "(ДС + ДЗ + CCТ) / КЗ");
	ЗаполнитьПоказатели("ОборотныеАктивы", НСтр("ru='Чистые оборотные активы';uk='Чисті оборотні активи'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 2", "(ДС + ДЗ + ССТ) - КЗ");
	ЗаполнитьПоказатели("ОбеспечСобствОК", НСтр("ru='Обеспеченность собственным оборотным капиталом';uk='Забезпеченість власним оборотним капіталом'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "((ДС + ДЗ + ССТ) - КЗ) / (ДС + ДЗ + ССТ)");
	ЗаполнитьПоказатели("КМаневренности" , НСтр("ru='Коэффициент маневренности собственного оборотного капитала';uk='Коефіцієнт маневреності власного оборотного капіталу'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "ССТ / ((ДС + ДЗ + ССТ) - КЗ)");
	ЗаполнитьПоказатели("ДоляТАвОбА"     , НСтр("ru='Доля труднореализуемых активов в общей величине оборотных активов';uk='Частка активів, що важко реалізуються, у загальній величині оборотних активів'", КодЯзыкаПечать),
	                    ИСТИНА, "ЧЦ = 15; ЧДЦ = 5", "ССТ / (ДС + ДЗ + ССТ)");
	ЗаполнитьПоказатели("Соотношение"    , НСтр("ru='Соотношение труднореализуемых и легкореализуемых активов';uk='Співвідношення активів, що  важко реалізуються і легко реалізуються'", КодЯзыкаПечать),
	...
	Для Каждого Показатель Из Показатели Цикл
		...
		Если Показатель.Использование = Истина Тогда
			...
			Если ИмяПоказателя = "Показатели" Тогда
				...
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Денежные средства';uk='Грошові кошти'", КодЯзыкаПечать), "ДенежныеСредства"         , ФорматПоказателя, "ДС", 1);
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Дебиторская задолженность';uk='Дебіторська заборгованість'", КодЯзыкаПечать) , "ДебиторскаяЗадолженность" , ФорматПоказателя, "ДЗ", 1);
				ДокументРезультат.НачатьГруппуСтрок("ДебиторскаяЗадолженность", Ложь);
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Дебиторская задолженность контрагентов';uk='Дебіторська заборгованість контрагентів'", КодЯзыкаПечать), "ДебиторскаяЗадолженностьКонтрагентов" , ФорматПоказателя, "", 2);
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Дебиторская задолженность подотчетных лиц';uk='Дебіторська заборгованість підзвітних осіб'", КодЯзыкаПечать), "ДебиторскаяЗадолженностьПодотчетников" , ФорматПоказателя, "", 2);
				ДокументРезультат.ЗакончитьГруппуСтрок();
				
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Кредиторская задолженность';uk='Кредиторська заборгованість'", КодЯзыкаПечать), "КредиторскаяЗадолженность", ФорматПоказателя, "КЗ", 1);
				ДокументРезультат.НачатьГруппуСтрок("КредиторскаяЗадолженность", Ложь);
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Кредиторская задолженность контрагентов';uk='Кредиторська заборгованість контрагентів'", КодЯзыкаПечать), "КредиторскаяЗадолженностьКонтрагентов", ФорматПоказателя, "", 2);
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Кредиторская задолженность подотчентых лиц';uk='Кредиторська заборгованість підзвітних осіб'", КодЯзыкаПечать), "КредиторскаяЗадолженностьПодотчетников", ФорматПоказателя, "", 2);
				ДокументРезультат.ЗакончитьГруппуСтрок();
				
				ДобавитьПараметр(Макет, ДокументРезультат, СтруктураПоказателейРасчета, НСтр("ru='Себестоимость товаров (за исключением принятых на комиссию)';uk='Собівартість товарів (за винятком прийнятих на комісію)'", КодЯзыкаПечать), "ТМЦ", ФорматПоказателя, "ССТ", 1);
...
мНазваниеОтчета = НСтр("ru='Структура оборотных средств + финансовый анализ';uk='Структура оборотних коштів + фінансовий аналіз'", КодЯзыкаПечать);
