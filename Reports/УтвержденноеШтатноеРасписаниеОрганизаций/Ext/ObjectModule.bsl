		...
		Пока ВыборкаСтрокШтРасписания.СледующийПоЗначениюПоля("Должность") Цикл 
			...
			Пока ВыборкаСтрокШтРасписания.Следующий() Цикл 
				...
				Если ВыборкаСтрокШтРасписания.РазмерНадбавки <> NULL Тогда
					...
					Если ВыборкаСтрокШтРасписания.СпособРасчета  = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой Тогда
			...
			Если ВывестиПодвалЛиста Тогда
				...
				ОбластьШапкиНадбавок.Текст = НСтр("ru='Надбавки, грн';uk='Надбавки, грн'");
...
Процедура СформироватьОтчет(ДокументРезультат, Дописать = Ложь) Экспорт
	...
	ДокументРегистратор = ПараметрОтбора;

	Если НЕ ЗначениеЗаполнено(ДокументРегистратор) тогда
		ТекстВыборкиИспользуемыхНадбавок = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		                                   |	НадбавкиПоШтатномуРасписаниюОрганизацийСрезПоследних.ВидНадбавки КАК ВидНадбавки
		                                   |ИЗ
		                                   |	РегистрСведений.НадбавкиПоШтатномуРасписаниюОрганизаций.СрезПоследних(&ДатаАктуальности, ПодразделениеОрганизации.Владелец = &Организация) КАК НадбавкиПоШтатномуРасписаниюОрганизацийСрезПоследних";

	Иначе	

		Организация = ДокументРегистратор.Организация;
		ТекстВыборкиИзменяемыхПозицийРасписаний = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		                                          |	НеуникальныеПозицииШтатного.Подразделение КАК Подразделение,
		                                          |	НеуникальныеПозицииШтатного.Должность КАК Должность
		                                          |ИЗ
		                                          |	(ВЫБРАТЬ
		                                          |		НадбавкиДока.Подразделение КАК Подразделение,
		                                          |		НадбавкиДока.Должность КАК Должность
		                                          |	ИЗ
		                                          |		Документ.ИзменениеШтатногоРасписанияОрганизаций.Надбавки КАК НадбавкиДока
		                                          |	
		                                          |	ГДЕ
		                                          |		НадбавкиДока.Ссылка = &ДокументРегистратор
		                                          |	
		                                          |	ОБЪЕДИНИТЬ ВСЕ
		                                          |	
		                                          |	ВЫБРАТЬ
		                                          |		РасписаниеДока.Подразделение,
		                                          |		РасписаниеДока.Должность
		                                          |	ИЗ
		                                          |		Документ.ИзменениеШтатногоРасписанияОрганизаций.ШтатныеЕдиницы КАК РасписаниеДока
		                                          |	
		                                          |	ГДЕ
		                                          |		РасписаниеДока.Ссылка = &ДокументРегистратор И
		                                          |		(РасписаниеДока.КоличествоСтавок <> 0)) КАК НеуникальныеПозицииШтатного";
				  
												
		ТекстВыборкиИспользуемыхНадбавок = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		                                   |	ИзменениеШтатногоРасписанияОрганизацииНадбавки.ВидНадбавки КАК ВидНадбавки
		                                   |ИЗ
		                                   |	Документ.ИзменениеШтатногоРасписанияОрганизаций.Надбавки КАК ИзменениеШтатногоРасписанияОрганизацииНадбавки
		                                   |
		                                   |ГДЕ
		                                   |	ИзменениеШтатногоРасписанияОрганизацииНадбавки.Ссылка = &ДокументРегистратор";
	КонецЕсли;
	...
	|" + ?(НЕ ЗначениеЗаполнено(ДокументРегистратор),"","
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (" + ТекстВыборкиИзменяемыхПозицийРасписаний + ") КАК ПозицииШтатногоИзменеямыеДоком
	|					ПО Расписание.ПодразделениеОрганизации = ПозицииШтатногоИзменеямыеДоком.Подразделение И Расписание.Должность = ПозицииШтатногоИзменеямыеДоком.Должность
	|				  
	|") + "
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ (" + ТекстВыборкиИспользуемыхНадбавок + ")  КАК ИспользуемыеНадбавки
	|			ПО (ИСТИНА)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НадбавкиПоШтатномуРасписаниюОрганизаций.СрезПоследних(&ДатаАктуальности, ПодразделениеОрганизации.Владелец = &Организация) КАК НадбавкиПоШтатномуРасписанию
	...
	|			" + ?(НЕ ЗначениеЗаполнено(ДокументРегистратор),""," И НадбавкиПоШтатномуРасписанию.Регистратор = &ДокументРегистратор") + "
	|	ГДЕ
	...
	Запрос.УстановитьПараметр("НадбавкаСуммой",Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой);  
	...
	ОбластьШапкаПослеНадбавок.Параметры.ШтатВКоличестве = НСтр("ru='штат в количестве ';uk='штат у кількості '")+ВсегоСтавокОрганизации+НСтр("ru=' единиц';uk=' одиниць'");
