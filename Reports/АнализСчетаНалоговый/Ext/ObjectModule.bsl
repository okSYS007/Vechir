...
Функция ЗаголовокОтчета() Экспорт
	Если ЗначениеЗаполнено(Счет) Тогда
		Возврат  НСтр("ru='Анализ счета ';uk='Аналіз рахунку '") + Счет + НСтр("ru=' (налоговый)';uk=' (податковий)'");		
	Иначе
		Возврат  НСтр("ru='Анализ счета <не выбран счет> (налоговый)';uk='Аналіз рахунку <не обрано рахунок> (податковий)'");		
	КонецЕсли;	
КонецФункции
...
Функция СформироватьЗаголовок() Экспорт
	...
	Если ПоВалютам Тогда
		...
		ТекстПроСписокПоказателей = ТекстПроСписокПоказателей + НСтр("ru=', валютная сумма';uk=', валютна сума'");
	
	КонецЕсли; 

	Если ПоКоличеству Тогда
	
		ТекстПроСписокПоказателей = ТекстПроСписокПоказателей + НСтр("ru=', количество';uk=', кількість'");
	
	КонецЕсли; 
	
	ТекстПроИтоги = "";
	Если ПоСубсчетамИСубконто Тогда
		ТекстПроИтоги = ТекстПроИтоги + НСтр("ru=', субсчетам';uk=', субрахункам'")
	КонецЕсли;

	Если ПостроительОтчета.ИзмеренияСтроки.Количество() > 0 Тогда
		ТекстПроИтоги = ТекстПроИтоги + НСтр("ru=', субконто: ';uk=', субконто: '") + УправлениеОтчетами.СформироватьСтрокуИзмерений(ПостроительОтчета.ИзмеренияСтроки);
	КонецЕсли;
	
	Если ПоКорСубсчетамИСубконто Тогда
		ТекстПроИтоги = ТекстПроИтоги + НСтр("ru=', кор.субсчетам и субконто';uk=', кор.субрахункам і субконто'")
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	Если Лев(Выборка.Группировка(), 8) = "Субконто" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, НСтр("ru='Карточка счета ';uk='Картка рахунку '") + СчетРасшифровки);
	...
	ИначеЕсли Выборка.Группировка() = "КорСчет" Тогда
		...
		СписокРасшифровкиДт.Добавить(ПараметрыРасшифровкиДт, НСтр("ru='Отчет по проводкам ';uk='Звіт по проводках'") + СчетРасшифровки);
		...
		СписокРасшифровкиКт.Добавить(ПараметрыРасшифровкиКт, НСтр("ru='Отчет по проводкам ';uk='Звіт по проводках'") + СчетРасшифровки);
		...
		КонецЦикла;
		СписокРасшифровки.Добавить(ПараметрыРасшифровки, НСтр("ru='Отчет по проводкам ';uk='Звіт по проводках'") + СчетРасшифровки);
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Если Счет.Пустая() Тогда
		Предупреждение(НСтр("ru='Не выбран счет!';uk='Не обраний рахунок!'"), 60);
		Возврат;
	КонецЕсли;
	...
	ОтборПоВидамСубконто = БухгалтерскиеОтчеты.УстановитьОтборНаВидыСубконтоПоПостроителюОтчета(ПостроительОтчета, Счет, ЗапросОбороты);
	ОтборПоВидамСубконто = БухгалтерскиеОтчеты.УстановитьОтборНаВидыСубконтоПоПостроителюОтчета(ПостроительОтчета, Счет, ЗапросОстатки);

	ТекстОбороты = ТекстОбороты + "
	|ИЗ
	|	РегистрБухгалтерии."+ИмяРегистраБухгалтерии+".Обороты(&ДатаНач, &ДатаКон, "+Периодичность+", Счет В ИЕРАРХИИ (&Счет), "+ОтборПоВидамСубконто+", " + Сред(ТекстОтбор, 3) + ") КАК Таблица
	|";
	
	ТекстОстатки = ТекстОстатки + "
	|ИЗ
	|	РегистрБухгалтерии."+ИмяРегистраБухгалтерии+".ОстаткиИОбороты(&ДатаНач, &ДатаКон, "+Периодичность+", ДвиженияИГраницыПериода, счет В ИЕРАРХИИ (&счет), "+ОтборПоВидамСубконто+", " + Сред(ТекстОтбор, 3) + ") КАК Таблица
	|";
