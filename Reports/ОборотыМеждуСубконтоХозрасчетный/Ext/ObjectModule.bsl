...
Функция ЗаголовокОтчета() Экспорт
	Возврат НСтр("ru='Обороты между субконто';uk='Обороти між субконто'");
...
Функция ПолучитьОписаниеНастроек()
	...
	СтрокаОписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Анализируется %1 в корреспонденции с %2';uk='Аналізується %1 у кореспонденції з %2'"), Сред(СтрокаСубконто, 3), Сред(СтрокаКорСубконто, 3));
...
Функция СформироватьЗаголовок() Экспорт
	...
	Если ПостроительОтчета.ИзмеренияСтроки.Количество() > 0 Тогда
		ЗаголовокОтчета.Параметры.ТекстПроГруппировки = НСтр("ru='Группировки: ';uk='Групування: '") + УправлениеОтчетами.СформироватьСтрокуИзмерений(ПостроительОтчета.ИзмеренияСтроки);
	КонецЕсли;
...
Процедура УстановитьПараметрыЗапроса(Запрос)
	...
	БухгалтерскиеОтчеты.УдалитьНеЗаполненныеЭлементыМассива(МассивВидовСубконто);
	БухгалтерскиеОтчеты.УдалитьНеЗаполненныеЭлементыМассива(МассивКорВидовСубконто);
...
Функция ПолучитьРасшифровку(ОтборДляРасшифровки, БазоваяРасшифровка=Неопределено)
	...
	СписокРасшифровки.Добавить(Расшифровка, НСтр("ru='Отчет по проводкам';uk='Звіт по проводках'"));
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Если ВидыСубконто.Количество()=0 Тогда
		Предупреждение(НСтр("ru='Не указаны анализируемые виды субконто';uk='Не зазначені аналізовані види субконто'"), 60);
		Возврат;
	КонецЕсли;
	
	Если КорВидыСубконто.Количество()=0 Тогда
		Предупреждение(НСтр("ru='Не указаны анализируемые виды корреспондирующих субконто';uk='Не зазначені аналізовані види кореспондуючих субконто'"), 60);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Для каждого Элемент Из МассивКорВидовСубконто Цикл
		...
		Поле.ТипЗначения = Элемент.ТипЗначения;
		Поле.Представление = НСтр("ru='Кор ';uk='Кор '")+Элемент.Наименование;
	КонецЦикла;
	
	Поле = ПостроительОтчета.ДоступныеПоля.Найти("ВалютаКор");
	Поле.Представление = НСтр("ru='Кор Валюта';uk='Кор Валюта'");
