...
Функция ПроверитьНастройки(ЗаголовокДействия)
	...
	Если НЕ ЗначениеЗаполнено(мДатаОтчета) Тогда
		ЕстьОшибки = Истина;
		ТекстОшибки = НСтр("ru='Не настроена дата формирования отчета.';uk='Не настроєна дата формування звіту.'");
		#Если Клиент Тогда
			Предупреждение(ТекстОшибки,,ЗаголовокДействия);
		#Иначе
			ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,, ЗаголовокДействия);
		#КонецЕсли
	КонецЕсли; 
	
	Если Показатели.Строки.НайтиСтроки(Новый Структура("ЭтоГруппа", Ложь), Истина).Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не выбраны показатели отчета.';uk='Не вибрані показники звіту.'");
	...
	Если НЕ ОтборДинамикаПозитивная 
		...
		ТекстОшибки = НСтр("ru='Не установлен отбор по динамике.';uk='Не встановлений відбір по динаміці.'");
		ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,ЕстьОшибки, ЗаголовокДействия);
	КонецЕсли; 
	...
	Если НЕ ОтборСостояниеХорошее 
		...
		ТекстОшибки = НСтр("ru='Не установлен отбор по состоянию.';uk='Не встановлений відбір по стану.'");
		ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки,ЕстьОшибки, ЗаголовокДействия);
	КонецЕсли; 
...
Процедура СохранитьНастройку() Экспорт
	...
	//СтруктураНастроек.Удалить("ТекДата"); // Дату не сохраняем
...
Процедура ПроверитьЗначенияПараметровПоказателя(ЗначенияНастроек, ПараметрыПанели, Отказ) Экспорт
	...
	Если ЗначенияНастроек.Свойство("СтандартнаяДатаНачала") Тогда
		Если НЕ ЗначениеЗаполнено(ЗначенияНастроек.СтандартнаяДатаНачала.Дата) Тогда
			СтрокаПредупреждения = СтрокаПредупреждения + НСтр("ru='"
"- дата формирования отчета';uk='"
"- дата формування звіту'");
		КонецЕсли; 
	КонецЕсли; 
	
	ПараметрыДанных = ЗначенияНастроек.НастройкиКомпоновщика.ПараметрыДанных;
	ПроверитьЗначениеПараметра("СмещениеПериода", НСтр("ru='Смещение периода';uk='Зміщення періоду'"), ПараметрыДанных.Элементы, СтрокаПредупреждения);
	ПроверитьЗначениеПараметра("Горизонт", "Горизонт", ПараметрыДанных.Элементы, СтрокаПредупреждения);
	ПроверитьЗначениеПараметра("Периодичность", НСтр("ru='Периодичность';uk='Періодичність'"), ПараметрыДанных.Элементы, СтрокаПредупреждения);
	
	Если СтрокаПредупреждения <> "" Тогда
		Отказ = Истина;
		Предупреждение(НСтр("ru='Необходимо заполнить параметры:';uk='Необхідно заповнити параметри:'") + СтрокаПредупреждения,, НСтр("ru='Формирование отчета';uk='Формування звіту'"));
	КонецЕсли; 
	...
	Для НомерСтроки = 1 По Результат.ВысотаТаблицы Цикл
		Если КолонкаИндикаторТренда <> Неопределено Тогда
			...
			Если ОбластьТренд.Текст = КодИндикаторДинамикаНегативная Тогда
				...
				РисунокТрендНисходящий.Высота = РисунокТрендНисходящий.Высота - СмещениеВысоты;
				ОбластьТренд.Текст = "";
			ИначеЕсли ОбластьТренд.Текст = КодИндикаторДинамикаСтабильная Тогда
				...
				РисунокТрендСтабильный.Высота = РисунокТрендСтабильный.Высота - СмещениеВысоты;
				ОбластьТренд.Текст = "";
			ИначеЕсли ОбластьТренд.Текст = КодИндикаторДинамикаПозитивная Тогда
				...
				РисунокТрендВосходящий.Высота = РисунокТрендВосходящий.Высота - СмещениеВысоты;
				ОбластьТренд.Текст = "";
			КонецЕсли; 
		КонецЕсли; 
		...
		Если КолонкаИндикаторСостояния <> Неопределено Тогда
			...
			Если ОбластьСостояние.Текст = КодИндикаторСостояниеНегативное Тогда
				...
				РисунокСостояниеНегативное.Высота = РисунокСостояниеНегативное.Высота - СмещениеВысоты;
			...
			ИначеЕсли ОбластьСостояние.Текст = КодИндикаторСостояниеСтабильное Тогда
				...
				РисунокСостояниеСтабильное.Высота = РисунокСостояниеСтабильное.Высота - СмещениеВысоты;
				ОбластьСостояние.Текст = "";
			ИначеЕсли ОбластьСостояние.Текст = КодИндикаторСостояниеПозитивное Тогда
				...
				РисунокСостояниеПозитивное.Высота = РисунокСостояниеПозитивное.Высота - СмещениеВысоты;
				ОбластьСостояние.Текст = "";
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла; 
...
Функция ПолучитьЗначениеПараметраПоказателя(ПоказательОтчета, ПараметрОтчета) Экспорт
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПараметрПоказателя.Значение) Тогда
			Если ПараметрПоказателя.Параметр = "Периодичность"
...
Процедура ЗаполнитьПараметрыПоказателя(ЗначенияНастроек) Экспорт
	...
	Для каждого ЭлПараметрСКД Из СКД.Параметры Цикл
		...
		Если ЭлПараметрСКД.ОграничениеИспользования Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура СоздатьВычисляемыеПоляОтчета(ЕстьПоказательПлан)
	...
	НовоеВычисляемоеПоле.Заголовок = НСтр("ru='Изменение (абс)';uk='Зміна (абс)'");
	...
	НовоеВычисляемоеПоле.Заголовок = НСтр("ru='Изменение %';uk='Зміна %'");
	...
	Если ЕстьПоказательПлан Тогда
		...
		НовоеВычисляемоеПоле.Заголовок = НСтр("ru='Отклонение (абс)';uk='Відхилення (абс)'");
		НовоеВычисляемоеПоле.Выражение = "ТекущееЗначение - ЗначениеПлан";
		НовоеВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2));
		
		НовоеВычисляемоеПоле = СхемаКомпоновкиДанных.ВычисляемыеПоля.Добавить();
		НовоеВычисляемоеПоле.ПутьКДанным = "ПроцентВыполнения";
		НовоеВычисляемоеПоле.Заголовок = НСтр("ru='% Выполнения';uk='% Виконання'");
		НовоеВычисляемоеПоле.Выражение = 
		...
		НовоеВычисляемоеПоле.Заголовок = НСтр("ru='Отклонение %';uk='Відхилення %'");
		НовоеВычисляемоеПоле.Выражение = 
...
Процедура ДобавитьДополнительныеПоляВНаборДанных(НаборДанных)
	...
	НовоеПоле.Заголовок   = НСтр("ru='Порядок показателя';uk='Порядок показника'");
	...
	НовоеПоле.Заголовок   = НСтр("ru='Ключ показателя';uk='Ключ показника'");
	...
	НовоеПоле.Заголовок   = НСтр("ru='Группа показателей';uk='Група показників'");
	НовоеПоле.ТипЗначения = Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(100));
...
Процедура СоздатьСхемуКомпоновкиДанных() Экспорт
	...
	//НастройкиКомпоновкиОтчета = КомпоновщикНастроек.ПолучитьНастройки();
	//
	//ТиповыеОтчеты.ЗагрузитьВСхемуНастройкиКомпоновщика(СхемаКомпоновкиДанных, НастройкиКомпоновкиОтчета);
...
Процедура ИнициализацияОбъекта()
	...
	ОтборДинамикаВосходящая = Истина;
	ОтборДинамикаНисходящая = Истина;
	ОтборДинамикаСтабильная = Истина;
	ОтборСостояниеНегативное = Истина;
	ОтборСостояниеПозитивное = Истина;
	ОтборСостояниеСтабильное = Истина;
