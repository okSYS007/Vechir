...
Процедура ЗаполнитьНачальныеНастройки(ВосстановлениеНастройки = Ложь) Экспорт
	...
	мНазваниеОтчета = НСтр("ru='Оперативный товарный календарь';uk='Оперативний товарний календар'", КодЯзыкаПечать);
...
Функция СформироватьТекстЗапроса(Источник,КопияГруппировкиОтчета, Первый = Ложь)
	...
	Для Каждого СтрокаГруппировки Из КопияГруппировкиОтчета Цикл
		...
		Если Найти(ОписаниеПоляЗапроса, ".Свойство") > 0 Тогда
			...
			Иначе
				Предупреждение(НСтр("ru='Не найдено свойство ';uk='Не знайдена властивість '") + СтрокаГруппировки.ИмяПоля);
				ЕстьОшибки = Истина;
			КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Каждого Строка Из ТаблицаСписокВсехФильтров Цикл
		...
		Если НайденныеСтроки.Количество() = 1 Тогда
			...
			СтрФильтры=СтрФильтры		+", "+ЛокализацияПовтИсп.ПолучитьПереводТекста(НайденныеСтроки[0].ПредставлениеФильтра)+ " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(СтрТипФильтра) 
	...
	Для Каждого СтрокаПоказатели Из ПоказателиОтчета Цикл // Формирование списка итогов запроса.
		...
		СтрПоказатели = СтрПоказатели + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(СтрокаПоказатели.ПредставлениеПоказателя);
	КонецЦикла;
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			...
			ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Оперативный товарный календарь';uk='Оперативний товарний календар'", КодЯзыкаПечать);

			ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на ';uk='По стану замовлень на '", КодЯзыкаПечать) +Формат(ДатаОтчета,"ДЛФ=ДД");

			Если  ДатаОтчета = '00010101000000' Тогда
				ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на момент последнего движения ';uk='По стану замовлень на момент останнього руху '", КодЯзыкаПечать);
			Иначе
				ОбластьЗначение.Параметры.Период = НСтр("ru='По состоянию заказов на ';uk='По стану замовлень на '", КодЯзыкаПечать) + Формат(ДатаОтчета, "ДФ = ""дд.ММ.гггг""");
			КонецЕсли;
	...
	СтруктураПараметров.ЗначениеГруппировкиГор.Параметры.ПредставлениеГруппировки=НСтр("ru='Остаток на дату отчета';uk='Залишок на дату звіту'", КодЯзыкаПечать);
	ДокументРезультат.Присоединить(СтруктураПараметров.ЗначениеГруппировкиГор);

	СтруктураПараметров.ЗначениеГруппировкиГор.Параметры.ПредставлениеГруппировки=НСтр("ru='Итого';uk='Разом'", КодЯзыкаПечать);
	ДокументРезультат.Присоединить(СтруктураПараметров.ЗначениеГруппировкиГор);
