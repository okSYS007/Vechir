...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|	ПродажиКомпанииОбороты.НДСПрибыли                     КАК НДСПрибыли,
	|	ПродажиКомпанииОбороты.Выручка                        КАК Выручка,
	|	ПродажиКомпанииОбороты.НДСВыручки                     КАК НДСВыручки,
	...
	|		МАКСИМУМ(НДСПрибыли)                    КАК НДСПрибыли,
	|		МАКСИМУМ(Выручка)                       КАК Выручка,
	|		МАКСИМУМ(НДСВыручки)                    КАК НДСВыручки,
	...
	|					ИНАЧЕ (ПродажиОбороты.НДСОборот - ЦеныНоменклатуры.НДСЕдиницы * ПродажиОбороты.КоличествоОборот)
	|			  		КОНЕЦ)                                 	                 КАК НДСПрибыли,
	|			СУММА(ПродажиОбороты.СтоимостьОборот)                            КАК Выручка,
	|			СУММА(ПродажиОбороты.НДСОборот) 								 КАК НДСВыручки,
	...
	|					  ИНАЧЕ (СебестоимостьКомпанииОбороты.НДСОборот / СебестоимостьКомпанииОбороты.КоличествоОборот)
	...
	мСтруктураСоответствияИмен = Новый Структура("Контрагент, КонтактноеЛицо, ИсточникИнформацииПриОбращении, ДокументПродажи", "Контрагент", НСтр("ru='Контактное лицо события';uk='Контактна особа події'"), НСтр("ru='Источник информации при обращении покупателя';uk='Джерело інформації при зверненні покупця'"), НСтр("ru='Документ продажи';uk='Документ продажу'"));
...
Функция СформироватьСтрокуПериода() Экспорт
	...
	Иначе
		...
		Если ДатаНачала = '00010101000000' ИЛИ ДатаОкончания = '00010101000000' Тогда
			...
			ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '") + Формат(ДатаНачала, "ДФ = ""дд.ММ.гггг""; ДП = ""...""") 
							+ " - "      + Формат(ДатаОкончания, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");

		Иначе

			Если ДатаНачала <= ДатаОкончания Тогда
				ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '") + ПредставлениеПериода(НачалоДня(ДатаНачала), КонецДня(ДатаОкончания), "ФП = Истина");
			Иначе
...
СтрокаПоказателя.ПредставлениеПоказателя  = НСтр("ru='Сумма выручки в (';uk='Сума виторгу у ('", КодЯзыкаПечать) +  мВалютаУпрУчета + ")";
СтрокаПоказателя = ПоказателиОтчета.Добавить();
СтрокаПоказателя.ИмяПоказателя = "ВыручкаБезНДС";
СтрокаПоказателя.ПредставлениеПоказателя  = НСтр("ru='Сумма выручки без НДС в (';uk='Сума виторгу без ПДВ у('", КодЯзыкаПечать) + мВалютаУпрУчета + ")";
СтрокаПоказателя = ПоказателиОтчета.Добавить();
СтрокаПоказателя.ИмяПоказателя = "Прибыль";
СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Сумма прибыли в (';uk='Сума прибутку у ('", КодЯзыкаПечать) + мВалютаУпрУчета + ")";
СтрокаПоказателя = ПоказателиОтчета.Добавить();
СтрокаПоказателя.ИмяПоказателя = "КоличествоРасходныхДокументов";
СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество продаж';uk='Кількість продажів'", КодЯзыкаПечать);
СтрокаПоказателя = ПоказателиОтчета.Добавить();
СтрокаПоказателя.ИмяПоказателя = "КоличествоПроданныхТоваров";
СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество проданных товаров';uk='Кількість проданих товарів'", КодЯзыкаПечать);
СтрокаПоказателя = ПоказателиОтчета.Добавить();
СтрокаПоказателя.ИмяПоказателя = "КоличествоСобытий";
СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество событий';uk='Кількість подій'", КодЯзыкаПечать);
