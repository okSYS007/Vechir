	...
	Иначе
		...
		Если НомерРелиза = Неопределено 
		 Или Не НомерРелиза = мНомерРелизаПараметровНастройки Тогда
			...
			Сообщить(НСтр("ru='Параметры настройки инициализированы значениями по умолчанию.';uk='Параметри настройки ініціалізовані значеннями по умовчанню.'"), СтатусСообщения.Информация);
		Иначе
...
Функция СоздатьИмяФайла(Расширение, ОтносительноеИмя = "") Экспорт
	...
	Если КаталогФормирования = "<Не выбран каталог>"
	 Или ПустаяСтрока(КаталогФормирования) Тогда
		...
		Если ФормироватьВФайл Тогда
			Сообщить(НСтр("ru='Рапорт сохранен в каталоге - ';uk='Рапорт збережений в каталозі - '") + ИмяФайла, СтатусСообщения.Информация);
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ИмяФайла = КаталогВременныхФайлов();
			Если ФормироватьВФайл Тогда
				Сообщить(НСтр("ru='Неверно указан каталог публикования рапорта!';uk='Невірно вказаний каталог публікування рапорту!'"), СтатусСообщения.ОченьВажное);
				Сообщить(НСтр("ru='Рапорт сохранен в каталоге - ';uk='Рапорт збережений в каталозі - '") + ИмяФайла, СтатусСообщения.Информация);
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьОтчет(СтрДействие, Параметры = 0) Экспорт
	...
	Если СтрДействие = "Формирование" Тогда
		...
		Секция.Параметры.ЗаголовокРапорта = НСтр("ru='Рапорт руководителю на ';uk='Рапорт керівнику на '", КодЯзыкаПечать) + Формат( ДатаКон, "ДЛФ=DD");
...
Процедура Продажи_ОбъемПродажФормирование()
	...
	Иначе
		...
		Сообщить(НСтр("ru='Не указан параметр <Периодичность> для показателя <Объем продаж>';uk='Не вказаний параметр <Періодичність> для показника <Обсяг продажів>'"), СтатусСообщения.Внимание);
		Возврат;
	...
	Если ТипСравнения = "План продаж" Тогда
		...
		Иначе
			...
			Сообщить(НСтр("ru='Не указан параметр <Периодичность планирования> для показателя <Объем продаж>';uk='Не вказаний параметр <Періодичність планування> для показника <Обсяг продажів>'"), СтатусСообщения.Внимание);
			Возврат;
	...
	Иначе
		Сообщить(НСтр("ru='Не указан  параметр <Показатель для сравнения> для показателя <Объем продаж>';uk='Не вказано параметр <Показник для порівняння> для показника <Обсяг продажів>'"), СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Продажи за ';uk='Продажі за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Продажи за период: ';uk='Продажі за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура ОборотныеСредства_ДенежныеСредстваФормирование()
	...
	//Если ВидОтчета = "Таблица" Тогда
		...
		ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти = НСтр("ru='Денежные средства';uk='Грошові кошти'", КодЯзыкаПечать);
...
Процедура ОборотныеСредства_ПодотчетФормирование()
	...
	ОбластьЗаголовокОбласти=Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти=НСтр("ru='Расчеты с подотчетными лицами';uk='Розрахунки з підзвітними особами'", КодЯзыкаПечать);
	ТабДок.Вывести(ОбластьЗаголовокОбласти);
	ТабДок.Вывести(Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
...
Процедура ОборотныеСредства_ДебиторскаяЗадолженностьФормирование()
	...
	ОбластьЗаголовокОбласти=Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти=НСтр("ru='Общая дебиторская задолженность';uk='Загальна дебіторська заборгованість'", КодЯзыкаПечать);
	ТабДок.Вывести(ОбластьЗаголовокОбласти);
	ТабДок.Вывести(Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
...
Процедура ОборотныеСредства_КредиторскаяЗадолженностьФормирование()
	...
	ОбластьЗаголовокОбласти=Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти=НСтр("ru='Общая кредиторская задолженность';uk='Загальна кредиторська заборгованість'", КодЯзыкаПечать);
	ТабДок.Вывести(ОбластьЗаголовокОбласти);
	ТабДок.Вывести(Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
...
Процедура ОборотныеСредства_ОстаткиТоваровФормирование()
	...
	ОбластьЗаголовокОбласти=Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти=НСтр("ru='Остатки товаров';uk='Залишки товарів'", КодЯзыкаПечать);
	ТабДок.Вывести(ОбластьЗаголовокОбласти);
	ТабДок.Вывести(Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
...
Процедура ОборотныеСредства_ОбщийИтогФормирование()
	...
	Макет = мМакет.ПолучитьОбласть("ОборотныеСредства");
	ОбластьЗаголовокОбласти = Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти = НСтр("ru='Итого оборотные средства';uk='Разом оборотні кошти'", КодЯзыкаПечать);
	ТабДок.Вывести( ОбластьЗаголовокОбласти);
	ТабДок.Вывести( Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СУММА(ОстаткиТоваровОстатки.КоличествоОстаток * ПартииОстаток.СтоимостьОстаток / ПартииОстаток.КоличествоОстаток) КАК Сумма
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОстаткиОпт.Склад КАК Склад,
	|		ОстаткиОпт.Номенклатура КАК Номенклатура,
	|		ОстаткиОпт.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
	|		ОстаткиОпт.СерияНоменклатуры КАК СерияНоменклатуры,
	|		ОстаткиОпт.КоличествоОстаток КАК КоличествоОстаток
	|	ИЗ
	|		РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаНач, ) КАК ОстаткиОпт
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ОстаткиРозница.Склад,
	|		ОстаткиРозница.Номенклатура,
	|		ОстаткиРозница.ХарактеристикаНоменклатуры,
	|		ОстаткиРозница.СерияНоменклатуры,
	|		ОстаткиРозница.КоличествоОстаток
	|	ИЗ
	|		РегистрНакопления.ТоварыВРознице.Остатки(&ДатаНач, ) КАК ОстаткиРозница) КАК ОстаткиТоваровОстатки
	...
	|		ИЗ
	|			РегистрНакопления.ПартииТоваровНаСкладах.Остатки(&ДатаНач, ) КАК ПартииТоваровНаСкладахОстатки
	|		ГДЕ
	|			ПартииТоваровНаСкладахОстатки.КоличествоОстаток <> 0) КАК ПартииОстаток
...
Процедура ОборотныеСредства_ИтогоКапиталФормирование()
	...
	Макет = мМакет.ПолучитьОбласть("ОборотныеСредства");
	ОбластьЗаголовокОбласти = Макет.ПолучитьОбласть("ЗаголовокОбласти");
	ОбластьЗаголовокОбласти.Параметры.ЗаголовокОбласти = НСтр("ru='Итого оборотный капитал';uk='Разом оборотний капітал'", КодЯзыкаПечать);
	ТабДок.Вывести( ОбластьЗаголовокОбласти);
	ТабДок.Вывести( Макет.ПолучитьОбласть("ЗаголовокТаблицы"));
...
Процедура ПоступленияИПлатежи_ПланируемыеПоступленияПлатежиРасшифровка(Параметры)
	...
	Если Форма.Открыта() Тогда
		...
		Ответ = Вопрос(НСтр("ru='Предыдущая форма расшифровки не была завершена."
"Завершить?';uk='Попередня форма розшифровки не була завершена."
"Завершити?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура Заказы_ЗакрытыеЗаказыФормирование()
	...
	Заголовок = НСтр("ru='Закрытые заказы за период: ';uk='Закриті замовлення за період: '", КодЯзыкаПечать) + Формат(ДатаНач, "ДФ=dd.MM.yyyy") + " - " + Формат(ДатаКон,  "ДФ=dd.MM.yyyy");
...
Процедура ДенежныеСредства_ОстатокСредствВКассахФормирование()
	...
	СтрПериод = НСтр("ru='Остаток денежных средств в кассах на ';uk='Залишок грошових коштів в касах на '", КодЯзыкаПечать) + Формат( ДатаКонцаПериода, "ДЛФ=DD");
...
Процедура ДенежныеСредства_ОстатокСредствНаСчетахФормирование()
	...
	СтрПериод = НСтр("ru='Остаток денежных средств на банковских счетах на ';uk='Залишок грошових коштів на банківських рахунках на '", КодЯзыкаПечать) + Формат( ДатаКонцаПериода, "ДЛФ=DD");
...
Процедура ДенежныеСредства_РасходПоСтатьямФормирование()
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Расход денежных средств в разрезе статей движения денежных средств за ';uk='Витрата грошових коштів в розрізі статей руху грошових коштів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Расход денежных средств в разрезе статей движения денежных средств за период: ';uk='Витрата грошових коштів в розрізі статей руху грошових коштів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
	...
	Если ПрошлыйПериод = "Да" Тогда
		...
		Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
			СтрПериод = НСтр("ru='Расход денежных средств в разрезе статей движения денежных средств за ';uk='Витрата грошових коштів в розрізі статей руху грошових коштів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПрошлогоПериода, ДатаКонцаПрошлогоПериода);
		Иначе
			СтрПериод = НСтр("ru='Расход денежных средств в разрезе статей движения денежных средств за период: ';uk='Витрата грошових коштів в розрізі статей руху грошових коштів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПрошлогоПериода, ДатаКонцаПрошлогоПериода);
		КонецЕсли;
...
Процедура ДенежныеСредства_ПриходПоСтатьямФормирование()
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Приход денежных средств в разрезе статей движения денежных средств за ';uk='Надходження грошових коштів в розрізі статей руху грошових коштів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Приход денежных средств в разрезе статей движения денежных средств за период: ';uk='Надходження грошових коштів в розрізі статей руху грошових коштів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
	...
	Если ПрошлыйПериод = "Да" Тогда
		...
		Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
			СтрПериод = НСтр("ru='Приход денежных средств в разрезе статей движения денежных средств за ';uk='Надходження грошових коштів в розрізі статей руху грошових коштів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПрошлогоПериода, ДатаКонцаПрошлогоПериода);
		Иначе
			СтрПериод = НСтр("ru='Приход денежных средств в разрезе статей движения денежных средств за период: ';uk='Надходження грошових коштів в розрізі статей руху грошових коштів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПрошлогоПериода, ДатаКонцаПрошлогоПериода);
		КонецЕсли;
...
Процедура Продажи_ОбъемОтгрузкиФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Продажи_ОбъемОтгрузки") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Объем отгрузки"" значение периодичности не может превышать период анализа';uk='Показник ""Обсяг відвантаження"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Отгрузка за ';uk='Відвантаження за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Отгрузка за период: ';uk='Відвантаження за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Продажи_ВаловаяПрибыльФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Продажи_ВаловаяПрибыль") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Валовая прибыль"" значение периодичности не может превышать период анализа';uk='Показник ""Валовий прибуток"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Валовая прибыль за ';uk='Валовий прибуток за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Валовая прибыль за период: ';uk='Валовий прибуток за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Продажи_ВаловаяПрибыльПроцентФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Продажи_ВаловаяПрибыльПроцент") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Валовая прибыль процентом"" значение периодичности не может превышать период анализа';uk='Показник ""Валовий прибуток відсотком"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Валовая прибыль в процентах за ';uk='Валовий прибуток у відсотках за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Валовая прибыль в процентах за период: ';uk='Валовий прибуток у відсотках за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Взаиморасчеты_ДебиторскаяЗадолженностьКонтрагентовФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ДебиторскаяЗадолженностьКонтрагентов") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Дебиторская задолженность контрагентов"" значение периодичности не может превышать период анализа';uk='Показник ""Дебіторська заборгованість контрагентів"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Дебиторская задолженность контрагентов за ';uk='Дебіторська заборгованість контрагентів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Дебиторская задолженность контрагентов за период: ';uk='Дебіторська заборгованість контрагентів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Взаиморасчеты_ДебиторскаяЗадолженностьКомпанииФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ДебиторскаяЗадолженностьКомпании") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Дебиторская задолженность компании"" значение периодичности не может превышать период анализа';uk='Показник ""Дебіторська заборгованість компанії"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
...
//попоравлено
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Дебиторская задолженность компании за ';uk='Дебіторська заборгованість компанії за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Дебиторская задолженность компании за период: ';uk='Дебіторська заборгованість компанії за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Заказы_НеисполненныеЗаказыПокупателейФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Заказы_НеисполненныеЗаказыПокупателей") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Количество неисполненных заказов покупателей"" значение периодичности не может превышать период анализа';uk='Показник ""Кількість невиконаних замовлень покупців"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
...
//Ничего не изменилось
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Количество неисполненных заказов покупателей за ';uk='Кількість невиконаних замовлень покупців за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Количество неисполненных заказов покупателей за период: ';uk='Кількість невиконаних замовлень покупців за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Заказы_НеисполненныеЗаказыПоставщикамФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Заказы_НеисполненныеЗаказыПоставщикам") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Количество неисполненных заказов поставщикам"" значение периодичности не может превышать период анализа';uk='Показник ""Кількість невиконаних замовлень постачальникам"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
...
//Ничего не изменилось
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Количество неисполненных заказов поставщикам за ';uk='Кількість невиконаних замовлень постачальникам за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Количество неисполненных заказов поставщикам за период: ';uk='Кількість невиконаних замовлень постачальникам за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Взаиморасчеты_ДебиторскаяЗадолженностьПодотчетниковФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ДебиторскаяЗадолженностьПодотчетников") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Дебиторская задолженность подотчетников"" значение периодичности не может превышать период анализа';uk='Показник ""Дебіторська заборгованість підзвітних осіб"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Дебиторская задолженность подотчетников за ';uk='Дебіторська заборгованість підзвітних осіб за  '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Дебиторская задолженность подотчетников за период: ';uk='Дебіторська заборгованість підзвітних осіб за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Взаиморасчеты_ДебиторскаяЗадолженностьПодотчетниковРасшифровка(Параметры)
	...
	Если Форма.Открыта() Тогда
		Форма.Активизировать();
		Ответ = Вопрос(НСтр("ru='Предыдущая форма задолженности подотчетников не была завершена."
"Завершить?';uk='Попередня форма заборгованості підзвітних осіб не була завершена."
"Завершити?'"), РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Да);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			Форма.Закрыть();
		КонецЕсли;
	КонецЕсли;
...
Процедура Взаиморасчеты_ПросроченнаяЗадолженностьПодотчетниковФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ПросроченнаяЗадолженностьПодотчетников") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Просроченная дебиторская задолженность подотчетников"" значение периодичности не может превышать период анализа';uk='Показник ""Прострочена дебіторська заборгованість підзвітних осіб"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Просроченная дебиторская задолженность подотчетников за ';uk='Прострочена дебіторська заборгованість підзвітних осіб за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Просроченная дебиторская задолженность подотчетников за период: ';uk='Прострочена дебіторська заборгованість підзвітних осіб за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура СебестоимостьТоварныхЗапасовФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("СебестоимостьТоварныхЗапасов") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Себестоимость товарных запасов"" значение периодичности не может превышать период анализа';uk='Показник ""Собівартість товарних запасів"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Себестоимость товарных запасов за ';uk='Собівартість товарних запасів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Себестоимость товарных запасов за период: ';uk='Собівартість товарних запасів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура ЛиквидностьСкладаФормирование()
	...
	Если НЕ ЗначениеЗаполнено(ТипЦен) Тогда
		Сообщить(НСтр("ru='Показатель ""Оценка ликвидности склада"" не указан тип цен для оценки стоимости товарных запасов';uk='Показник ""Оцінка ліквідності складу"" не вказаний тип цін для оцінки вартості товарних запасів'"), СтатусСообщения.БезСтатуса);
		Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если СтруктураПараметровПоказателей.Свойство("ЛиквидностьСклада") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Оценка ликвидности склада"" значение периодичности не может превышать период анализа';uk='Показник ""Оцінка ліквідності складу"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Оценка ликвидности склада за ';uk='Оцінка ліквідності складу за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Оценка ликвидности склада за период: ';uk='Оцінка ліквідності складу за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура ЛиквидностьСкладаРасшифровка(Параметры)
	...
	Форма = Отчеты.СтоимостнаяОценкаСкладаВЦенахКомпании.ПолучитьФорму("ГлавнаяФорма", Параметры["ЭтаФорма"], "ЛиквидностьСкладаРасшифровка");
	Если Форма.Открыта() Тогда
		Форма.Активизировать();
		Ответ = Вопрос(НСтр("ru='Предыдущая форма расшифровки не была завершена."
"Завершить?';uk='Попередня форма розшифровки не була завершена."
"Завершити?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	...
	Форма.мНазваниеОтчета = "Стоимостная оценка склада в ценах компании";
	Форма.РаскрашиватьИзмерения      = Истина;
	Форма.ПоказыватьЗаголовок  = Ложь;
...
Процедура Взаиморасчеты_ЗадолженностьКомиссионеровФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ЗадолженностьКомиссионеров") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Задолженность комиссионеров"" значение периодичности не может превышать период анализа';uk='Показник ""Заборгованість комісіонерів"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Задолженность комиссионеров за ';uk='Заборгованість комісіонерів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Задолженность комиссионеров за период: ';uk='Заборгованість комісіонерів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура Взаиморасчеты_ПросроченнаяЗадолженностьКомиссионеровФормирование()
	...
	Если СтруктураПараметровПоказателей.Свойство("Взаиморасчеты_ПросроченнаяЗадолженностьКомиссионеров") Тогда
		...
		Если Поз1 > Поз2 Тогда
			Сообщить(НСтр("ru='Показатель ""Просроченная задолженность комиссионеров"" значение периодичности не может превышать период анализа';uk='Показник ""Прострочена заборгованість комісіонерів"" значення періодичності не може перевищувати період аналізу'"), СтатусСообщения.БезСтатуса);
			Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Просроченная задолженность комиссионеров за ';uk='Прострочена заборгованість комісіонерів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Просроченная задолженность комиссионеров за период: ';uk='Прострочена заборгованість комісіонерів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура КоэфПривлеченияПокупателейФормирование()
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Коэффициент удержания и привлечения покупателей за ';uk='Коефіцієнт утримання і залучення покупців за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Коэффициент удержания и привлечения покупателей за период: ';uk='Коефіцієнт утримання і залучення покупців за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
...
Процедура РейтингМенеджеровФормирование()
	...
	Если НЕ ЗначениеЗаполнено(КритерийОценки) Тогда
		Сообщить(НСтр("ru='Показатель ""Рейтинг лучших менеджеров"" не указан критерий оценки эффективности деятельности менеджеров';uk='Показник ""Рейтинг кращих менеджерів"" не вказаний критерій оцінки ефективності діяльності менеджерів'"), СтатусСообщения.БезСтатуса);
		Сообщить(НСтр("ru='Показатель не сформирован!';uk='Показник не сформований!'"), СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Рейтинг лучших менеджеров за ';uk='Рейтинг кращих менеджерів за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Рейтинг лучших менеджеров за период: ';uk='Рейтинг кращих менеджерів за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
	СтрПериод = СтрПериод + Символы.ПС + НСтр("ru='Критерий оценки: ';uk='Критерій оцінки: '", КодЯзыкаПечать) + КритерийОценки;
...
Процедура РейтингИсточниковИнформацииФормирование()
	...
	Если НачалоДня(ДатаНачалаПериода) = НачалоДня(ДатаКонцаПериода) Тогда
		СтрПериод = НСтр("ru='Рейтинг лучших источников информации за ';uk='Рейтинг кращих джерел інформації за '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	Иначе
		СтрПериод = НСтр("ru='Рейтинг лучших источников информации за период: ';uk='Рейтинг кращих джерел інформації за період: '", КодЯзыкаПечать) + ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода);
	КонецЕсли;
