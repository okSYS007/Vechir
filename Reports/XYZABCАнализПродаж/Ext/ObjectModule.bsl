...
Функция СформироватьСтрокуПериода() Экспорт
	...
	Иначе
		...
		Если мДатаНачала = '00010101000000' ИЛИ ДатаОкончания = '00010101000000' Тогда
			...
			ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '") + Формат(мДатаНачала, "ДФ = ""дд.ММ.гггг""; ДП = ""...""") 
							+ " - "      + Формат(ДатаОкончания, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");

		Иначе

			Если мДатаНачала <= ДатаОкончания Тогда
				ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '") + ПредставлениеПериода(НачалоДня(мДатаНачала), КонецДня(ДатаОкончания), "ФП = Истина");
			Иначе
...
Функция СформироватьСтрокуНазванияОтчета() Экспорт
	...
	КонецЦикла;
	СтрокаНазванияОтчета = СтрокаНазванияОтчета + НСтр("ru='-анализ продаж';uk='-аналіз продажів'");
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мСтруктураСоответствияИмен = Новый Структура("Номенклатура, Контрагент, МенеджерПокупателя, ДокументПродажи", "Номенклатура", "Контрагент", НСтр("ru='Менеджер покупателя';uk='Менеджер покупця'"), НСтр("ru='Расходный документ';uk='Видатковий документ'"));
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если Периодичность = Перечисления.Периодичность.ПустаяСсылка() Тогда
		Предупреждение(НСтр("ru='Укажите периодичность.';uk='Вкажіть періодичність.'"));
		Возврат;
	КонецЕсли; 
	...
	Если НЕ УправлениеОтчетами.ЗадатьПараметрыОтбораПоКатегориям(ПостроительОтчета, мСтруктураДляОтбораПоКатегориям) Тогда
		Предупреждение(НСтр("ru='По одной категории нельзя устанавливать несколько отборов ';uk='По одній категорії не можна встановлювати кілька відборів '", КодЯзыкаПечать));
		Возврат;
	КонецЕсли;
	...
	Секция.Параметры.СтрокаПараметров = НСтр("ru='Анализируемый объект: ';uk=""Об'єкт, що аналізується: """, КодЯзыкаПечать)+мСписокОбъектов.НайтиПоЗначению(ОбъектАнализа).Представление+", параметр: "+Показатели.Найти(ПараметрАнализа).Представление;
	Таб.Вывести(Секция);
	...
	Если НЕ ПустаяСтрока(СтрокаГруппировок) Тогда
		...
		СтрокаГруппировок = НСтр("ru='Группировки строк: ';uk='Групування рядків: '", КодЯзыкаПечать) + СтрокаГруппировок;
	...
	Если ОтборФлагКласс Тогда
		Если ОтборВидСравненияКласс = ВидСравнения.Равно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='XYZ-класс = ';uk='XYZ-клас = '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКласс);
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.НеРавно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='XYZ-класс <> ';uk='XYZ-клас <> '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКласс);
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.ВСписке Тогда
			СтрокаСписка = "";
			...
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='XYZ-класс в списке ';uk='XYZ-клас у списку '", КодЯзыкаПечать)+ СтрокаСписка;
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.НеВСписке Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ОтборЗначениеКласс Цикл
				Если НЕ ПустаяСтрока(СтрокаСписка) Тогда
				...
				СтрокаСписка = СтрокаСписка + СокрЛП(ЭлементСписка.Значение);
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='XYZ-класс не в списке ';uk='XYZ-клас не у списку '", КодЯзыкаПечать)+ СтрокаСписка;
		КонецЕсли;
	КонецЕсли; 
	...
	Если ОтборФлагКлассABC Тогда
		Если ОтборВидСравненияКлассABC = ВидСравнения.Равно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс = ';uk='АВС-клас = '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКлассABC);
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.НеРавно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс <> ';uk='АВС-клас <> '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКлассABC);
		ИначеЕсли ОтборВидСравненияКлассABC = ВидСравнения.ВСписке Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ОтборЗначениеКлассABC Цикл
				...
				СтрокаСписка = СтрокаСписка + СокрЛП(ЭлементСписка.Значение);
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс в списке ';uk='АВС-клас не у списку '", КодЯзыкаПечать)+ СтрокаСписка;
		ИначеЕсли ОтборВидСравненияКлассABC = ВидСравнения.НеВСписке Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ОтборЗначениеКлассABC Цикл
				Если НЕ ПустаяСтрока(СтрокаСписка) Тогда
				...
				СтрокаСписка = СтрокаСписка + СокрЛП(ЭлементСписка.Значение);
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс не в списке ';uk='АВС-клас не у списку '", КодЯзыкаПечать)+ СтрокаСписка;
	...
	Если НЕ ПустаяСтрока(СтрокаОтборов) Тогда
		ПоследняяСтрока = ПоследняяСтрока + 1;
		СтрокаОтборов = НСтр("ru='Отбор: ';uk='Відбір: '", КодЯзыкаПечать) + СтрокаОтборов;
	...
	Если НЕ ПустаяСтрока(СтрокаПорядка) Тогда
		ПоследняяСтрока = ПоследняяСтрока + 1;
		СтрокаПорядка = НСтр("ru='Сортировка: ';uk='Сортування: '", КодЯзыкаПечать) + СтрокаПорядка;
	...
	Если ГруппировкаОбъекта = Неопределено Тогда
		...
		Предупреждение(НСтр( "ru='Группировка по объекту анализа ""';uk='Группування по об'єкту аналізу ""'", КодЯзыкаПечать) + ПредставлениеОбъектаАнализа + НСтр( "ru='"", должна присутствовать обязательно!';uk='"", повинна бути присутня обов'язково!'", КодЯзыкаПечать));
		Возврат;
	Иначе
...
мСписокОбъектов.Добавить("МенеджерПокупателя", НСтр("ru='Менеджер покупателя';uk='Менеджер покупця'"));
...
НовыйПоказатель.Представление = НСтр("ru='Сумма выручки в валюте упр.учета (';uk='Сума виручки в іноземній валюті упр.обліку ('", КодЯзыкаПечать) + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";

НовыйПоказатель = Показатели.Добавить();
НовыйПоказатель.Имя = "СуммаВаловойПрибыли";
НовыйПоказатель.Представление = НСтр("ru='Сумма валовой прибыли в валюте упр.учета (';uk='Сума валового прибутку у валюті упр.обліку ('", КодЯзыкаПечать) + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";

НовыйПоказатель = Показатели.Добавить();
НовыйПоказатель.Имя = "КоличествоПроданныхТоваров";
НовыйПоказатель.Представление = НСтр("ru='Количество проданных товаров';uk='Кількість проданих товарів'", КодЯзыкаПечать);      

НовыйПоказатель = Показатели.Добавить();
НовыйПоказатель.Имя = "КоличествоПродаж";
НовыйПоказатель.Представление = НСтр("ru='Количество продаж';uk='Кількість продажів'", КодЯзыкаПечать);
