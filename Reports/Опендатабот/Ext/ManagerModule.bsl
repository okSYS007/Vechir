...
Процедура ОткрытьФормуСервисаОпендатабот(ПередаваемыйПараметр) Экспорт
	
	Если Не ОпендатаботСервер.Опендатабот_Права() Тогда
		Возврат;
	КонецЕсли;
	ДанныеДляСтрокиПоиска = ПередаваемыйПараметр.СтрокаПоиска;
	ДанныеДляЗаголовка = ПередаваемыйПараметр.Наименование;
	Ссылка = ПередаваемыйПараметр.ССылка;
	

	ФормаОпендатабот = Отчеты.Опендатабот.ПолучитьФорму("Форма",,Ссылка);
	
	Параметры = Новый Структура;
	Параметры.Вставить("ДанныеДляСтрокиПоиска", ДанныеДляСтрокиПоиска);
	Параметры.Вставить("ДанныеДляЗаголовка", ДанныеДляЗаголовка);
	ФормаОпендатабот.Параметры = Параметры;
	ФормаОпендатабот.Открыть();
	
КонецПроцедуры

Процедура ВыполнитьДействиеПриЗакрытии() Экспорт
...
Функция ФормированиеОтчета(ПараметрыОтчета) Экспорт
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыОтчета.СтрокаПоиска) Тогда
		...
		Если НЕ (ЭтоЮридическоеЛицо  ИЛИ ЭтоФОП) Тогда 
			ДанныеОтчета.ОписаниеОшибки = НСтр("ru='Некорректная длина кода контрагента';uk='Некоректна довжина коду контрагента.'");
