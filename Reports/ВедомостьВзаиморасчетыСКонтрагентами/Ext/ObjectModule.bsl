...
Процедура УстановитьТекстЗапроса(ЕстьПолеРегистратор = Истина)
	...
	|	ИЗ
	|		(ВЫБРАТЬ
	|			ВзаиморасчетыБезДокументов.Организация КАК Организация,
	|			ВзаиморасчетыБезДокументов.Контрагент КАК Контрагент,
	|			ВзаиморасчетыБезДокументов.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|			ВзаиморасчетыБезДокументов.Сделка КАК Сделка,
	|			НЕОПРЕДЕЛЕНО КАК ДокументРасчетовСКонтрагентом,
	|			ВзаиморасчетыБезДокументов.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|			ВзаиморасчетыБезДокументов.Регистратор КАК Регистратор,
	|			ВзаиморасчетыБезДокументов.Период КАК Период,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, ДЕНЬ) КАК ПериодДень,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, НЕДЕЛЯ) КАК ПериодНеделя,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, ДЕКАДА) КАК ПериодДекада,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, МЕСЯЦ) КАК ПериодМесяц,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, КВАРТАЛ) КАК ПериодКвартал,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, ПОЛУГОДИЕ) КАК ПериодПолугодие,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыБезДокументов.Период, ГОД) КАК ПериодГод,
	|			ВзаиморасчетыБезДокументов.СуммаВзаиморасчетовНачальныйОстаток КАК СуммаВзаиморасчетовНачальныйОстаток,
	|			ВзаиморасчетыБезДокументов.СуммаВзаиморасчетовКонечныйОстаток КАК СуммаВзаиморасчетовКонечныйОстаток,
	|			ВзаиморасчетыБезДокументов.СуммаВзаиморасчетовПриход КАК СуммаВзаиморасчетовПриход,
	|			ВзаиморасчетыБезДокументов.СуммаВзаиморасчетовРасход КАК СуммаВзаиморасчетовРасход,
	|			ВзаиморасчетыБезДокументов.СуммаУпрНачальныйОстаток КАК СуммаУпрНачальныйОстаток,
	|			ВзаиморасчетыБезДокументов.СуммаУпрКонечныйОстаток КАК СуммаУпрКонечныйОстаток,
	|			ВзаиморасчетыБезДокументов.СуммаУпрПриход КАК СуммаУпрПриход,
	|			ВзаиморасчетыБезДокументов.СуммаУпрРасход КАК СуммаУпрРасход
	|		{ВЫБРАТЬ
	|			Организация.* КАК Организация,
	|			Контрагент.* КАК Контрагент,
	|			ДоговорКонтрагента.* КАК ДоговорКонтрагента,
	...
	|			ВзаиморасчетыПоДокументам.ДоговорКонтрагента.ВалютаВзаиморасчетов,
	|			ВзаиморасчетыПоДокументам.Регистратор,
	|			ВзаиморасчетыПоДокументам.Период,
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, ДЕНЬ),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, НЕДЕЛЯ),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, ДЕКАДА),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, МЕСЯЦ),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, КВАРТАЛ),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, ПОЛУГОДИЕ),
	|			НАЧАЛОПЕРИОДА(ВзаиморасчетыПоДокументам.Период, ГОД),
	...
	Если УниверсальныйОтчет.ИспользоватьСвойстваИКатегории Тогда
		...
		УниверсальныйОтчет.ДобавитьСвойстваИКатегорииДляПоля("Взаиморасчеты.ДоговорКонтрагента", "ДоговорКонтрагента", НСтр("ru='Договор контрагента';uk='Договір контрагента'"), ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Справочник_ДоговорыКонтрагентов);
		УниверсальныйОтчет.ДобавитьСвойстваИКатегорииДляПоля("Взаиморасчеты.Сделка", "Сделка", НСтр("ru='Сделка';uk='Угода'"), ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Документы);
		УниверсальныйОтчет.ДобавитьСвойстваИКатегорииДляПоля("Взаиморасчеты.Сделка", "Сделка", НСтр("ru='Сделка';uk='Угода'"), ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Документ_ЗаказПокупателя);
		УниверсальныйОтчет.ДобавитьСвойстваИКатегорииДляПоля("Взаиморасчеты.ДокументРасчетовСКонтрагентом", "ДокументРасчетовСКонтрагентом", НСтр("ru='Документ расчетов с контрагентом';uk='Документ розрахунків з контрагентом'"), ПланыВидовХарактеристик.НазначенияСвойствКатегорийОбъектов.Документы);
...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ДокументРасчетовСКонтрагентом", НСтр("ru='Документ расчетов с контрагентом';uk='Документ розрахунків з контрагентом'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаВзаиморасчетовНачальныйОстаток", НСтр("ru='Сумма взаиморасчетов начальный остаток';uk='Сума взаєморозрахунків початковий залишок'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаВзаиморасчетовКонечныйОстаток", НСтр("ru='Сумма взаиморасчетов конечный остаток';uk='Сума взаєморозрахунків кінцевий залишок'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаВзаиморасчетовПриход", НСтр("ru='Сумма взаиморасчетов приход';uk='Сума взаєморозрахунків прибуток'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаВзаиморасчетовРасход", НСтр("ru='Сумма взаиморасчетов расход';uk='Сума взаєморозрахунків витрата'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаУпрНачальныйОстаток", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр + НСтр("ru=' (нач. ост.)';uk=' (поч. зал.)'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаУпрКонечныйОстаток", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр + НСтр("ru=' (кон. ост.)';uk=' (кін. зал.)'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаУпрПриход", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр + НСтр("ru=' (приход)';uk=' (прибуток)'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("СуммаУпрРасход", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр + НСтр("ru=' (расход)';uk=' (витрата)'"));
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ВалютаВзаиморасчетов", НСтр("ru='Валюта взаиморасчетов';uk='Валюта взаєморозрахунків'"));
	
	// Добавление показателей
	// Необходимо вызывать для каждого добавляемого показателя.
	// УниверсальныйОтчет.ДобавитьПоказатель(<ИмяПоказателя>, <ПредставлениеПоказателя>, <ВключенПоУмолчанию>, <Формат>, <ИмяГруппы>, <ПредставлениеГруппы>);
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаВзаиморасчетовНачальныйОстаток", НСтр("ru='нач. остаток';uk='поч. залишок'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаВзаиморасчетов", НСтр("ru='Сумма взаиморасчетов';uk='Сума взаєморозрахунків'"));
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаВзаиморасчетовПриход", НСтр("ru='приход';uk='прибуток'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаВзаиморасчетов", НСтр("ru='Сумма взаиморасчетов';uk='Сума взаєморозрахунків'"));
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаВзаиморасчетовРасход", НСтр("ru='расход';uk='витрата'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаВзаиморасчетов", НСтр("ru='Сумма взаиморасчетов';uk='Сума взаєморозрахунків'"));
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаВзаиморасчетовКонечныйОстаток",  НСтр("ru='кон. остаток';uk='кін. залишок'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаВзаиморасчетов", НСтр("ru='Сумма взаиморасчетов';uk='Сума взаєморозрахунків'"));
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаУпрНачальныйОстаток", НСтр("ru='нач. остаток';uk='поч. залишок'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаУпр", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр);
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаУпрПриход", НСтр("ru='приход';uk='прибуток'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаУпр", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр);
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаУпрРасход", НСтр("ru='расход';uk='витрата'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаУпр", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр);
	УниверсальныйОтчет.ДобавитьПоказатель("СуммаУпрКонечныйОстаток", НСтр("ru='кон. остаток';uk='кін. залишок'"), Истина, "ЧЦ=15; ЧДЦ=2", "СуммаУпр", НСтр("ru='Сумма ';uk='Сума '") + ВалютаУпр);
