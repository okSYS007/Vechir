...
Процедура ДобавитьОбластьЗаголовок(Макет)
	...
	Если ЛокализацияПовтИсп.ТестовыйРежим() Тогда
		Сообщить(НСтр("ru='Отчет ';uk='Звіт '") + мНазваниеОтчета);
	КонецЕсли; 
	...
	ИначеЕсли мРежимВводаПериода = 1 Тогда // на дату
		...
		Если ДатаКон = '00010101' Тогда
			...
			ОписаниеПериода = НСтр("ru='на ';uk='на '") + Формат(ТекущаяДата(), "ДП = ...");
	...
	Если НЕ ПустаяСтрока(ПредставлениеГруппировкиКолонок) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Группировки колонок:';uk='Групування колонок:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеГруппировкиКолонок;
	...
	Для каждого Отбор из ПостроительОтчета.Отбор Цикл
		...
		Если Отбор.Использование И Отбор.Имя <> "Периодичность" Тогда
			...
			Если Отбор.ВидСравнения = ВидСравнения.Интервал Тогда
                ...
                ПредставлениеОтборы = ПредставлениеОтборы + НСтр("ru=' > ';uk=' > '");
			...
			Для ИндексСтолбца = 3 По ИменованнаяОбластьАвтоматическогоМакетаИзмерение.ШиринаТаблицы Цикл
				...
				Если ЭтоПолеСвязаноСИзмерением(ВыбранноеПоле.ПутьКДанным, ИзмерениеСтроки.ПутьКДанным, ДляИерархии) Тогда
					...
					Если ЭтоОбластьДляВыводаПоля(ОбластьИзмеренияСтрокиАвтоматическогоМакета, ВыбранноеПоле.Имя, ВыбранноеПоле.ПутьКДанным) Тогда
                        ...
                        Если Положение = "Передгрупуванням" Тогда
                            Положение = "Передгруппировкой";
                        ИначеЕсли Положение = "Замістьгрупування" Тогда
                            Положение = "Вместогруппировки";
                        ИначеЕсли Положение = "Післягрупування" Тогда
...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	Для каждого Ресурс Из МетаданныеРегистра.Ресурсы Цикл
		...
		Иначе
			...
			ДобавитьПоказатель(Ресурс.Имя + "НачальныйОстаток", НСтр("ru='Нач. остаток';uk='Поч. залишок'"), Истина, "ЧЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.Разрядность, "ЧГ=0") + " ; ЧДЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти, "ЧГ=0"), Ресурс.Имя, Ресурс.Представление());
			ДобавитьПоказатель(Ресурс.Имя + "Приход", НСтр("ru='Приход';uk='Прибуток'"), Истина, "ЧЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.Разрядность, "ЧГ=0") + " ; ЧДЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти, "ЧГ=0"), Ресурс.Имя, Ресурс.Представление());
			ДобавитьПоказатель(Ресурс.Имя + "Расход", НСтр("ru='Расход';uk='Витрата'"), Истина, "ЧЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.Разрядность, "ЧГ=0") + " ; ЧДЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти, "ЧГ=0"), Ресурс.Имя, Ресурс.Представление());
			ДобавитьПоказатель(Ресурс.Имя + "КонечныйОстаток", НСтр("ru='Кон. остаток';uk='Кін. залишок'"), Истина, "ЧЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.Разрядность, "ЧГ=0") + " ; ЧДЦ = " + Формат(Ресурс.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти, "ЧГ=0"), Ресурс.Имя, Ресурс.Представление());
...
Процедура ДобавитьВТекстЗапросаДанныеОФизлице(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, Параметры = Неопределено, СоответствиеДанныхОФизлицеИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДатаРождения", НСтр("ru='Дата рождения';uk='Дата народження'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ИНН", НСтр("ru='ИНН';uk='ІПН'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "КодИМНС", НСтр("ru='Код ИМНС';uk='Код ІМНС'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Комментарий", НСтр("ru='Комментарий';uk='Коментар'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "СтраховойНомерПФР", НСтр("ru='Страховой номер ПФР';uk='Страховий номер ПФР'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Пол", НСтр("ru='Пол';uk='Стать'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "МестоРождения", НСтр("ru='Место рождения';uk='Місце народження'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ФиоПолное", "ФИО (полностью)");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ФиоКраткое", НСтр("ru='Фамилия И. О.';uk='Прізвище І. Б.'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Возраст", "Возраст");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "СтепеньРодства", НСтр("ru='Степень родства';uk='Ступінь споріднення'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ИмяРодственника", НСтр("ru='Имя родственника';uk=""Ім'я родича"""));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ГодРожденияРодственника", НСтр("ru='Год рождения родственника';uk='Рік народження родича'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ВозрастРодственника", НСтр("ru='Возраст родственника';uk='Вік родича'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ВидОбразования", НСтр("ru='Вид образования';uk='Вид освіти'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "УчебноеЗаведение", НСтр("ru='Учебное заведение';uk='Навчальний заклад'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Специальность", НСтр("ru='Специальность';uk='Спеціальність'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Диплом", НСтр("ru='Диплом';uk='Диплом'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ГодОкончания", НСтр("ru='Год окончания учебного заведения';uk='Рік закінчення навчального закладу'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Квалификация", НСтр("ru='Квалификация';uk='Кваліфікація'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ТД_Организация", НСтр("ru='Трудовая деятельность: организация';uk='Трудова діяльність: організація'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ТД_ДатаНачала", НСтр("ru='Трудовая деятельность: начало работы';uk='Трудова діяльність: початок роботи'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ТД_ДатаОкончания", НСтр("ru='Трудовая деятельность: окончание работы';uk='Трудова діяльність: закінчення роботи'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ТД_Должность", НСтр("ru='Трудовая деятельность: должность';uk='Трудова діяльність: посада'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Язык", НСтр("ru='Знание языков: язык';uk='Знання мов: мова'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "СтепеньЗнанияЯзыка", НСтр("ru='Знание языков: степень владения';uk='Знання мов: ступінь володіння'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументВид", НСтр("ru='Вид документа';uk='Вид документу'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументСерия", НСтр("ru='Серия документа';uk='Серія документа'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументНомер", НСтр("ru='Номер документа';uk='Номер документа'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументДатаВыдачи", НСтр("ru='Дата выдачи документа';uk='Дата видачі документа'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументКемВыдан", НСтр("ru='Кем выдан документ';uk='Ким виданий документ'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ДокументКодПодразделения", НСтр("ru='Код подразделения, выдавшего документ';uk='Код підрозділу, який видав документ'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Страна", НСтр("ru='Страна';uk='Країна'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "НеИмеетПравоНаПенсию", НСтр("ru='Не имеет права на пенсию';uk='Не має права на пенсію'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентомРФ", НСтр("ru='Не является налоговым резидентом РФ';uk='Не є податковим резидентом РФ'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ОтношениеКВоинскойОбязанности", НСтр("ru='Отношение к воинской обязанности';uk=""Відношення до військового обов'язку"""));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ОтношениеКВоинскомуУчету", НСтр("ru='Отношение к воинскому учету';uk='Відношення до військового обліку'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Годность", НСтр("ru='Годность к воинской службе';uk='Придатність до військової служби'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Звание", НСтр("ru='Воинское звание';uk='Військове звання'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ЗабронированОрганизацией", НСтр("ru='Забронирован организацией';uk='Заброньований організацією'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "НаличиеМобпредписания", НСтр("ru='Наличие мобпредписания';uk='Наявність мобприпису'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Военкомат", НСтр("ru='Военкомат';uk='Військкомат'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ВУС", НСтр("ru='ВУС';uk='ВОС'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "КатегорияЗапаса", НСтр("ru='Категория запаса';uk='Категорія запасу'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Состав", НСтр("ru='Состав (профиль)';uk='Склад (профіль)'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "НомерКомандыПартии", НСтр("ru='Номер команды, партии';uk='Номер команди, партії'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "СемейноеПоложение", НСтр("ru='Семейное положение';uk='Сімейний стан'"));
...
Процедура УстановитьПредставленияПолей(СтруктураПредставлениеПолей, ПостроительОтчета) Экспорт
	...
	СтруктураПредставлениеПолей.Вставить("Периодичность", НСтр("ru='Периодичность итогов';uk='Періодичність підсумків'"));
...
Процедура РасчетШириныКолонок(ТабличныйДокумент) Экспорт
	...
	Состояние(НСтр("ru='Выполняется расчет ширины колонок...';uk='Виконується розрахунок ширини колонок...'"));
...
Функция ПолучитьПоложениеВыбранногоПоля(Имя) Экспорт
	...
	ПоложениеПоУмолчанию = НСтр("ru='После группировки';uk='Після групування'");
...
Функция ДобавитьДополнительноеПоле(ПутьКДанным, Размещение = Неопределено, Положение = 3) Экспорт
	...
	Если Положение = 1 Тогда
		...
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, НСтр("ru='Перед группировкой';uk='Перед групуванням'"));
		
	ИначеЕсли Положение = 2 Тогда
		
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, НСтр("ru='Вместо группировки';uk='Замість групування'"));
		
	ИначеЕсли Положение = 3 Тогда
		
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, НСтр("ru='После группировки';uk='Після групування'"));
...
Процедура ДобавитьПанельБыстрыеОтборы(Объект, Форма) Экспорт
	...
	Если КоличествоБыстрыхОтборов = 0 Тогда
		...
		НадписьОтборыОтсутствуют.Заголовок = НСтр("ru='Отчет не содержит быстрых отборов';uk='Звіт не містить швидких відборів'");
			...
			Если Форма.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Найти("Подменю") <> Неопределено Тогда
				...
				Если ПодменюДействия.Кнопки.Найти("Справка") <> Неопределено Тогда
					...
					Если мДействиеКоманднаяПанельФормыПечать <> Неопределено Тогда
						...
						КнопкаПечать = ПодменюДействия.Кнопки.Вставить(ПодменюДействия.Кнопки.Индекс(ПодменюДействия.Кнопки.Справка), "Печать", ТипКнопкиКоманднойПанели.Действие, НСтр("ru='Печать...';uk='Друк...'"), мДействиеКоманднаяПанельФормыПечать);
				...
				Иначе
					...
					Если мДействиеКоманднаяПанельФормыПечать <> Неопределено Тогда
						...
						КнопкаПечать = ПодменюДействия.Кнопки.Добавить("Печать", ТипКнопкиКоманднойПанели.Действие, НСтр("ru='Печать...';uk='Друк...'"), мДействиеКоманднаяПанельФормыПечать);
						КнопкаПечать.Картинка = БиблиотекаКартинок.Печать;
						КнопкаПечать.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.P,, Истина);
...
Процедура УстановитьЗаголовокФормы(Объект, Отчет, Форма) Экспорт
	...
	Если Объект.мТекущаяНастройка <> Неопределено И Объект.мТекущаяНастройка.Свойство("НаименованиеНастройки") И Объект.мТекущаяНастройка.НаименованиеНастройки <> Неопределено Тогда
		...
		Форма.Заголовок = мНазваниеОтчета + НСтр("ru='. Настройка: ';uk='. Настройка: '") + СокрЛП(Объект.мТекущаяНастройка.НаименованиеНастройки);
