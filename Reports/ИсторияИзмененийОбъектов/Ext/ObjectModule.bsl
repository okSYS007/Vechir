...
Процедура СформироватьОтчет(ТЧОтчета, знач СписокВерсий) Экспорт
	...
	ОбщийШаблон.КодЯзыкаМакета = КодЯзыкаПечать;
	// 37490 >>
	...
	Для Каждого ЭлементМодРеквизит Из ТаблицаИзмененийРеквизитов Цикл
		Если ЭлементМодРеквизит._Модификация = Истина Тогда
			...
			Если ОписаниеРеквизита = Неопределено Тогда
				Если НаименованиеРеквизита = "Date" Тогда
					ОписаниеРеквизита = Новый Структура("Тип", Новый ОписаниеТипов("Дата"));
					
				ИначеЕсли НаименованиеРеквизита = "Code"
					Или НаименованиеРеквизита = "Number"
					Или НаименованиеРеквизита = "Description"
					Или НаименованиеРеквизита = "Name" Тогда
					
					ОписаниеРеквизита = Новый Структура("Тип", Новый ОписаниеТипов("Строка"));
					
				ИначеЕсли НаименованиеРеквизита = "Posted"
					  Или НаименованиеРеквизита = "DeletionMark"
					  Или НаименованиеРеквизита = "IsFolder" Тогда
					ОписаниеРеквизита = Новый Структура("Тип", Новый ОписаниеТипов("Булево"));
...
Функция ПолучитьСтрокуXMLПоНомеруВерсии(НомерВерсии)
	...
	Если ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка получения версии объекта из информационной базы';uk=""Помилка отримання версії об'єкта з інформаційної бази""");
	Иначе
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
		ВерсияОбъекта.Записать(ИмяВременногоФайла);
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
