...
Процедура ВывестиСтроку(Выборка, СтруктураПараметров, Номер)
	...
	Иначе
		...
		Если (СтруктураПараметров.НомерОбхода=1) И (ТипЗаписиВыборки = ТипЗаписиЗапроса.ОбщийИтог) Тогда 
			ЗначениеГруппировки=НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать); // Первый обход запроса по взаиморасчетам: записываются значения итогов.
		ИначеЕсли (СтруктураПараметров.НомерОбхода=1) И (СокрЛП("" + Выборка[ИмяГруппировки])=СокрЛП(""+Заказ)) Тогда
			ЗначениеГруппировки=НСтр("ru='Текущий заказ';uk='Поточне замовлення'", КодЯзыкаПечать); 	// Первый обход запроса по взаиморасчетам: выводятся
													// данные по текущему заказу.
		ИначеЕсли (СтруктураПараметров.НомерОбхода=2) И (ТипЗаписиВыборки = ТипЗаписиЗапроса.ОбщийИтог) Тогда 
			ЗначениеГруппировки=НСтр("ru='Другие заказы по договору';uk='Інші замовлення по договору'", КодЯзыкаПечать); 	// Второй обход запроса по взаиморасчетам:
	...
	Если ЗначениеГруппировки<>НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать) Тогда  // При первом обходе запроса по взаиморасчетам итоговая строка не выводится.

		Если (ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоГруппировке) И (ЗначениеГруппировки<>НСтр("ru='Текущий заказ';uk='Поточне замовлення'", КодЯзыкаПечать)) Тогда
			СдвигУровня = СтруктураПараметров.СтруктураСдвигУровняГруппировок[ИмяГруппировки];
		ИначеЕсли (ТипЗаписиВыборки=ТипЗаписиЗапроса.ОбщийИтог) ИЛИ (ЗначениеГруппировки=НСтр("ru='Текущий заказ';uk='Поточне замовлення'", КодЯзыкаПечать)) Тогда
			СдвигУровня=0;
		КонецЕсли;
	...
	Иначе
		...
		Если (ЗначениеГруппировки=НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать)) или (ЗначениеГруппировки=НСтр("ru='Текущий заказ';uk='Поточне замовлення'", КодЯзыкаПечать)) Тогда 
		...
		ИначеЕсли ЗначениеГруппировки=НСтр("ru='Другие заказы по договору';uk='Інші замовлення по договору'", КодЯзыкаПечать) Тогда
		...
		Если ЗначениеГруппировки=НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать) Тогда
		...
		Если (ЗначениеГруппировки=НСтр("ru='Текущий заказ';uk='Поточне замовлення'", КодЯзыкаПечать)) 
			ИЛИ (ЗначениеГруппировки=НСтр("ru='Другие заказы по договору';uk='Інші замовлення по договору'", КодЯзыкаПечать)) 
			ИЛИ (ЗначениеГруппировки=НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать))  Тогда
...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Состояние заказа по документу ';uk='Стан замовлення по документу '", КодЯзыкаПечать)
			+ Заказ+Символы.ПС+" на "
			+ ?(ДатаАнализа='00010101000000',НСтр("ru='момент последнего движения';uk='момент останнього руху'", КодЯзыкаПечать),Формат(ДатаАнализа,НСтр( "ru='ДФ=""дд ММММ гггг ''г.'' ЧЧ:мм:сс'""';uk='ДФ=""дд ММММ гггг ''г.'' ЧЧ:мм:сс'"";Л=uk_UA'", КодЯзыкаПечать)));
	...
	Если Не ТипЗаказа = "Внутренний" Тогда
		...
		ОбластьЗначение.Параметры.Контрагент = НСтр("ru='Контрагент: ';uk='Контрагент: '", КодЯзыкаПечать)+Заказ.Контрагент;
		ОбластьЗначение.Параметры.РасшифровкаКонтрагент=Заказ.Контрагент;
		ОбластьЗначение.Параметры.Договор = НСтр("ru='Договор контрагента: ';uk='Договір контрагента: '", КодЯзыкаПечать)+Заказ.ДоговорКонтрагента;
	...
	Если Не ТипЗаказа = "Внутренний" Тогда
		...
		ОбластьПоказатель.Параметры.ПредоплатаТекст = НСтр("ru='Предоплата  ';uk='Передплата '", КодЯзыкаПечать)
					+	Формат(Заказ.ДоговорКонтрагента.ПроцентПредоплаты,"ЧЦ=5; ЧДЦ=2; ЧН=""0,00""")+"%";
		ОбластьПоказатель.Параметры.ВалютаВзаиморасчетов =НСтр("ru='Валюта взаиморасчетов: ';uk='Валюта взаєморозрахунків: '", КодЯзыкаПечать)
		...
		СтруктураПараметров.ЗначениеГруппировки.Параметры.ЗначениеГруппировки = НСтр("ru='Всего';uk='Всього'", КодЯзыкаПечать);
