...
Функция СформироватьЗаголовок() Экспорт
	...
	Если Не ПустаяСтрока(СтрФильтры) Тогда
		ОбластьОтбор.Параметры.ТекстПроОтбор = НСтр("ru='Отбор: ';uk='Відбір: '") + СтрФильтры;
...
Функция ЗаголовокОтчета() Экспорт
	...
	Возврат НСтр("ru='Карточка субконто (налоговый)';uk='Картка субконто (податковий)'");
...
Процедура ПолучитьТекстЗапроса(Запрос)
	...
	Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	НАЧАЛОПЕРИОДА(ДвиженияССубконто.Период, " + ?(Период = "" или Период = "РЕГИСТРАТОР", "ДЕНЬ", Период)+") КАК НачПериода,
...
Функция ПараметрыОтчетаКорректны()
	...
	Если Субконто.Количество()=0 Тогда
		Предупреждение(НСтр("ru='Отчет не может быть построен. Не задан ни один вид субконто.';uk='Звіт не може бути побудований. Не заданий жоден вид субконто.'"), 60);
		Возврат Ложь;
	КонецЕсли;
	
	Для каждого стр Из Субконто Цикл
		Если НЕ ЗначениеЗаполнено(стр.ВидСубконто) Тогда
			Предупреждение(НСтр("ru='Отчет не может быть построен. Не задан вид субконто в строке настройки ';uk='Звіт не може бути побудований. Не заданий вид субконто в рядку настройки '")+стр.НомерСтроки+".", 60);
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка) Экспорт
	...
	ОблСальдо.Параметры.ОписательСальдо = НСтр("ru='Сальдо на ';uk='Сальдо на '") + формат(ДатаНач, "ДФ=dd.MM.yyyy");
	...
	ВывестиПодИтог(ДокументРезультат, ВыборкаОбщие, СтруктураПараметров, НСтр("ru='Итого за период';uk='Разом за період'"));
	
	ДокументРезультат.ЗакончитьАвтогруппировкуСтрок();
	
	// Вывод конечного сальдо
	ОблСальдоПараметры = ОблСальдо.Параметры;
	ОблСальдоПараметры.ОписательСальдо = НСтр("ru='Сальдо на ';uk='Сальдо на '") + формат(ДатаКон, "ДФ=dd.MM.yyyy");
