...
Функция СформироватьСтрокуПериода(КодЯзыкаПечать)
	...
	Если ДатаНачала = '00010101000000' И ДатаОкончания = '00010101000000' Тогда
		ОписаниеПериода     = ОписаниеПериода + НСтр("ru='Период анализа не установлен';uk='Період аналізу не встановлений'", КодЯзыкаПечать);
	Иначе
		Если ДатаНачала = '00010101000000' ИЛИ ДатаОкончания = '00010101000000' Тогда
			ОписаниеПериода = ОписаниеПериода + НСтр("ru='Период анализа: ';uk='Період аналізу: '", КодЯзыкаПечать) + Формат(ДатаНачала, "ДФ = ""дд.ММ.гггг""; ДП = ""...""") 
							+ " - "      + Формат(ДатаОкончания, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
		Иначе
			Если ДатаНачала <= ДатаОкончания Тогда
				ОписаниеПериода = ОписаниеПериода + НСтр("ru='Период анализа: ';uk='Період аналізу: '", КодЯзыкаПечать) + ПредставлениеПериода(НачалоДня(ДатаНачала), КонецДня(ДатаОкончания), "ФП = Истина");
			Иначе
...
Функция СобратьТекстЗапросаУсловий(ЗапросУсловий)
	...
	Если ГруппировкаДоговорКонтрагента Тогда
		...
		СписокГруппировок.Добавить("ДоговорКонтрагента", НСтр("ru='Договор контрагента';uk='Договір контрагента'"));
	КонецЕсли; 
	Если ГруппировкаДокументУсловий Тогда
		СтрокаГруппировок = Символы.ПС + СтрокаГруппировок + "ДокументУсловий" + ", ";
		СписокГруппировок.Добавить("ДокументУсловий", НСтр("ru='Документ условий';uk='Документ умов'"));
	КонецЕсли; 
	...
	Если ГруппировкаДокументВыполненияУсловий Тогда
		СписокГруппировок.Добавить("ДокументВыполненияУсловий", НСтр("ru='Документ выполнения условий';uk='Документ виконання умов'"));
	КонецЕсли; 
...
Процедура ЗаполнитьДокументыВыполненияУсловий(СтрокаДерева, ТаблицаЗапроса, ИндексСтроки, СписокГруппировок, ТаблицаЗапросаВыполненияУсловий, Знач ТаблицаДатВыполненияУсловийНоменклатуры, ТаблицаЗапросаВыполненияУсловийБезДокумента, ФлагВыводитьСтроки)
	...
	Если ДоговорДерева = Неопределено ИЛИ РодительСтрокСДатами = Неопределено Тогда
		Сообщить(НСтр("ru='Ошибка обработки';uk='Помилка обробки'"));
		Возврат;
	КонецЕсли; 
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Секция.Параметры.ПредставлениеСумма = НСтр("ru='Сумма, ';uk='Сума, '", КодЯзыкаПечать) + СокрЛП(глЗначениеПеременной("ВалютаУправленческогоУчета"));
	Таб.Вывести(Секция);
...
Поле = ДоступныеПоляОтбора.Добавить("ДоговорКонтрагента", НСтр("ru='Договор контрагента';uk='Договір контрагента'"), Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
Поле.Отбор = Истина;

Поле = ДоступныеПоляОтбора.Добавить("ДокументУсловий", НСтр("ru='Документ условий';uk='Документ умов'"), Новый ОписаниеТипов("ДокументСсылка.УсловияПоставокПоДоговорамКонтрагентов"));
Поле.Отбор = Истина;

Поле = ДоступныеПоляОтбора.Добавить("Номенклатура", НСтр("ru='Номенклатура условий';uk='Номенклатура умов'"), Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
Поле.Отбор = Истина;

Поле = ДоступныеПоляОтбора.Добавить("ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры условий';uk='Характеристика номенклатури умов'"), Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
Поле.Отбор = Истина;
