	...
	Если ЗначениеЗаполнено(Счет) Тогда
		...
		Если ПолнаяОбработка Тогда
			...
			Если Счет.НалоговыйУчет Тогда
				...
				НоваяСтрока.Представление  = "Налоговое назначение";
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = НСтр("ru='Оборотно-сальдовая ведомость по счету ';uk='Оборотно-сальдова відомість по рахунку '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + Счет + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
