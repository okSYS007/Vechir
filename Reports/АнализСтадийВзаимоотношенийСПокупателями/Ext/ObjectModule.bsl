...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мСтруктураСоответствияИмен = Новый Структура("Контрагент, , СтадияПокупателя, КлассПостоянногоПокупателя","Контрагент", НСтр("ru='Стадия взаимоотношений с покупателем';uk='Стадія взаємовідносин з покупцем'"), НСтр("ru='XYZ-класс постоянного покупателя';uk='XYZ-клас постійного покупця'"));
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Иначе
		ПредставлениеПараметра = НСтр("ru='Параметр распределения: ';uk='Параметр розподілу: '", КодЯзыкаПечать) + ПараметрРаспределения + НСтр("ru=', в валюте упр.учета (';uk=', у валюті упр.обліку ('", КодЯзыкаПечать) + СокрЛП(глЗначениеПеременной("ВалютаУправленческогоУчета").Наименование) + ")";
	КонецЕсли; 
