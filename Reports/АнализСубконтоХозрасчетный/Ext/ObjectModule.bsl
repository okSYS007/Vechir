...
Функция СформироватьЗаголовок() Экспорт
	...
	ТекстПроСписокРесурсов = НСтр("ru='Выводимые данные: сумма';uk='Виведені дані: сума'");
	
	Если ПоВалютам Тогда
	
		ТекстПроСписокРесурсов = ТекстПроСписокРесурсов + НСтр("ru=', валютная сумма';uk=', валютна сума'");
	
	КонецЕсли; 

	Если ПоКоличеству Тогда
	
		ТекстПроСписокРесурсов = ТекстПроСписокРесурсов + НСтр("ru=', количество';uk=', кількість'");
	...
	ТекстПроИтоги = НСтр("ru='Виды субконто: ';uk='Види субконто: '") + Сред(ТекстПроИтоги, 3);
...
Функция ЗаголовокОтчета() Экспорт
	Возврат НСтр("ru='Анализ субконто';uk='Аналіз субконто'");
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Если Субконто.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не указан ни один вид субконто';uk='Не зазначений жоден вид субконто'"), 60);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	Если Лев(Выборка.Группировка(), СтрДлина(Выборка.Группировка()) - 1) = "Субконто" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСубконто, НСтр("ru='Карточка субконто ';uk='Картка субконто '") + Выборка.Группировка());
	...
	ИначеЕсли Выборка.Группировка() = "Счет" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, НСтр("ru='Карточка счета ';uk='Картка рахунку '") + Выборка.Счет);
...
Процедура ВывестиСчета(Выборка, СтруктураПараметров, ОтборДляРасшифровки = Неопределено)
	...
	Пока ВыборкаПоСчетам.Следующий() Цикл
		...
		Если ПоСубсчетам ИЛИ (Уровень = СтруктураПараметров.МассивСубконто.Количество() И мИспользоватьКлассыСчетовВКачествеГрупп) Тогда
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если Не ПолеСчет = Неопределено Тогда
		...
		ПолеСчет.Представление = НСтр("ru='Счет';uk='Рахунок'");
