...
Процедура ИнициализацияПостроителяОтчета() Экспорт
	...
	мСтруктураПредставлениеПолей.Вставить("МестаХранения", НСтр("ru='Места хранения';uk='Місця зберігання'"));
	мСтруктураПредставлениеПолей.Вставить("Организация", НСтр("ru='Организация';uk='Організація'"));
	мСтруктураПредставлениеПолей.Вставить("Штрихкод", "Штрихкод");
	мСтруктураПредставлениеПолей.Вставить("КодНоменклатурыКонтрагента", НСтр("ru='Код поставщика';uk='Код постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("АртикулНоменклатурыКонтрагента", НСтр("ru='Артикул поставщика';uk='Артикул постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("НаименованиеНоменклатурыКонтрагента", НСтр("ru='Наименование поставщика';uk='Найменування постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("ШтрихКодНоменклатурыКонтрагента", НСтр("ru='Штрихкод поставщика';uk='Штрихкод постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("ЕдиницаНоменклатурыКонтрагента", НСтр("ru='Единица поставщика';uk='Одиниця постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("Проект", "Проект");
	мСтруктураПредставлениеПолей.Вставить("ЦенаПоставщика", НСтр("ru='Цена поставщика';uk='Ціна постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("ВалютаЦеныПоставщика", НСтр("ru='Валюта цены поставщика';uk='Валюта ціни постачальника'"));
	мСтруктураПредставлениеПолей.Вставить("ЗаказыПоставщикамБольшиеТиражи", НСтр("ru='Заказано поставщикам (большие тиражи)';uk='Замовлено постачальникам (великі тиражі)'"));
	мСтруктураПредставлениеПолей.Вставить("ЗаказаноПоставщикам", НСтр("ru='Заказано поставщикам';uk='Замовлено постачальникам'"));
...
Процедура УстановитьПредставленияКолонок(ТабличноеПоле) Экспорт
	...
	Для Индекс = 1 по КоличествоОстатки Цикл
		...
		ИначеЕсли ПустаяСтрока(Склады.Получить(Индекс - 1).Представление) Тогда
			...
			Колонки["Остатки_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='Склад: ';uk='Склад: '") + Склады.Получить(Индекс - 1).Склад.Наименование;
	...
	Для Индекс = 1 по КоличествоЦены Цикл
		...
		ИначеЕсли ПустаяСтрока(ТипыЦен.Получить(Индекс - 1).Представление) Тогда
			...
			Колонки["Цена_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='Цена: ';uk='Ціна: '") + ТипыЦен.Получить(Индекс - 1).ТипЦен.Наименование;
			Колонки["ВалютаЦены_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='Валюта: ';uk='Валюта: '") + ТипыЦен.Получить(Индекс - 1).ТипЦен.Наименование;
			
		Иначе
			
			Колонки["Цена_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = ТипыЦен.Получить(Индекс - 1).Представление;
			Колонки["ВалютаЦены_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='Валюта: ';uk='Валюта: '") + ТипыЦен.Получить(Индекс - 1).Представление;
	...
	Для Индекс = 1 по КоличествоПродажи Цикл
		...
		Иначе
			...
			ИначеЕсли ПериодичностьПродаж < 2 Тогда
				...
				Колонки["Продажи_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='С ';uk='З '") + Формат(ПериодНачало, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]) + Символы.ПС + НСтр("ru='по ';uk='по'") + Формат(ПериодОкончание, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]);
				Колонки["ПродажиПроцент_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0")].ТекстШапки = НСтр("ru='С ';uk='З '") + Формат(ПериодНачало, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]) + Символы.ПС + НСтр("ru='по ';uk='по'") + Формат(ПериодОкончание, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]) + "(%)";
