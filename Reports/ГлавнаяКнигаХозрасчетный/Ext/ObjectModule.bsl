...
Функция СформироватьЗаголовок() Экспорт
    ...
    ОписаниеПериода = БухгалтерскиеОтчеты.СформироватьСтрокуВыводаПараметровПоДатам(ДатаНач, ДатаКон, ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	...
	ТекстПроСписокПоказателей = НСтр("ru='Выводимые данные: сумма';uk='Виведені дані: сума'", КодЯзыкаПечать);
	...
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = НСтр("ru='Детализация по ';uk='Деталізація по '", КодЯзыкаПечать) + ТекстПроИтоги;
	КонецЕсли;
...
Функция ЗаголовокОтчета() Экспорт
	Возврат НСтр("ru='Главная книга';uk='Головна книга'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Функция ВыполнитьОбщийЗапрос()
	...
	Состояние(НСтр("ru='Выполнение запроса';uk='Виконання запиту'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	ФорматПериода = ПолучитьСтрокуФорматаПериода(Период,КодЯзыкаПечать);
...
Функция ПолучитьСтрокуФорматаПериода(Знач Период,КодЯзыкаПечать) Экспорт
...
Процедура ВывестиШапкуТаблицы(СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	НачалоЗаголовка.Параметры.ОписаниеСчета = НСтр("ru='Главная книга. Счет ';uk='Головна книга. Рахунок '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов())+Выборка.СчетПредставление+" """+Выборка.СчетНаименование+"""";
	Если ЗначениеЗаполнено(Период) Тогда
		НачалоЗаголовка.Параметры.Заголовок = НСтр("ru='Период';uk='Період'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	КонецЕсли;
...
Процедура ВывестиСчет(ОбщийИтог, СтруктураПараметров, ПоказыватьЗаголовок)
	...
	Состояние(НСтр("ru='Сбор оборотов по счету ';uk='Збір оборотів по рахунку '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов())+ОбщийИтог.СчетПредставление);
...
Функция ПолучитьСписокРасшифровок(ПараметрыРасшифровки, ОтчетПоПроводкам = Истина)
	...
	Если ПараметрыРасшифровки<>Неопределено Тогда
		...
		СписокРасшифровки.Добавить(КопияРасшифровки, НСтр("ru='Обороты счета ';uk='Обороти рахунку '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + СтрокаСчета);
		
		Если ОтчетПоПроводкам Тогда
			КопияРасшифровки = БухгалтерскиеОтчеты.СоздатьКопиюСоответствияСтруктуры(ПараметрыРасшифровки);
			КопияРасшифровки.Вставить("ИмяОбъекта","ОтчетПоПроводкам");
			СписокРасшифровки.Добавить(КопияРасшифровки, НСтр("ru='Отчет по проводкам ';uk='Звіт по проводках'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + СтрокаСчета);
		КонецЕсли;
