...
Процедура СформироватьВнешнийИсточникДанных(ДатаОтчета)Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПоложенныеЕжегодныеОтпускаРаботников.СрезПоследних(&ДатаОтчета, ) КАК ПоложенныеОтпуска
	               ...
	               |		ИЗ
	               |			РегистрСведений.РаботникиОрганизаций.СрезПоследних КАК Рабы
	               |		ГДЕ
	...
	|	ЕстьNULL(ОборотыОтпусков.ПоложеноДнейОтпуска,0) -  ЕстьNULL(ОборотыОтпусков.ИспользованоДнейОтпуска,0) - ЕстьNULL(ОборотыОтпусков.КомпенсированоДнейОтпуска,0) КАК ДоступныйОстатокОтпуска
