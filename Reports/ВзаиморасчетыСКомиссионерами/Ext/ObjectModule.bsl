...
Процедура ПроставитьЗначенияБыстрыхОтборов(ФормаБыстрыхОтборов) Экспорт
	...
	Для каждого Строки Из ФильтрыОтчета Цикл
		...
		Если    Строки.ТипФильтра = "РАВНО" 	ИЛИ Строки.ТипФильтра = "ДОРІВНЮЄ"
			ИЛИ Строки.ТипФильтра = "НЕ РАВНО" 	ИЛИ Строки.ТипФильтра = "НЕ ДОРІВНЮЄ" Тогда
...
Процедура ЗаполнитьНачальныеНастройки(ВосстановлениеНастройки = Ложь) Экспорт
	...
	мНазваниеОтчета = НСтр("ru='Взаиморасчеты с комиссионерами';uk='Взаєморозрахунки з комісіонерами'");
	...
	Для НСтр = 1 По ОбластьПоказателиВысота Цикл
		...
		Если Врег(СтдВалюта) = "УПР" Тогда
			СтрПоказатели.ПредставлениеПоля = СтрПоказатели.ПредставлениеПоля + " (" + мВалютаУправленческогоУчета + ")";
		ИначеЕсли Врег(СтдВалюта) = "РЕГЛ" Тогда
			СтрПоказатели.ПредставлениеПоля = СтрПоказатели.ПредставлениеПоля + " (" +мВалютаРегламентированногоУчета + ")";
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Запрос.Текст) Тогда
		...
		Состояние(НСтр("ru='Получение списка свойств';uk='Отримання списку властивостей'"));
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Договор взаиморасчетов';uk='Договір взаєморозрахунків'");
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Заказ покупателя';uk='Замовлення покупця'");
...
Функция СформироватьТекстЗапроса(Источник,КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей)
	...
	Для Каждого СтрокаГруппировки Из КопияГруппировкиОтчета Цикл
		...
		Если Найти(СтрокаГруппировки.ОписаниеГруппировки, ".Свойство") > 0 Тогда
			...
			Если ОписаниеПоляЗапроса<>"&НеВыводить" Тогда
				...
				Иначе
					Предупреждение(НСтр("ru='Не найдено свойство ';uk='Не знайдена властивість '") + СтрокаГруппировки.ИмяГруппировки);
					ЕстьОшибки = Истина;
				КонецЕсли;
	...
	Для Каждого СтрокаФильтры Из ТаблицаСписокВсехФильтров Цикл
		...
		Если Найти(ОписаниеФильтра, ".Свойство") > 0 Тогда
			...
			Иначе
				Предупреждение(НСтр("ru='Не найдено свойство ';uk='Не знайдена властивість '") + СтрокаФильтры.ИмяПоля);
				ЕстьОшибки = Истина;
			КонецЕсли;
		...
		ИначеЕсли Найти(ОписаниеФильтра, ".Категория") > 0 Тогда
			...
			Если НайдСтрока <> Неопределено Тогда
				...
				СтрокаИсточник.ИмяПоляОбъект = ИмяПоляОбъект;//?(ИмяПоляОбъект = "&НеВыводить",ИмяКлюча,ИмяПоляОбъект
				СтрокаИсточник.СтрокаПараметров = СтрокаИсточник.СтрокаПараметров + ",  &Параметр" + НомерФильтра;
		...
		Если    Врег(СтрокаФильтры.ТипФильтра) = "РАВНО" 				ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "ДОРІВНЮЄ"
			...
			ОператорСравнения = " В ";
		ИначеЕсли Врег(СтрокаФильтры.ТипФильтра) = "В СПИСКЕ ПО ИЕРАРХИИ" ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "В СПИСКУ ПО ІЄРАРХІЇ" Тогда
		...
		ИначеЕсли Врег(СтрокаФильтры.ТипФильтра) = "НЕ В СПИСКЕ ПО ИЕРАРХИИ" ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "НЕ В СПИСКУ ПО ІЄРАРХІЇ" Тогда
	...
	Если Источник="ТоварыПереданные.ОстаткиНачало" Тогда

		ТекстИсточника="
			|РегистрСведений.КурсыВалют.СрезПоследних(&ДатаКон, Валюта = &ВалютаУпрУчета) КАК УпрВалюты,
			|РегистрНакопления.ТоварыПереданные.Остатки(&ДатаНач, СтатусПередачи <> &ВозвратнаяТара";
	...
	ИначеЕсли Источник="ТоварыПереданные.ОстаткиКонец" Тогда
		
		ТекстИсточника="
			|РегистрСведений.КурсыВалют.СрезПоследних(&ДатаКон, Валюта = &ВалютаУпрУчета) КАК УпрВалюты,
			|РегистрНакопления.ТоварыПереданные.Остатки(&ДатаКон, СтатусПередачи <> &ВозвратнаяТара";
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Каждого Строка Из ГруппировкиОтчета цикл
		...
		СтрГруппировки=СтрГруппировки+ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка.ПредставлениеГруппировки);
	...
	Для Каждого Строка Из ТаблицаСписокВсехФильтров Цикл
		...
		Иначе
			...
			СтрФильтры=СтрФильтры+", "+ЛокализацияПовтИсп.ПолучитьПереводТекста(НайденныеСтроки[0].ПредставлениеФильтра)+" "+СтрТипФильтра+" [";
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				// Если не задано представление, берем его из имени
				...
				СтрФильтры=СтрФильтры+ЛокализацияПовтИсп.ПолучитьПереводТекста(НайденнаяСтрока.ЗначениеФильтра)+", ";
			КонецЦикла;
	...
	Если ПоказателиОтчета.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не выбрано ни одного показателя!';uk='Не обрано жодного показника!'"), 60);
		ЕстьОшибки = Истина;
		Возврат;
	КонецЕсли;
	...
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "&НеВыводить");//ТоварыПереданныеОстаткиИОбороты.Сделка
	...
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",              "&НеВыводить"); //ТоварыПереданныеОстаткиИОбороты.Номенклатура
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры","&НеВыводить");//ТоварыПереданныеОстаткиИОбороты.ХарактеристикаНоменклатуры
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",         "&НеВыводить");//ТоварыПереданныеОстаткиИОбороты.СерияНоменклатуры

	СтруктураДоступныхПоказателей.Вставить("КоличествоНачальныйОстаток","ТоварыПереданныеОстаткиИОбороты.КоличествоОстаток");
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстаток", "ТоварыПереданныеОстаткиИОбороты.КоличествоОстаток*ТоварыПереданныеОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ТоварыПереданныеОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйОстаток",     "ТоварыПереданныеОстаткиИОбороты.СуммаВзаиморасчетовОстаток
	...
	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("ТоварыПереданные.ОстаткиНачало",
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоКонечныйОстаток", "ТоварыПереданныеОстаткиИОбороты.КоличествоОстаток");
	СтруктураДоступныхПоказателей.Вставить("КоличествоПриход",          0);
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовНачальныйОстаток", "0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовКонечныйОстаток", "ТоварыПереданныеОстаткиИОбороты.КоличествоОстаток*ТоварыПереданныеОстаткиИОбороты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ТоварыПереданныеОстаткиИОбороты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент");
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовПриход",          "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйОстаток",     "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйОстаток",      "ТоварыПереданныеОстаткиИОбороты.СуммаВзаиморасчетовОстаток
	                                                                    |*(КурсыВалютСрезПоследних.Курс/КурсыВалютСрезПоследних.Кратность)
	                                                                    |/(УпрВалюты.Курс/УпрВалюты.Кратность)");
	СтруктураДоступныхПоказателей.Вставить("СуммаПриход",               "0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоРасход",          "0");
	...
	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("ТоварыПереданные.ОстаткиКонец",
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей);
	...
	СтруктураДоступныхГруппировок.Вставить("Контрагент",                "ТоварыПереданныеОбороты.Контрагент");
	...
	СтруктураДоступныхГруппировок.Вставить("Организация",               "ТоварыПереданныеОбороты.Организация");
	...
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "&НеВыводить"); //ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.Сделка
	...
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовРасход",          "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаРасход",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",        "ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.СуммаВзаиморасчетовОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",       "0");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаРасход",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",        "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "ВзаиморасчетыСКонтрагентамиОстаткиИОбороты.СуммаВзаиморасчетовОстаток");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",       "0");
	...
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "ВЫБОР КОГДА ОтчетКомиссионераОПродажахТовары.Ссылка.СделкаПоРеализации = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) ТОГДА НЕОПРЕДЕЛЕНО ИНАЧЕ ОтчетКомиссионераОПродажахТовары.Ссылка.СделкаПоРеализации КОНЕЦ");
	...
	Для Каждого СтрокаПоказатели Из ПоказателиОтчета Цикл // Формирование списка итогов запроса.
		...
		Если СтрокаПоказатели.Сортировка <> 0 Тогда
			...
			СтрокаУпорядочивания = СтрокаУпорядочивания + ": " + Нрег(ПредставлениеПоляСортировки) 
			                                            + ?(СтрокаПоказатели.Сортировка = 1,
														НСтр("ru=' по возрастанию';uk=' за зростанням'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()),
														НСтр("ru=' по убыванию';uk=' за спаданням'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		КонецЕсли;

		// Строка для шапки отчета
		СтрПоказатели = СтрПоказатели + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(СтрокаПоказатели.ПредставлениеПоказателя) + СтрокаУпорядочивания;
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Взаиморасчеты с комиссионерами';uk='Взаєморозрахунки з комісіонерами'", КодЯзыкаПечать);
			Если ДатаНач = '00010101000000' И ДатаКон = '00010101000000' Тогда
				ОбластьЗначение.Параметры.Период = НСтр("ru='Период: без ограничения ';uk='Період: без обмеження '", КодЯзыкаПечать) 
												+ ?(ТолькоПросроченные,НСтр("ru=', только просроченные авансы ';uk=', тільки прострочені аванси '", КодЯзыкаПечать),"")
												+ СтрОшибки;
			Иначе
				Если ДатаНач = '00010101000000' ИЛИ ДатаКон = '00010101000000' Тогда
					ОбластьЗначение.Параметры.Период = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + Формат(ДатаНач, "ДФ = ""дд.ММ.гггг""; ДП = ""без ограничения""") 
														  + " - " + Формат(ДатаКон, "ДФ = ""дд.ММ.гггг""; ДП = ""без ограничения""") 
														  + ?(ТолькоПросроченные,НСтр("ru=', только просроченные авансы ';uk=', тільки прострочені аванси '"),"")
														  + СтрОшибки;
				Иначе
					ОбластьЗначение.Параметры.Период = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) 	+ ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон))
																	+ ?(ТолькоПросроченные,НСтр("ru=', только просроченные авансы ';uk=', тільки прострочені аванси '"),"")
																	+ СтрОшибки;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли Префикс = "Середина" Тогда
			ОбластьЗначение.Параметры.Фильтры = НСтр("ru='Отбор: ';uk='Відбір: '", КодЯзыкаПечать) + СтрФильтры;
		ИначеЕсли Префикс = "Низ" Тогда
			ОбластьЗначение.Параметры.Группировки = НСтр("ru='Группировки: ';uk='Групування: '", КодЯзыкаПечать) + СтрГруппировки;
			ОбластьЗначение.Параметры.Показатели  = НСтр("ru='Показатели: ';uk='Показники: '", КодЯзыкаПечать)  + СтрПоказатели;
		КонецЕсли;
...
мСтруктураТиповФильтров.Вставить("ОдноИз", НСтр("ru='Одно из:';uk='Одне з:'"));
мСтруктураТиповФильтров.Вставить("ВсеКроме", НСтр("ru='Все, кроме:';uk='Всі, крім:'"));
...
мСтруктураНазванийПолейСортировки.Вставить("НачальныйДолг",    НСтр("ru='Долг нач.';uk='Борг поч.'"));
мСтруктураНазванийПолейСортировки.Вставить("КонечныйДолг",     НСтр("ru='Долг кон.';uk='Борг кін.'"));
мСтруктураНазванийПолейСортировки.Вставить("НачальныйОстаток", НСтр("ru='Остаток нач.';uk='Залишок поч.'"));
мСтруктураНазванийПолейСортировки.Вставить("КонечныйОстаток",  НСтр("ru='Остаток кон.';uk='Залишок кін.'"));
мСтруктураНазванийПолейСортировки.Вставить("Приход",           НСтр("ru='Передано';uk='Передано'"));
мСтруктураНазванийПолейСортировки.Вставить("Расход",           НСтр("ru='Реализовано';uk='Реалізовано'"));
мСтруктураНазванийПолейСортировки.Вставить("Оплата",           НСтр("ru='Оплачено';uk='Оплачено'"));
мСтруктураНазванийПолейСортировки.Вставить("Вознаграждение",    НСтр("ru='Вознагр.';uk='Винаг.'"));
