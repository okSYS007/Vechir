...
Процедура ПередВыводомОтчета(МакетКомпоновки) Экспорт
	...
	МассивИменМакетовГруппировкиСчет = Новый Массив;
	Для Каждого МакетГруппировки Из МакетГруппировкиСчет Цикл
		МассивИменМакетовГруппировкиСчет.Добавить(МакетГруппировки.Имя);
	КонецЦикла;
	
	МассивИменМакетовГруппировкиВалюта = Новый Массив;
	Для Каждого МакетГруппировки Из МакетГруппировкиВалюта Цикл
		МассивИменМакетовГруппировкиВалюта.Добавить(МакетГруппировки.Имя);
	КонецЦикла;
		
	ДанныеОтчета.Вставить("МакетВалюта"          , МассивИменМакетовГруппировкиВалюта);
	ДанныеОтчета.Вставить("МакетШапкиОтчета"     , МакетШапкиОтчета.Имя);
	ДанныеОтчета.Вставить("МакетСчет"            , МассивИменМакетовГруппировкиСчет);
	ДанныеОтчета.Вставить("МакетПодвал"          , МакетПодвалаОтчета.Имя);
	...
	СмещениеПоСтроке = МакетКомпоновки.Макеты[ДанныеОтчета.МакетШапкиОтчета].Макет[0].Ячейки.Количество() - 7;
	ДанныеОтчета.Вставить("СмещениеПоСтроке", СмещениеПоСтроке);
...
Процедура ПередВыводомЭлементаРезультата(МакетКомпоновки, ДанныеРасшифровки, ЭлементРезультата, Отказ = Ложь) Экспорт
	...
	Если ЭлементРезультата.ЗначенияПараметров.Количество() > 0
		...
		И ДанныеОтчета.МакетВалюта.Найти(ЭлементРезультата.Макет) <> Неопределено
		И ЭлементРезультата.ЗначенияПараметров.П1.Значение = Null Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если ДанныеОтчета.ПоказателиОтчета.ВалютнаяСумма.Значение
		И ДанныеОтчета.КоличествоПоказателей > 1 Тогда
		КоличествоПоказателей = ДанныеОтчета.КоличествоПоказателей - 1;
	Иначе
		КоличествоПоказателей = ДанныеОтчета.КоличествоПоказателей;
	КонецЕсли;
	...
	Если ЭлементРезультата.ЗначенияПараметров.Количество() > 0 
		И ЭлементРезультата.ЗначенияПараметров.Найти("П1") <> Неопределено
		И ЗначениеЗаполнено(ЭлементРезультата.Макет)
		И ДанныеОтчета.МакетШапкиОтчета <> ЭлементРезультата.Макет
		И ДанныеОтчета.МакетПодвал <> ЭлементРезультата.Макет Тогда
		
		//Накапливаем суммы по корневым счетам
		Если ДанныеОтчета.МакетСчет.Найти(ЭлементРезультата.Макет) <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(ЗначениеСчет.Родитель) И Не ЗначениеСчет.Забалансовый Тогда
				Для Индекс = 0 По КоличествоПоказателей - 1 Цикл
					Для ПодИндекс = 1 По 6 Цикл
						...
						Ячейка = СтрокаМакета.Ячейки[ПодИндекс + ДанныеОтчета.СмещениеПоСтроке];
	...
	ИначеЕсли ЭлементРезультата.Макет = ДанныеОтчета.МакетПодвал Тогда
		Для Индекс = 0 По КоличествоПоказателей - 1 Цикл


			Для ПодИндекс = 1 По 6 Цикл
				СтрокаМакета = МакетКомпоновки.Макеты[ДанныеОтчета.МакетПодвал].Макет[Индекс];
				Ячейка = СтрокаМакета.Ячейки[ПодИндекс + ДанныеОтчета.СмещениеПоСтроке];
				ИмяПараметра = Строка(Ячейка.Элементы[0].Значение);
				
				ЭлементРезультата.ЗначенияПараметров[ИмяПараметра].Значение = ДанныеОтчета.ЗначенияПоказателей[ПодИндекс - 1][Индекс];
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеОтчета.Группировка Цикл	
		...
		Если СубконтоДетализации.Количество() > 0 Тогда
			...
			Если Индекс > 1 Тогда
				Для ИндексПсевдонима = 1 По 4 Цикл
				...
				КонецЦикла;
			КонецЕсли;
			Для ИндексСубконто = СубконтоДетализации.Количество() + 1 По 4 Цикл
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = НСтр("ru='Оборотно-сальдовая ведомость';uk='Оборотно-сальдова відомість'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
