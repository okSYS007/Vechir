...
Функция СформироватьЗаголовок(МакетОтчета, КоличествоКолонок, ТолькоЗаголовок, ВыводитьДетальныеЗаписи)
	...
	ИначеЕсли мРежимВводаПериода =  1 Тогда   // на дату
		...
		Если ДатаКон = '00010101000000' Тогда
			...
			Если ТолькоЗаголовок Тогда
				ОписаниеПериода = НСтр("ru='на текущее время ';uk='на поточний час '", ЯзыкФормированияПечатныхФормОтчетов) + Формат(ТекущаяДата(), "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
			Иначе
	...
	ИначеЕсли мРежимВводаПериода = 2 Тогда   // месяц
		...
		Если ДатаНач = '00010101000000' Тогда
			ОписаниеПериода = НСтр("ru='Период не установлен';uk='Період не встановлений'", ЯзыкФормированияПечатныхФормОтчетов);
		Иначе
			ОписаниеПериода = "" + ПредставлениеПериода(НачалоМесяца(ДатаНач), КонецМесяца(ДатаНач), "ФП = Истина" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
		КонецЕсли;
	...
	ИначеЕсли мРежимВводаПериода = 3 Тогда   // квартал
		...
		Если ДатаНач = '00010101000000' Тогда
			ОписаниеПериода = НСтр("ru='Период не установлен';uk='Період не встановлений'", ЯзыкФормированияПечатныхФормОтчетов);
		Иначе
			ОписаниеПериода = "" + ПредставлениеПериода(НачалоКвартала(ДатаНач), КонецКвартала(ДатаНач), "ФП = Истина" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
		КонецЕсли;
	...
	ИначеЕсли мРежимВводаПериода = 4 Тогда   // год
		Если ДатаНач = '00010101000000' Тогда
			ОписаниеПериода = НСтр("ru='Период не установлен';uk='Період не встановлений'", ЯзыкФормированияПечатныхФормОтчетов);
		Иначе
			ОписаниеПериода = "" + ПредставлениеПериода(НачалоГода(ДатаНач), КонецГода(ДатаНач), "ФП = Истина" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
		КонецЕсли;
	...
	Для Сч=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		...
		СписокИзмерений = СписокИзмерений 
						  + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ИзмеренияСтроки[Сч].Представление)
						  + " "  + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.ИзмеренияСтроки[Сч].ТипИзмерения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы());
	...
	Для Сч=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		...
		Если ПостроительОтчета.Отбор[Сч].Использование И Не ПостроительОтчета.Отбор[Сч].Имя = "Периодичность" Тогда
			...
			СписокОтбор = СписокОтбор 
						  + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.Отбор[Сч].Представление)
			              + " "  + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.Отбор[Сч].ВидСравнения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) 
						  + " "  + ПостроительОтчета.Отбор[Сч].Значение;
	...
	ЗаголовокОтчета.Параметры.Измерения  = НСтр("ru='Итоги по: ';uk='Підсумки по: '", ЯзыкФормированияПечатныхФормОтчетов) + Сред(СписокИзмерений,2);
	...
	ЗаголовокОтчета.Параметры.Отбор      = НСтр("ru='Отбор:';uk='Відбір:'", ЯзыкФормированияПечатныхФормОтчетов) + Сред(СписокОтбор,2);
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь, УказыватьРасшифровки = Истина) Экспорт
	...
	ОбластьИтого = МакетОтчета.Область("ОбластьИтого");
	ОбластьИтого.Текст = НСтр("ru='ИТОГО:';uk='ВСЬОГО:'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
//					КонецЕсли;
	...
	Иначе
		...
		Если СтруктураПараметров.КоличествоКолонокГруппировок > 1 И
			...
			МакетКолонкаШапки.Область(2,1).Текст = НСтр("ru='Общий итог';uk='Загальний підсумок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
