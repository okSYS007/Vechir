...
//   Строка
//
Функция СформироватьСтрокуПериода(КодЯзыкаПечать)

	ОписаниеПериода = "";
	
	Если ДатаОтчета = '00010101000000' Тогда
		ОписаниеПериода = НСтр("ru='Дата анализа потребностей: ';uk='Дата аналізу потреб: '", КодЯзыкаПечать) + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy") + ", ";
	Иначе
		ОписаниеПериода = НСтр("ru='Дата анализа потребностей: ';uk='Дата аналізу потреб: '", КодЯзыкаПечать) + Формат(ДатаОтчета, "ДФ=dd.MM.yyyy") + ", ";
	КонецЕсли; 
	
	// Вывод заголовка, описателя периода и фильтров и заголовка
	ОписаниеПериода = ОписаниеПериода + НСтр("ru='Интервал анализа потребностей: ';uk='Інтервал аналізу потреб: '", КодЯзыкаПечать);
	Если Отбор.ДатаПотребности.Использование Тогда
		...
		ИначеЕсли Отбор.ДатаПотребности.ВидСравнения = ВидСравнения.ВСписке Тогда
			ОписаниеПериода = ОписаниеПериода + НСтр("ru='в списке ';uk='у списку '", КодЯзыкаПечать) + Строка(Отбор.ДатаПотребности.Значение);
		ИначеЕсли Отбор.ДатаПотребности.ВидСравнения = ВидСравнения.НеВСписке Тогда
			ОписаниеПериода = ОписаниеПериода + НСтр("ru='не в списке ';uk='не в списку '", КодЯзыкаПечать) + Строка(Отбор.ДатаПотребности.Значение);
		...
		ИначеЕсли Отбор.ДатаПотребности.ВидСравнения = ВидСравнения.Интервал
			...
			Если Отбор.ДатаПотребности.ЗначениеС > Отбор.ДатаПотребности.ЗначениеПо И ЗначениеЗаполнено(Отбор.ДатаПотребности.ЗначениеПо) Тогда
				ОписаниеПериода = ОписаниеПериода + НСтр("ru='Неправильно задан период анализа!';uk='Неправильно заданий період аналізу!'", КодЯзыкаПечать);
			Иначе
		...
		КонецЕсли;
	Иначе
		ОписаниеПериода = ОписаниеПериода + НСтр("ru='не установлен';uk='не встановлений'", КодЯзыкаПечать);
	КонецЕсли;
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Секция.Параметры.СтрокаИнтервал = СформироватьСтрокуПериода(КодЯзыкаПечать);
	Таб.Вывести(Секция);
	...
	Если Группировки.Количество() > 0 Тогда
		...
		Для каждого Группировка Из Группировки Цикл
			...
			КонецЕсли; 
			СтрокаГруппировок = СтрокаГруппировок + ЛокализацияПовтИсп.ПолучитьПереводТекста(Группировка.Представление);
		КонецЦикла; 
		
		Секция.Параметры.СтрокаГруппировок = НСтр("ru='Группировки строк: ';uk='Групування рядків: '", КодЯзыкаПечать) + СтрокаГруппировок;
		Таб.Вывести(Секция);
	...
	Если НЕ ПустаяСтрока(СтрокаОтборов) Тогда
		...
		Секция.Параметры.СтрокаОтборов = НСтр("ru='Отбор: ';uk='Відбір: '", КодЯзыкаПечать) + СтрокаОтборов;
		Таб.Вывести(Секция);
		мКоличествоВыведенныхСтрокЗаголовка = мКоличествоВыведенныхСтрокЗаголовка + 1;
	КонецЕсли; 
	...
	Если КопияГруппировок.Найти("ЗаказПоставщику", "Имя") = Неопределено Тогда
		...
		НоваяГруппировка.Представление = НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'", КодЯзыкаПечать);
	КонецЕсли; 
	...
	Для каждого СтрокаТекущегоУровня Из СтрокаПрошлогоУровня.Строки Цикл
		...
		Если ТипЗнч(ЗначениеГруппировки) = Тип("Дата") Тогда
			Если ЗначениеГруппировки = '00010101000000' Тогда
				СтрокаГруппировки = НСтр("ru='Не заявлено в плане потребности';uk='Не заявлено у плані потреби'");
			Иначе
		...
		ИначеЕсли ТипЗнч(ЗначениеГруппировки) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
			Если НЕ ЗначениеГруппировки.Пустая() Тогда
				СтрокаГруппировки = "Заказ поставщику №" + Строка(ЗначениеГруппировки.Номер) + НСтр("ru=' от ';uk=' від '") + Формат(ЗначениеГруппировки.Дата, "ДФ=dd.MM.yyyy") + НСтр("ru=' (поставка ';uk=' (поставка '") + ?(ЗначениеГруппировки.ДатаПоступления = '00010101000000', НСтр("ru='не указана';uk='не указана'"), Формат(ЗначениеГруппировки.ДатаПоступления, "ДФ=dd.MM.yyyy")) + ")";
			КонецЕсли; 
		ИначеЕсли ТипЗнч(ЗначениеГруппировки) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Если НЕ ЗначениеГруппировки.Пустая() Тогда
				СтрокаГруппировки = ЗначениеГруппировки.Метаданные().Синоним + " №" + Строка(ЗначениеГруппировки.Номер) + НСтр("ru=' от ';uk=' від '") + Формат(ЗначениеГруппировки.Дата, "ДФ=dd.MM.yyyy") + НСтр("ru=' (отгрузка ';uk=' (відвантаження '") + ?(ЗначениеГруппировки.ДатаОтгрузки = '00010101000000', НСтр("ru='не указана';uk='не указана'"), Формат(ЗначениеГруппировки.ДатаОтгрузки, "ДФ=dd.MM.yyyy")) + ")";
			КонецЕсли; 
		Иначе
...
Поле = ДоступныеПоля.Добавить("Номенклатура", НСтр("ru='Номенклатура потребности';uk='Номенклатура потреби'"), Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
Поле.Измерение = Истина;
Поле.Отбор = Истина;

Поле = ДоступныеПоля.Добавить("ДатаПотребности", НСтр("ru='Дата потребности';uk='Дата потреби'"), Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
Поле.Измерение = Истина;
Поле.Отбор = Истина;
...
Поле = ДоступныеПоля.Добавить("ЗаказПокупателя", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"), Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
Поле.Измерение = Истина;
Поле.Отбор = Истина;
...
Поле = ДоступныеПоля.Добавить("ЗаказПоставщику", НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"), Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
Поле.Измерение = Истина;
...
Поле = ДоступныеПоля.Добавить("ОтветственныйЗаПокупки", НСтр("ru='Ответственный за покупки';uk='Відповідальний за купівлі'"), Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
Поле.Измерение = Истина;
Поле.Отбор = Истина;
