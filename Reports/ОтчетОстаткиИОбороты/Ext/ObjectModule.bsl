	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если ПостроительОтчета.ДоступныеПоля.Найти("Регистратор")<>Неопределено Тогда
			...
			Если ПостроительОтчета.ВыбранныеПоля.Найти("Регистратор")=Неопределено Тогда
				...
				СписокВыбора.Добавить(0, НСтр("ru='По документам движения';uk='По документах руху'"));
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь)	Экспорт
	...
	Если ВыводитьДетальныеЗаписи Тогда
		...
		Для Каждого Инд ИЗ МассивИндексовНезависимыхВыбранныхПолей Цикл
			...
			МакетОтчета.Область(ОбластьШапкаТаблицыНиз.Низ,1+1+Сч).Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ЗаголовокКолонки);
	...
	Заголовок = ЛокализацияПовтИсп.ПолучитьПереводТекста(мНазваниеОтчета);
	...
	Для Сч=0 По ПостроительОтчета.ВыбранныеПоля.Количество()-1 Цикл
		...
		Для Инд = 0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
			...
			Если ПостроительОтчета.ВыбранныеПоля[Сч].ПутьКДанным <> ПостроительОтчета.ИзмеренияСтроки[Инд].ПутьКДанным
				...
				СписокКолонок = СписокКолонок +", "+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ВыбранныеПоля[Сч].Представление);
		...
		Если ВыводитьДетальныеЗаписи Тогда
			...
			СписокПолейДетальныхЗаписей = СписокПолейДетальныхЗаписей +", "+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ВыбранныеПоля[Сч].Представление);
	...
	Для Сч=0 По Показатели.Количество()-1 Цикл
		...
		Если Показатели[Сч].Использование Тогда
			...
			СписокПоказателей = СписокПоказателей +", "+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Показатели[Сч].Представление);
	...
	ЗаголовокОтчета.Параметры.Период     = НСтр("ru='Период: ';uk='Період: '", ЯзыкФормированияПечатныхФормОтчетов) + ОписаниеПериода;
	...
	Если НЕ ПустаяСтрока(СтрокаГруппировок) Тогда
		СтрокаГруппировок = НСтр("ru='Группировки строк: ';uk='Групування рядків: '", ЯзыкФормированияПечатныхФормОтчетов) + СтрокаГруппировок;
	КОнецЕсли;
	ЗаголовокОтчета.Параметры.Измерения  = СтрокаГруппировок;

	ЗаголовокОтчета.Параметры.Показатели = НСтр("ru='Показатели: ';uk='Показники: '", ЯзыкФормированияПечатныхФормОтчетов) + Сред(СписокПоказателей,2);
	
	ЗаголовокОтчета.Параметры.Колонки = 
	?((ПостроительОтчета.ИзмеренияСтроки.Количество()>0 И Не ПустаяСтрока(СписокКолонок)), НСтр("ru='Дополнительные поля: ';uk='Додаткові поля: '", ЯзыкФормированияПечатныхФормОтчетов) + Сред(СписокКолонок, 2)+" ", "") 
	+ ?(ВыводитьДетальныеЗаписи, НСтр("ru='Поля детальных записей: ';uk='Поля детальних записів: '", ЯзыкФормированияПечатныхФормОтчетов) + Сред(СписокПолейДетальныхЗаписей, 2),"");
	...
	Если НЕ ПустаяСтрока(СтрокаОтборов) Тогда
		СтрокаОтборов = НСтр("ru='Отбор: ';uk='Відбір: '", ЯзыкФормированияПечатныхФормОтчетов) + СтрокаОтборов;
	КонецЕсли; 
	...
	Если ВыводитьПоказателиВСтроку Тогда
		...
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+0*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1*КоличествоПоказателей).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+0*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1*КоличествоПоказателей).Текст, ЯзыкФормированияПечатныхФормОтчетов);
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+1*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+2*КоличествоПоказателей).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+1*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+2*КоличествоПоказателей).Текст, ЯзыкФормированияПечатныхФормОтчетов);
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+2*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+3*КоличествоПоказателей).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+2*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+3*КоличествоПоказателей).Текст, ЯзыкФормированияПечатныхФормОтчетов);	
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+3*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+4*КоличествоПоказателей).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+1+3*КоличествоПоказателей, ОбластьШапкаТаблицы.Верх+1, 1+КоличествоКолонок+4*КоличествоПоказателей).Текст, ЯзыкФормированияПечатныхФормОтчетов);
		...
		Для Каждого Показатель Из Показатели Цикл
			Если Показатель.Использование = Истина Тогда
				...
				Если Стр <> Неопределено Тогда
					Представление = ЛокализацияПовтИсп.ПолучитьПереводТекста(Стр.ПредставлениеПоля);
				Иначе
		...
		КонецЦикла;
	Иначе
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+2, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+2).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+2, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+2).Текст, ЯзыкФормированияПечатныхФормОтчетов);
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+3, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+3).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+3, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+3).Текст, ЯзыкФормированияПечатныхФормОтчетов);
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+4, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+4).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+4, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+4).Текст, ЯзыкФормированияПечатныхФормОтчетов);	
		МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+5, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+5).Текст =
			ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+5, ОбластьШапкаТаблицы.Верх+1, КоличествоКолонок+5).Текст, ЯзыкФормированияПечатныхФормОтчетов);
	КонецЕсли;
	...
	МакетОтчета.Область(ОбластьОбщиеИтоги.Верх, 2, ОбластьОбщиеИтоги.Верх, 2).Текст = 
		ЛокализацияПовтИсп.ПолучитьПереводТекста(МакетОтчета.Область(ОбластьОбщиеИтоги.Верх, 2, ОбластьОбщиеИтоги.Верх, 2).Текст);
	...
	Если ПостроительОтчета.ИзмеренияСтроки.Количество()>0 Тогда
		...
		МакетШапкиТаблицы.Область(2,1+1).Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ЗаголовокКолонки);

	ИначеЕсли НЕ ВыводитьДетальныеЗаписи Тогда
		МакетШапкиТаблицы.Область(2,1+1).Текст = НСтр("ru='Общий итог';uk='Загальний підсумок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	КонецЕсли; 
...
СтруктураФорматаПолей.Вставить("ПериодНеделя",    НСтр("ru='ДФ = ''""Неделя с"" дд.ММ.гггг ''';uk='ДФ = ''""Тиждень з"" дд.ММ.гггг '''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
СтруктураФорматаПолей.Вставить("ПериодДекада",    НСтр("ru='ДФ = ''""Декада с"" дд.ММ.гггг ''';uk='ДФ = ''""Декада з"" дд.ММ.гггг '''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
СтруктураФорматаПолей.Вставить("ПериодМесяц",     НСтр("ru='ДФ = ''ММММ гггг ""г.""''';uk='ДФ = ''ММММ гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
СтруктураФорматаПолей.Вставить("ПериодКвартал",   НСтр("ru='ДФ = ''к ""квартал"" гггг ""г.""''';uk='ДФ = ''к ""квартал"" гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
СтруктураФорматаПолей.Вставить("ПериодПолугодие", НСтр("ru='ДФ = ''""Полугодие с"" дд.ММ.гггг''';uk='ДФ = ''""Півріччя з"" дд.ММ.гггг'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
СтруктураФорматаПолей.Вставить("ПериодГод",       НСтр("ru='ДФ = ''гггг ""г.""''';uk='ДФ = ''гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
