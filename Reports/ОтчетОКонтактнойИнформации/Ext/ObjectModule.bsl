...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если ОбъектОтчета = ТипЗнч(Неопределено) ИЛИ ОбъектОтчета = Неопределено Тогда
		Предупреждение(НСтр("ru='Задайте объект отчета.';uk=""Задайте об'єкт звіту."""));
		Возврат;
	КонецЕсли;
	...
	СтруктураПредставлениеПолей = Новый Структура("Вид, Объект", НСтр("ru='Вид котактной информации';uk='Вид котактной інформації'"), мСписокОбъектовОтчета.НайтиПоЗначению(ОбъектОтчета).Представление);
...
Процедура СформироватьОтчет(Таб, ТолькоЗаголовок = Ложь) Экспорт
	...
	СекцияШапка.Параметры.НаименованиеОтчета = НСтр("ru='Отчет о контактной информации, объект - ';uk=""Звіт про контактну інформацію, об'єкт - """, КодЯзыкаПечать) + ЛокализацияПовтИсп.ПолучитьПереводТекста(мСписокОбъектовОтчета.НайтиПоЗначению(ОбъектОтчета).Представление);
	Таб.Вывести(СекцияШапка);
	
	СекцияИнтервал = мМакет.ПолучитьОбласть("ШапкаИнтервал");
	СекцияИнтервал.Параметры.СтрокаИнтервал = НСтр("ru='Дата формирования отчета: ';uk='Дата формування звіту: '", КодЯзыкаПечать) + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy");
	...
	Если ПостроительОтчета.ИзмеренияСтроки.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Необходимо выбрать хотя бы одну группировку в отчете.';uk='Необхідно вибрати хоча б одне групування в звіті.'"));
		Возврат;
	КонецЕсли; 
	
	Если СписокВидовКИ.Количество() = 0 И НЕ ОбрабатыватьПерсональныеВидыКИ Тогда
		Предупреждение(НСтр("ru='Для формирования отчета необходимо отметить виды контактной информации в настройке отчета.';uk='Для формування звіту необхідно зазначити види контактної інформації в настройці звіту.'"));
		Возврат;
	КонецЕсли; 
	...
	Для каждого СтрокаТаблицы Из ВидыКонтактнойИнформации Цикл
		...
		Секция.Параметры.ИмяПоказателя = ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(СтрокаТаблицы.ВидКонтактнойИнформации.Тип)) + ": " + СокрЛП(СтрокаТаблицы.ВидКонтактнойИнформации);
	...
	Пока ВыборкаВидов.Следующий() Цикл
		...
		Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
			...
			Секция = мМакет.ПолучитьОбласть("ШапкаВидыКИ|Показатель");
			Секция.Параметры.ИмяПоказателя = ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ВыборкаДетальныхЗаписей.Тип)) + ": " + СокрЛП(ВыборкаДетальныхЗаписей.Вид);
			Таб.Присоединить(Секция);
			мКолВоСтолбов = мКолВоСтолбов + 1;
