...
//СПИСКИ ЗНАЧЕНИЙ
	...
	УдержанияОрганизаций = ПланыВидовРасчета.Удержания;
...
////////////////////////////////////////////////////////////////////////////////////
	...
	списокНачисленоКолонка[3].Добавить(ОсновныеНачисления.ПустаяСсылка());
	
	списокНачисленоКолонка[4] = Новый СписокЗначений; //оплата праці за час перебування у відрядженні
	списокНачисленоКолонка[4].Добавить(ОсновныеНачисления.Командировка);
	...
	списокНачисленоКолонка[10].Добавить(ОсновныеНачисления.ПустаяСсылка());
	...
	списокНачисленоКолонка[13].Добавить(ОсновныеНачисления.ПустаяСсылка());
	...
	списокНачисленоКолонка[18].Добавить(ОсновныеНачисления.ПустаяСсылка());	
	...
	списокНачисленоКолонка[36].Добавить(ОсновныеНачисления.ДоплатаДоСреднегоЗаработка);
	списокНачисленоКолонка[36].Добавить(ОсновныеНачисления.ДоплатаДоМЗП);
	...
	списокНачисленоКолонка[41].Добавить(ОсновныеНачисления.ПустаяСсылка());
	...
	СписокНачисленоКолонка[42].Добавить(ОсновныеНачисления.ОплатаПоСреднемуБЛПоБеременностиИРодам);
	СписокНачисленоКолонка[42].Добавить(ОсновныеНачисления.ОплатаПоСреднемуБЛТравмаНаПроизводстве);
	списокНачисленоКолонка[42].Добавить(ОсновныеНачисления.ОплатаПоСреднемуБЛ);
	...
	списокНачисленоКолонка[44].Добавить(ОсновныеНачисления.ПустаяСсылка());
	...
	списокНачисленоКолонка[48].Добавить(ОсновныеНачисления.ПустаяСсылка());
...
////////////////////////////////////////////////////////////////////////////////////	
	...
	списокУдержаноКолонка[7].Добавить(УдержанияОрганизаций.ИндексацияАлиментов);
...
Процедура ПроверитьНеучтенныеВидыРасчета() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Результат <> 0 Тогда
			Сообщить(НСтр("ru='Для работника: ';uk='Для працівника: '") + Выборка.Сотрудник + НСтр("ru=' присутствует вид расчета (';uk=' присутній вид розрахунку ('") + Выборка.ВидРасчета + НСтр("ru=') с ненулевым начислением. Измените настройки!';uk=') з ненульовим нарахуванням. Змініть настройки!'"),СтатусСообщения.Важное);
		КонецЕсли;
