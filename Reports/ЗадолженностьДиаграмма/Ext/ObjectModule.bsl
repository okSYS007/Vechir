...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если Не ЗначениеЗаполнено(ВидЗадолженности) Тогда
		...
		Если Найти(ОбщийОтчет.мНазваниеОтчета, "Дебиторская")>0 
				ИЛИ Найти(ОбщийОтчет.мНазваниеОтчета, "Дебіторська")>0 Тогда
				
			ВидЗадолженности = "Дебиторская";
			
		ИначеЕсли найти(ОбщийОтчет.мНазваниеОтчета, "Кредиторская")>0 
					ИЛИ Найти(ОбщийОтчет.мНазваниеОтчета, "Кредиторська")>0 Тогда
		
			ВидЗадолженности = "Кредиторская";
			
		Иначе
			
			Сообщить(НСтр("ru='Не задан тип задолженности (переменная объекта мНазваниеОтчета)';uk=""Не заданий тип заборгованості (змінна об'єкта мНазваниеОтчета)"""), СтатусСообщения.ОченьВажное);
	...
	НСтр("ru='Валюта взаиморасчетов';uk='Валюта взаєморозрахунків'"),
	"Сделка",
	НСтр("ru='Сумма в валюте упр. учета';uk='Сума у валюті упр. обліку'"));
	...
	Если ОбщийОтчет.Показатели.Найти("Сумма", "Имя") = Неопределено Тогда
		ОбщийОтчет.ЗаполнитьПоказатели("Сумма", НСтр("ru='Сумма в (';uk='Сума у ('") + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")", Ложь, "ЧЦ=15; ЧДЦ=2");
	КонецЕсли;
