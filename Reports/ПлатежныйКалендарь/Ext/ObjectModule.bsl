...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мНазваниеОтчета=НСтр("ru='Платежный календарь';uk='Платіжний календар'", КодЯзыкаПечать);
	мНаДату = Ложь;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь, РапортРуководителю=Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаКон) И ДатаКон < ДатаНач Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Конечная дата периода отчета не должна быть меньше начальной даты!';uk='Кінцева дата періоду звіту не повинна бути менше початкової дати!'"));
	...
	Если НЕ ЕстьГруппировки Тогда
		Сообщить(НСтр("ru='Необходимо выбрать хотя бы одну основную группировку.';uk='Необхідно вибрати хоча б одне основне групування.'", КодЯзыкаПечать));
		Возврат;
	КонецЕсли;
	...
	Если НЕ РапортРуководителю Тогда // выводим развернутый заголовок
		...
		Для Сч = 1 По 3 Цикл
			...
			Если Префикс = "Верх" Тогда
				...
				Иначе
					Если ДатаНач = '00010101000000' ИЛИ ДатаКон = '00010101000000' Тогда
						ОбластьЗначение.Параметры.Период =НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + Формат(ДатаНач, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+";ДФ = ""дд.ММ.гггг""; ДП = ""без ограничения""") 
						+ " - " + Формат(ДатаКон, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+";ДФ = ""дд.ММ.гггг""; ДП = ""без ограничения""") + СтрОшибки;
					Иначе
						ОбластьЗначение.Параметры.Период = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)) + СтрОшибки;
					КонецЕсли;
				КонецЕсли;
			...
			ИначеЕсли Префикс = "Низ" Тогда
				...
				ОбластьЗначение.Параметры.Группировки = НСтр("ru='Группировки: ';uk='Групування: '", КодЯзыкаПечать)+ СтрГруппировкиВерт;
				ДокументРезультат.Присоединить(ОбластьЗначение);
	...
	Если ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
		...
		Если ПустаяСтрока(ЗначениеТекущейГруппировки) Тогда
			Если ТипЗнч(Выборка[ИмяГруппировки])=Тип("СправочникСсылка.Кассы") Тогда
				ЗначениеТекущейГруппировки = НСтр("ru='Касса не указана';uk='Каса не вказана'", КодЯзыкаПечать);
			ИначеЕсли ТипЗнч(Выборка[ИмяГруппировки])=Тип("СправочникСсылка.БанковскиеСчета") Тогда
				ЗначениеТекущейГруппировки = НСтр("ru='Банковский счет не указан';uk='Банківський рахунок не вказаний'", КодЯзыкаПечать);
			Иначе
				ЗначениеТекущейГруппировки = НСтр("ru='Не указано';uk='Не зазначено'", КодЯзыкаПечать);
			КонецЕсли;
		КонецЕсли;

		ЗначениеГруппировки = ЗначениеГруппировки + ЗначениеТекущейГруппировки;

		Если ЗначениеРасшифровки = Неопределено И Найти(ИмяГруппировки,"ВидДвижения")=0 И НЕ ЗначениеТекущейГруппировки = НСтр("ru='Не указано';uk='Не зазначено'", КодЯзыкаПечать) Тогда 
			ЗначениеРасшифровки = Выборка[ИмяГруппировки];
		КонецЕсли;

	ИначеЕсли ТипЗаписиВыборки = ТипЗаписиЗапроса.ОбщийИтог Тогда
		ЗначениеГруппировки = НСтр("ru='ИТОГО: ';uk='РАЗОМ: '", КодЯзыкаПечать);
	КонецЕсли;
