...
Функция ПолучитьСотрудников( УсловиеНаФизлицо = "" )
	...
	|			(Состояния.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускПоБеременностиИРодам)
	|			 И Состояния.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускПоУходуЗаРебенком) 
	|			 И Состояния.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован) ))
	|			" + УсловиеНаФизлицо + "
	|
	|			ОБЪЕДИНИТЬ ВСЕ
	|			ВЫБРАТЬ
	...
	|			 ИЛИ СостоянияДо.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован))
	...
	|					  ИЛИ Состояния.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован))            
    |					И	Состояния.Период >= &парамНачало
	|					И	Состояния.Период <= &парамКонец
	|
...
Функция ПолучитьОтбор(знач ПараметрОтбора = "") Экспорт
	...
	УсловиеОтбора = "";
	Если ПараметрОтбора = "Ж" ИЛИ ПараметрОтбора = "ЖСЕЛ" ИЛИ ПараметрОтбора = "ЖОТ18ДО35" ИЛИ ПараметрОтбора = "ЖОТ36ДО60" ИЛИ ПараметрОтбора = "ЖОТ60" 
	...
	Если ПараметрОтбора = "СЕЛ" ИЛИ ПараметрОтбора = "ЖСЕЛ" ИЛИ ПараметрОтбора = "ИНВСЕЛ" ИЛИ ПараметрОтбора = "ЖИНВСЕЛ" Тогда
		...
		ЗапросСЖ.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	СотрудникиОрганизаций.Ссылка КАК Сотрудник,
		|	КонтактнаяИнформация.Поле5 КАК НаселенныйПункт
		|ИЗ
		|	Справочник.СотрудникиОрганизаций КАК СотрудникиОрганизаций
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
		|		ПО СотрудникиОрганизаций.Физлицо = КонтактнаяИнформация.Объект.Ссылка
		...
		Пока Выборка.Следующий() Цикл
			Если Не СокрЛП(Выборка.НаселенныйПункт) = "" Тогда
	...
	Если ПараметрОтбора = "ОТ18ДО35" ИЛИ ПараметрОтбора = "ЖОТ18ДО35" ИЛИ ПараметрОтбора = "ИНВОТ18ДО35" ИЛИ ПараметрОтбора = "ЖИНВОТ18ДО35" Тогда
		УсловиеОтбора = УсловиеОтбора + " И Работники.Сотрудник.ФизЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
										| И Работники.Сотрудник.ФизЛицо.ДатаРождения <= &парамНачалоИнтервала
										| И Работники.Сотрудник.ФизЛицо.ДатаРождения >= &парамОкончаниеИнтервала";
		Запрос.УстановитьПараметр("парамНачалоИнтервала", КонецМесяца(ДобавитьМесяц(КонецГода(ДатаНачала), -18*12)));
		Запрос.УстановитьПараметр("парамОкончаниеИнтервала", НачалоГода(ДобавитьМесяц(КонецГода(ДатаНачала), -35*12)));
	КонецЕсли;
	Если ПараметрОтбора = "ОТ36ДО60" ИЛИ ПараметрОтбора = "ЖОТ36ДО60" ИЛИ ПараметрОтбора = "ИНВОТ36ДО60" ИЛИ ПараметрОтбора = "ЖИНВОТ36ДО60" Тогда
		УсловиеОтбора = УсловиеОтбора + " И Работники.Сотрудник.ФизЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
										| И Работники.Сотрудник.ФизЛицо.ДатаРождения <= &парамНачалоИнтервала
										| И Работники.Сотрудник.ФизЛицо.ДатаРождения >= &парамОкончаниеИнтервала";
		Запрос.УстановитьПараметр("парамНачалоИнтервала", КонецМесяца(ДобавитьМесяц(КонецГода(ДатаНачала), -36*12)));
		Запрос.УстановитьПараметр("парамОкончаниеИнтервала", НачалоГода(ДобавитьМесяц(КонецГода(ДатаНачала), -60*12)));
	КонецЕсли;
	Если ПараметрОтбора = "ОТ60" ИЛИ ПараметрОтбора = "ЖОТ60" 
		ИЛИ ПараметрОтбора = "ИНВОТ60" ИЛИ ПараметрОтбора = "ЖИНВОТ60" Тогда
		УсловиеОтбора = УсловиеОтбора + " И Работники.Сотрудник.ФизЛицо.ДатаРождения <> ДАТАВРЕМЯ(1,1,1)
										| И Работники.Сотрудник.ФизЛицо.ДатаРождения < &парамНачалоИнтервала";
		Запрос.УстановитьПараметр("парамНачалоИнтервала", НачалоГода(ДобавитьМесяц(КонецГода(ДатаНачала), -60*12)));
	КонецЕсли;
	...
	Если ПараметрОтбора = "ИНВ" ИЛИ ПараметрОтбора = "ЖИНВ" ИЛИ ПараметрОтбора = "ИНВСЕЛ" ИЛИ ПараметрОтбора = "ЖИНВСЕЛ"
	...
	ИначеЕсли ПараметрОтбора = "ДОПГАРАНТИИ" Тогда  
	...
	КонецЕсли; 
	Если  ПараметрОтбора = "ИНВ" ИЛИ ПараметрОтбора = "ПЕНСИОНЕРЫ" ИЛИ ПараметрОтбора = "ПЕН" ИЛИ ПараметрОтбора = "ИНОСТРАНЦЫ" ИЛИ ПараметрОтбора = "ДОПГАРАНТИИ" ИЛИ ПараметрОтбора = "ДОПГАРАНТИИНЕИНВ" ИЛИ ПараметрОтбора = "ДОПГАРАНТИИ10ЛЕТ"
		ИЛИ ПараметрОтбора = "ЖИНВ" ИЛИ ПараметрОтбора = "ИНВСЕЛ" ИЛИ ПараметрОтбора = "ЖИНВСЕЛ" 
		ИЛИ ПараметрОтбора = "ИНВОТ18ДО35" ИЛИ ПараметрОтбора = "ИНВОТ36ДО60" ИЛИ ПараметрОтбора = "ИНВОТ60" 
		ИЛИ ПараметрОтбора = "ЖИНВОТ18ДО35" ИЛИ ПараметрОтбора = "ЖИНВОТ36ДО60" ИЛИ ПараметрОтбора = "ЖИНВОТ60" Тогда
