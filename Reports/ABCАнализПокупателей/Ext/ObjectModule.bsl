...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мСтруктураСоответствияИмен = Новый Структура("Контрагент, ABCКлассПокупателя","Контрагент", НСтр("ru='ABC-Класс покупателя';uk='ABC-Клас покупця'"));
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если ПараметрАВСРаспределения = Неопределено ИЛИ (ТипЗнч(ПараметрАВСРаспределения) = Тип("ПеречислениеСсылка.ПараметрыABCКлассификацииПокупателей") И ПараметрАВСРаспределения.Пустая()) Тогда
		ПредставлениеПараметра = НСтр("ru='Параметр распределения не задан в учетной политике.';uk='Параметр розподілу не заданий в обліковій політиці.'", КодЯзыкаПечать);
	Иначе
		ПредставлениеПараметра = НСтр("ru='Параметр распределения:';uk='Параметр розподілу:'", КодЯзыкаПечать) + ПараметрАВСРаспределения + НСтр("ru=', в валюте упр.учета (';uk=', у валюті упр.обліку ('", КодЯзыкаПечать) + СокрЛП(глЗначениеПеременной("ВалютаУправленческогоУчета").Наименование) + ")";
	КонецЕсли; 
	Секция.Параметры.ПредставлениеПараметра = ПредставлениеПараметра;
	Таб.Вывести(Секция);
	
	Секция = Макет.ПолучитьОбласть("ШапкаИнтервал");
	Секция.Параметры.СтрокаИнтервал =  НСтр("ru='Период: на ';uk='Період: на '", КодЯзыкаПечать) + ?(ДатаОтчета = Дата("00010101000000"), (НСтр("ru='текущее время ';uk='поточний час '", КодЯзыкаПечать) + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy")),Формат(ДатаОтчета, "ДФ=dd.MM.yyyy"));
	Таб.Вывести(Секция);
	...
	ВывестиСтроки(Таб, Макет, РезультатЗапроса, 0, ТаблицаЗапросаПрошлыхКлассов, КодЯзыкаПечать);
...
// Возвращаемое значение
//  НЕТ
Процедура ВывестиСтроки(Таб, Макет, ТекущаяВыборка, ИндексТекущейГруппировки, ТаблицаЗапросаПрошлыхКлассов, КодЯзыкаПечать)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НаименованиеГруппировки = "Контрагент" Тогда
			СтрокаВыводаСтадии = НСтр("ru='Не задана';uk='Не задана'", КодЯзыкаПечать);
				...
				Если ВыборкаДетальныхЗаписей.ТипЗаписи() = ТипЗаписиЗапроса.ДетальнаяЗапись Тогда
					Если ТипЗнч(ВыборкаДетальныхЗаписей.Стадия) = Тип("ПеречислениеСсылка.СтадииВзаимоотношенийСПокупателями") Тогда
						...
						Если ВыборкаДетальныхЗаписей.Стадия = Перечисления.СтадииВзаимоотношенийСПокупателями.ПостоянныйПокупатель Тогда
							...
							Иначе
								СтрокаВыводаСтадии = СтрокаВыводаСтадии + НСтр("ru=' (Клас не задан)';uk=' (Клас не заданий)'", КодЯзыкаПечать);
							КонецЕсли; 
						КонецЕсли; 
					КонецЕсли; 
					Прервать;
		...
		ВывестиСтроки(Таб, Макет, Выборка, ИндексТекущейГруппировки+1, ТаблицаЗапросаПрошлыхКлассов, КодЯзыкаПечать);
