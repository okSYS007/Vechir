	...
	Иначе
		...
		Иначе
			...
			Если ДатаНачала <= ДатаОкончания Тогда
				ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + ПредставлениеПериода(НачалоДня(ДатаНачала), КонецДня(ДатаОкончания), "ФП = Истина");
			Иначе
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если ОтборФлагКласс Тогда
		Если ОтборВидСравненияКласс = ВидСравнения.Равно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс = ';uk='АВС-клас = '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКласс);
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.НеРавно Тогда
			СтрокаОтборов = СтрокаОтборов + НСтр("ru='АВС-класс <> ';uk='АВС-клас <> '", КодЯзыкаПечать)+ СокрЛП(ОтборЗначениеКласс);
		ИначеЕсли ОтборВидСравненияКласс = ВидСравнения.ВСписке Тогда
			СтрокаСписка = "";
	...
	Если СуммаПроцентов <> 100 Тогда
		Предупреждение(НСтр("ru='Сумма процентов критериев распределения по АВС-классам должна быть равна 100%';uk='Сума відсотків критеріїв розподілу за АВС-класами повинна дорівнювати 100%'", КодЯзыкаПечать));
		Возврат;
	КонецЕсли; 
	...
	Если ГруппировкаОбъекта = Неопределено Тогда
		...
		Предупреждение(НСтр("ru='Группировка по объекту анализа ""';uk='Групування по об''єкту аналізу ""'", КодЯзыкаПечать) + ПредставлениеОбъектаАнализа + НСтр("ru='"", должна присутствовать обязательно!';uk='"", повинна бути присутньою обов''язково!'", КодЯзыкаПечать));
		Возврат;
	Иначе
...
НовыйПоказатель.Представление = НСтр("ru='Сумма выручки без НДС в валюте упр.учета (';uk='Сума виручки без ПДВ у валюті упр.обліку ('", КодЯзыкаПечать) + глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";
