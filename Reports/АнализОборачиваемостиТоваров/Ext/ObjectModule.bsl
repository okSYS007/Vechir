...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ЗаполнитьПоказатели("Остаток",          НСтр("ru='Средний остаток за период';uk='Середній залишок за період'", КодЯзыкаПечать),
	                    ИСТИНА,   "ЧЦ = 15; ЧДЦ = 3");
	ЗаполнитьПоказатели("Расход",           НСтр("ru='Расход за период';uk='Витрата за період'", КодЯзыкаПечать),
	                    ИСТИНА,   "ЧЦ = 15; ЧДЦ = 3");
	ЗаполнитьПоказатели("КОборачиваемости", НСтр("ru='Коэффициент оборачиваемости';uk='Коефіцієнт оборотності'", КодЯзыкаПечать),
	                    ИСТИНА,   "ЧЦ = 15; ЧДЦ = 5");
	ЗаполнитьПоказатели("СрокХранения",     НСтр("ru='Средний срок хранения в днях';uk='Середній строк зберігання в днях'", КодЯзыкаПечать),
	                    ИСТИНА,   "ЧЦ = 15; ЧДЦ = 2");

	ЗаполнитьПредставление("СтатусТоваров",              НСтр("ru='Статус товаров';uk='Статус товарів'", КодЯзыкаПечать),              ИСТИНА, ЛОЖЬ  );
	Если СтатусТоваров = "ТоварыНаСкладах" Тогда
		ЗаполнитьПредставление("Склад",                      НСтр("ru='Склад';uk='Склад'", КодЯзыкаПечать),                       ИСТИНА, ИСТИНА);
	ИначеЕсли СтатусТоваров = "ТоварыУКомиссионеров" Тогда 
		ЗаполнитьПредставление("Комиссионер",                НСтр("ru='Комиссионер';uk='Комісіонер'", КодЯзыкаПечать),                 ЛОЖЬ,   ИСТИНА);
	Иначе
		ЗаполнитьПредставление("Склад",                      НСтр("ru='Склад';uk='Склад'", КодЯзыкаПечать),                       ИСТИНА, ИСТИНА);
		ЗаполнитьПредставление("Комиссионер",                НСтр("ru='Комиссионер';uk='Комісіонер'", КодЯзыкаПечать),                 ЛОЖЬ,   ИСТИНА);
	КонецЕсли;
	ЗаполнитьПредставление("Номенклатура",               НСтр("ru='Номенклатура';uk='Номенклатура'", КодЯзыкаПечать),                ИСТИНА, ИСТИНА);
	ЗаполнитьПредставление("ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'", КодЯзыкаПечать), ЛОЖЬ,   ЛОЖЬ  );
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	СписокИзмерений = "";
	Для Сч=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		СписокИзмерений = СписокИзмерений + ", " + ПостроительОтчета.ИзмеренияСтроки[Сч].Представление + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.ИзмеренияСтроки[Сч].ТипИзмерения));
	КонецЦикла; 
	...
	Секция.Параметры.Период          = НСтр("ru='Данные за период: ';uk='Дані за період: '", КодЯзыкаПечать) + Формат(ДатаНачала, "ДФ = ""дд.ММ.гггг""; ДП = ""...""")
	                                 + " - " + Формат(ДатаКонца, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
	Секция.Параметры.Измерения       = НСтр("ru='Итоги по: ';uk='Підсумки по: '", КодЯзыкаПечать)       + Сред(СписокИзмерений, 2);
	Секция.Параметры.Показатели      = НСтр("ru='Показатели: ';uk='Показники: '", КодЯзыкаПечать)     + Сред(СписокПоказателей, 2);
	Секция.Параметры.Отбор           = НСтр("ru='Отбор:';uk='Відбір:'", КодЯзыкаПечать)           + Сред(СписокОтбор,2);
	...
	Для к = 1 по КоличествоЗапросов Цикл
		...
		Иначе
            ...
            + ОбщегоНазначения.ПолучитьИмяЭлементаПеречисленияПоЗначению(Периодичность)
...
мСписокСтатусовТоваров.Добавить("ВсеТовары", НСтр("ru='Все товары';uk='Всі товари'"));
мСписокСтатусовТоваров.Добавить("ТоварыНаСкладах", НСтр("ru='Товары на складах';uk='Товари на складах'"));
мСписокСтатусовТоваров.Добавить("ТоварыУКомиссионеров", НСтр("ru='Товары у комиссионеров';uk='Товари в комісіонерів'"));

// 37490 <<
КодЯзыкаПечать = ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов();
// 37490 >>
мНазваниеОтчета = НСтр("ru='Анализ оборачиваемости товаров';uk='Аналіз оборотності товарів'", КодЯзыкаПечать);
