...
Процедура ПроставитьЗначенияБыстрыхОтборов(ФормаБыстрыхОтборов) Экспорт
	...
	Для каждого Строки Из ФильтрыОтчета Цикл
		...
		Если Строки.ТипФильтра = "РАВНО" ИЛИ Врег(Строки.ТипФильтра) = "ДОРІВНЮЄ"
			ИЛИ Строки.ТипФильтра = "НЕ РАВНО" ИЛИ Врег(Строки.ТипФильтра) = "НЕ ДОРІВНЮЄ" Тогда
...
Процедура ЗаполнитьНачальныеНастройки(ВосстановлениеНастройки = Ложь) Экспорт
	...
	мНазваниеОтчета = НСтр("ru='Взаиморасчеты с комитентами';uk='Взаєморозрахунки з комітентами'");
	...
	Для НСтр = 1 По ОбластьГруппировкиВысота Цикл
		...
		Если СтрГруппировки.ВозможенФильтр Тогда
			...
			Если Назначение <> Неопределено Тогда
				...
				СтрФильтрыКатегории.ПредставлениеПоля = НСтр("ru='Категории ';uk='Категорії '") + НРег(СтрГруппировки.ПредставлениеПоля);
...
Функция ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчета(ТаблицаДокумента,Источник)
	...
	|			" + ТаблицаДокумента + "."+Источник+" * КурсыВалютыУпрУчета.Кратность / (КурсыВалютыУпрУчета.Курс * 1) // ограничение разрядов выполняется как при записи в регистр
	|		ИНАЧЕ 
	|			0
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Взаиморасчеты с комитентами';uk='Взаєморозрахунки з комітентами'", КодЯзыкаПечать);
			Если ДатаНач = '00010101000000' И ДатаКон = '00010101000000' Тогда
				ОбластьЗначение.Параметры.Период = НСтр("ru='Период: без ограничения ';uk='Період: без обмеження '", КодЯзыкаПечать) + СтрОшибки;
			Иначе
				...
				Иначе
					ОбластьЗначение.Параметры.Период = НСтр("ru='Период: ';uk='Період: '", КодЯзыкаПечать) + ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон)) + СтрОшибки;
				КонецЕсли;
			КонецЕсли;
...
мСтруктураНазванийПолейСортировки.Вставить("НачальныйОстатокРеализовано", НСтр("ru='Не отчитались нач.';uk='Не відзвітували поч.'"));
мСтруктураНазванийПолейСортировки.Вставить("КонечныйОстаток"			, НСтр("ru='Остаток кон.';uk='Залишок кін.'"));
мСтруктураНазванийПолейСортировки.Вставить("КонечныйОстатокРеализовано" , НСтр("ru='Не отчитались кон.';uk='Не відзвітували кін.'"));
мСтруктураНазванийПолейСортировки.Вставить("Приход"						, НСтр("ru='Принято';uk='Прийнято'"));
мСтруктураНазванийПолейСортировки.Вставить("ПриходРеализовано"			, НСтр("ru='Реализовано';uk='Реалізовано'"));
мСтруктураНазванийПолейСортировки.Вставить("РасходРеализовано"			, НСтр("ru='Отчитались';uk='Відзвітували'"));
