...
Процедура СформироватьОтчет(ДокументРезультат, Режим, ВозвращаемоеСообщениеОбОшибке = Неопределено) Экспорт
	...
	Если Не ПроцедурыУправленияПерсоналом.ЗаполненРегламентированныйПроизводственныйКалендарь(НачалоМесяца(ДатаНач),КонецДня(ДатаКон)) Тогда
	...
	ЗапросТабель.УстановитьПараметр("КонецПериода", КонецДня(ДатаКон));
	...
	ОбластьДанныхШапка.Параметры.ДатаКон     = КонецДня(ДатаКон);
	...
	Если Режим = "Табель" Тогда
		...
		Иначе
			...
			|	";           
			Для НомерДня = 2 По День(Мин(РабочаяДата, КонецДня(ДатаКон))) Цикл
		...
		|ИЗ
		|	РегистрРасчета.Начисления.ФактическийПериодДействия(Назначение В (ВЫБРАТЬ Сотрудник ИЗ ВТСотрудники)) КАК ФактическийПериодДействия
		|ГДЕ
		|	ПериодДействия = &Месяц
		|	И ФактическийПериодДействия.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени <> &ВидВремениПустаяСсылка
		|	И НЕ ФактическийПериодДействия.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени.ВидВремени  В ( Значение(Перечисление.ВидыВремени.ПустаяСсылка), Значение(Перечисление.ВидыВремени.ДополнительноОплачиваемоеВПределахНормы), Значение(Перечисление.ВидыВремени.ЧасовоеНеотработанное))";
		ЗапросТабель.Выполнить();
		...
		|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ПериодЗавершения,
		|	СостояниеРаботников.Состояние КАК СостояниеЗавершения
		...
		|		НеотфильтрованноеВремя.Сотрудник.Физлицо.Пол КАК Пол,
		...
		|УПОРЯДОЧИТЬ ПО
		|	Организация, ПодразделениеОрганизации, СотрудникНаименование, НеотфильтрованноеВремя.Сотрудник, Должность, ДатаТабеля, ПолеСортировки";
		...
		Если ГруппироватьПодразделения Тогда
			...
			Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Организация") Цикл
				...
				Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ПодразделениеОрганизации") Цикл
					...
					ОбластьДанныхПодвал.Параметры.ФИООтветственного = ОбщегоНазначения.ИмяФамилияВФорматеДСТУ(ОбластьДанныхПодвал.Параметры.ФИООтветственного);
					ОбластьДанныхПодвал.Параметры.ФИОРуководителя = ОбщегоНазначения.ИмяФамилияВФорматеДСТУ(ОбластьДанныхПодвал.Параметры.ФИОРуководителя);
					ОбластьДанныхПодвал.Параметры.ФИОКадровика = ОбщегоНазначения.ИмяФамилияВФорматеДСТУ(ОбластьДанныхПодвал.Параметры.ФИОКадровика);					
					...
					Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("СотрудникНаименование") Цикл
						Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Сотрудник") Цикл
							...
							Если НЕ ВыборкаПоРаботнику.Пол.Пустая() Тогда
		                    	Пол = ?(ВыборкаПоРаботнику.Пол = Перечисления.ПолФизическихЛиц.Мужской,"ч","ж"); 
							Иначе
								Пол = "";
							КонецЕсли;	
							ОбластьДанныхСтрока.Параметры.Пол = Пол;
							...
							Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ДатаТабеля") Цикл
								...
								Пока ВыборкаПоРаботнику.Следующий() Цикл
									...
									Если ГрафаНеявок(ВыборкаПоРаботнику.ВидВремени)>2 Тогда
										...
										Если ВыборкаПоРаботнику.ВнутрисменныеОтклоненияЧасов = 0 И ВыборкаПоРаботнику.НеотработаноЧасов = 0 
											И НЕ (ВыборкаПоРаботнику.ДатаТабеля >= ПроведениеРасчетов.ДатаИзмененияПорядка1266() 
											И (ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Болезнь
											ИЛИ ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам))
											И НЕ ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам Тогда
									...
									ИначеЕсли ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Вечерние Тогда
									...
									Если ВыборкаПоРаботнику.ВнутрисменныеОтклоненияВидИспользованияРабочегоВремени <> NULL 
										...
										И НЕ(ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Вечерние
									...
									ИначеЕсли НЕ(ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Вечерние
		...
		Иначе
			Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Организация") Цикл
				...
				Если ОтборПодразделения Тогда
					ОбластьДанныхШапка.Параметры.ИмяПодразделения = ВыборкаПоРаботнику.ПодразделениеОрганизации;
				КонецЕсли;
	...
	ИначеЕсли Режим = "Бланк" Тогда 
		...
		|		КОНЕЦ, РаботникиОрганизаций.Сотрудник.Физлицо.Наименование) КАК СотрудникНаименование,
		|	ДолжностиРаботников.Организация КАК Организация,
		|	ПОДСТРОКА(ДолжностиРаботников.Организация.НаименованиеПолное, 1, 250) КАК ИмяОрганизации,
		...
		Иначе
			ЗапросТабель.Текст = ЗапросТабель.Текст + "
			|	ПО ДолжностиРаботников.Организация = Руководитель.СтруктурнаяЕдиница";
		КонецЕсли;
		ЗапросТабель.Текст = ЗапросТабель.Текст + "
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&КонецПериода, ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.РуководительКадровойСлужбы)) КАК РуководительКадровойСлужбыОрганизации
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&КонецПериода, ) КАК ФИОКадровикаОрганизации
		|		ПО РуководительКадровойСлужбыОрганизации.ФизическоеЛицо = ФИОКадровикаОрганизации.ФизЛицо
		|	ПО ДолжностиРаботников.Организация = РуководительКадровойСлужбыОрганизации.СтруктурнаяЕдиница
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&КонецПериода, ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.РуководительКадровойСлужбы)) КАК РуководительКадровойСлужбыПодразделения
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&КонецПериода, ) КАК ФИОКадровикаПодразделения
		|		ПО РуководительКадровойСлужбыПодразделения.ФизическоеЛицо = ФИОКадровикаПодразделения.ФизЛицо
