	...
	Иначе
		...
		Для Каждого СтрокаТела Из  тбПоказателиТело Цикл
			...
			Иначе
				...
				Если (ИмяСхемы = "J1201210" ИЛИ ИмяСхемы = "F1201210") 
					И ((ЛЕВ(СтрокаТела.Имя,5) = "R0301") ИЛИ (ЛЕВ(СтрокаТела.Имя,5) = "R0302")) Тогда
...
//R0302G4
//R0302G5 
					ИмяВСтруктуре = ЛЕВ(СтрокаТела.Имя,5)+ "_" + Сред(СтрокаТела.Имя,6);
					
					ОписаниеОшибки = "";
					ВыгрузитьПоказатель(элТело, СтрокаТела.Имя,
										ПреобразоватьТипXML(ПолучитьИзСтруктуры(ИмяВСтруктуре, НужнаяСтруктура),
															СтрокаТела.Тип,
				...
				ИначеЕсли НЕ (ИмяСхемы = "R1201501" И СтрокаТела.Имя = "INN") Тогда
...
Функция ПреобразоватьТипXMLОбщий(Значение,Тип,Результат,ОписаниеОшибки = "")
	...
	ИначеЕсли (ВРег(Тип) = ВРег("DGI5i")) Тогда
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				...
				ИЛИ Контроль > 99999
				ИЛИ Цел(Контроль) <> Контроль Тогда
				ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 1000 до 99999';uk='Значення не є цілим числом від 1000 до 99999'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 1000 до 99999';uk='Значення не є цілим числом від 1000 до 99999'");
		КонецПопытки;
		
	ИначеЕсли (ВРег(Тип) = ВРег("DGI5"))
	...
	ИначеЕсли (ВРег(Тип) = ВРег("DGI6"))
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				...
				ИЛИ Контроль > 999999
				ИЛИ Цел(Контроль) <> Контроль Тогда
				ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 100000 до 999999';uk='Значення не є цілим числом від 100000 до 99999'");
			КонецЕсли;
		Исключение
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGKATOTTG") 
		...
		Иначе
			Попытка
				Контроль = Число(Сред(Значение, 3));
				Если Лев(Значение,2)= "UA" 
					И Контроль<99999999999999999 Тогда
					Результат = Значение;
				Иначе
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGIBAN") Тогда
		...
		Если Лев(Результат, 2) <> "UA" Тогда
			Контроль=0;
		ИначеЕсли ДлинаКода > 29 Тогда
			Контроль = 0;
		Иначе
			Для Сч = 3 По ДлинаКода Цикл
	...
	ИначеЕсли ВРег(Тип) = ВРег("KOD_KPColumn")
		  ИЛИ ВРег(Тип) = ВРег("KOD_KP")
		  Тогда
		 ...
		 ИЛИ ВРег(Тип) = ВРег("NumZOColumn") Тогда
			Если Найти("0123456789", Лев(Результат,1)) > 0 Тогда
				...
				Иначе
					...
					КонецЦикла;
				КонецЕсли;
			ИначеЕсли Лев(Результат, 1) = "П" Тогда
				// если начинается с "П" - то девять следующих цифр
				Если СтрДлина(Результат) <> 10 Тогда
					Контроль=0;
				Иначе
					Для Сч = 2 По 10 Цикл
					...
					КонецЦикла;
				КонецЕсли;
			ИначеЕсли Лев(Результат, 1) = "F" Тогда	
				// по сле символа F - любые знаки
			ИначеЕсли Лев(Результат, 2) = "БК" Тогда	
				Если СтрДлина(Результат) <> 10 Тогда
					Контроль=0;
				ИначеЕсли Найти("АБВГДЕЄЖЗИІКЛМНОПРСТУФХЦЧШЩЮЯ",Сред(Результат,3,1)) = 0
					  ИЛИ Найти("АБВГДЕЄЖЗИІКЛМНОПРСТУФХЦЧШЩЮЯ",Сред(Результат,4,1)) = 0 Тогда
					Контроль=0;
				Иначе
					Для Сч = 5 По 10 Цикл
						Если Найти("0123456789", Сред(Результат,Сч,1)) = 0 Тогда
					...
					КонецЦикла;
				КонецЕсли;
			ИначеЕсли СтрДлина(Результат) <> 9 Тогда
				Контроль = 0;
			Иначе
				Если Найти("АБВГДЕЄЖЗИІКЛМНОПРСТУФХЦЧШЩЮЯабвгдеєжзиіклмнопрстуфхцчшщюя",Сред(Результат,1,1)) = 0
				 ИЛИ Найти("АБВГДЕЄЖЗИІКЛМНОПРСТУФХЦЧШЩЮЯабвгдеєжзиіклмнопрстуфхцчшщюя",Сред(Результат,2,1)) = 0
				 ИЛИ Найти("АБВГДЕЄЖЗИІКЛМНОПРСТУФХЦЧШЩЮЯабвгдеєжзиіклмнопрстуфхцчшщюя",Сред(Результат,3,1)) = 0 Тогда
					Контроль = 0;
				Иначе
					Для Сч = 4 По 9 Цикл
						Если Найти("0123456789", Сред(Результат,Сч,1)) = 0 Тогда
			...
			КонецЕсли;
			Если Контроль = 0 Тогда
				ОписаниеОшибки = "Неправильно указан код по ДРФО либо Серия и номер паспорта.
								 |Значение указывается в одном из форматов:
								 |	XXXXXXXXXX (десять цифр кода ДРФО)
								 |	БКССXXXXXX (""БК""СС - серия, XXXXXX - номер паспорта в виде книжечки)
								 |  ПXXXXXXXXX (""П"" - фикс. символ, далее 9 цифр номера паспорта в виде карточки)
								 |  CCCXXXXXX  (три буквы серии и 6 цифр номера документа)
								 |	F...	   (""F"" - фикс. символ, далее любая последовательность символов)";
				Контроль = -1;	
			КонецЕсли;
			
		ИначеЕсли Найти("0123456789", Лев(Результат,1)) > 0 Тогда
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGPass") //<!--Загальний тип "№ паспорта" (HPASS)-->
		...
		Если Контроль = 0 Тогда
			...
			ОписаниеОшибкиРу = "Неправильно Серия и номер паспорта для физ. лица, у которого нет кода.
							 |Паспорт указывается в формате ССНННННН (СС - серия, НННННН - номер паспорта)
