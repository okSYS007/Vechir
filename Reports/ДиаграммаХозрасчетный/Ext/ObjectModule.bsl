...
Функция ЗаголовокОтчета() Экспорт
	Возврат НСтр("ru='Диаграмма';uk='Діаграма'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Функция СформироватьЗаголовок() Экспорт
	...
	соотВид     = Новый Структура("НачальныйОстаток, КонечныйОстаток, Оборот", 
									НСтр("ru='начальный остаток';uk='початковий залишок'", КодЯзыкаПечать), 
									НСтр("ru='конечный остаток';uk='кінцевий залишок'", КодЯзыкаПечать), 
									НСтр("ru='оборот';uk='оборот'", КодЯзыкаПечать)
								 );
	соотСторона = Новый Структура("Дт, Кт", 
									НСтр("ru='дебетовый';uk='дебетовий'", КодЯзыкаПечать), 
									НСтр("ru='кредитовый';uk='кредитовий'", КодЯзыкаПечать)
								 );
	соотРесурс  = Новый Структура("Сумма, Количество, ВалютнаяСумма", 
									НСтр("ru='сумма';uk='сума'", КодЯзыкаПечать), 
									НСтр("ru='количество';uk='як число'", КодЯзыкаПечать), 
									НСтр("ru='валютная сумма';uk='валютна сума'", КодЯзыкаПечать)
								 );
	
	ЗаголовокОтчета = ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ТипДиаг), КодЯзыкаПечать) + НСтр("ru=' по счету ';uk=' по рахунку '", КодЯзыкаПечать) + Строка(Счет);
	...
	Если Не ПустаяСтрока(СтрокаОтборов) Тогда
		ЗаголовокОтчета = ЗаголовокОтчета + Символы.ПС + НСтр("ru='Отбор : ';uk='Відбір : '", КодЯзыкаПечать) + СтрокаОтборов;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Период) Тогда
		...
		Если ЗначениеЗаполнено(Период) Тогда
			Если Врег(Период) = "ГОД" Тогда
				ФорматПериода = НСтр("ru='ДФ = ""гггг """"г.""""""';uk='ДФ = ""гггг """"р.""""""'", КодЯзыкаПечать);
			ИначеЕсли Врег(Период) = "ПОЛУГОДИЕ" Тогда
				ФорматПериода = НСтр("ru='ДФ = """"""Полугодие с"""" дд.ММ.гггг """"""';uk='ДФ = """"""Півріччя з"""" дд.ММ.гггг """"""'", КодЯзыкаПечать);
			ИначеЕсли Врег(Период) = "КВАРТАЛ" Тогда
				ФорматПериода = НСтр("ru='ДФ = ""к"""" квартал"""" гггг """"г.""""""';uk='ДФ = ""к"""" квартал"""" гггг """"р.""""""'", КодЯзыкаПечать);
			ИначеЕсли Врег(Период) = "МЕСЯЦ" Тогда
				ФорматПериода = НСтр("ru='ДФ = ""ММММ гггг """"г.""""""';uk='ДФ = ""ММММ гггг """"р.""""""'", КодЯзыкаПечать) + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД();
			ИначеЕсли Врег(Период) = "ДЕКАДА" Тогда
				ФорматПериода = НСтр("ru='ДФ = """"""Декада с"""" дд.ММ.гггг """"""';uk='ДФ = """"""Декада з"""" дд.ММ.гггг """"""'", КодЯзыкаПечать);
			ИначеЕсли Врег(Период) = "НЕДЕЛЯ" Тогда
				ФорматПериода = НСтр("ru='ДФ = """"""Неделя с"""" дд.ММ.гггг """"""';uk='ДФ = """"""Тиждень із"""" дд.ММ.гггг """"""'", КодЯзыкаПечать);
...
Процедура СформироватьОтчет(Диаграмма) Экспорт
	...
	Если ДатаНач > ДатаКон И ДатаКон <> '00010101000000' Тогда
		Предупреждение(НСтр("ru='Дата начала периода не может быть больше даты конца периода';uk='Дата початку періоду не може бути більше дати кінця періоду'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), 60);
		Возврат;
	КонецЕсли;

	Если ПостроительОтчета.ИзмеренияСтроки.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не задано ни одной серии. Диаграмма не может быть построена.';uk='Не задано жодної серії. Діаграма не може бути побудована.'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), 60);
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка) Экспорт
	...
	Если СписокДоступныхПолей.Количество()>0 Тогда
		...
		Выбор = СписокДоступныхПолей.ВыбратьЭлемент(НСтр("ru='Выберите способ расшифровки';uk='Виберіть спосіб розшифровки'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), СписокДоступныхПолей[СписокДоступныхПолей.Количество()-1]);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ПолеСчет = ПостроительОтчета.ДоступныеПоля.найти("Счет");
	
	Если Не ПолеСчет = Неопределено Тогда
		
		ПолеСчет.Представление = НСтр("ru='Счет';uk='Рахунок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
