...
Функция ЗаголовокОтчета() Экспорт
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Карточка счета %1 (налоговый)';uk='Картка рахунку %1 (податковий)'"), Счет);
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Пока Проводки.Следующий() Цикл
		...
		Если Проводки.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
			...
			Если ОбластьОборота = Неопределено Тогда
				...
				ОбластьОборота.Параметры.ОписательПериода = НСтр("ru='Обороты за ';uk='Обороти за '") + БухгалтерскиеОтчеты.ПолучитьПериодДатСтрокой(Период, Проводки.Период);
		...
		Иначе
			...
			Если (Проводки.ВалютнаяСуммаДт <> NULL )
				...
				ОбластьМакет.Параметры.Заполнить(Проводки);
				ОбластьМакет.Параметры.ОписательВалюты = НСтр("ru='В валюте :';uk='У валюті :'");
				ОбластьМакет.Параметры.Расшифровка = СтруктураРасшифровки;
				ДокументРезультат.Вывести(ОбластьМакет);
	...
	ОбластьМакет.Параметры.ОписательПериода = НСтр("ru='Обороты за период';uk='Обороти за період'");
