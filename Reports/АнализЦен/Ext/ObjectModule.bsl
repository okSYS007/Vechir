...
Процедура ВывестиКолонкуШапки(Выборка, Знач Индекс, СтруктураПараметров, ДокументРезультат, ДеревоКолонкиОтчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (НомерЭлементаВыборки = 1) И (Индекс <> 0) Тогда
			...
			ДокументРезультат.Область(2+ДокументРезультат.ВысотаТаблицы-МакетКолонкаШапки.ВысотаТаблицы+Индекс,ДокументРезультат.ШиринаТаблицы - МакетКолонкаШапки.ШиринаТаблицы + 1).Текст = ?(НазваниеПоля = NULL, НСтр("ru='Собственные цены';uk='Власні ціни'"), НазваниеПоля);
		Иначе
			...
			НазваниеПоля = Выборка[Выборка.Группировка()];
			МакетКолонкаШапки.Область(2+Индекс,1).Текст = ?(НазваниеПоля = NULL, НСтр("ru='Собственные цены';uk='Власні ціни'"), НазваниеПоля);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	ЗаполнитьПоказатели("Цена", НСтр("ru='Цена';uk='Ціна'", КодЯзыкаПечать), ИСТИНА, "ЧЦ = 15; ЧДЦ = 2");
	ЗаполнитьПоказатели("ПроцентОтклоненияОтЦеныКомпании", НСтр("ru='Отклонение от цены компании в %%';uk='Відхилення від ціни компанії в %%'", КодЯзыкаПечать), ИСТИНА, "ЧЦ = 15; ЧДЦ = 2");

	ЗаполнитьПредставление("Номенклатура",               "Номенклатура",                "Строки",  ИСТИНА);
	ЗаполнитьПредставление("ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "Строки",  ИСТИНА);
	ЗаполнитьПредставление("Контрагент",                 "Контрагент",                  "Колонки", ИСТИНА);
	ЗаполнитьПредставление("ТипЦен",                     НСтр("ru='Тип цен';uk='Тип цін'"),                     "Колонки", ИСТИНА);
	ЗаполнитьПредставление("ПериодЦен",                  НСтр("ru='Дата регистрации цены';uk='Дата реєстрації ціни'"),       "Колонки", ИСТИНА);
	...
	"Номенклатура",
	НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"),
	"Контрагент",
	НСтр("ru='Тип цен';uk='Тип цін'"),
	НСтр("ru='Дата регистрации цены';uk='Дата реєстрації ціни'"),
	"Цена",
	НСтр("ru='Процент отклонения от цены компании';uk='Відсоток відхилення від ціни компанії'"));
	...
	Если ИспользоватьСвойстваИКатегории Тогда
		...
		НоваяСтрока.Представление = НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'");
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ТолькоЗаголовок = Ложь) Экспорт
	...
	Если ПостроительОтчета.ИзмеренияСтроки.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не задано ни одной группировки для строк отчета!';uk='Не вказано жодного групування для рядків звіту!'"));
		Возврат;
	КонецЕсли;
	...
	Если КурсВалюты.Курс = 0 тогда
		Предупреждение(НСтр("ru='Курс валюты отчета ""';uk='Курс валюти звіту ""'") + ВалютаОтчета.Наименование+""" на дату " + строка(ДатаКурса)+НСтр("ru=' не определен или нулевой. Формирование отчета невозможно.';uk=' не визначений або нульовий. Формування звіту неможливо.'"));
		возврат;
	КонецЕсли;
	
	Если КурсВалюты.Кратность = 0 тогда
		Предупреждение(НСтр("ru='Кратность курса валюты отчета ""';uk='Кратність курсу валюти звіту ""'") + ВалютаОтчета.Наименование+""" на дату " + строка(ДатаКурса)+НСтр("ru=' не определена или нулевая. Формирование отчета невозможно.';uk=' не визначена або нульова. Формування звіту неможливо.'"));
		возврат;
	КонецЕсли;
	...
	СтруктураПредставлениеПолей.Вставить("Цена", НСтр("ru='Цена в ';uk='Ціна у '", КодЯзыкаПечать)+ВалютаОтчета);
	мТаблицаПоказатели.Найти("Цена", "ИмяПоля").ПредставлениеПоля = НСтр("ru='Цена в ';uk='Ціна у '", КодЯзыкаПечать)+ВалютаОтчета;
	Показатели.Найти("Цена", "Имя").Представление = НСтр("ru='Цена в ';uk='Ціна у '", КодЯзыкаПечать)+ВалютаОтчета;
	МакетОтчета = ПолучитьМакет("Макет");
	// 37490 <<
	ОбластьИтого = МакетОтчета.Область("ОбластьИтого");
	ОбластьИтого.Текст = НСтр("ru='ИТОГО:';uk='ВСЬОГО:'", КодЯзыкаПечать);
	// 37490 >>
	...
	Для Сч=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		СписокИзмерений = СписокИзмерений + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ИзмеренияСтроки[Сч].Представление) + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.ИзмеренияСтроки[Сч].ТипИзмерения));
	КонецЦикла; 

	СписокКолонок = "";
	Для Сч=0 По ПостроительОтчета.ИзмеренияКолонки.Количество()-1 Цикл
		СписокКолонок = СписокКолонок + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.ИзмеренияКолонки[Сч].Представление) + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.ИзмеренияКолонки[Сч].ТипИзмерения));
	КонецЦикла; 
	...
	Для Сч=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		Если ПостроительОтчета.Отбор[Сч].Использование Тогда
			СписокОтбор = СписокОтбор + ", " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПостроительОтчета.Отбор[Сч].Представление) + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПостроительОтчета.Отбор[Сч].ВидСравнения)) + " " + ПостроительОтчета.Отбор[Сч].Значение;
		КонецЕсли;
	КонецЦикла; 
	...
	Если ДатаОтчета = '00010101000000' Тогда
		Если ТолькоЗаголовок Тогда
			ОписаниеПериода = НСтр("ru='на текущее время ';uk='на поточний час '", КодЯзыкаПечать) + Формат(ТекущаяДата(), "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
		Иначе
			ОписаниеПериода = НСтр("ru='на ';uk='на '", КодЯзыкаПечать) + Формат(ТекущаяДата(), "ДФ = ""дд.ММ.гггг ЧЧ:мм:сс""; ДП = ""...""");
		КонецЕсли; 
	Иначе
		ОписаниеПериода = НСтр("ru='на конец дня ';uk='на кінець дня '", КодЯзыкаПечать) + Формат(ДатаОтчета, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
	КонецЕсли;
	
	ЗаголовокОтчета.Параметры.Период          = НСтр("ru='Данные ';uk='Дані '", КодЯзыкаПечать)+ ОписаниеПериода;
	ЗаголовокОтчета.Параметры.Измерения       = НСтр("ru='Строки: ';uk='Рядки: '", КодЯзыкаПечать) + Сред(СписокИзмерений, 2);
	ЗаголовокОтчета.Параметры.Колонки         = НСтр("ru='Колонки: ';uk='Колонки: '", КодЯзыкаПечать) + Сред(СписокКолонок, 2);
	ЗаголовокОтчета.Параметры.Показатели      = НСтр("ru='Показатели: ';uk='Показники: '", КодЯзыкаПечать)  + Сред(СписокПоказателей, 2);;
	ЗаголовокОтчета.Параметры.Отбор           = НСтр("ru='Отбор: ';uk='Відбір: '", КодЯзыкаПечать)      + Сред(СписокОтбор,2);
...
// 37490 >>
мНазваниеОтчета = НСтр("ru='Анализ цен';uk='Аналіз цін'", КодЯзыкаПечать);
