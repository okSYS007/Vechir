...
Процедура ЗаполнитьНачальныеНастройки(ВосстановлениеНастройки = Ложь) Экспорт
	...
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество полученной тары (в ед. хранения)';uk='Кількість отриманої тари (в од. зберігання)'", КодЯзыкаПечать);
	СтрокаПоказателя.ИспользованиеПоказателя = Истина;

	СтрокаПоказателя = ПоказателиОтчета.Добавить();
	СтрокаПоказателя.ИмяПоказателя = "КоличествоБазовыхЕдПолученных";
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество полученной тары (в базовых ед.)';uk='Кількість отриманої тари (у базових од.)'", КодЯзыкаПечать);

	СтрокаПоказателя = ПоказателиОтчета.Добавить();
	СтрокаПоказателя.ИмяПоказателя = "СуммаВзаиморасчетовПолученных";
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Сумма полученной тары в валюте взаиморасчетов';uk='Сума отриманої тари у валюті взаєморозрахунків'", КодЯзыкаПечать);
	СтрокаПоказателя.ИспользованиеПоказателя = Истина;
	СтрокаПоказателя = ПоказателиОтчета.Добавить();
	
	СтрокаПоказателя.ИмяПоказателя = "КоличествоПереданных";
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество переданной тары (в ед. хранения)';uk='Кількість переданої тари (в од. зберігання)'", КодЯзыкаПечать);
	СтрокаПоказателя.ИспользованиеПоказателя = Истина;

	СтрокаПоказателя = ПоказателиОтчета.Добавить();
	СтрокаПоказателя.ИмяПоказателя = "КоличествоБазовыхЕдПереданных";
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Количество переданной тары (в базовых ед.)';uk='Кількість переданої тари (у базових од.)'", КодЯзыкаПечать);

	СтрокаПоказателя = ПоказателиОтчета.Добавить();
	СтрокаПоказателя.ИмяПоказателя = "СуммаВзаиморасчетовПереданных";
	СтрокаПоказателя.ПредставлениеПоказателя = НСтр("ru='Сумма переданной тары в валюте взаиморасчетов';uk='Сума переданої тари у валюті взаєморозрахунків'", КодЯзыкаПечать);
	СтрокаПоказателя.ИспользованиеПоказателя = Истина;

	СтрокаГруппировки = ГруппировкиОтчета.Добавить();
	СтрокаГруппировки.ИмяГруппировки = "Контрагент";
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Контрагент';uk='Контрагент'", КодЯзыкаПечать);
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Договор контрагента';uk='Договір контрагента'", КодЯзыкаПечать);
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Валюта взаиморасчетов';uk='Валюта взаєморозрахунків'", КодЯзыкаПечать);
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Номенклатура';uk='Номенклатура'", КодЯзыкаПечать);
	...
	СтрокаГруппировки.ПредставлениеГруппировки = НСтр("ru='Единица хранения остатков';uk='Одиниця зберігання залишків'", КодЯзыкаПечать);
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Каждого Строка Из ГруппировкиОтчета цикл
		...
		Если ПустаяСтрока(Строка.ИмяГруппировки) Тогда
			Предупреждение(НСтр("ru='Не указано имя группировки!';uk=""Не вказано ім'я групування!"""));
			Возврат;
		КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаСписокВсехФильтров Цикл
		...
		Если ПустаяСтрока(Строка.ИмяФильтра) Тогда
			Предупреждение(НСтр("ru='Не указано имя отбора!';uk=""Не вказано ім'я відбору!"""));
			Возврат;
		КонецЕсли;
	...
	Если ПоказателиОтчета.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не выбрано ни одного показателя!';uk='Не обрано жодного показника!'"));
		ЕстьОшибки = Истина;
		Возврат;
	КонецЕсли;
	...
	Для Каждого Строка Из ПоказателиОтчета цикл
		Если ПустаяСтрока(Строка.ИмяПоказателя) Тогда
			Предупреждение(НСтр("ru='Не указано имя показателя!';uk=""Не вказано ім'я показника!"""));
			Возврат;
		КонецЕсли;
		// Если не задано представление, берем его из имени
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Ведомость по возвратной таре';uk='Відомість по зворотній тарі'", КодЯзыкаПечать);
...
мСтруктураНазванийПолейСортировки.Вставить("НачальныйОстаток", НСтр("ru='Нач.ост.';uk='Поч.зал.'"));
мСтруктураНазванийПолейСортировки.Вставить("КонечныйОстаток",  НСтр("ru='Кон.ост.';uk='Кін.зал.'"));
мСтруктураНазванийПолейСортировки.Вставить("ЛимитНачальныйОстаток", НСтр("ru='Лимит нач.';uk='Ліміт поч.'"));
мСтруктураНазванийПолейСортировки.Вставить("ЛимитКонечныйОстаток",  НСтр("ru='Лимит кон.';uk='Ліміт кін.'"));
мСтруктураНазванийПолейСортировки.Вставить("Приход", НСтр("ru='Приход';uk='Надходження'"));
мСтруктураНазванийПолейСортировки.Вставить("Расход", НСтр("ru='Расход';uk='Видаток'"));
