...
Функция ПолучитьАВСКласс()
	...
	Если ТаблицаЗапроса.Количество() = 0 Тогда
		Возврат НСтр("ru='Не задан';uk='Не заданий'", КодЯзыкаПечать);
	Иначе
...
Функция ПолучитьСтадиюВзаимоотношений()
	...
	Если ТаблицаЗапроса.Количество() = 0 Тогда
		Возврат НСтр("ru='Не задана';uk='Не задана'", КодЯзыкаПечать);
	Иначе
...
Функция ПолучитьИсточникИнформацииПриОбращении()
	...
	Иначе
		ИсточникИнформации = НСтр("ru='Не указан';uk='Не зазначений'", КодЯзыкаПечать);
	КонецЕсли;
...
Процедура ВывестиДанные(Таб, ПорядковыйНомер, Имя)
	...
	Если Имя = "ОбщиеДанные" Тогда
		...
		Если ЗначениеЗаполнено(КонтрагентОтчета.ЮрФизЛицо) Тогда
			Если КонтрагентОтчета.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
				Секция.Параметры.ЮрФизЛицо = НСтр("ru=' (Юридическое лицо)';uk=' (Юридична особа)'", КодЯзыкаПечать);
			Иначе //Физическое лицо
				Секция.Параметры.ЮрФизЛицо = НСтр("ru=' (Физическое лицо)';uk=' (Фізична особа)'", КодЯзыкаПечать);
			КонецЕсли; 
		Иначе
			Секция.Параметры.ЮрФизЛицо = НСтр("ru='Не задано';uk='Не задано'", КодЯзыкаПечать);
		КонецЕсли; 
		...
		Если ЗначениеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент) Тогда
			...
			Если ЗначениеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент.ЮрФизЛицо) Тогда
				Если КонтрагентОтчета.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
					СтрокаГоловногоКонтрагента = СтрокаГоловногоКонтрагента + НСтр("ru=' (Юридическое лицо)';uk=' (Юридична особа)'", КодЯзыкаПечать);
				Иначе //Физическое лицо
					СтрокаГоловногоКонтрагента = СтрокаГоловногоКонтрагента + НСтр("ru=' (Физическое лицо)';uk=' (Фізична особа)'", КодЯзыкаПечать);
				КонецЕсли; 
			КонецЕсли; 
		...
		Иначе
			Секция.Параметры.ГоловнойКонтрагент = НСтр("ru='Не задан';uk='Не заданий'", КодЯзыкаПечать);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет) тогда
			...
			Если ЗначениеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет.Банк) Тогда
				...
				Секция.Параметры.МФО        = КонтрагентОтчета.ОсновнойБанковскийСчет.Банк.Код;
	...
	ИначеЕсли Имя = "РежимРаботы" Тогда
		...
		Секция.Параметры.График = ?(ПустаяСтрока(КонтрагентОтчета.РасписаниеРаботыСтрокой), НСтр("ru='Не задан';uk='Не заданий'", КодЯзыкаПечать), КонтрагентОтчета.РасписаниеРаботыСтрокой);
		Таб.Вывести(Секция,2);
	...
	ИначеЕсли Имя = "КонтЛица" Тогда
		...
		Для каждого СтрокаКЛ Из ТаблицаКЛ Цикл
			...
			Если ЗначениеЗаполнено(СтрокаКЛ.Роль) Тогда
				ПредставлениеКонтактногоЛица = ПредставлениеКонтактногоЛица + НСтр("ru=' (роль: ';uk=' (роль: '") + СокрЛП(СтрокаКЛ.Роль) + ")";
			КонецЕсли; 
	...
	ИначеЕсли Имя = "ПараметрыПоставщикаПокупателя" Тогда
		...
		Если КонтрагентОтчета.Покупатель Тогда
			...
			Секция.Параметры.Менеджер = ?(КонтрагентОтчета.ОсновнойМенеджерПокупателя.Пустая(),НСтр("ru='НЕТ';uk='НІ'", КодЯзыкаПечать),СокрЛП(КонтрагентОтчета.ОсновнойМенеджерПокупателя.Наименование));
			Таб.Вывести(Секция,2);
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если Не ЗначениеЗаполнено(КонтрагентОтчета) Тогда
		Предупреждение(НСтр("ru='Выберите контрагента!';uk='Виберіть контрагента!'", КодЯзыкаПечать));
		Возврат;
	КонецЕсли;
	
	Если КонтрагентОтчета.ЭтоГруппа Тогда
		Предупреждение(НСтр("ru='Отчет нельзя формировать по группе элементов!';uk='Звіт не можна формувати по групі елементів!'", КодЯзыкаПечать));
		Возврат;
	КонецЕсли; 
...
СписокНастроек.Добавить("ОбщиеДанные",НСтр("ru='Общие данные контрагента';uk='Загальні дані контрагента'")                           ,Истина);
СписокНастроек.Добавить("РежимРаботы",НСтр("ru='Режим работы контрагента';uk='Режим роботи контрагента'")                           ,Истина);
СписокНастроек.Добавить("ПараметрыПоставщикаПокупателя",НСтр("ru='Параметры поставщика и покупателя';uk='Параметри постачальника і покупця'"),Истина);
СписокНастроек.Добавить("КонтИнф",НСтр("ru='Контактная информация контрагента';uk='Контактна інформація контрагента'")                      ,Истина);
СписокНастроек.Добавить("КонтЛица",НСтр("ru='Контактные лица контрагента';uk='Контактні особи контрагента'")                           ,Истина);
СписокНастроек.Добавить("СвойстваИЗначения",НСтр("ru='Прочие свойства и значения контрагента';uk='Інші властивості і значення контрагента'")       ,Истина);
