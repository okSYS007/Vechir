...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ОбменДанными.Загрузка Тогда
		Если ПустаяСтрока(Идентификатор) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан идентификатор обработки обслуживания!';uk='Не вказаний ідентифікатор обробки обслуговування!'"));
			Отказ = Истина;
		Иначе
			Запрос = Новый Запрос(
			"ВЫБРАТЬ ПЕРВЫЕ 1
			...
			Если Не Запрос.Пустой() Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Идентификатор ""';uk='Ідентифікатор ""'")
				                 + Идентификатор
				                 + НСтр("ru='"" уже используется для элемента с кодом ';uk='"" вже використовується для елемента з кодом '")
				                 + Запрос.Код);
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
