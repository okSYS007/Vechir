...
Функция ПроверитьЗаполнениеОбязательныхПолей() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(АдресЭлектроннойПочты) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указан адрес (E-mail) учетной записи.';uk='Не зазначена адреса (E-mail) облікового запису.'");
	КонецЕсли; 

	Если НЕ ЗначениеЗаполнено(SMTPСервер) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указано имя SMTP сервера.';uk=""Не зазначене ім'я SMTP сервера.""");
	КонецЕсли; 

	Если НЕ ЗначениеЗаполнено(ПортSMTP) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указан порт SMTP сервера.';uk='Не зазначений порт SMTP сервера.'");
	КонецЕсли; 

	Если НЕ ЗначениеЗаполнено(POP3Сервер) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указано имя POP3 сервера.';uk=""Не зазначене ім'я POP3 сервери.""");
	КонецЕсли; 

	Если НЕ ЗначениеЗаполнено(ПортPOP3) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указан порт POP3 сервера.';uk='Не зазначений порт POP3 сервери.'");
	КонецЕсли; 

	Если НЕ ЗначениеЗаполнено(Логин) Тогда
		ТекстОшибок = ТекстОшибок + Символы.ПС + НСтр("ru='Не указано имя пользователя для доступа к почтовому серверу.';uk=""Не зазначене ім'я користувача для доступу до поштового сервера.""");
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменДанными.Загрузка Тогда
		...
		СтрокаОшибки = НСтр("ru='Учетная запись ""';uk='Обліковий запис ""'") + Наименование + "<" + АдресЭлектроннойПочты + ">" + НСтр("ru=' не записана.';uk=' не записана.'");
		
		мНовыеГруппы = Неопределено;
		
		Если НЕ Константы.ИспользованиеВстроенногоПочтовогоКлиента.Получить() Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В настройках учета не установлено использование встроенного почтового клиента.';uk='У настройках обліку не встановлене використання убудованого поштового клієнта.'"),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Наименование) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указано представление учетной записи.';uk='Не зазначене представлення облікового запису.'"),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ГруппаУдаленные) И ЕстьСсылки(ГруппаУдаленные) Тогда
			ОбщегоНазначения.СообщитьОбОшибке((НСтр("ru='В группе ""';uk='У групі ""'") + ГруппаУдаленные + НСтр("ru='"" есть электронные письма. Данную группу нельзя устанавливать в качестве группы удаленных писем.';uk='"" є електронні листи. Дану групу не можна встановлювати як групу вилучених листів.'")),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(ГруппаЧерновики) И ЕстьСсылки(ГруппаЧерновики) Тогда
			ОбщегоНазначения.СообщитьОбОшибке((НСтр("ru='В группе ""';uk='У групі ""'") + ГруппаЧерновики + НСтр("ru='"" есть электронные письма. Данную группу нельзя устанавливать в качестве группы черновиков.';uk='"" є електронні листи. Дану групу не можна встановлювати як групу чернеток.'")),, СтрокаОшибки);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если ТаблицаПроверкиГрупп.Количество() > 0 Тогда
			...
			Для каждого СтрокаТаблицы Из ТаблицаПроверкиГрупп Цикл
				Если СтрокаТаблицы.Количество > 1 Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='У предопределенных групп Входящие, Исходящие, Удаленные и Черновики не должно быть повторяющихся значений.';uk='У напередвизначених груп Вхідні, Вихідні, Вилучені й Чернетки не повинно бути значень, що повторюються.'"),, СтрокаОшибки);
					Отказ = Истина;
					Возврат;
				КонецЕсли; 
			КонецЦикла; 
		...
		Если АвтоПолучениеОтправкаСообщений Тогда
			Если НЕ ЗначениеЗаполнено(ОтветственныйЗаАвтоПолучениеОтправкуСообщений) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан ответственный за автополучение/отправку писем.';uk='Не зазначений відповідальний за автоотримання/відправлення листів.'"),, СтрокаОшибки);
				Отказ = Истина;
			ИначеЕсли НЕ ЗначениеЗаполнено(ИнтервалАвтоПолученияОтправкиСообщений) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан интервал автополучения/отправки писем.';uk='Не зазначений інтервал автоотримання/відправлення листів.'"),, СтрокаОшибки);
				Отказ = Истина;
			Иначе
				...
				КонецЕсли; 
				Если Отказ Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='У ответственного за автополучение/отправку нет прав на запись писем.';uk='У відповідального за автоотримання/відправлення немає прав на запис листів.'"),, СтрокаОшибки);
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
