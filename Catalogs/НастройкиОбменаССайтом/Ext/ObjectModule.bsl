...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ОбменТоварами 
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Должен быть выбран хотя бы один из режимов выгрузки: ""Выгрузка товаров"" или ""Обмен заказами""!';uk='Повинен бути вибраний хоча б один з режимів вивантаження: ""Вивантаження товарів"" або ""Обмін замовленнями""!'"), Отказ);
	...
	Если ВыгружатьНаСайт Тогда
		...
		Если Не ЗначениеЗаполнено(HTTPОбменАдресСайта) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан WEB - сайт для обмена данными!';uk='Не вказаний WEB - сайт для обміну даними!'"), Отказ);
		КонецЕсли;
		
	Иначе
		
		Если Не ЗначениеЗаполнено(КаталогВыгрузки) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан каталог для обмена данными!';uk='Не вказаний каталог для обміну даними!'"), Отказ);
		КонецЕсли;
	...
	Если ВыгружатьТолькоИзменения Тогда
		...
		Если ОбменТоварами И НЕ ЗначениеЗаполнено(УзелОбменаТоварами) ТОгда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Узел плана обмена для контроля изменений товаров не указан!';uk='Вузол плану обміну для контролю змін товарів не вказаний!'"), Отказ);
		КонецЕсли;
		
		Если ОбменЗаказами И НЕ ЗначениеЗаполнено(УзелОбменаЗаказами) ТОгда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Узел плана обмена для контроля изменений заказов не указан!';uk='Вузол плану обміну для контролю змін замовлень не вказаний!'"), Отказ);
		КонецЕсли;
