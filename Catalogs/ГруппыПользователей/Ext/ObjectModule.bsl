	...
	#Если Клиент Тогда
		Если МнджрВрТблСоставГруппы <> Неопределено Тогда
			...
			Если НЕ ПустаяСтрока(СписокПользователей) Тогда												
				Сообщить(НСтр("ru='Для пользователей ';uk='Для користувачів '") + СписокПользователей + "), ");
				Сообщить(НСтр("ru='новые значения настроек, зависящие от вхождения в группы пользователей (значения доп. прав, дата запрета изменения данных и т.п.),';uk='нові значення настройок, що залежать від входження до групи користувачів (значення дод. прав, дата заборони зміни даних тощо),'"));
				Сообщить(НСтр("ru='вступят в силу только после того, как они перезапустят сеанс работы с программой.';uk='вступлять в силу тільки після того, як вони перезапустять сеанс роботи з програмою.'"));
			КонецЕсли;		
		КонецЕсли;
	#КонецЕсли
КонецПроцедуры
