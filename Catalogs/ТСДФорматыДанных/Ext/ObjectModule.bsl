...
Функция ПолучитьСоответствиеПолейТСД(Штрихкод, Номенклатура, ЕдиницаИзмерения,
	...
	КодНоменклатуры = СокрЛП(Номенклатура.Код);
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ОбменДанными.Загрузка Тогда
		...
		Если НЕ ЗначениеЗаполнено(Наименование) Тогда
			Ошибки = НСтр("ru=' - Не задано имя формата.';uk="" - Не задано ім'я формату.""");
		КонецЕсли;
		...
		Если Не ЕстьШтрихКод Тогда
			Если Не ПустаяСтрока(Ошибки) Тогда
				...
				|";
			КонецЕсли;
			Ошибки = Ошибки + НСтр("ru=' - Параметр номенклатуры ""Штрихкод"" должен выгружаться в обязательном порядке.';uk=' - Параметр номенклатури ""Штрихкод"" повинен вивантажуватися в обов''язковому порядку.'");
		КонецЕсли;
		...
		Если Не ЗаполненыПараметрыНоменклатуры Тогда
			Если Не ПустаяСтрока(Ошибки) Тогда
				...
				|";
			КонецЕсли;
			Ошибки = Ошибки + НСтр("ru=' - Обнаружены незаполненные параметры номенклатуры.';uk=' - Виявлені незаповнені параметри номенклатури.'");
		КонецЕсли;
		...
		Если Не ЗаполненыПоляТСД Тогда
			Если Не ПустаяСтрока(Ошибки) Тогда
				...
				|";
			КонецЕсли;
			Ошибки = Ошибки + НСтр("ru=' - Обнаружены незаполненные поля терминала.';uk=' - Виявлені незаповнені поля терміналу.'");
		КонецЕсли;
		...
		Если Не УникальныеПараметрыНоменклатуры Тогда
			Если Не ПустаяСтрока(Ошибки) Тогда
				...
				|";
			КонецЕсли;
			Ошибки = Ошибки + НСтр("ru=' - Обнаружен параметр номенклатуры, выгружаемый в два поля одновременно.';uk=' - Виявлено параметр номенклатури, що вивантажується в два поля одночасно.'");
		КонецЕсли;
		...
		Если Не УникальныеПоляТСД Тогда
			Если Не ПустаяСтрока(Ошибки) Тогда
				...
				|";
			КонецЕсли;
			Ошибки = Ошибки + НСтр("ru=' - Обнаружено поле терминала, в которое выгружаются несколько параметров одновременно.';uk=' - Виявлено поле терміналу, в яке вивантажуються декілька параметрів одночасно.'");
		КонецЕсли;
		
		Если Не ПустаяСтрока(Ошибки) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='При попытке записи были обнаружены следующие ошибки:"
"';uk='При спробі запису були виявлені наступні помилки:"
"'") + Ошибки);
			Отказ = Истина;
		КонецЕсли;
