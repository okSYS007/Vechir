...
Процедура ПриЗаписи(Отказ) 
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при записи изменений в настройку обмена ""';uk='Помилка при записі змін у настройку обміну ""'") + Ссылка + """ : " + ОписаниеОшибки(), Отказ);
	КонецПопытки;	
...
Процедура ПередЗаписью(Отказ)
	...
	Если БазоваяПоставка
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В базовой версии недоступен обмен через распределенную информационную базу';uk='У базовій версії недоступний обмін через розподілену інформаційну базу'"), Отказ);	
	...
	Если ЭтотУзелОбмена = УзелИнформационнойБазы Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В качестве узла обмена выбран узел, соответствующий текущей информационной базе.';uk='У якості вузла обміну обраний вузол, що відповідає поточній інформаційній базі.'"), Отказ);
	...
	Если OnLineОбмен Тогда
		...
		Если (ПроизводитьПриемСообщений
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заданы правила обмена данными для выгрузки в базу обмена';uk='Не задані правила обміну даними для вивантаження в базу обміну'"), Отказ);		
		...
		Если ТипНастройки = Перечисления.ТипыАвтоматическогоОбменаДанными.ОбменЧерезComСоединение Тогда
			...
			Если ПроизводитьПриемСообщений Тогда
				...
				Если РазмерХранилищаПравилЗагрузки = 0 Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заданы правила для выгрузки из базы обмена в текущую базу';uk='Не задані правила для вивантаження з бази обміну в поточну базу'"), Отказ);		
			...
			Если ТипИнформационнойБазыДляПодключения Тогда
				...
				Если НЕ ЗначениеЗаполнено(КаталогИнформационнойБазыДляПодключения) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнено поле ""Каталог информационной базы для подключения""';uk='Не заповнене поле ""Каталог інформаційної бази для підключення""'"), Отказ);
				КонецЕсли;
				
			Иначе
				
				Если НЕ ЗначениеЗаполнено(ИмяСервераИнформационнойБазыДляПодключения) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнено поле ""Сервер 1С:Предприятия""';uk='Не заповнене поле ""Сервер 1С:Підприємства""'"), Отказ);
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(ИмяИнформационнойБазыНаСервереДляПодключения) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнено поле ""Имя информационной базы""';uk='Не заповнене поле ""Ім''я інформаційної бази""'"), Отказ);
				КонецЕсли;
