...
Функция ПолучитьПредставлениеПериодичности(Периодичность) 
	...
	Если Периодичность = 6 Тогда
		Представление = НСтр("ru='по дням;uk='по дням'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 7 Тогда
		Представление = НСтр("ru='по неделям';uk='по тижнях'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 8 Тогда
		Представление = НСтр("ru='по декадам';uk='по декадах'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 9 Тогда
		Представление = НСтр("ru='по месяцам';uk='по місяцях'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 10 Тогда
		Представление = НСтр("ru='по кварталам';uk='по кварталах'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 11 Тогда
		Представление = НСтр("ru='по полугодиям';uk='по півріччях'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	ИначеЕсли Периодичность = 12 Тогда
		Представление = НСтр("ru='по годам';uk='по роках'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	КонецЕсли;
...
Функция ПолучитьТекстОбособленныхПодразделений(Организация) Экспорт
	...
	ТекстОрганизации = ТекстОрганизации + НСтр("ru=' с обособленными подразделениями';uk=' с відокремленими підрозділами'");
...
Функция ПолучитьПредставлениеПериода(ОтчетОбъект = Неопределено, НачалоПериода = Неопределено, КонецПериода = Неопределено, ТолькоДаты  = Ложь) Экспорт
	...
	ФорматЛ = "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + "; ";
	
	Если ЗначениеЗаполнено(КонецПериода) Тогда 
		Если КонецПериода >= НачалоПериода Тогда
			ТекстПериод = ?(ТолькоДаты, "", " за ") + ПредставлениеПериода(НачалоДня(НачалоПериода), КонецДня(КонецПериода), ФорматЛ + "ФП = Истина");
		Иначе
			ТекстПериод = "";
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(НачалоПериода) И Не ЗначениеЗаполнено(КонецПериода) Тогда
		ТекстПериод = ?(ТолькоДаты, "", " за ") + ПредставлениеПериода(НачалоДня(НачалоПериода), КонецДня(Дата(3999, 11, 11)), ФорматЛ + "ФП = Истина");
...
Функция ПолучитьТекстОрганизация(ОтчетОбъект = Неопределено, Организация = Неопределено, ВключатьОбособленныеПодразделения = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		СведенияОбОрганизации = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Организация, '20200101');
		ТекстОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,");
		Если ПустаяСтрока(ТекстОрганизации) Тогда
			ТекстОрганизации = СведенияОбОрганизации.Представление;
		КонецЕсли;
...
Функция ПолучитьЗапрещенныеПоля(ОтчетОбъект, Режим = "Группировка") Экспорт
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомостьПоСчету" Тогда
		Если Режим = "Группировка" Тогда
			...
			КонецЕсли;
			Для Индекс = КоличествоСубконто + 1 По 4 Цикл
				НоваяСтрока = РодителиПоля.Добавить();
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОборотыМеждуСубконто" Тогда
		Если Режим = "Группировка" Тогда
			...
			КонецЕсли;
			Для Индекс = КоличествоКорСубконто + 1 По 4 Цикл
				НоваяСтрока = РодителиПоля.Добавить();
				НоваяСтрока.Поле = "КорСубконто" + Индекс;
			КонецЦикла;
			НоваяСтрока = РодителиПоля.Добавить();
...
Процедура ЗаполнитьДополнительныеПоляПоУмолчанию(ОтчетОбъект) Экспорт
	...
	Для Каждого Группировка Из ДанныеОтчета.Группировка Цикл 
		...
		ИначеЕсли ТипПоля.СодержитТип(Тип("ДокументСсылка.ПоступлениеТоваровУслуг"))
			...
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПоступлениеТоваровУслугВНТТ"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ВозвратТоваровПоставщику"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ВозвратТоваровПоставщикуИзНТТ"))
			...
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.ПоступлениеНМА"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.РегистрацияВходящегоНалоговогоДокумента"))
			ИЛИ ТипПоля.СодержитТип(Тип("ДокументСсылка.СчетНаОплатуПоставщика"))
...
Процедура ВывестиОтчет(ОтчетОбъект, Результат, ДанныеРасшифровки, ВыводВФормуОтчета, ВнешниеНаборыДанных, ЗаполнятьДанныеРасшифровки = Истина, НастройкаКомпоновкиДанных = Неопределено) Экспорт
	...
	Локализация.ПроверитьПереводСхемыКомпоновкиДанных(
					Схема, 
					Строка(ОтчетОбъект), 
					// считаем, что наименования субконто заполнены на языке ИБ. Если язык ИБ совпадает с языком формирования отчетов - наименования субконто не переводим
					(ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов() <> ЛокализацияПовтИсп.КодЯзыкаИнформационнойБазы()) // ПереводитьСубконто
				);
	...
	Попытка
		...
		Локализация.ПеревестиМакетКомпоновкиДанных(
						МакетКомпоновки,
						,
						,
						// считаем, что наименования субконто заполнены на языке ИБ. Если язык ИБ совпадает с языком формирования отчетов - наименования субконто не переводим
...
Процедура ВыводЗаголовкаОтчета(ОтчетОбъект, Результат) Экспорт
	...
	Макет.КодЯзыкаМакета = ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов();
	...
	Если ПоказыватьВыводимыеДанные Тогда 
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Выводимые данные:';uk='Виведені дані:'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
		ОбластьОписаниеНастроек.Параметры.ОписаниеНастроекОтчета = ЛокализацияПовтИсп.ПолучитьПереводДляОтчетовНаСКД(ТекстВыводимыеДанные);
		Результат.Вывести(ОбластьОписаниеНастроек);
	КонецЕсли;
	
	// Отбор
	ТекстОтбор = Локализация.ПеревестиОтборСКД(ОтчетОбъект.КомпоновщикНастроек);
	ТекстПодразделение = "";
	
	Если Не ПустаяСтрока(ТекстОтбор) Тогда
		ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Отбор:';uk='Відбір:'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
	...
	Если ИдентификаторОбъекта <>  "ОтчетОбъект.ОборотноСальдоваяВедомость" 
		...
		Для Каждого СтрокаПорядок Из ОтчетОбъект.КомпоновщикНастроек.Настройки.Порядок.Элементы Цикл
			Если СтрокаПорядок.Использование Тогда
				ТекстСортировка = ТекстСортировка + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПолучитьСвойствоПоля(ОтчетОбъект.КомпоновщикНастроек, СтрокаПорядок.Поле));
				ТекстСортировка = ТекстСортировка + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(СтрокаПорядок.ТипУпорядочивания)) + ", ";
			КонецЕсли;
		КонецЦикла;
		ТекстСортировка = Лев(ТекстСортировка, СтрДлина(ТекстСортировка) - 2);
		
		Если Не ПустаяСтрока(ТекстСортировка) Тогда
			ОбластьОписаниеНастроек.Параметры.ИмяНастроекОтчета      = НСтр("ru='Сортировка:';uk='Сортування:'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
		...
		Если ПервыйЭлемент <> Неопределено Тогда
			...
			Для каждого Отбор из МассивПолей Цикл
				Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
					...
					ИначеЕсли Найти(Отбор.Поле, "Субконто") = 1 И (ИдентификаторОбъекта = "ОтчетОбъект.АнализСчета" ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.ОборотыСчета") Тогда
						ПолеСубконто = СтрЗаменить(Отбор.Поле, "Субконто", "Субконто" + БухТипРесурса);
						ТиповыеОтчеты.ДобавитьОтбор(ФормаРасшифровка.КомпоновщикНастроек, ПолеСубконто, Отбор.Значение, ?(Отбор.Иерархия, ВидСравненияКомпоновкиДанных.ВИерархии, ВидСравненияКомпоновкиДанных.Равно));	
...
Процедура ОбработкаРасшифровкиОтчетовДляРуководителя(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ЗадолженностьПокупателей"
		...
		Если Субконто1 <> Неопределено И Субконто2 <> Неопределено Тогда
			Показатель = "Контрагент: " + Строка(Субконто1) + Символы.ПС + НСтр("ru='Договор: ';uk='Договір: '") + Строка(Субконто2);
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОстаткиДенежныхСредств" Тогда
		...
		Если ТипЗнч(КорСубконто1) = Тип("СправочникСсылка.Валюты") 
			...
			Показатель = Показатель + Символы.ПС + НСтр("ru='Валюта: ';uk='Валюта: '") + КорСубконто1;
		КонецЕсли;
...
Функция ПолучитьПредставлениеНастройкиДляИстории(ОтчетОбъект) Экспорт 
	...
	Исключение
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомостьПоСчету"
			...
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АнализСчета"
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.ОборотыСчета" Тогда 
			Префикс = НСтр("ru='По счету ';uk='По рахунку '") + ОтчетОбъект.Счет;
		КонецЕсли;
		...
		СоответствиеПоказателей.Вставить("Контроль"         , "Разн.");
...
Процедура ЗаполнитьДенежныеСредства(СтрокаТаблицы, ПредшествующаяДата, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель = НСтр("ru='Остаток денежных средств';uk='Залишок грошових коштів'");
...
Процедура ЗаполнитьПоступлениеДенежныхСредств(СтрокаТаблицы, ПредшествующаяДата, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель = НСтр("ru='Поступление денежных средств';uk='Надходження грошових коштів'");
