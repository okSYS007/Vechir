...
Процедура РегистрироватьНовогоКонтрагента(Элемент, Текст, Значение, СтандартнаяОбработка, мТайпингКонтрагента, 
											мТекстТайпингаКонтрагента, мПоследнееЗначениеЭлементаТайпингаКонтрагент, 
											ЭтаФорма, Контрагент, КонтактноеЛицо, Модифицированность, 
											ДанныеНезарегистрированногоКонтрагента, ИспользоватьСервисПактум = Ложь) Экспорт
	...
	Если ТипЗнч(Элемент.Значение) <> Тип("Строка") Тогда
		...
		ТекстВопроса = НСтр("ru='Контрагент не найден. Зарегистрировать данные нового контрагента?';uk='Контрагент не знайдений. Зареєструвати дані нового контрагента?'");
		...
		Если ИспользоватьСервисПактум И Пактум_Сервер.Пактум_Права() Тогда
			
			ФормаРегистрацииНовогоКонтрагента = Справочники.Контрагенты.ПолучитьФормуНовогоЭлемента(, ЭтаФорма);
			ФормаРегистрацииНовогоКонтрагента.Наименование       = Текст;
			Если Пактум_Сервер.ВероятноЕДРПОУ(СокрЛП(Текст)) Тогда
				ФормаРегистрацииНовогоКонтрагента.КодПоЕДРПОУ = Текст;
				Если СтрДлина(Текст)=10 Тогда
					ФормаРегистрацииНовогоКонтрагента.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ФизЛицо;
					ФормаРегистрацииНовогоКонтрагента.ЭлементыФормы.ПанельЮрФизЛицо.Страницы.ФизЛицо.Видимость=Истина;
					ФормаРегистрацииНовогоКонтрагента.ЭлементыФормы.ПанельЮрФизЛицо.Страницы.ЮрЛицо.Видимость=Ложь;
				КонецЕсли;
			КонецЕсли;
			
			ФормаРегистрацииНовогоКонтрагента.Пактум_ОткрытДляЗаполненияСервисом = Истина;
			ФормаРегистрацииНовогоКонтрагента.Открыть();
			ЭтаФорма.Пактум_КонтрагентВыбран = Истина;
...
Процедура НачалоВыбораКонтактногоЛицаКонтрагента(Элемент, СтандартнаяОбработка, Контрагент = Неопределено) Экспорт
	...
	ФормаВыбора.Заголовок = НСтр("ru='Выбор контактного лица контрагента';uk='Вибір контактної особи контрагента'");
	ФормаВыбора.Открыть();
...
Процедура СформироватьАнализУсловийДоговораВзаиморасчетов(ТекущийПользователь, Ссылка) Экспорт
	...
	ЭлементОтбора = Отчет.Отбор.Добавить("ДокументУсловий",, НСтр("ru='Документ условий';uk='Документ умов'"));
...
Функция ПолучитьСтрокуУсловияДР() Экспорт
	...
	Возврат НСтр("ru='Поздравить с днём рождения';uk='Привітати з днем народження'");
...
Процедура ЗаписатьДанныеНезарегистрированногоКонтрагента(Ссылка, ДанныеНезарегистрированногоКонтрагента, Отказ) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Не удалось записать данные контрагента.';uk='Не вдалося записати дані контрагента.'"));
		Отказ = Истина;
	КонецПопытки;
...
Функция ПолучитьСписокИнтерваловНапоминания() Экспорт
	...
	СписокИнтервалов.Добавить(5*60,НСтр("ru='5 мин.';uk='5 хв.'"));
	СписокИнтервалов.Добавить(10*60,НСтр("ru='10 мин.';uk='10 хв.'"));
	СписокИнтервалов.Добавить(15*60,НСтр("ru='15 мин.';uk='15 хв.'"));
	СписокИнтервалов.Добавить(30*60,НСтр("ru='30 мин.';uk='30 хв.'"));
	СписокИнтервалов.Добавить(1*60*60,НСтр("ru='1 час';uk='1 година'"));
	СписокИнтервалов.Добавить(2*60*60,НСтр("ru='2 часа';uk='2 години'"));
	СписокИнтервалов.Добавить(4*60*60,НСтр("ru='4 часа';uk='4 години'"));
	СписокИнтервалов.Добавить(8*60*60,НСтр("ru='8 часов';uk='8 годин'"));
	СписокИнтервалов.Добавить(1*24*60*60,НСтр("ru='1 день';uk='1 день'"));
	СписокИнтервалов.Добавить(2*24*60*60,НСтр("ru='2 дня';uk='2 дні'"));
	СписокИнтервалов.Добавить(3*24*60*60,НСтр("ru='3 дня';uk='3 дні'"));
	СписокИнтервалов.Добавить(4*24*60*60,НСтр("ru='4 дня';uk='4 дні'"));
	СписокИнтервалов.Добавить(1*7*24*60*60,НСтр("ru='1 неделя';uk='1 тиждень'"));
