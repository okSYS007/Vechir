...
//	ДоПараметры - структура дополнительных параметров. по умолчанию - неопределено
//
Процедура ЗаполнитьШапкуДокументаПоОснованию(ДокументОбъект, ДокументОснование, ДопПараметры = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ЕстьРеквизитДокумента("СкладГруппа", МетаданныеДокументаОснования) Тогда
		...
		Иначе
			Если ТипЗнч(ДопПараметры) = Тип("Структура") 
				И ДопПараметры.Свойство("Склад") <> Неопределено 
				И ЗначениеЗаполнено(ДопПараметры.Склад) Тогда
				ДокументОбъект.Склад = ДопПараметры.Склад;
			Иначе
				ДокументОбъект.Склад = ?(ТипЗнч(ДокументОснование.СкладГруппа) = Тип("СправочникСсылка.Склады"), ДокументОснование.СкладГруппа, Неопределено);
			КонецЕсли;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДоговорКонтрагента", МетаданныеДокумента)
	   ...
	   И (НЕ ОбщегоНазначения.ЕстьРеквизитДокумента("Организация", МетаданныеДокумента) 
		...
		Иначе
			Если ТипЗнч(ДопПараметры) = Тип("Структура") 
				И ДопПараметры.Свойство("ДоговорКонтрагента") <> Неопределено 
				И ЗначениеЗаполнено(ДопПараметры.ДоговорКонтрагента) Тогда
				ДокументОбъект.ДоговорКонтрагента = ДопПараметры.ДоговорКонтрагента;
			Иначе
				ДокументОбъект.ДоговорКонтрагента = ДокументОснование.ДоговорКонтрагента;
			КонецЕсли;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("АвторасчетНДС", МетаданныеДокумента)
	   И ОбщегоНазначения.ЕстьРеквизитДокумента("АвторасчетНДС", МетаданныеДокументаОснования) Тогда
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ОтражатьВУправленческомУчете", МетаданныеДокумента) Тогда
		...
		Иначе
			ДокументОбъект.ОтражатьВУправленческомУчете = НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "НеОтражатьДокументыВУправленческомУчете");;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ОтражатьВБухгалтерскомУчете", МетаданныеДокумента) Тогда
		...
		Иначе
			ДокументОбъект.ОтражатьВБухгалтерскомУчете = НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "НеОтражатьДокументыВБухгалтерскомУчете");;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, ТипОперации = "", ПараметрОбъектКопирования = Неопределено, ПараметрОснование = неопределено, СтруктураПараметровДляПолученияДоговора = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ЕстьРеквизитДокумента("КурсВзаиморасчетов", МетаданныеДокумента) Тогда
		...
		Если СтруктураКурсаДокумента.Курс <> ДокументОбъект.КурсВзаиморасчетов 
		 Или СтруктураКурсаДокумента.Кратность <> ДокументОбъект.КратностьВзаиморасчетов Тогда
			Ответ = Вопрос(НСтр("ru='Курс валюты изменился. Поменять курс в документе?';uk='Курс валюти змінився. Змінити курс в документі?'"), РежимДиалогаВопрос.ДаНет);
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ПредставительОрганизации", МетаданныеДокумента) И 
		НЕ ЗначениеЗаполнено(ДокументОбъект.ПредставительОрганизации) Тогда
	  ДокументОбъект.ПредставительОрганизации = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОсновнойПредставительОрганизации");
	  Если НЕ ЗначениеЗаполнено(ДокументОбъект.ПредставительОрганизации) И ОбщегоНазначения.ЕстьРеквизитДокумента("Ответственный", МетаданныеДокумента) Тогда
		ДокументОбъект.ПредставительОрганизации = ДокументОбъект.Ответственный.ФизЛицо;
	  КонецЕсли;
  	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапкиДокумента(ДокументОбъект, СтруктураОбязательныхПолей, Отказ, Заголовок) Экспорт
	...
	Для каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
		...
		//Дандали В.27.10 на случай, если нет этого реквизита в документа
		Попытка
		...
		ИначеЕсли ТипыПланыСчетов.СодержитТип(ТипЗнч(Значение)) тогда
			...
			Если Значение.ЗапретитьИспользоватьВПроводках Тогда
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Реквизит ""%1"" : счет %2 ""%3"" нельзя использовать в проводках.';uk='Реквізит ""%1"" : рахунок %2 ""%3"" не можна використовувати в проводках.'"), СокрЛП(ПредставлениеРеквизита), СокрЛП(Значение), Значение.Наименование);
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения, Отказ, Заголовок);
			КонецЕсли;
...
Процедура ПриИзмененииЗначенияОрганизации(ДокументОбъект, СтруктураПараметровДляПолученияДоговора= Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("Организация", МетаданныеДокумента) Тогда
		...
		//Если ОбщегоНазначения.ЕстьРеквизитДокумента("ОбособленноеПодразделениеОрганизации", МетаданныеДокумента) 
		//	И ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
		//	
		//	ОбособленноеПодразделениеПоУмолчанию = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОсновноеОбособленноеПодразделениеОрганизации");
		//	
		//	// Если ОбособленноеПодразделение не заполнено.
		//	Если НЕ ЗначениеЗаполнено(ДокументОбъект.ОбособленноеПодразделениеОрганизации) Тогда
		//		
		//		// Если ПодразделениеПоУмолчанию сооответствует выбранной организации, подставляем его.
		//		Если ОбособленноеПодразделениеПоУмолчанию.Владелец = ДокументОбъект.Организация Тогда
		//			ДокументОбъект.ОбособленноеПодразделениеОрганизации = ОбособленноеПодразделениеПоУмолчанию;
		//			ДокументОбъект.ПоОбособленномуПодразделению = НЕ ДокументОбъект.ОбособленноеПодразделениеОрганизации.Пустая();
		//		КонецЕсли;
		//	
		//	Иначе // Если ПодразделениеОрганизации заполнено.
		//		// Если ПодразделениеОрганизации не соответствует выбранной организации.
		//		Если НЕ ДокументОбъект.ОбособленноеПодразделениеОрганизации.Владелец = ДокументОбъект.Организация Тогда
		//				
		//			ДокументОбъект.ОбособленноеПодразделениеОрганизации = Справочники.ОбособленныеПодразделенияОрганизаций.ПустаяСсылка();
		//			ДокументОбъект.ПоОбособленномуПодразделению = Ложь;
		//			
		//			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Значение в поле ""Обособленное подразделение"" не соответствует выбранной организации и было очищено.';uk='Значення в полі ""Відокремлений підрозділ"" не відповідає обраній організації й було очищено.'"));
		//				
		//		КонецЕсли;	
		//	КонецЕсли;
		//КонецЕсли;
...
Процедура ИзменитьЦеныВалюту(ДокументОбъект, СпособЗаполненияЦен, СтруктураРеквизитовДокумента, 
	...
	Если СтруктураНеобрабатываемыхТабличныхЧастей = Неопределено Тогда
		СтруктураНеобрабатываемыхТабличныхЧастей = Новый Структура;
	КонецЕсли;
	СтруктураНеобрабатываемыхТабличныхЧастей.Вставить("ДокументыРасчетовСКонтрагентом");
	...
	Иначе
		...
		Если (СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатурыКонтрагентов
		 ...
		 ИЛИ (СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры
			...
			СтрокаВозможныхВыборов = ?(СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры, НСтр("ru='покупателями или с комиссионерами';uk='покупцями або з комісіонерами'"), НСтр("ru='поставщиками или комитентами';uk='постачальниками або комітентами'"));
		КонецЕсли;
		...
		Если НЕ МожноПродолжать Тогда
			...
			Иначе
#Если Клиент Тогда
				ТекстПредупреждения = НСтр("ru='В документе можно выбирать только договора с ';uk='У документі можна вибирати тільки договори з '")
...
#Если Клиент Тогда
			...
			Предупреждение(НСтр("ru='Выбранный договор принадлежит другому контрагенту. "
"Будет восстановлен старый договор.';uk='Обраний договір належить іншому контрагентові. "
...
"відмінні від установлених у документі:'");
		   ...
		   И (ЗначениеЗаполнено(Договор.ВалютаВзаиморасчетов)) Тогда
			...
			Если  ДокументОбъект.ВалютаДокумента <> НовыйВалютаДокумента Тогда
				...
				ТекстВопроса = ТекстВопроса + Символы.ПС + 
				                 НСтр("ru='Валюта документа: ""';uk='Валюта документа: ""'") +
				                ?(НЕ ЗначениеЗаполнено(ДокументОбъект.ВалютаДокумента), НСтр("ru='Не задана';uk='Не задана'"),СокрЛП(ДокументОбъект.ВалютаДокумента)) +
				                  НСтр("ru='"", договора: ""';uk='"", договору: ""'") +  СокрЛП(НовыйВалютаДокумента) + """.";
			КонецЕсли;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ЕстьРеквизитДокумента("ТипЦен", МетаданныеДокумента) Тогда
			...
			Если ДокументОбъект.ТипЦен <> НовыйТипЦен Тогда
				...
				ТекстВопроса = ТекстВопроса + Символы.ПС +
				                 НСтр("ru='Тип цен документа: ""';uk='Тип цін документа: ""'") +
				                 ?(НЕ ЗначениеЗаполнено(ДокументОбъект.ТипЦен), НСтр("ru='Не задан';uk='Не заданий'"),СокрЛП(ДокументОбъект.ТипЦен)) +
				                   НСтр("ru='"", договора: ""';uk='"", договору: ""'") + СокрЛП(НовыйТипЦен) + """.";
			КонецЕсли;
		КонецЕсли;

		Если НеобходимоПересчитать Тогда
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Пересчитать документ в соответствии с договором?';uk='Перерахувати документ відповідно до договору?'");
#Если Клиент Тогда
...
#Если Клиент Тогда
		Если НЕ ЗначениеЗаполнено(СтарыйДоговорКонтрагента) Тогда
			ДокументОбъект.ДоговорКонтрагента = Неопределено;
		Иначе
			ТекстПредупреждения = НСтр("ru='Валюта документа не совпадает ни с валютой договора, ни с валютой регламентированного учета!';uk='Валюта документа не збігається ні з валютою договору, ні з валютою регламентованого обліку!'")
			                    + Символы.ПС;
...
Процедура ПроверитьЗаполнениеРеквизитов(ОбъектПроверки, СписокИменРеквизитов, Отказ, ЖесткаяПроверка = Истина) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокИменРеквизитов Цикл
		...
		Если НЕ ЗначениеЗаполнено(ОбъектПроверки[ЭлементСписка.Значение]) Тогда
			...
			СообщениеОНевозможностиЗаписи = НСтр("ru='Реквизит ""';uk='Реквізит ""'")+СокрЛП(ЭлементСписка.Представление)+НСтр("ru='"" не заполнен!';uk='"" не заповнений!'");
...
Процедура ВыбратьАдресДоставки(СтандартнаяОбработка, ЭтаФорма, ДокументОбъект) Экспорт
	...
	Иначе
		Если ДокументОбъект.Контрагент.Пустая() Тогда
			ВвестиЗначение(ДокументОбъект.Контрагент, НСтр("ru='Выберите контрагента';uk='Виберіть контрагента'"));
	...
	Если НЕ КонтрагентДокумент.Пустая() Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитДокумента("Грузополучатель", ДокументОбъект.Метаданные())
			И ЗначениеЗаполнено(ДокументОбъект.Грузополучатель) Тогда
		
			ФормаВыбора.РегистрСведенийСписок.Отбор.Объект.Значение = ДокументОбъект.Грузополучатель;	
		...
		Если СтруктураАдреса <> Неопределено Тогда
			...
			Если НЕ ИзменятьДополнениеКАдресуДоставки Тогда
				Ответ = Вопрос(НСтр("ru='Дополнение к адресу доставки отличается от текущего значения в документе. Поменять дополнение к адресу доставки?';uk='Додаток до адреси доставки відрізняється від поточного значення в документі. Поміняти додаток до адресу доставки?'"), 
					РежимДиалогаВопрос.ДаНет, , , НСтр("ru='Изменение дополнения к адресу доставки';uk='Зміна доповнення до адреси доставки'"));
...
Функция ПолучитьСледующийДокумент(СтруктураОчередей, ДатаОкончания, МаксимальноеКоличествоДокументовВВыборке, Организация)
	СтруктураВозврата = Новый Структура("Ссылка, Дата, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете, Организация",Неопределено,Неопределено,Ложь,Ложь,Ложь,Организация);
...
Процедура ВыполнитьВосстановление (ДатаКонцаОбработки,
	...
	Если НЕ (ОтражатьВУправленческомУчете
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не выбран ни один из учетов!';uk='Не обраний жоден з обліків!'"));
		Возврат;
	...
	НачатьТранзакцию();
	#Если Клиент Тогда
		Состояние(НСтр("ru='Начало транзакции';uk='Початок транзакції'"));
	#КонецЕсли
	...
	Пока ЕстьДокументы Цикл
		...
		#Если Клиент Тогда
			Состояние(НСтр("ru='Выборка документов для проведения.';uk='Вибірка документів для проведення.'"));
		#КонецЕсли
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗакрытиеЗаказовПокупателей") Тогда
			...
			#Если Клиент Тогда
				Состояние(НСтр("ru='Проведение документа ';uk='Проведення документа '") + ДокументСсылка);
			#КонецЕсли
				
			ТаблицаСписания = ПолучитьТаблицуСтрокЗакрытиеЗаказовПокупателей(ДокументСсылка, Организация, ОтражатьВУправленческомУчете, ОтражатьВБухгалтерскомУчете);
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ВводВЭксплуатациюОС") Тогда
			...
			Если ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование тогда 
				...
				#Если Клиент Тогда
					Состояние(НСтр("ru='Обработано ';uk='Опрацьовано'")+ОбработаноСтрок+НСтр("ru='. фиксация транзакции.';uk='. фіксація транзакції.'"));
				#КонецЕсли
				...
				Попытка 
					...
					Сообщить(НСтр("ru='Перепроведен документ ';uk='Перепроведений документ '") + ДокументСсылка+ НСтр("ru='. Возможно изменение стоимости ОС.';uk='. Можлива зміна вартості ОЗ.'"));
				Исключение
					ОтменитьТранзакцию();
					Сообщить(НСтр("ru='Не удалось заблокировать документ ';uk='Не вдалося заблокувати документ '") + ДокументСсылка);
					НачатьТранзакцию();
				КонецПопытки;
			КонецЕсли;
		...
		Иначе
			...
			Если (Бух ИЛИ Упр)
				...
				Если ТипЗНЧ(ДокументСсылка) = Тип("ДокументСсылка.ЗакрытиеМесяца") Тогда
					ДокументМоментВремени = Новый МоментВремени(КонецМесяца(ДокументСсылка.Дата), ДокументСсылка);
				Иначе
		...
		Если ОбработаноСтрок >= МаксимальноеКоличествоСтрокВТранзакции Тогда
			...
			#КонецЕсли
			ЗафиксироватьТранзакцию();
			Сообщить(НСтр("ru='Обработано ';uk='Опрацьовано'")+ОбработаноСтрок+НСтр("ru='. Транзакция зафиксирована';uk='. Транзакція зафіксована'"));
...
Процедура ОтправитьДокументПоПочтеCommerceML(СсылкаНаДокумент, Параметр = Неопределено) Экспорт
		
	ОбработкаВыгрузки = Обработки.ВыгрузкаДанныхCommerceML.Создать();
	ОбработкаВыгрузки.ОтправитьИзДокумента(СсылкаНаДокумент,Параметр);
...
Функция НапечататьПриходРасходСРозничныхСкладовВЦенахАТТ(Док, ВалютаРегламентированногоУчета, ЭтоПриход) Экспорт
	...
	Если ЭтоПриход Тогда
		...
		ТекстНетДвижений = НСтр("ru='Данным документом не выполнен приход ни на один розничный склад!';uk='Даним документом не виконане надходження на жоден роздрібний склад!'");

		ТекстВсегоПринятоОтпущено = НСтр("ru='Всего принято ';uk='Всього прийнято '", КодЯзыкаПечать);
		ТекстПринялОтпустил = НСтр("ru='Принял';uk='Прийняв'", КодЯзыкаПечать);
	Иначе
		...
		ТекстНетДвижений = НСтр("ru='Данным документом не выполнен расход ни с одного розничного склада!';uk='Даним документом не виконаний видаток з жодного роздрібного складу!'");

		ТекстВсегоПринятоОтпущено = НСтр("ru='Всего отпущено ';uk='Всього відпущено '", КодЯзыкаПечать);
		ТекстПринялОтпустил = НСтр("ru='Отпустил';uk='Відпустив'", КодЯзыкаПечать);
	КонецЕсли;
	...
	Иначе
		...
		ЗаголовокДокумента = Локализация.ПолучитьЛокализованноеПредставление(Док, КодЯзыкаПечать);
		...
		Пока ВыборкаСклады.Следующий() Цикл
			...
			ОбластьСклад.Параметры.ПредставлениеСклада = СокрЛП(ВыборкаСклады.Склад) + ?(ЗначениеЗаполнено(ВыбМОЛ), НСтр("ru=' (материально-ответственное лицо: ';uk=' (матеріально-відповідальна особа: '", КодЯзыкаПечать) + ВыбМОЛ + ")", "");
			...
			ОбластьСуммаПрописью.Параметры.ИтоговаяСтрока = ТекстВсегоПринятоОтпущено + НомерСтроки + НСтр("ru=' наименований, на сумму ';uk=' найменувань, на суму '", КодЯзыкаПечать) + ОбщегоНазначения.ФорматСумм(Сумма, ВалютаРегламентированногоУчета);
			ОбластьСуммаПрописью.Параметры.СуммаПрописью  = ОбщегоНазначения.СформироватьСуммуПрописью(Сумма, ВалютаРегламентированногоУчета, КодЯзыкаПечать);
