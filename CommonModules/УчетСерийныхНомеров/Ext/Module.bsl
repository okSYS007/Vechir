...
Процедура ПроверитьКоличествоСерийныхНомеровВДокументе(ДокументОбъект, ИмяТЧ) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаНоменклатуры Цикл
		...
		Если СтрокаТабличнойЧасти.Количество <> КоличествоСерийныхНомеров Тогда
			#Если Клиент Тогда
			Сообщить(НСтр("ru='Количество серийных номеров не соответствует количеству номенклатуры (строка № ';uk='Кількість серійних номерів не відповідає кількості номенклатури (рядок № '") + СтрокаТабличнойЧасти.НомерСтроки + НСтр("ru=' таб. часть ""';uk=' таб. частина ""'") + ИмяТЧ + """)");
			#КонецЕсли
		КонецЕсли;
...
Функция ПолучитьЗначениеРазрядаСерийногоНомера(Источник, Значение, РазмерРазряда, СтруктураПараметров) Экспорт
	...
	НовоеЗначение = Прав(Строка(НовоеЗначение), РазмерРазряда);
...
Процедура ОбработкаПодбораНоменклатурыПоСерийномуНомеру(ЗначениеВыбора, СерийныеНомера,
	...
	Если СерийныйНомер <> Неопределено Тогда
		Если СерийныеНомера.Найти(СерийныйНомер, "СерийныйНомер") <> Неопределено Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Серийный номер ""';uk='Серійний номер ""'") + СокрЛП(СерийныйНомер.Код) + НСтр("ru='"" уже присутствует в списке серийных номеров.';uk='"" вже присутня у списку серійних номерів.'"));
...
//
Функция ПолучитьПоследнееЗначениеСчетчика(ВидСчетчика, Организация, Подразделение, НоменклатурнаяГруппа, Номенклатура) Экспорт
...
Функция ПечатьСерийныхНомеров(ТекДок, ИмяТабЧасти) Экспорт
	...
	Область.Параметры.ТекстЗаголовок = ОбщегоНазначения.СформироватьЗаголовокДокумента( ТекДок, , КодЯзыкаПечать);
