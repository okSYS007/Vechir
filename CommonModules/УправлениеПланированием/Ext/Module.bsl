...
Процедура ВыровнятьДатуПоНачалуПериода(ДатаПланирования, Периодичность, НазваниеДаты = Неопределено, ВыводитьСообщение = Ложь) Экспорт
	...
	Если ВыводитьСообщение И ДатаИзменена Тогда
		...
		Если НазваниеДаты = Неопределено Тогда
			...
			НазваниеДаты = НСтр("ru='Дата начала периода ';uk='Дата початку періоду '");
			
		КонецЕсли;
		
		ОбщегоНазначения.СообщитьОбОшибке(НазваниеДаты + НСтр("ru='изменена в соответствии с установленной периодичностью.';uk='змінена у відповідності з встановленою періодичністю.'"));
...
Процедура ВыровнятьДатуПоКонцуПериода(ДатаПланирования, Периодичность, НазваниеДаты = Неопределено, ВыводитьСообщение = Ложь) Экспорт
	...
	Если ВыводитьСообщение и ДатаИзменена Тогда
		...
		Если НазваниеДаты = Неопределено Тогда
			...
			НазваниеДаты = НСтр("ru='Дата окончания периода ';uk='Дата закінчення періоду '");
...
Процедура ВыровнятьПериод(ДатаНачПериода = Неопределено, ДатаКонПериода = Неопределено, Периодичность = Неопределено) Экспорт
	...
	ТекущаяПериодичность_Имя =  ОбщегоНазначения.ПолучитьИмяЭлементаПеречисленияПоЗначению(ТекущаяПериодичность);

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(&ДатаНач, " + ТекущаяПериодичность_Имя + ") КАК ДатаНач,
	|	КОНЕЦПЕРИОДА(&ДатаКон, " + ТекущаяПериодичность_Имя + ") КАК ДатаКон");
