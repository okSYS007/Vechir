...
Функция ИспользоватьВнутренниеЗаказы(Сообщать = Ложь) Экспорт
	...
	Если Не Флаг И Сообщать Тогда
		Сообщить(НСтр("ru='Использование внутренних заказов в данной конфигурации отключено.';uk='Використання внутрішніх замовлень у даній конфігурації відключено.'"), СтатусСообщения.ОченьВажное);
	КонецЕсли;
...
Функция ОстаткиОборудованияПоЗаказуПоставщику( Заказ, Договор, Знач КонДата, Знач СтатусПартии = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РегЗаказы.Номенклатура                 КАК Номенклатура,
	|	ВЫБОР КОГДА РегЗаказы.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент IS NULL ИЛИ РегЗаказы.ЕдиницаИзмерения.Коэффициент IS NULL ТОГДА 
	|		РегЗаказы.Цена
	|	ИНАЧЕ
	|		РегЗаказы.Цена * РегЗаказы.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент / РегЗаказы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Цена,
...
Процедура ЗаполнитьТабЧастьОборудованиеПоЗаказуПоставщику(ДокОбъект, ТабЧасть, ТабОстатки, ДопПараметры = Неопределено) Экспорт
	...
	ТабОстатки = ТабОстатки.Выгрузить();
...
Процедура ЗаполнитьОстаткамиТоваровСРезервомПоВнутреннемуЗаказу( ЭтотОбъект, Заказ, ТабЧасть, Склад, Знач КонДата, СтатусыПартий = Неопределено) Экспорт
	...
	ЕстьДокументРезерва = Не Метаданные.НайтиПоТипу(ТипЗнч(ТабЧасть)).Реквизиты.Найти("ДокументРезерва") = Неопределено;
	ЕстьВнутреннийЗаказ = Не Метаданные.НайтиПоТипу(ТипЗнч(ТабЧасть)).Реквизиты.Найти("ВнутреннийЗаказ") = Неопределено;
	ЕстьЗаказ			= Не Метаданные.НайтиПоТипу(ТипЗнч(ТабЧасть)).Реквизиты.Найти("Заказ") = Неопределено;
		...
		Пока ВыборкаПоХарактеристике.Следующий() цикл
			...
			Пока Выборка.Следующий() цикл
				...
				Если ВРезерве > 0 Тогда
					...
					Если ЕстьДокументРезерва Тогда
						НоваяСтрока.ДокументРезерва = Заказ;
					КонецЕсли;
					Если ЕстьВнутреннийЗаказ Тогда
						НоваяСтрока.ВнутреннийЗаказ = Заказ;	
					КонецЕсли;
...
Функция ДокументЗаписан(Ссылка,флСообщать=истина) Экспорт
	Если не ЗначениеЗаполнено(Ссылка) Тогда
		Если флСообщать Тогда
			Сообщить(НСтр("ru='Новый документ не записан! Невозможно сформировать отчет ""Анализ заказа""';uk='Новий документ не збережений! Неможливо сформувати звіт ""Аналіз замовлення""'"), СтатусСообщения.Важное);
		КонецЕсли;
		Возврат ложь;
	КонецЕсли;
	Возврат истина;
...
Функция ПравоДоступаКОтчету(ИмяОтчета, флСообщать=истина) Экспорт
	...
	КонецЕсли;
	Если флСообщать Тогда
		Сообщить(НСтр("ru='Нарушение прав доступа! Отчет: ';uk='Порушення прав доступу! Звіт: '")+МетаданныеОтчета.Синоним,СтатусСообщения.Важное);
	КонецЕсли;
	Возврат ложь;
КонецФункции
...
Процедура СформироватьОтчетАнализЗаказа(ЗаказСсылка,флПроверятьЗаписьЗаказа=ложь, флСообщатьОбОшибках) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ЗаказСсылка) 
		...
		Предупреждение(НСтр("ru='Не выбран заказ!';uk='Не вибрано замовлення!'"));
		Возврат;
	КонецЕсли;
...
Процедура Заказ_ЗаполнитьТабличныеЧастиВозможнымРазмещением(Параметры, Товары, ВозвратнаяТара) Экспорт
	...
	Если Авторезервирование Тогда
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрНакопления.ТоварыОрганизаций.Остатки(, Организация = &Организация И Комиссионер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) И Номенклатура В (&МассивНоменклатуры) "+?(ВестиУчетПоДопРазрезамРегл, " И Качество = &Новый", "")+") КАК ТоварыОрганизацийОстатки
	...
	ПраваНаПревышениеОстатковПоОрганизации = УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации();
...
Процедура ЗаполнитьЗаказПокупателяВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	Если ОбщегоНазначения.ЕстьТабЧастьДокумента("Материалы", МетаданныеДокумента) Тогда
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ЗаказПоставщику", МетаданныеДокумента, "Материалы") Тогда
...
Процедура ЗаполнитьЗаказПоставщикуВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	Для каждого СтрокаТЧ Из ДокументОбъект.Услуги Цикл
		СтрокаТЧ.ЗаказПоставщику = ЗаказИзШапки;
	КонецЦикла;
	Если ОбщегоНазначения.ЕстьТабЧастьДокумента("Оборудование", МетаданныеДокумента) Тогда
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ЗаказПоставщику", МетаданныеДокумента, "Оборудование") Тогда
...
Функция ИспользоватьЗаказВТабличнойЧасти(ДокументВидОперации, ДоговорКонтрагента, ВидОперации = "Реализация") Экспорт
	...
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='ИспользоватьЗаказВТабличнойЧасти: передан не верный вид операции документа.';uk='ИспользоватьЗаказВТабличнойЧасти: переданий не вірний вид операції документа.'"));
...
Функция ПодготовитьТаблицуПричинЗакрытияЗаказов(Заголовок, ДокСсылка, ИмяРеквизитаЗаказ, ИмяДокумента, ТаблицаПоЗаказам=неопределено) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаПричиныЗакрытия Цикл		
		Если ЗначениеЗаполнено(СтрокаТаблицы.ПредыдущееЗакрытие) Тогда
			#Если Клиент Тогда
			СтрокаСообщения = НСтр("ru='Внимание! ';uk='Увага! '")+СтрокаТаблицы.ЗаказПредставление+НСтр("ru=' уже был закрыт документом ';uk=' вже був закритий документом '")+СтрокаТаблицы.ПредыдущееЗакрытиеПредставление+НСтр("ru='"
"Причина закрытия данного заказа в учетных регистрах изменена не будет.';uk='"
"Причина закриття даного замовлення в облікових регістрах змінена не буде.'");
...
Процедура ПроверитьРезервированиеТарыПодЗаказСОбособленнымУчетом(ДокументОбъект,ИмяРевизитаЗаказ="", ЗаказВШапке=ложь, ИмяРеквизитаСклад="", Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект.ВозвратнаяТара Цикл
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(СтрокаТаблицы.НомерСтроки) +
		     НСтр("ru='"" табличной части ""';uk='"" табличної частини ""'") + ПредставлениеТабличнойЧасти + НСтр("ru='"": выполняется резервирование под заказ покупателя с обособленным учетом. ';uk='"": виконується резервування під замовлення покупця з відокремленим обліком. '") +
			 НСтр("ru='Резервировать возвратную тару под такие заказы запрещено ';uk='Резервувати зворотну тару під такі замовлення заборонено '"), Отказ, Заголовок);
...
Процедура ПроверитьРеквизитыЗаказов(ДокументОбъект, ИмяТабличнойЧасти, ИмяРевизитаЗаказ, 
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ]) И 
			...
			Если ДокЗаказ.Организация <> ДокументОбъект.Организация и не ДокументОбъект.Организация.А_УправленческийУчет Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='выбран заказ с неверным реквизитом ""Организация"". ';uk='обране замовлення з невірним реквізитом ""Організація"". '") +
				                   НСтр("ru='Заказы можно указывать только от организации - ';uk='Замовлення можна вказувати тільки від організації '") + СокрЛП(ДокументОбъект.Организация), Отказ, Заголовок);
			КонецЕсли;

			Если ДокЗаказ.Контрагент <> ДокументОбъект.Контрагент Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='выбран заказ с неверным реквизитом ""Контрагент"". ';uk='обране замовлення з невірним реквізитом ""Контрагент"". '") +
				                   НСтр("ru='Заказы можно указывать только с контрагентом - ';uk='Замовлення можна вказувати тільки з контрагентом - '") + СокрЛП(ДокументОбъект.Контрагент), Отказ, Заголовок);
			КонецЕсли;

			//Дандали В. 15.04.2023 
			//убрал Эту проверку т.к. с заказа нужно создавать несколько реализаций с разным видом договора
			//Если ДокЗаказ.ДоговорКонтрагента <> ДокументОбъект.ДоговорКонтрагента и не ДокументОбъект.Организация.А_УправленческийУчет Тогда
			//	ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='выбран заказ с неверным реквизитом ""Договор контрагента"". ';uk='обране замовлення з невірним реквізитом ""Договір контрагента"". '") +
			//	                   НСтр("ru='Заказы можно указывать только с договором - ';uk='Замовлення можна вказувати тільки з договором - '") + СокрЛП(ДокументОбъект.ДоговорКонтрагента), Отказ, Заголовок);
			//КонецЕсли;
			//Дандали В. 15.04.2023
			
			// Проверим даты Заказов, они не должны быть больше даты документа Реализации
			Если ДокЗаказ.Дата > ДокументОбъект.Дата Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата и время Заказа больше даты и времени документа!';uk='дата і час Замовлення більше дати та часу документа!'"), Отказ, Заголовок);
			КонецЕсли;
...
Процедура УстановитьДатуОплатыПоДоговору(ДокОбъект, РежимВызова = "", мСтароеЗначениеДоговора=неопределено)Экспорт
	...
	Иначе	
		...
		КонецЕсли;
	КонецЕсли;
	Если НачалоДня(ДокОбъект.ДатаОплаты) <> НачалоДня(ДатаОплатыНовая) Тогда
...
#Если Клиент Тогда
		...
		Если НЕ ПустаяСтрока(РежимВызова) Тогда
			...
			Если ТекстВопроса<>"" Тогда
				Ответ = Вопрос(ТекстВопроса + НСтр("ru=' Пересчитать дату оплаты?';uk=' Перерахувати дату оплати?'"), 
...
Процедура ДвиженияСтатусыЗаказов(Документ, Статус, тчДанные = Неопределено, ДокументДвижения = Неопределено) Экспорт
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		
		Для Каждого стрТовар из тчДанные Цикл
			
			НабЗап = РегистрыСведений.СтатусыЗаказов.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПокупателя.Установить(стрТовар.Заказ);
			...
			Если НабЗап.Количество() > 0 Тогда
				НабЗап[0].НоменклатураПоставщика 	= стрТовар.Номенклатура;
				НабЗап[0].Статус 					= ?(НабЗап[0].Статус = Перечисления.СтатусыЗаказов.КЗаказу,Статус,НабЗап[0].Статус) ;
				НабЗап[0].ДокументДвижения 			= ДокументДвижения;
				НабЗап.Записать();
			Иначе
				НовСтр = НабЗап.Добавить();
				ЗаполнитьЗначенияСвойств(НовСтр, стрТовар); 
				
				НовСтр.ЗаказПокупателя 	= Документ;
				Если НЕ ЗначениеЗаполнено(НовСтр.Статус) Тогда
					НовСтр.Статус = Перечисления.СтатусыЗаказов.КЗаказу;
				КонецЕсли;
		...
		//Внутренний заказ
		Если ТипЗнч(Документ.ДокументОснование) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
			
			ВнутреннийЗаказ 		= Документ.ДокументОснование;
			ТоварыВнутреннегоЗаказа = ВнутреннийЗаказ.Товары;
			
			Для Каждого стрТовар из тчДанные Цикл
				
				стрПоиска = Новый Структура;
				стрПоиска.Вставить("УникальныйИдентификаторСтроки", стрТовар.УникальныйИдентификаторСтроки);
				НайденныеСтроки = ТоварыВнутреннегоЗаказа.НайтиСтроки(стрПоиска);
				
				Если НайденныеСтроки.Количество() > 0 Тогда
					МенЗап 									= РегистрыСведений.СтатусыВнутреннихЗаказов.СоздатьМенеджерЗаписи();
					МенЗап.ВнутреннийЗаказ 					= ВнутреннийЗаказ;
					МенЗап.УникальныйИдентификаторСтроки 	= стрТовар.УникальныйИдентификаторСтроки;
					МенЗап.Прочитать();
					МенЗап.СтатусТовара = Перечисления.СтатусыЗаказов.Заказан;
					МенЗап.ДатаПрихода	= Документ.Дата;
					МенЗап.Записать();	
				КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Документ) = ТиП("ДокументСсылка.ПоступлениеТоваровУслуг") 
		...
		Для Каждого стрТовар из тчДанные Цикл
			...
			НабЗап = РегистрыСведений.СтатусыЗаказов.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПокупателя.Установить(стрТовар.Заказ);
			НабЗап.Отбор.НоменклатураПоставщика.Установить(стрТовар.Номенклатура);
			НабЗап.Прочитать();
			
			Если НабЗап.Количество() > 0 Тогда
				НабЗап[0].Статус 					= стрТовар.Статус;
				НабЗап[0].ДокументДвижения 			= ДокументДвижения;
				НабЗап.Записать();
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда  
		
		//Внутренний заказ
		Если ТипЗнч(Документ.ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику") 
			И ТипЗнч(Документ.ДокументОснование.ДокументОснование) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
			
			ВнутреннийЗаказ 		= Документ.ДокументОснование.ДокументОснование;
			ТоварыВнутреннегоЗаказа = ВнутреннийЗаказ.Товары;
			ТоварыЗаказаПоставщику	= Документ.ДокументОснование.Товары;
			
			Для Каждого стрТовар из ТоварыЗаказаПоставщику Цикл
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					МенЗап.УникальныйИдентификаторСтроки 	= стрТовар.УникальныйИдентификаторСтроки;
					МенЗап.Прочитать();
					МенЗап.СтатусОплаты = Перечисления.СтатусОплатыЗаказов.Оплачен;
					МенЗап.ДатаОплаты 	= Документ.ДатаРасхода;
					МенЗап.Записать();	
				КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее")
		...
		или ТипЗнч(Документ) = Тип("ДокументСсылка.УпрРасходныйКассовыйОрдер") Тогда
		
		Для Каждого Элем из Документ.РасшифровкаПлатежа Цикл
			
			Если НЕ ЗначениеЗаполнено(Элем.Сделка) Тогда
				Продолжить;
			КонецЕсли;
			Для Каждого стрТовар из Элем.Сделка.Товары цикл
				НабЗап = РегистрыСведений.СтатусОплатыЗаказов.СоздатьНаборЗаписей();
				НабЗап.Отбор.ЗаказПокупателя.Установить(Элем.Сделка.ДокументОснование);	
				НабЗап.Отбор.УникальныйИдентификаторСтроки.Установить(стрТовар.УникальныйИдентификаторСтроки);;	
				НабЗап.Прочитать();
				
				Для Каждого Запись ИЗ НабЗап Цикл
					
					Если Элем.Сделка.СуммаДокумента = Документ.СуммаДокумента Тогда  
						Запись.Статус 			= Перечисления.СтатусОплатыЗаказов.Оплачен;
					Иначе
						Запись.Статус 			= Перечисления.СтатусОплатыЗаказов.ЧастичноОплачен;
					КонецЕсли;
					Запись.ДокументДвижения = ДокументДвижения;
				КонецЦикла;
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
		...
		Если тчДанные.количество() Тогда
			
			Для Каждого стрТовар из тчДанные Цикл
				
				МенЗап = РегистрыСведений.СтатусыВнутреннихЗаказов.СоздатьМенеджерЗаписи();
				ЗаполнитьЗначенияСвойств(МенЗап, стрТовар); 
				МенЗап.ВнутреннийЗаказ = Документ;
				
				Если НЕ ЗначениеЗаполнено(стрТовар.СтатусТовара) Тогда
					МенЗап.СтатусТовара = Статус;
				Иначе
					МенЗап.СтатусТовара = стрТовар.СтатусТовара;
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(стрТовар.СтатусОплаты) Тогда
					МенЗап.СтатусОплаты = Перечисления.СтатусОплатыЗаказов.КОплате;
				Иначе
					МенЗап.СтатусОплаты = стрТовар.СтатусОплаты
				КонецЕсли;
	...
	Иначе 
		...
		Если тчДанные.количество() Тогда
			...
			Для Каждого стрТовар из тчДанные Цикл
				НабЗап = РегистрыСведений.СтатусыЗаказов.СоздатьНаборЗаписей();
				НабЗап.Отбор.ЗаказПокупателя.Установить(Документ);
				НабЗап.Отбор.УникальныйИдентификаторСтроки.Установить(стрТовар.УникальныйИдентификаторСтроки);
		...
		Если тчДанные.количество() Тогда 
			...
			Для Каждого стрТовар из тчДанные Цикл
				...
				НовСтр.ЗаказПокупателя 	= Документ;
				Если НЕ ЗначениеЗаполнено(НовСтр.Статус) Тогда
					НовСтр.Статус = Перечисления.СтатусОплатыЗаказов.КОплате;
				КонецЕсли;
				НабЗап.Записать();	
			КонецЦикла;
...
Процедура ДвиженияСтатусыЗаказовПоставщику(Документ, Статус, тчДанные = Неопределено, ДокументДвижения = Неопределено) Экспорт
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		
				
		Для Каждого стрТовар из тчДанные Цикл
			НабЗап = РегистрыСведений.СтатусыЗаказовПоставщику.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПоставщика.Установить(Документ);
			НабЗап.Отбор.УникальныйИдентификаторСтроки.Установить(стрТовар.УникальныйИдентификаторСтроки);
			НабЗап.Прочитать();
			
			
			если НабЗап.Количество() Тогда 
				Если НЕ ЗначениеЗаполнено(НабЗап[0].Статус) Тогда
					НабЗап[0].Статус = ?(НабЗап[0].Статус = Перечисления.СтатусыЗаказов.ПустаяСсылка(),Статус,НабЗап[0].Статус);
				КонецЕсли;
			...
			НовСтр.НоменклатураПоставщика = стрТовар.Номенклатура;
			НовСтр.ЗаказПоставщика 	= Документ;
			Если НЕ ЗначениеЗаполнено(НовСтр.Статус) Тогда
				НовСтр.Статус = ?(НовСтр.Статус = Перечисления.СтатусыЗаказов.ПустаяСсылка(),Статус,НовСтр.Статус);
			КонецЕсли;
		КонецЕсли;
				НабЗап.Записать();
		...
		НабЗап = РегистрыСведений.СтатусОплатыЗаказовПоставщику.СоздатьНаборЗаписей();
		НабЗап.Отбор.ЗаказПоставщика.Установить(Документ);
		НабЗап.Прочитать();
	...
	ИначеЕсли ТипЗнч(Документ) = ТиП("ДокументСсылка.ПоступлениеТоваровУслуг") 
		...
		Для Каждого стрТовар из тчДанные Цикл
			...
			НабЗап = РегистрыСведений.СтатусыЗаказовПоставщику.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПоставщика.Установить(ДокументДвижения);
			НабЗап.Отбор.НоменклатураПоставщика.Установить(стрТовар.Номенклатура);
			НабЗап.Прочитать();
		...
		//Дандали В. 24.11.2025 +
		ВнутреннийЗаказ = ЕстьДокументВнутреннийЗаказ(ДокументДвижения);
		Если ЗначениеЗаполнено(ВнутреннийЗаказ) Тогда
			
			НабЗап = РегистрыСведений.СтатусыВнутреннихЗаказов.СоздатьНаборЗаписей();
			НабЗап.Отбор.ВнутреннийЗаказ.Установить(ВнутреннийЗаказ);
			НабЗап.Прочитать();
			...
			Для Каждого стрТовар из тчДанные Цикл
				...
				НайденныеСтрокиВЗаказе = ВнутреннийЗаказ.Товары.НайтиСтроки(СтрПоиска);
				
				Для Каждого стрТоварвЗаказе из НайденныеСтрокиВЗаказе Цикл
					
					СтрПоиска = Новый Структура;
					СтрПоиска.Вставить("ВнутреннийЗаказ", ВнутреннийЗаказ);
					СтрПоиска.Вставить("УникальныйИдентификаторСтроки", стрТоварвЗаказе.УникальныйИдентификаторСтроки);
					
					Для Каждого СтрНабЗап из НабЗап Цикл
						
						Если СтрНабЗап.УникальныйИдентификаторСтроки = стрТоварвЗаказе.УникальныйИдентификаторСтроки Тогда
							Если стрТовар.Количество = 0 Тогда
								СтрНабЗап.СтатусТовара = Перечисления.СтатусыЗаказов.НаСкладе;
							Иначе
								СтрНабЗап.СтатусТовара = Перечисления.СтатусыЗаказов.ЧастичноОприходован;
							КонецЕсли;
							Прервать;
						КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") 
		...
		Для Каждого стрТовар из тчДанные Цикл
			...
			Если НабЗап.Количество() > 0 Тогда
				НабЗап[0].Статус 					= стрТовар.Статус;
				НабЗап[0].ДокументДвижения 			= Документ;
				НабЗап.Записать();
			КонецЕсли;
			
		КонецЦикла;
		
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") 
		...
		Для Каждого Элем из Документ.РасшифровкаПлатежа Цикл
			...
			КонецЕсли;
			НабЗап = РегистрыСведений.СтатусОплатыЗаказовПоставщику.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПоставщика.Установить(Элем.Сделка);			
			НабЗап.Прочитать();
		...
		//Дандали В. 24.11.2025 +
		ВнутреннийЗаказ = ЕстьДокументВнутреннийЗаказ(Элем.Сделка);
		Если ЗначениеЗаполнено(ВнутреннийЗаказ) Тогда
...
Процедура УдалитьДвиженияСтатусЗаказа(ПолностьюОчистить = Ложь, Данные, Статус, ДокументДвижения = Неопределено) Экспорт
	
	Если ПолностьюОчистить Тогда
		НабЗап = РегистрыСведений.СтатусыЗаказов.СоздатьНаборЗаписей();
		НабЗап.Отбор.ЗаказПокупателя.Установить(Данные);
		НабЗап.Прочитать();
		НабЗап.Очистить();
		НабЗап.Записать();
		Возврат;
	...
	Если ТипЗнч(Данные) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		
		тчДанные = Данные.Товары;
		Для Каждого стрТовар из тчДанные Цикл
			...
			Если НабЗап.Количество() > 0 Тогда
				НабЗап[0].НоменклатураПоставщика 	= "";
				НабЗап[0].Статус 					= Статус;
				НабЗап[0].ДокументДвижения 			= "";
				НабЗап.Записать();
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Данные) = ТиП("ДокументСсылка.ПоступлениеТоваровУслуг") 
		ИЛИ ТипЗнч(Данные) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		...
		Для Каждого стрТовар из тчДанные Цикл
			...
			Если НабЗап.Количество() > 0 Тогда
				...
				НабЗап[0].ДокументДвижения = ?(ДокументДвижения = Неопределено, "", ДокументДвижения);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
		
		тчДанные = Данные.РасшифровкаПлатежа;
		Для Каждого стрТовар из тчДанные Цикл
			
			Если НЕ ЗначениеЗаполнено(стрТовар.Сделка) Тогда				
				Продолжить;
			КонецЕсли;
			
			НабЗап = РегистрыСведений.СтатусОплатыЗаказов.СоздатьНаборЗаписей();
			НабЗап.Отбор.ЗаказПокупателя.Установить(стрТовар.Сделка.ДокументОснование);
			НабЗап.Прочитать();
			
			Для Каждого Запись из НабЗап Цикл				
				Запись.Статус 			= Статус;
				Запись.ДокументДвижения = стрТовар.Сделка;
			КонецЦикла;
...
Процедура ЗакрытиеЗаказов() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ ЗаказПоставщикуТовары.Ссылка.СтатусЗаказа В (&СтатусыЗаказов)
		...
		|
		|ВЫБРАТЬ
		|	ЗаказПоставщикуОборудование.Ссылка,
		|	СУММА(ЗаказПоставщикуОборудование.Количество)
		|ИЗ
		|	Документ.ЗаказПоставщику.Оборудование КАК ЗаказПоставщикуОборудование
		|ГДЕ
		|	НЕ ЗаказПоставщикуОборудование.Ссылка В (&СтатусыЗаказов)
		|	И ЗаказПоставщикуОборудование.Ссылка.Проведен
		|	И НЕ ЗаказПоставщикуОборудование.Ссылка.ПометкаУдаления
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗаказПоставщикуОборудование.Ссылка
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ЗаказыПоставщику.Ссылка,
		|	СУММА(ВТ_ЗаказыПоставщику.Количество) КАК Количество
		|ПОМЕСТИТЬ ВТ_ЗаказПоставщикуИтог
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПриходныйОрдерНаТоварыТовары.Ссылка.ДокументОснования КАК ЗаказПоставщику,
		|	ВТ_ЗаказПоставщикуИтог.Количество КАК КоличествоЗаказ,
		|	СУММА(ПриходныйОрдерНаТоварыТовары.Количество) КАК КоличествоПриходный,
		|	ПриходныйОрдерНаТоварыТовары.Ссылка КАК Приходный
		|ПОМЕСТИТЬ ВТ_ЗаказыПриходные
		|ИЗ
		|	ВТ_ЗаказПоставщикуИтог КАК ВТ_ЗаказПоставщикуИтог
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары.Товары КАК ПриходныйОрдерНаТоварыТовары
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ПриходныйОрдерНаТоварыТовары.Ссылка.ДокументОснования
		|ГДЕ
		|	ПриходныйОрдерНаТоварыТовары.Ссылка.ДокументОснования В
		|			(ВЫБРАТЬ
		|				ВТ_ЗаказыПоставщику.Ссылка
		|			ИЗ
		|				ВТ_ЗаказыПоставщику КАК ВТ_ЗаказыПоставщику)
		|	И ПриходныйОрдерНаТоварыТовары.Ссылка.Проведен
		|	И НЕ ПриходныйОрдерНаТоварыТовары.Ссылка.ПометкаУдаления
		|
		|СГРУППИРОВАТЬ ПО
		|	ПриходныйОрдерНаТоварыТовары.Ссылка,
		|	ВТ_ЗаказПоставщикуИтог.Количество,
		|	ПриходныйОрдерНаТоварыТовары.Ссылка.ДокументОснования
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ЗаказыПриходные.ЗаказПоставщику,
		|	ВТ_ЗаказыПриходные.КоличествоЗаказ,
		|	СУММА(ВТ_ЗаказыПриходные.КоличествоПриходный) КАК КоличествоПриходный,
		|	ВТ_ЗаказыПриходные.Приходный
		|ПОМЕСТИТЬ ВТ_ЗаказыПриходныеИтог
		|ИЗ
		|	ВТ_ЗаказыПриходные КАК ВТ_ЗаказыПриходные
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗаказыПриходные.Приходный,
		|	ВТ_ЗаказыПриходные.ЗаказПоставщику,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СУММА(ПоступлениеТоваровУслугТовары.Количество) КАК КоличествоПТУ,
		|	ВТ_ЗаказПоставщикуИтог.Ссылка
		|ПОМЕСТИТЬ ВТ_ЗаказыПТУ
		|ИЗ
		|	ВТ_ЗаказПоставщикуИтог КАК ВТ_ЗаказПоставщикуИтог
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ПоступлениеТоваровУслугТовары.Ссылка.Сделка
		|ГДЕ
		...
		|	ВТ_ЗаказыПриходные КАК ВТ_ЗаказыПриходные
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
		|		ПО ВТ_ЗаказыПриходные.Приходный = ПоступлениеТоваровУслугТовары.ПриходныйОрдер
		|ГДЕ
		|	НЕ ПоступлениеТоваровУслугТовары.Ссылка.ПометкаУдаления
		|	И ПоступлениеТоваровУслугТовары.Ссылка.Проведен
		|	И ПоступлениеТоваровУслугТовары.Ссылка.Сделка = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗаказыПриходные.ЗаказПоставщику
		|
		...
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ПоступлениеТоваровУслугОборудование.Ссылка.Сделка
		|ГДЕ
		|	НЕ ПоступлениеТоваровУслугОборудование.Ссылка.ПометкаУдаления
		...
		|	ВТ_ЗаказыПриходные КАК ВТ_ЗаказыПриходные
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Оборудование КАК ПоступлениеТоваровУслугОборудование
		|		ПО ВТ_ЗаказыПриходные.Приходный = ПоступлениеТоваровУслугОборудование.ПриходныйОрдер
		|ГДЕ
		|	НЕ ПоступлениеТоваровУслугОборудование.Ссылка.ПометкаУдаления
		|	И ПоступлениеТоваровУслугОборудование.Ссылка.Проведен
		|	И ПоступлениеТоваровУслугОборудование.Ссылка.Сделка = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗаказыПриходные.ЗаказПоставщику
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВТ_ЗаказыПТУ.КоличествоПТУ) КАК КоличествоПТУ,
		|	ВТ_ЗаказыПТУ.Ссылка
		|ПОМЕСТИТЬ ВТ_ЗаказПТУИтог
		|ИЗ
		|	ВТ_ЗаказыПТУ КАК ВТ_ЗаказыПТУ
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗаказыПТУ.Ссылка
		|;
		...
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВТ_ЗаказПоставщикуИтог.Ссылка,
		|	ИСТИНА КАК СтатусЗакрытия
		|ПОМЕСТИТЬ ВТ_ЗакрытиеЗаказов
		|ИЗ
		|	ВТ_ЗаказПоставщикуИтог КАК ВТ_ЗаказПоставщикуИтог
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗакрытиеЗаказовПоставщикам.Заказы КАК ЗакрытиеЗаказовПоставщикамЗаказы
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ЗакрытиеЗаказовПоставщикамЗаказы.ЗаказПоставщику
		|ГДЕ
		|	НЕ ЗакрытиеЗаказовПоставщикамЗаказы.Ссылка.ПометкаУдаления
		|	И ЗакрытиеЗаказовПоставщикамЗаказы.Ссылка.Проведен
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ЗаказПоставщикуИтог.Ссылка КАК ЗаказПоставщику,
		|	МАКСИМУМ(ВТ_ЗаказПоставщикуИтог.Количество) КАК КоличествоЗаказ,
		|	МАКСИМУМ(ЕСТЬNULL(ВТ_ЗаказыПриходныеИтог.КоличествоПриходный, 0)) КАК КоличествоПриходный,
		|	МАКСИМУМ(ЕСТЬNULL(ВТ_ЗаказПТУИтог.КоличествоПТУ, 0)) КАК КоличествоПТУ,
		|	ВТ_ЗакрытиеЗаказов.СтатусЗакрытия,
		|	МАКСИМУМ(ВТ_ЗаказыПриходныеИтог.Приходный.Дата) КАК ДатаПрихода
		|ПОМЕСТИТЬ ВТ_Итог
		|ИЗ
		|	ВТ_ЗаказПоставщикуИтог КАК ВТ_ЗаказПоставщикуИтог
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗаказыПриходныеИтог КАК ВТ_ЗаказыПриходныеИтог
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ВТ_ЗаказыПриходныеИтог.ЗаказПоставщику
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗаказПТУИтог КАК ВТ_ЗаказПТУИтог
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ВТ_ЗаказПТУИтог.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакрытиеЗаказов КАК ВТ_ЗакрытиеЗаказов
		|		ПО ВТ_ЗаказПоставщикуИтог.Ссылка = ВТ_ЗакрытиеЗаказов.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗаказПоставщикуИтог.Ссылка,
		|	ВТ_ЗакрытиеЗаказов.СтатусЗакрытия
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Итог.ЗаказПоставщику КАК Заказ,
		|	ВТ_Итог.КоличествоЗаказ,
		|	ВТ_Итог.КоличествоПриходный КАК КоличествоПриходный,
		|	ВТ_Итог.КоличествоПТУ КАК КоличествоПТУ,
		|	ЕСТЬNULL(ВТ_Итог.СтатусЗакрытия, ЛОЖЬ) КАК СтатусЗакрытия,
		|	ВЫБОР
		|		КОГДА ВТ_Итог.КоличествоПТУ = 0
		|			ТОГДА ВЫБОР
		|					КОГДА ВТ_Итог.КоличествоЗаказ = ВТ_Итог.КоличествоПриходный
		|						ТОГДА ""Голубой""
		|					КОГДА ВТ_Итог.КоличествоЗаказ <> ВТ_Итог.КоличествоПриходный
		|						ТОГДА ""Желтый""
		|				КОНЕЦ
		|		КОГДА ВТ_Итог.КоличествоЗаказ = ВТ_Итог.КоличествоПриходный
		|				И ВТ_Итог.КоличествоЗаказ = ВТ_Итог.КоличествоПТУ
		|			ТОГДА ""Белый""
		|		КОГДА ВТ_Итог.КоличествоПриходный = ВТ_Итог.КоличествоПТУ
		|				И ВТ_Итог.КоличествоПриходный <> ВТ_Итог.КоличествоЗаказ
		|			ТОГДА ""Красный""
		|		КОГДА ВТ_Итог.СтатусЗакрытия
		|			ТОГДА ""Белый""
		|		ИНАЧЕ ""Белый""
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗаказыПоставщику
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗаказыПриходные
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗаказыПриходныеИтог
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗаказыПТУ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗаказПТУИтог
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ЗакрытиеЗаказов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_Итог";
	
	МассСтатусов = Новый Массив;
	МассСтатусов.Добавить(Перечисления.СтатусыЗаказаПокупателей.Отменен);
	МассСтатусов.Добавить(Перечисления.СтатусыЗаказаПокупателей.Закрыт);
	Запрос.УстановитьПараметр("СтатусыЗаказов", МассСтатусов);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Заказ.СтатусЗаказа = Перечисления.СтатусыЗаказаПокупателей.Закрыт Тогда
			Продолжить;
		КонецЕсли;
		
		ДокОб 				= ВыборкаДетальныеЗаписи.Заказ.ПолучитьОбъект();
		ДокОб.СтатусЗаказа 	= Перечисления.СтатусыЗаказаПокупателей.Закрыт;
		Попытка
			ДокОб.Записать();
		Исключение
		КонецПопытки;
...
Функция ЕстьДокументВнутреннийЗаказ(ЗаказПоставщика)
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВнутреннийЗаказТовары.УникальныйИдентификаторСтроки,
		|	ВнутреннийЗаказТовары.Ссылка КАК ВнутреннийЗаказ
		|ИЗ
		|	Документ.ВнутреннийЗаказ.Товары КАК ВнутреннийЗаказТовары
		|ГДЕ
		|	НЕ ВнутреннийЗаказТовары.Ссылка.ПометкаУдаления
		|	И ВнутреннийЗаказТовары.Ссылка.Проведен
		|	И ВнутреннийЗаказТовары.Размещение = &ДокументОснование";
	
	Запрос.УстановитьПараметр("ДокументОснование", ЗаказПоставщика);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.ВнутреннийЗаказ;
	КонецЦикла;
