...
//	ДатаНач
//  ДатаКон
Функция СформироватьСтрокуОграниченийПоДатамДляФормы(Знач ДатаНач, Знач ДатаКон, КодЯзыкаПечать = "ru") Экспорт
	...
	Иначе
		...
		Иначе
			...
			Если ДатаНач <= ДатаКон Тогда
				ОписаниеПериода = "" + ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+"; ФП = Истина");
			Иначе
...
Процедура ОбработкаРедактированиеСубконто(Элемент, ТекущиеДанныеТаблицыРедактирования) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Счет) Тогда
		Предупреждение(НСтр("ru='Не выбран счет!';uk='Не обраний рахунок!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработчикПриИзмененииСчета(ТабличноеПоле, Знач ТабПолеРазвернутогоСальдо) Экспорт
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		...
		Если НЕ НайденСпособРазворота Тогда
			...
			Если ТабПолеРазвернутогоСальдо Тогда
				ВидНастройки = НСтр("ru='развернутого сальдо ';uk='розгорнутого сальдо '");
			Иначе
				ВидНастройки = НСтр("ru='итогов по субсчетам и субконто ';uk='підсумків по субрахунках і субконто '");
			КонецЕсли;
			Предупреждение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Счет ""%1"" не имеет ни субсчетов, ни субконто."
"Настройка %2 не имеет смысла!';uk='Рахунок ""%1"" не має ні субрахунків, ні субконто."
"Настройка %2 не має змісту!'"), Счет.Код, ВидНастройки));
...
Функция ОпределитьНаличиеДублирующегосяПараметраВИзмерениях(ПостроительОтчета) Экспорт
	...
	Для Сч = 0 По ПостроительОтчета.ИзмеренияСтроки.Количество() - 1  Цикл
		...
		Если СоответствиеТекущихИзмерений[ПутьКДанным] <> Неопределено Тогда
			...
			Предупреждение(НСтр("ru='Повторяющееся поле детализации ""';uk='Повторюване поле деталізації ""'") + ПостроительОтчета.ИзмеренияСтроки[Сч].Представление + """!");
			Возврат Истина;
	...
	Для Каждого ИмяПоказателя Из СтруктураПараметров.МассивПоказателей Цикл
		...
		Если ВыведеноПоказателей > 1 Тогда
			...
			Если ИмяПоказателя = "ВалютнаяСумма" Тогда
				ОбластьСтрокаПоказателяСчет.Параметры.СубконтоПредставление = НСтр("ru='Валютная сумма';uk='Валютна сума'");
			Иначе
...
Процедура СформироватьОтчетОборотноСальдовойВедомости(ОбъектОтчета, ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0,
	...
	Знач ВидУчета = Неопределено, Знач Валюта = Неопределено, Знач ОтражениеВНУ = Неопределено, 
	Знач Сценарий = Неопределено, Знач ВВалютеСценария = Неопределено, УчитыватьКлассыСчетов = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ОбъектОтчета.ПоСубсчетамИСубконто
			// Если не выбрано ПоСубсчетамИСубконто, то выводим 2 верхних уровня - классы счетов и счета
			И  Уровень > ?(УчитыватьКлассыСчетов,1,0) Тогда
...
Процедура ЗаполнитьПараметрыРасшифровкиОборотноСальдовойВедомости(Знач ДатаНач, Знач ДатаКон, Знач ИмяРегистраБухгалтерии,
	...
	Если Лев(ИмяГруппировки, СтрДлина(ИмяГруппировки) - 1) = "Субконто" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, НСтр("ru='Карточка счета ';uk='Картка рахунку '") + Выборка.СчетПредставление);
		
	ИначеЕсли ИмяГруппировки = "Счет" Тогда

		СписокРасшифровки.Добавить(ПараметрыОСВПоСчету,    НСтр("ru='ОСВ по счету ';uk='ОСВ по рахунку '") + Выборка.СчетПредставление);
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, НСтр("ru='Карточка счета ';uk='Картка рахунку '") + Выборка.СчетПредставление);
		СписокРасшифровки.Добавить(ПараметрыАнализСчета,   НСтр("ru='Анализ счета ';uk='Аналіз рахунку '") + Выборка.СчетПредставление);
		
		Если НЕ (Месяц(ДатаНач) = Месяц(ДатаКон) И Год(ДатаНач) = Год(ДатаКон))
			ИЛИ (ДатаНач = '00010101' И ДатаКон = '00010101') Тогда
			
			СписокРасшифровки.Добавить(ПараметрыОборотыСчетаМесяц,  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обороты счета %1 по месяцам';uk='Обороти рахунку %1 по місяцях'"), Выборка.СчетПредставление));
			
		КонецЕсли;
		
		СписокРасшифровки.Добавить(ПараметрыОборотыСчетаДень, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обороты счета %1 по дням';uk='Обороти рахунку %1 по днях'"), Выборка.СчетПредставление));
...
Процедура ДокументРезультатОбработкаРасшифровкиОборотноСальдовойВедомости(Элемент, Расшифровка, 
	...
	Если ТипЗнч(Расшифровка) = Тип("СписокЗначений") Тогда
		...
		Иначе
			ВыбранноеЗначение = Расшифровка.ВыбратьЭлемент(НСтр("ru='Выберите расшифровку';uk='Виберіть розшифровку'"), Расшифровка[0]);
		КонецЕсли;
...
Функция ПолучитьПериодДатСтрокой(Знач Период, Знач ДатаПериода) Экспорт
	...
	Возврат ПредставлениеПериода(ДатаНачала, ДатаОкончания, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса())+"; ФП=Истина")
...
//Функция формирует строку ограничений для запроса по ограничениям построителя отчетов
Функция ПолучитьТекстОграниченийПоПостроителюОтчетаСПрефиксом(ПостроительОтчета, Запрос, ПрефиксУсловий = "") Экспорт
	...
	Для Каждого СтрокаОтбора Из ПостроительОтчета.Отбор Цикл
		...
		Если СтрокаОтбора.ВидСравнения = ВидСравнения.ВИерархии
			...
			Если ТипВыражения <> "" Тогда
				...
				ТекстСтрокиПоля = "Выразить(" + ПрефиксУсловий + СтрокаОтбора.ПутьКДанным + " КАК " + ТипВыражения + МетаданныеОграничения.Имя + ")"
		...
		Если ТекстСтрокиПоля = "" Тогда
			...
			ТекстСтрокиПоля = ПрефиксУсловий + СтрокаОтбора.ПутьКДанным;
...
Процедура ВывестиДанныеОстатковКарточкиСчетаВМакет(ЗапросПоОстаткам, Знач ОбластьМакет, Знач Счет, Знач ЗапросНаНачало, 
	...
	Если ЗапросНаНачало Тогда
		...
		ОбластьМакет.Параметры.ОписательСальдо = НСтр("ru='Сальдо на начало';uk='Сальдо на початок'");
		ВыводитьИтогПоКоличеству = Счет.Количественный;
		
	Иначе
		
		ОбластьМакет.Параметры.ОписательСальдо = НСтр("ru='Сальдо на конец';uk='Сальдо на кінець'");
		ВыводитьИтогПоКоличеству = Счет.Количественный;
	...
	Если Счет.Вид = ВидСчета.АктивноПассивный Тогда
		...
		Иначе
			ОбластьМакет.Параметры.СуммаСальдоДт   = 0;
...
Функция ПолучитьСтрокуФорматаПериода(Знач Период) Экспорт
	...
	Если ЗначениеЗаполнено(Период) Тогда
		...
		ИначеЕсли Врег(Период) = "МЕСЯЦ" Тогда
			ФорматПериода = НСтр("ru='ДФ = ""ММММ гггг """"г.""""""';uk='ДФ = ""ММММ гггг """"р.""""""'")+";Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса());
		ИначеЕсли Врег(Период) = "ДЕКАДА" Тогда
			ФорматПериода = НСтр("ru='ДФ = """"""Декада с"""" дд.ММ.гггг """"""';uk='ДФ = """"""Декада з"""" дд.ММ.гггг """"""'");
		ИначеЕсли Врег(Период) = "НЕДЕЛЯ" Тогда
			ФорматПериода = НСтр("ru='ДФ = """"""Неделя с"""" дд.ММ.гггг """"""';uk='ДФ = """"""Тиждень із"""" дд.ММ.гггг """"""'");
...
Процедура ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, Знач ИмяОбластиВывода, Знач Выборка, 
	Знач ДокументРезультат, Знач ПоСубсчетам, Знач ВидДвижения, МассивРесурсов, СписокСчетов, Знач ДополнениеКУровнюВывода = 0, УчитыватьКлассыСчетов = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПоСубсчетам Тогда
			...
			Если ВерхнийУровень < Выборка.Уровень() Тогда
				// для Хозрасчетного будем выводить 2 верхних уровня - классы счетов и счета
				Если УчитыватьКлассыСчетов Тогда 
					Если ВерхнийУровень < Выборка.Уровень() - 1 Тогда
						Продолжить;
					КонецЕсли;
				Иначе
					// для остальных выводим только верхний уровень
					Продолжить;
				КонецЕсли;
			Иначе
...
Процедура ВывестиПолныйЗаголовокОтчетаОборотовСчета(Выборка = Неопределено, ШиринаТаблицы, ОбъектОтчет, Макет, ДокументРезультат,
	...
	Знач ОбщийИтог = Неопределено, УчитыватьКлассыСчетов = Ложь) Экспорт
	...
	Если ОбъектОтчет.ОборотДтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотДтКорсчет", Выборка,
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, ВидДвиженияБухгалтерии.Дебет, МассивРесурсов, СписокДт, ДополнениеКУровнюВывода, УчитыватьКлассыСчетов);
	...
	Если ОбъектОтчет.ОборотКтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотКтКорсчет", Выборка, 
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, ВидДвиженияБухгалтерии.Кредит, МассивРесурсов, СписокКт, ДополнениеКУровнюВывода, УчитыватьКлассыСчетов);
...
Процедура ВывестиИтогиОборотноСальдовойВедомостиПоСчету(Знач Выборка, Знач ИмяРегистраБухгалтерии, Знач МассивПоказателей, Знач ФорматПоказателей,
	Знач Счет, Знач ОбластьИтогиСтрокаЧисла, Знач ОбластьИтогиЧислаМакет, Знач ОбластьИтогиСтрокаСчет, Знач ДокументРезультат, ВыводитьРазвернутоеСальдо = Ложь, СтруктураПараметров = Неопределено) Экспорт
	...
	Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
		...
		Если ВыводитьРазвернутоеСальдо И НачальныйОстатокДт <> Неопределено Тогда
			...
			//Дандали В. 07.08.2025 +
			Если СтруктураПараметров <> Неопределено
				И СтруктураПараметров.Свойство("ТолькоКонечныйОстаток") Тогда
...
Процедура ВывестиПоказателиОборотноСальдовойВедомостиПоСчету(Выборка, СтруктураПараметров, Знач Счет, 
	...
	Для Каждого ИмяПоказателя Из СтруктураПараметров.МассивПоказателей Цикл
		...
		//Дандали В. 07.08.2025 +
		Если СтруктураПараметров.Свойство("ТолькоКонечныйОстаток") Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтруктураПараметров.СтруктураОграничений.ПоВалютам Тогда
			...
			Пока ВыборкаПоВалютам.Следующий() Цикл
				...
				ВыводимаяОбласть.Параметры.Заполнить(Выборка);
				ВыводимаяОбласть.Параметры.СубконтоПредставление = НСтр("ru='Валюта ';uk='Валюта '") + ВыборкаПоВалютам.ВалютаПредставление;
...
Процедура ЗаполнитьРасшифровкуДляСчета(ОтчетОбъект, Область, Выборка, СтруктураПараметров, Знач ИмяРегистраБухгалтерии,
	...
	Если Лев(ИмяГруппировки, 8) = "Субконто" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, НСтр("ru='Карточка счета ';uk='Картка рахунку '") + РасшифровываемыйСчетПредставление);
	...
	Для каждого ЭлементОтбора Из ТаблицаОтборов Цикл
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВСписке Тогда
			...
			Для каждого ЭлементСписка Из ЭлементОтбора.Значение Цикл
				...
				КонецЕсли;
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + ЭлементОтбора.Представление + НСтр("ru=' в списке ';uk=' у списку '")+ ?(НЕ ЗначениеЗаполнено(ЭлементОтбора.Значение), "Пусто", СтрокаСписка);
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВСпискеПоИерархии Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ЭлементОтбора.Значение Цикл
				Если НЕ ПустаяСтрока(СтрокаСписка) Тогда
				...
				КонецЕсли;
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + ЭлементОтбора.Представление + НСтр("ru=' в списке по иерархии ';uk=' у списку по ієрархії '")+ ?(НЕ ЗначениеЗаполнено(ЭлементОтбора.Значение), "Пусто", СтрокаСписка);
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВСписке Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ЭлементОтбора.Значение Цикл
				Если НЕ ПустаяСтрока(СтрокаСписка) Тогда
				...
				КонецЕсли;
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + ЭлементОтбора.Представление + НСтр("ru=' не в списке ';uk=' не в списку '")+ ?(НЕ ЗначениеЗаполнено(ЭлементОтбора.Значение), "Пусто", СтрокаСписка);
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВСпискеПоИерархии Тогда
			СтрокаСписка = "";
			Для каждого ЭлементСписка Из ЭлементОтбора.Значение Цикл
				Если НЕ ПустаяСтрока(СтрокаСписка) Тогда
				...
				КонецЕсли;
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов + ЭлементОтбора.Представление + НСтр("ru=' не в списке по иерархии ';uk=' не в списку по ієрархії '")+ ?(НЕ ЗначениеЗаполнено(ЭлементОтбора.Значение), "Пусто", СтрокаСписка);
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВИерархии Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВИерархии Тогда
			СтрокаОтборов = СтрокаОтборов + ЭлементОтбора.Представление + НСтр("ru=' не в иерархии ';uk=' не в ієрархії '")+ ?(НЕ ЗначениеЗаполнено(ЭлементОтбора.Значение), "Пусто", ЭлементОтбора.Значение);
