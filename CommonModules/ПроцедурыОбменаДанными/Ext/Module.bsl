...
Процедура ЗаполнитьНастройкуОбменаЗначениямиПоУмолчанию(НастройкаОбмена) Экспорт
	...
	Если Лев(СистемнаяИнформация.ВерсияПриложения,3) = "8.2" Тогда
		НастройкаОбмена.ВерсияПлатформыИнформационнойБазыДляПодключения = "V82";
	ИначеЕсли Лев(СистемнаяИнформация.ВерсияПриложения,3) = "8.3" Тогда
		НастройкаОбмена.ВерсияПлатформыИнформационнойБазыДляПодключения = "V83";
	КонецЕсли;	
...
Процедура ЗаполнитьНастройкуОбменаWEBЗначениямиПоУмолчанию(НастройкаОбмена) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НастройкаОбмена.ЕдиницаИзмеренияНовойНоменклатуры) Тогда 
		...
		Если НЕ ЗначениеЗаполнено(НастройкаОбмена.ЕдиницаИзмеренияНовойНоменклатуры) Тогда 
			НастройкаОбмена.ЕдиницаИзмеренияНовойНоменклатуры = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("2009");
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(НастройкаОбмена.ЕдиницаИзмеренияНовойНоменклатуры) Тогда 
...
Функция ВыполнитьУдалениеФайла(Знач ИмяФайлаУдаления, СтруктураНастроекОбменаДанными, 
	...
	Попытка
		...
		Если ВыводитьИнформациюВПротокол Тогда
			ДобавитьИнформациюВПротокол(НСтр("ru='Удаление файла: ';uk='Вилучення файлу: '") + ИмяФайлаУдаления, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		КонецЕсли;
	...
	Исключение
		...
		Если ВыводитьИнформациюВПротокол Тогда
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при попытке удаления файла: ';uk='Помилка при спробі вилучення файлу: '") + ИмяФайлаУдаления + " " + ОписаниеОшибки(), 
...
Функция ВыполнитьКопированиеФайла(Знач ИмяФайлаИсточника, Знач ИмяФайлаПриемника, 
	...
	Попытка
		...
		Если ВыводитьИнформациюВПротокол Тогда
			ДобавитьИнформациюВПротокол(НСтр("ru='Копирование файла из  ';uk='Копіювання файлу з '") + ИмяФайлаИсточника + " в " + ИмяФайлаПриемника, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		КонецЕсли;
	...
	Исключение
		...
		Если ВыводитьИнформациюВПротокол Тогда
			...
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при копировании файла из  ';uk='Помилка при копіюванні файлу з  '") + ИмяФайлаИсточника + " в " + ИмяФайлаПриемника + 
...
Процедура ЗагрузитьCообщениеСИзменениямиОтРИБУзла(СтруктураНастроекОбменаДанными, ЧтениеXML, КонфигурацияИзмененаПриОбменеРИБ = Ложь)
	...
	Исключение
		...
		Если СтруктураНастроекОбменаДанными.РучнойРежимЗапуска
			...
			СообщитьИнформациюОбОбмене(НСтр("ru='Для завершения чтения обновления конфигурации откройте Конфигуратор и выполните команду Конфигурация - Обновить конфигурацию базы данных.';uk='Для завершення читання оновлення конфігурації відкрийте Конфігуратор і виконайте команду Конфігурація - Оновити конфігурацію бази даних.'"),
...
Функция ЗаархивироватьФайл(Знач ИмяИсходногоФайла, СтруктураНастроекОбменаДанными)
	...
	Попытка
		...
		ДобавитьИнформациюВПротокол(НСтр("ru='Окончание сжатия файла обмена данными ';uk='Закінчення стиснення файлу обміну даними '") + ИмяИсходногоФайла + "." + Символы.ПС +
			НСтр("ru='Данные файла обмена помещены в файл ';uk='Дані файлу обміну поміщені у файл '") + СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений, СтруктураНастроекОбменаДанными.ДанныеПротокола);
	Исключение
...
Функция РазархивироватьФайл(СтруктураНастроекОбменаДанными) 
	...
	ДобавитьИнформациюВПротокол(НСтр("ru='Окончание распаковки файла обмена данными ';uk='Закінчення розпакування файлу обміну даними '") + СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений + НСтр("ru='."
"Распакованные данные помещены в файл ';uk='."
"Розпаковані дані поміщені у файл '") + ИмяРазархивированногоФайла, СтруктураНастроекОбменаДанными.ДанныеПротокола);
...
Процедура ОтправитьСообщениеОбОшибкеОбменаПриНеобходимости(СтруктураНастроекОбменаДанными, ОшибкаПриВыгрузке = Неопределено, ТекстСообщения = "")
	...
	Исключение
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Не удалось отправить письмо - уведомление о произошедшей ошибке:';uk='Не вдалося відправити лист - повідомлення про помилку, що виникла:'") + Символы.ПС + ОписаниеОшибки(), 
			СтруктураНастроекОбменаДанными, Истина, ОшибкаПриВыгрузке = Истина);
	Конецпопытки;
...
Функция ПроверитьНастройкиFTPПодключения(ДанныеНастройки, ДанныеПротокола = "", Знач ВывестиИнформациюВОкноСообщений = Истина,
	...
	Если ПустаяСтрока(ИмяFTPСервера) Тогда
		...
		СообщитьПростуюИнформацию(НСтр("ru='Не задан сервер обмена информацией через FTP.';uk='Не вказаний сервер обміну інформацією через FTP.'"), 
			ДанныеПротокола, ВывестиИнформациюВОкноСообщений, СтруктураОбменаДанными);
...
Процедура ПодготовитьОбменЧерезФайловыйРесурс(СтруктураНастроекОбменаДанными)
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		ИначеЕсли НаличиеФайлаОбменаXML И НаличиеФайлаОбменаZIP Тогда
			...
			СообщитьИнформациюОбОбмене(НСтр("ru='Время записи/изменения файла : ';uk='Час запису/зміни файлу : '") + ИтоговоеИмяФайлаОбменаДанными + НСтр("ru=' больше. Он выбран для обмена данными.';uk=' більше. Він обраний для обміну даними.'"), 
				СтруктураНастроекОбменаДанными);
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Иначе
			...
			Если МассивНайденныхФайлов.Количество() > 0 Тогда
				...
				СообщениеДляПользователя = НСтр("ru='Найденные файлы на FTP не соответствуют параметрам для выполнения обмена данными:"
" - расширение файла должно быть «XML» или «ZIP»;"
" - размер файла должен быть больше 0 байт;"
" - если в настройке обмена указано значение максимального пакета отличное от 0, то размер файла не должен превышать указанного в настройке значения ';uk='Знайдені файли на FTP не відповідають параметрам для виконання обміну даними:"
" - розширення файлу має бути XML або «ZIP»;"
" - розмір файлу повинен бути більше 0 байт;"
" - якщо в настройці обміну вказано значення максимального пакета відмінне від 0, то розмір файлу не повинен перевищувати зазначеного в настройці значення '")+ СтрокаМаксимальныйРазмерПакета + НСтр("ru=';"
" - дата создания файла на FTP должна быть больше даты файла, по которому последний раз происходил обмен данными ';uk=';"
" - дата створення файлу на FTP повинна бути більше дати файлу, за яким останній раз відбувався обмін даними '")+СтрокаДатаПоследнегоФайлаОбмена + ".";
...
Процедура ПодготовитьОбменЧерезПочтовыйРесурс(СтруктураНастроекОбменаДанными)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.ДанныеНастройки.УчетнаяЗаписьПриемаОтправкиСообщений) Тогда
		...
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Не задана учетная запись приема и отправки сообщений!';uk='Не заданий обліковий запис прийому й відправлення повідомлень!'"), Истина);
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Не задана учетная запись приема и отправки сообщений!';uk='Не заданий обліковий запис прийому й відправлення повідомлень!'"), Ложь);
	...
	Если СписокДоступныхЗаписей.Количество() = 0 Тогда
		...
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Нет доступных учетных записей для отправки и приема сообщений!';uk='Немає доступних облікових записів для відправлення й прийому повідомлень!'"), Истина);
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Нет доступных учетных записей для отправки и приема сообщений!';uk='Немає доступних облікових записів для відправлення й прийому повідомлень!'"), Ложь);
		Возврат;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Если СтруктураНастроекОбменаДанными.РучнойРежимЗапуска 
			...
			Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при получении почтовых сообщений: ';uk='Помилка при одержанні поштових повідомлень: '") + ТекстОшибок, СтруктураНастроекОбменаДанными, Истина, Ложь);	
			КонецЕсли;			
		...
		Если ФайлЗаписанНаДиск Тогда
			...
			Исключение
				...
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при сохранении вложения из письма: ';uk='Помилка при збереженні вкладення з листа: '") + ПисьмоИВложениеВНего + ". " + ОписаниеОшибки(), 
					СтруктураНастроекОбменаДанными, Истина, Ложь);	
				СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений = "";
		...
		Если НужноОтправлятьОтветноеПисьмо Тогда
			...
			Если СтруктураПисем <> Неопределено Тогда				   
				...
				Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
					...
					СтруктураНастроекОбменаДанными.РезультатЗаписиДанных = Ложь;
					СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при отправке ответного письма: ';uk='Помилка при відправленні відповідного листа: '") + ТекстОшибок, СтруктураНастроекОбменаДанными, Истина, Истина);	
	...
	Если ОпределитьБылоУдачноПроизведеноЧтениеДанных(СтруктураНастроекОбменаДанными) Тогда
		...
		Для Каждого ОбработанноеПисьмо Из СтруктураНастроекОбменаДанными.МассивОбработанныхПисем Цикл
			...
			Исключение
				...
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при попытке пометить письмо: ';uk='Помилка при спробі позначити лист: '") +
					Строка(ОбъектПисьмо) + НСтр("ru=' как прочтенное. Текст ошибки: ';uk=' як прочитане. Текст помилки: '") + ОписаниеОшибки(), СтруктураНастроекОбменаДанными, Истина, Ложь);	
...
Процедура ПолучитьПараметрыОбменаДанными(СтруктураНастроекОбменаДанными, МетаданныеУзлаОбмена, СтрокаИнформацияОбОбмене)
	...
	СтрокаИнформацияОбОбмене = НСтр("ru='обмена данными ';uk='обміну даними '") + СтрокаОписанияЗапуска + НСтр("ru=' для плана обмена ""';uk=' для плану обміну ""'") + ИмяПланаОбмена + НСтр("ru='"" узла ""';uk='"" вузла ""'") + СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы.Наименование + """";
...
Функция ВыполнитьДействияПередНачаломОбменаДанными(СтруктураНастроекОбменаДанными)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Автоматический';uk='Обмін даними.Автоматичний'"), УровеньЖурналаРегистрации.Информация, 
...
Функция ВыполнитьДействияПослеЗавершенияОбменаДанными(СтруктураНастроекОбменаДанными, ОбработкаОбменаПриемника = Неопределено)
	...
	СообщитьИнформациюОбОбмене(НСтр("ru='Обмен данными по настройке ""';uk='Обмін даними по настройці ""'") + Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки) + НСтр("ru='"" завершен (';uk='"" завершений ('") + Формат(ТекущаяДата(), "ДЛФ=В") + ").", 
		СтруктураНастроекОбменаДанными);
