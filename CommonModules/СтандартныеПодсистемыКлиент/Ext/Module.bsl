...
Функция Обработчик(Сценарий, КоличествоПараметров = 2)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Обработчик оповещения для более чем 2 параметров не поддерживается';uk='Обробник сповіщення для більш ніж 2 параметрів не підтримується'");
	КонецЕсли;
КонецФункции
