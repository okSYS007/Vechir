...
Функция ОбработкаРасшифровкиТиповогоОтчета(Расшифровка, СтандартнаяОбработка, ОтчетОбъект, ФормаОтчета, ДополнительныеРасшифровки = Неопределено) Экспорт
	...
	ИначеЕсли Команда = "РасшифроватьПоле" Тогда
		...
		СписокДоступныхРасшифровок.Добавить(НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		Если СписокДоступныхРасшифровок.Количество() > 1 тогда
			...
			КонецЕсли;
		Иначе
			Поле = НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
		КонецЕсли;
		
		Если Поле = НСтр("ru='Полный список полей для расшифровки...';uk='Повний список полів для розшифровки...'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) Тогда	
...
Процедура НарисоватьПериодПолемВыбора(ОтчетОбъект, ФормаОтчета, ЗначенияНастроек, Параметры, Лево, ЕстьПериодНаГорПанели)
	...
	Если ЗначениеПараметраНачалоПериода <> Неопределено
		...
		НовыйЭлемент.Подсказка = НСтр("ru='Выбор произвольного периода';uk='Вибір довільного періоду'");
		Лево = Лево + НовыйЭлемент.Ширина + 6;
...
Процедура НарисоватьДинамическийОтборСписок(ФормаОтчета, Индекс, Верх, ЦветФонаКнопки, СтрокаОтбора, ЭлементДинамическогоОтбора, ДоступноеПоле, ПараметрыИсполненияОтчета = Неопределено)
	...
	СписокВыбора.Добавить("", НСтр("ru='Не отбирать';uk='Не відбирати'"));
	СписокВыбора.Добавить("Выбранные", НСтр("ru='Только выбранные';uk='Тільки обрані'"));
	Если СтрокаОтбора.ДоступенВариантИсключить Тогда
		СписокВыбора.Добавить("Исключая", НСтр("ru='Кроме выбранных';uk='Окрім обраних'"));
	КонецЕсли;
	...
	Если СтрокаОтбора.СохранятьСписок Тогда
		...
		ЭлементНадпись.Подсказка = НСтр("ru='Сохранить этот список';uk='Зберегти цей список'");
		...
		ЭлементНадпись.Подсказка = НСтр("ru='Загрузить сохраненный список';uk='Завантажити список, що збережено'");
		ЭлементНадпись.ГиперСсылка = Истина;
		ЭлементНадпись.РазмерКартинки = РазмерКартинки.РеальныйРазмер;
...
Процедура НарисоватьЭлементУправленияТаблицейДиаграммой(ФормаОтчета, ОтчетОбъект, Индекс, СтрокаГруппировки, ЦветФонаКнопки, Группировки, ЭлементДинамическойГруппировки, ПараметрыИсполненияОтчета)
	...
	Для каждого Группировка из СтрокиГруппировки Цикл
		...
		СтрокаГруп.Представление         = Представление;
		Если СтрокаГруппировки["НастраиватьИерархию"] И ИспользоватьИерархию тогда
			СтрокаГруп.ПредставлениеИерархии = "" + Представление + НСтр("ru=' (по группам)';uk=' (по групах)'");
		КонецЕсли;
		СтрокаГруп.Использование         = Группировка.Значение.Использование;
		СтрокаГруп.ИспользованиеИерархии = ИспользоватьИерархию;
	КонецЦикла;
...
Процедура ПерерисоватьПанельНастроек(ОтчетОбъект, ФормаОтчета, ЗначенияНастроек = Неопределено, ПереноситьПараметры = истина) Экспорт
	...
	Для каждого ДопНастройка из ДополнительныеНастройкиОтчета Цикл
		Если Не ЕстьДополнительнаяСтраница тогда
			НоваяСтраница = ЭлементыФормы.ПанельЗакладок.Страницы.Добавить("ДинамическийОтбор" + "ДополнительнаяСтраница", НСтр("ru='Дополнительные настройки';uk='Додаткові настройки'"), "ДиаграммаГанта");
			ЕстьДополнительнаяСтраница = истина;
		КонецЕсли;
	...
	Если НЕ ЕстьЭлементыНаПанели тогда 
		НовыйЭлемент = ЭлементыФормы.Добавить(Тип("Надпись"), НСтр("ru='НадписьНеЗаполненаПанель';uk='НадписьНеЗаполненаПанель'"), Истина, ЭлементыФормы.ПанельПользователя);
		НовыйЭлемент.Верх      = 1;
		НовыйЭлемент.Лево      = 1;
		...
		НовыйЭлемент.Заголовок = НСтр("ru='         Панель пользователя не заполнена.';uk='         Панель користувача не заповнена.'");
...
Процедура СохранитьРезультатССохраненным(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	Если ТД = Неопределено Тогда
		Вопрос(НСтр("ru='Отсутствует сохраненный результат';uk='Відсутній збережений результат'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), РежимДиалогаВопрос.ОК);
		Возврат;
	ИначеЕсли ТД.ВысотаТаблицы = 0 Тогда
		Вопрос(НСтр("ru='Сохраненный результат пустой';uk='Збережений результат порожній'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), РежимДиалогаВопрос.ОК);
		Возврат;
	ИначеЕсли ФормаОтчета.ЭлементыФормы.Результат.ВысотаТаблицы = 0 Тогда
		Вопрос(НСтр("ru='Текущий результат пустой';uk='Поточний результат порожній'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), РежимДиалогаВопрос.ОК);
		Возврат;
	КонецЕсли;
	...
	Если СравнениеФайлов.Сравнить() Тогда
		Вопрос(НСтр("ru='Результаты совпадают';uk='Результати збігаються'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()), РежимДиалогаВопрос.ОК);
	Иначе
...
Процедура ВыбратьНастройкуФормы(СохраненнаяНастройка, ФормаВладелец, НастраиваемыйОбъект, РежимСохраненияНастройки) Экспорт
	...
	СписокДоступныхВариантов = ТиповыеОтчеты.ПолучитьСписокДоступныхВариантов(НастраиваемыйОбъект,,, ФормаВладелец.ОтчетОбъект); 
...
Процедура СохранитьТекущуюНастройкуКак(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	//СохраненныеНастрокий.ВыбратьНастройкуФормы(ОтчетОбъект.СохраненнаяНастройка, ФормаОтчета, ПолучитьИдентификаторОбъекта(ОтчетОбъект), Истина);
	ВыбратьНастройкуФормы(ОтчетОбъект.СохраненнаяНастройка, ФормаОтчета, ПолучитьИдентификаторОбъекта(ОтчетОбъект), Истина);
...
Процедура ОткрытьФормуСохраненнойНастройки(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	Если Не ОтчетОбъект.СохраненнаяНастройка.Пустая() Тогда
		...
		Если ОтчетОбъект.СохраненнаяНастройка.ПолучитьОбъект() = Неопределено Тогда
			ОтчетОбъект.СохраненнаяНастройка = Справочники.СохраненныеНастройки.ПустаяСсылка();
			Вопрос(НСтр("ru='Текущий вариант отчета удален.';uk='Поточний варіант звіту вилучений.'"), РежимДиалогаВопрос.ОК);
			ОбновитьТаблицуДоступныхНастроекПользователю(ОтчетОбъект);
			ОбновитьКнопкиВыбораНастроек(ФормаОтчета, ОтчетОбъект, ФормаОтчета.ПредставлениеНастройки, ФормаОтчета.РежимРедактированияНастройки);
			ОбновитьЗаголовокТиповогоОтчета(ОтчетОбъект, ФормаОтчета);
			Возврат;
		...
		ОбновитьКнопкиВыбораНастроек(ФормаОтчета, ОтчетОбъект, ФормаОтчета.ПредставлениеНастройки, ФормаОтчета.РежимРедактированияНастройки);
	Иначе
		Вопрос(НСтр("ru='Основной вариант не содержит описания и виден всем пользователям';uk='Основний варіант не містить опису й видний всім користувачам'"), РежимДиалогаВопрос.ОК);
	КонецЕсли;
...
Процедура ОбработкаЗакрытияНастройкиОтчета(ОтчетОбъект, ФормаОтчета, Отказ = Ложь, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ОтчетОбъект.СохраненнаяНастройка.Пустая() Тогда
		// Сохраним сам отчет
		РезультатВопроса = Вопрос(НСТР("ru='Сохранить изменения в основном варианте отчета?';uk='Зберегти зміни в основному варіанті звіту?'"), РежимДиалогаВопрос.ДаНетОтмена);
	...
	Иначе
		// Сохраним настройку
		РезультатВопроса = Вопрос(НСтр("ru='Сохранить изменения в варианте отчета ';uk='Зберегти зміни у варіанті звіту '") 
...
Процедура ОбработкаВыбораВариантаОтчета(ОтчетОбъект, ФормаОтчета, ВыбраннаяСохраненнаяНастройка) Экспорт 
	...
	Если Не ВыбраннаяСохраненнаяНастройка.Пустая() И ВыбраннаяСохраненнаяНастройка.ПолучитьОбъект() = Неопределено Тогда
		Вопрос(НСтр("ru='Операция невозможна. Вариант отчета удален.';uk='Операція неможлива. Варіант звіту вилучений.'"), РежимДиалогаВопрос.ОК);
		ОбновитьТаблицуДоступныхНастроекПользователю(ОтчетОбъект);
		ОбновитьКнопкиВыбораНастроек(ФормаОтчета, ОтчетОбъект, ФормаОтчета.ПредставлениеНастройки, ФормаОтчета.РежимРедактированияНастройки);
		Возврат;
	КонецЕсли;
...
Процедура ВыборВариантаОтчетаНаПанелиВариантов(ОтчетОбъект, ФормаОтчета, Элемент = Неопределено, СохраненаяНастройка = Неопределено) Экспорт
	...
	Если Не ВыбраннаяСохраненнаяНастройка.Пустая() И ВыбраннаяСохраненнаяНастройка.ПолучитьОбъект() = Неопределено Тогда
		Вопрос(НСтр("ru='Операция невозможна. Вариант отчета ""';uk='Операція неможлива. Варіант звіту ""'") + Элемент.ТекстНевыбраннойКартинки + НСтр("ru='"" удален.';uk='"" вилучений.'"), РежимДиалогаВопрос.ОК);
		ОбновитьТаблицуДоступныхНастроекПользователю(ОтчетОбъект);
		ОбновитьКнопкиВыбораНастроек(ФормаОтчета, ОтчетОбъект, ФормаОтчета.ПредставлениеНастройки, ФормаОтчета.РежимРедактированияНастройки);
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьСписокВТабличноеПоле(ОтчетОбъект, ЭлементыФормы, Элемент) Экспорт
	...
	Если Отказ Тогда
		Вопрос(НСтр("ru='Сохраненные списки отсутствуют';uk='Збережені списки відсутні'"), РежимДиалогаВопрос.ОК, , , НСтр("ru='Загрузка списка';uk='Завантаження списку'"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьСписокВТабличномПоле(ОтчетОбъект, ЭлементыФормы, Элемент) Экспорт
	...
	Если ЭлементТабличноеПоле.Значение.Количество() = 0 Тогда
		Вопрос(НСтр("ru='Список пуст';uk='Список порожній'"), РежимДиалогаВопрос.ОК);
		Возврат;
	КонецЕсли;
	...
	Результат = ВвестиЗначение(НазваниеСписка, НСтр("ru='Введите название сохраняемого списка';uk='Введіть назву  списку, що зберігається'"));
...
Процедура ОбработкаВыбораФормыОтчета(ОтчетОбъект, ФормаОтчета, ЗначениеВыбора, Источник) Экспорт
	...
	Если ТипЗнч(ЗначениеВыбора) = Тип("СхемаКомпоновкиДанных") Тогда
		...
		Если Не ЗаписьОтчетаДоступна(ОтчетОбъект) Тогда
			Предупреждение(НСтр("ru='Право на изменение отчета имеет только владелец';uk='Право на зміну звіту має тільки власник'"));
			Возврат;
		КонецЕсли;
...
Процедура ОбработкаНачалаВыбораОтчета(ОтчетОбъект, Элемент, СтандартнаяОбработка) Экспорт
	...
	ВыбранныйЭлемент = СписокОтчетов.ВыбратьЭлемент(НСтр("ru='Выберете отчет';uk='Виберете звіт'"));
...
Процедура СохранитьНастройкиФормыОтчета(ОтчетОбъект, ЭтаФорма) Экспорт
	...
	Иначе
		...
		НовыйЭлемент.Наименование        = НСтр("ru='Настройки формы отчета';uk='Настройка форми звіту'");
	...
	Если ЗначенияНастроек.Свойство("ДинамическиеПараметры") И ПараметрыПанели.Свойство("Параметры") тогда
		Если ПараметрыПанели.Параметры.Колонки.Найти("Заполнено") <> Неопределено тогда
			...
			Если СтрокиПарамтеровДляПроверки.Количество() > 0 тогда
				СтрокаПредупреждения = НСтр("ru='Не заполнен(а)';uk='Не заповнений(а)'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Процедура ВывестиТиповойОтчет(ОтчетОбъект, Результат, ДанныеРасшифровки, ВыводВФормуОтчета = Истина, ВнешниеНаборыДанных = Неопределено, ВыводитьШапкуОтчетаНаВсехСтраницах = истина)
	...
	Локализация.ПроверитьПереводСхемыКомпоновкиДанных(Схема, Строка(ОтчетОбъект),,ЭтоСтараяВерсияОтчета(ОтчетОбъект));
	...
	Попытка
		...
		#Если Клиент тогда
			Состояние(НСТР("ru='Если Вы хотите прервать вывод отчета, нажмите Ctrl+Break';uk='Якщо Ви хочете перервати виведення звіту, натисніть Ctrl+Break'"));
		#КонецЕсли
	...
	Исключение
		#Если Клиент тогда
			Предупреждение(НСтр("ru='Отчет не сформирован!';uk='Звіт не сформовано!'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + Символы.ПС + ПолучитьОписаниеРодительскойПричиныИнформацииОбОшибке(ИнформацияОбОшибке()));
		#КонецЕсли
	КонецПопытки;
...
Функция ПолучитьТаблицуСтандартныхПериодов()
	...
	// Произвольный
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "", "ПроизвольныйПериод", ВариантСтандартногоПериода.ПроизвольныйПериод, НСтр("ru='Произвольный период';uk='Довільний період'"));
	// День
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "День", "Предыдущий",    ВариантСтандартногоПериода.Вчера, НСтр("ru='Вчера';uk='Вчора'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "День", "Текущий",       ВариантСтандартногоПериода.Сегодня, НСтр("ru=' Сегодня';uk=' Сьогодні'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "День", "Следующий",     ВариантСтандартногоПериода.Завтра, НСтр("ru='  Завтра';uk=' Завтра'"));
	// Неделя
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Неделя", "Предыдущий",  ВариантСтандартногоПериода.ПрошлаяНеделя, НСтр("ru='Предыдущая неделя';uk='Попередній тиждень'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Неделя", "Текущий",     ВариантСтандартногоПериода.ЭтаНеделя, НСтр("ru=' Эта неделя';uk=' Цей тиждень'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Неделя", "Следующий",   ВариантСтандартногоПериода.СледующаяНеделя, НСтр("ru=' Следующая неделя';uk=' Наступний тиждень'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Неделя", "СНачала",     ВариантСтандартногоПериода.СНачалаЭтойНедели, НСтр("ru='С начала этой недели';uk='З початку цього тижня'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Неделя", "ДоКонца",     ВариантСтандартногоПериода.ДоКонцаЭтойНедели, НСтр("ru='До конца этой недели';uk='До кінця цього тижня'"));
	// Декада
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Декада", "Предыдущий",  ВариантСтандартногоПериода.ПрошлаяДекада, НСтр("ru='Предыдущая декада';uk='Попередня декада'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Декада", "Текущий",     ВариантСтандартногоПериода.ЭтаДекада, НСтр("ru=' Эта декада';uk=' Ця декада'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Декада", "Следующий",   ВариантСтандартногоПериода.СледующаяДекада, НСтр("ru='  Следующая декада';uk=' Наступна декада'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Декада", "СНачала",     ВариантСтандартногоПериода.СНачалаЭтойДекады, НСтр("ru='С начала этой декады';uk='З початку цієї декади'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Декада", "ДоКонца",     ВариантСтандартногоПериода.ДоКонцаЭтойДекады, НСтр("ru='До конца этой декады';uk='До кінця цієї декади'"));
	// Месяц
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Месяц", "Предыдущий",   ВариантСтандартногоПериода.ПрошлыйМесяц, НСтр("ru='Предыдущий месяц';uk='Попередній місяць'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Месяц", "Текущий",      ВариантСтандартногоПериода.ЭтотМесяц, НСтр("ru=' Этот месяц';uk=' Цей місяць'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Месяц", "Следующий",    ВариантСтандартногоПериода.СледующийМесяц, НСтр("ru='  Следующий месяц';uk=' Наступний місяць'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Месяц", "СНачала",      ВариантСтандартногоПериода.СНачалаЭтогоМесяца, НСтр("ru='С начала этого месяца';uk='З початку цього місяця'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Месяц", "ДоКонца",      ВариантСтандартногоПериода.ДоКонцаЭтогоМесяца, НСтр("ru='До конца этого месяца';uk='До кінця цього місяця'"));
	// Квартал
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Квартал", "Предыдущий", ВариантСтандартногоПериода.ПрошлыйКвартал, НСтр("ru='Предыдущий квартал';uk='Попередній квартал'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Квартал", "Текущий",    ВариантСтандартногоПериода.ЭтотКвартал, НСтр("ru=' Этот квартал';uk=' Цей квартал'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Квартал", "Следующий",  ВариантСтандартногоПериода.СледующийКвартал, НСтр("ru='  Следующий квартал';uk=' Наступний квартал'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Квартал", "СНачала",    ВариантСтандартногоПериода.СНачалаЭтогоКвартала, НСтр("ru='С начала этого квартала';uk='З початку цього кварталу'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Квартал", "ДоКонца",    ВариантСтандартногоПериода.ДоКонцаЭтогоКвартала, НСтр("ru='До конца этого квартала';uk='До кінця цього кварталу'"));
	// Полугодие
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Полугодие", "Предыдущий", ВариантСтандартногоПериода.ПрошлоеПолугодие, НСтр("ru='Предыдущее полугодие';uk='Попереднє півріччя'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Полугодие", "Текущий",    ВариантСтандартногоПериода.ЭтоПолугодие, НСтр("ru=' Это полугодие';uk=' Це півріччя'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Полугодие", "Следующий",  ВариантСтандартногоПериода.СледующееПолугодие, НСтр("ru='  Следующее полугодие';uk=' Наступне півріччя'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Полугодие", "СНачала",    ВариантСтандартногоПериода.СНачалаЭтогоПолугодия, НСтр("ru='С начала этого полугодия';uk='З початку цього півріччя'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Полугодие", "ДоКонца",    ВариантСтандартногоПериода.ДоКонцаЭтогоПолугодия, НСтр("ru='До конца этого полугодия';uk='До кінця цього півріччя'"));
	// Год
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Год", "Предыдущий",     ВариантСтандартногоПериода.ПрошлыйГод, НСтр("ru='Предыдущий год';uk='Попередній рік'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Год", "Текущий",        ВариантСтандартногоПериода.ЭтотГод, НСтр("ru=' Этот год';uk=' Цей рік'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Год", "Следующий",      ВариантСтандартногоПериода.СледующийГод, НСтр("ru='  Следующий год';uk=' Наступний рік'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Год", "СНачала",        ВариантСтандартногоПериода.СНачалаЭтогоГода, НСтр("ru='С начала этого года';uk='З початку цього року'"));
	ДобавитьСтрокуСтандартногоПериода(ТаблицаСтандартныхПериодов, "Год", "ДоКонца",        ВариантСтандартногоПериода.ДоКонцаЭтогоГода, НСтр("ru='До конца этого года';uk='До кінця цього року'"));
...
Функция ПолучитьТаблицуСтандартныхДатНачала()
	...
	// Произвольная
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "", "ПроизвольнаяДата",  ВариантСтандартнойДатыНачала.ПроизвольнаяДата, НСтр("ru='Произвольная дата';uk='Довільна дата'"));
	// День
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "День", "Предыдущий",    ВариантСтандартнойДатыНачала.НачалоПрошлогоДня, НСтр("ru='Начало вчерашнего дня';uk='Початок учорашнього дня'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "День", "Текущий",       ВариантСтандартнойДатыНачала.НачалоЭтогоДня, НСтр("ru=' Начало сегодняшнего дня';uk=' Початок сьогоднішнього дня'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "День", "Следующий",     ВариантСтандартнойДатыНачала.НачалоСледующегоДня, НСтр("ru='  Начало завтрашнего дня';uk='  Початок завтрашнього дня'"));
	// Неделя
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Неделя", "Предыдущий",  ВариантСтандартнойДатыНачала.НачалоПрошлойНедели, НСтр("ru='Начало предыдущей недели';uk='Початок попереднього тижня'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Неделя", "Текущий",     ВариантСтандартнойДатыНачала.НачалоЭтойНедели, НСтр("ru=' Начало этой недели';uk=' Початок цього тижня'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Неделя", "Следующий",   ВариантСтандартнойДатыНачала.НачалоСледующейНедели, НСтр("ru='  Начало следующей недели';uk='  Початок наступного тижня'"));
	// Декада
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Декада", "Предыдущий",  ВариантСтандартнойДатыНачала.НачалоПрошлойДекады, НСтр("ru='Начало предыдущей декады';uk='Початок попередньої декади'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Декада", "Текущий",     ВариантСтандартнойДатыНачала.НачалоЭтогоМесяца, НСтр("ru=' Начало этой декады';uk=' Початок цієї декади'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Декада", "Следующий",   ВариантСтандартнойДатыНачала.НачалоСледующейДекады, НСтр("ru='  Начало следующей декады';uk='  Початок наступної декади'"));
	// Месяц
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Месяц", "Предыдущий",   ВариантСтандартнойДатыНачала.НачалоПрошлогоМесяца, НСтр("ru='Начало предыдущего месяца';uk='Початок попереднього місяця'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Месяц", "Текущий",      ВариантСтандартнойДатыНачала.НачалоЭтогоМесяца, НСтр("ru=' Начало этого месяца';uk=' Початок цього місяця'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Месяц", "Следующий",    ВариантСтандартнойДатыНачала.НачалоСледующегоМесяца, НСтр("ru='  Начало следующего месяца';uk='  Початок наступного місяця'"));
	// Квартал
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Квартал", "Предыдущий", ВариантСтандартнойДатыНачала.НачалоПрошлогоКвартала, НСтр("ru='Начало предыдущего квартала';uk='Початок попереднього кварталу'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Квартал", "Текущий",    ВариантСтандартнойДатыНачала.НачалоЭтогоКвартала, НСтр("ru=' Начало этого квартала';uk=' Початок цього кварталу'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Квартал", "Следующий",  ВариантСтандартнойДатыНачала.НачалоСледующегоКвартала, НСтр("ru='  Начало следующего квартала';uk='  Початок наступного кварталу'"));
	// Полугодие
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Полугодие", "Предыдущий", ВариантСтандартнойДатыНачала.НачалоПрошлогоПолугодия, НСтр("ru='Начало предыдущего полугодия';uk='Початок попереднього півріччя'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Полугодие", "Текущий",    ВариантСтандартнойДатыНачала.НачалоЭтогоПолугодия, НСтр("ru=' Начало этого полугодия';uk=' Початок цього півріччя'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Полугодие", "Следующий",  ВариантСтандартнойДатыНачала.НачалоСледующегоПолугодия, НСтр("ru='  Начало следующего полугодия';uk='  Початок наступного півріччя'"));
	// Год
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Год", "Предыдущий",     ВариантСтандартнойДатыНачала.НачалоПрошлогоГода, НСтр("ru='Начало предыдущего года';uk='Початок попереднього року'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Год", "Текущий",        ВариантСтандартнойДатыНачала.НачалоЭтогоГода, НСтр("ru=' Начало этого года';uk=' Початок цього року'"));
	ДобавитьСтрокуСтандартнойДатыНачала(ТаблицаСтандартныхПериодов, "Год", "Следующий",      ВариантСтандартнойДатыНачала.НачалоСледующегоГода, НСтр("ru='  Начало этого года';uk='  Початок цього року'"));
	Возврат ТаблицаСтандартныхПериодов;
...
Функция ПолучитьСписокВыбораСтандартногоПериодаПользователя(Параметры)
	...
	Если Параметры.ПроизвольныйПериод Тогда
		СписокВыбораПериодовПользователя.Вставить(0, ВариантСтандартногоПериода.ПроизвольныйПериод, НСтр("ru='Произвольный период';uk='Довільний період'"));
	КонецЕсли;
...
Функция ПолучитьСписокВыбораСтандартнойДатыНачалаПользователя(Параметры)
	...
	Если Параметры.ПроизвольныйПериод Тогда
		СписокВыбораПериодовПользователя.Вставить(0, ВариантСтандартнойДатыНачала.ПроизвольнаяДата, НСтр("ru='Произвольная дата';uk='Довільна дата'"));
	КонецЕсли;
...
Функция ПолучитьПараметрыПанелиПользователяПоУмолчанию(ОтчетОбъект, ФормаОтчета = Неопределено) Экспорт
	...
	//	АналитическиеОтборы = ДобавитьИЗаполнитьСтроку(ДеревоНастроекСтандартныхСтраниц, Истина, "АналитическиеОтборы", НСтр("ru='Аналитические отборы';uk='Аналітичні відбори'"));
	//	ДобавитьИЗаполнитьСтроку(АналитическиеОтборы, Истина, "КоличествоЗаписей", НСтр("ru='Ограничение на количество записей';uk='Обмеження на кількість записів'"));
	//	ДобавитьИЗаполнитьСтроку(АналитическиеОтборы, Истина, "Порог", НСтр("ru='Порог существенности';uk='Поріг істотності'"));
	//	ДобавитьИЗаполнитьСтроку(АналитическиеОтборы, Истина, "Индикаторы", НСтр("ru='Индикаторы (тренд, состояние)';uk='Індикатори (тренд, стан)'"));
	//	ДобавитьИЗаполнитьСтроку(АналитическиеОтборы, Истина, "ABCКлассификация", "ABC - Классификация");
	//	ДобавитьИЗаполнитьСтроку(АналитическиеОтборы, Истина, "СкрытьНулевые", НСтр("ru='Скрытие нулевых строк и колонок';uk='Приховання нульових рядків і колонок'"));
	//КонецЕсли;
	ДобавитьИЗаполнитьСтроку(ДеревоНастроекСтандартныхСтраниц, Параметры, "Параметры", НСтр("ru='Параметры';uk='Параметри'"));
	ДобавитьИЗаполнитьСтроку(ДеревоНастроекСтандартныхСтраниц, Показатели, "Показатели", НСтр("ru='Показатели';uk='Показники'"));
	ДобавитьИЗаполнитьСтроку(ДеревоНастроекСтандартныхСтраниц, Отбор, "Отбор", НСтр("ru='Отбор';uk='Відбір'"));
	ДобавитьИЗаполнитьСтроку(ДеревоНастроекСтандартныхСтраниц, Порядок, "Порядок", НСтр("ru='Сортировка';uk='Сортування'"));
	...
	СписокДоступныхОтносительныхПериодов.Добавить("Предыдущий", НСтр("ru='Предыдущий';uk='Попередній'"), Истина);
...
Функция ПолучитьСтруктуруДинамическихПараметров(Параметры, ЭлементыФормы) Экспорт
	...
	Для каждого СтрокаПараметра Из Параметры.Параметры Цикл
		...
		Если ЭлементыФормы.Найти(НазваниеЭлементаУправления) <> Неопределено тогда
			...
			КонецЕсли;
		Иначе
			Сообщить(НСтр("ru='Не найден параметр ';uk='Не знайдений параметр'")+СтрокаПараметра.Параметр);
		КонецЕсли;
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Использование", Использование);
...
Процедура ОбновитьПредопределенныеВариантыОтчетов(СписокВариантов = Неопределено, Перезаписать = ложь) Экспорт
	...
	Если СписокВариантов = Неопределено тогда
		Для каждого СтрокаТаблицы из ТаблицаНастроек Цикл
			СписокНастроек.Добавить(СтрокаТаблицы.Макет,  НСтр("ru='Отчет: ';uk='Звіт:'") + СтрокаТаблицы.ОтчетПредставление + " - " +  СтрокаТаблицы.Представление);
			СоответсвиеИмяОтчета.Вставить(СтрокаТаблицы.Макет, СтрокаТаблицы.Отчет);
		КонецЦикла;
		СписокНастроек.ОтметитьЭлементы(НСтр("ru='Выберите варианты отчетов, которые требуется обновить:';uk='Виберіть варіанти звітів, які потрібно оновити:'"));
...
Процедура НачалоВыбораИзСпискаПредставленияПериода(Элемент, СтандартнаяОбработка, Период, ЭтаФорма, МинимальныйПериод, НачальноеЗначение = Неопределено)
	...
	Если ВРЕГ(МинимальныйПериод) = "ГОД" тогда
		НачалоПрошлогоГода = НачалоГода(ДобавитьМесяц(НачалоТекущегоГода, -108))-1;
		СписокВыбора.Добавить(НачалоПрошлогоГода, (Формат(НачалоТекущегоГода-1, "ДФ='yyyy'") + "..."));
	Иначе	
	...
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВРЕГ(МинимальныйПериод) = "ГОД" тогда
		Если Год(ВыбранныйЭлемент.Значение) = Год(НачалоСледующегоГода) или Год(ВыбранныйЭлемент.Значение) = Год(НачалоПрошлогоГода) тогда
