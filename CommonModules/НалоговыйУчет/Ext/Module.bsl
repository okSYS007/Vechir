...
Процедура ДвиженияПоЕдиномуНалогуРасчеты(ДокументОбъект,СтруктураШапкиДокумента,РеестрПлатежей,Отказ=Ложь) Экспорт
	...
	// Для платежных документов, дата берется из даты платежа
	//Дата		            = СтруктураШапкиДокумента.Дата;
	Если СтруктураШапкиДокумента.Свойство("ДатаОплаты") Тогда
		Дата	= СтруктураШапкиДокумента.ДатаОплаты;
	Иначе	
		Дата	= СтруктураШапкиДокумента.Дата;
	КонецЕсли; 
	...
	ВидОперации  = ?(ОбщегоНазначения.ЕстьРеквизитДокумента("ВидОперации", МетаданныеДокумента), СтруктураШапкиДокумента.ВидОперации, Неопределено);  
	...
	Для Каждого СтрокаПлатеж Из РеестрПлатежей Цикл
		...
		Если СтрокаПлатеж.СтоимостьОС > 0  
			И Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
...
Функция СоздатьСтруктуруТаблицыНалоговыхСумм() Экспорт
	...
	Таблица.Колонки.Добавить("СуммаВзаиморасчетов", 	ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	Таблица.Колонки.Добавить("БазаНДС", 				ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	Таблица.Колонки.Добавить("СуммаНДС", 				ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	Таблица.Колонки.Добавить("СуммаНДСПропорционально", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
...
Процедура ДобавитьКолонкиТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента, ПогрешностиОкругления, Приобретение = Истина) Экспорт
	...
	ТаблицаТоваров.Колонки.Добавить("ПроводкиСуммаНДСКурсНБУ"	 , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));	
	
	// сумма по налоговому учету налога на прибыль
	ТаблицаТоваров.Колонки.Добавить("ПроводкиСуммаБезНДСНУ" , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
...
Процедура ДвиженияПоРегистрамНалоговогоУчетаУпрощенныйНалоговыйУчет(ДокументОбъект, ТаблицаПоВторомуСобытиюНал = Неопределено, ДатаДвижений = Неопределено, НеУчитываемыеДвиженияДокумента = Неопределено, ДвиженияДокументаКакОстатки = Ложь, УчитываемыеДвиженияДокумента = Неопределено
	...
	ЗапросДвиженияДокумента.Текст = 
		  ...
		  |	СУММА(ПриобретенияНалоговыйУчет.СуммаНДСПропорционально) 	КАК СуммаНДСПропорционально,
 		  |	ПриобретенияНалоговыйУчет.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентомРегл						КАК ВестиПоДокументамРасчетовСКонтрагентомРегл,
		  ...
		  | 0,
 		  |	ПродажиНалоговыйУчет.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентомРегл,
	...
	ОстаткиРегистраПриобретение.УстановитьПараметр("МоментВремени", Новый МоментВремени(ДатаДвижений, ДокументОбъект.Ссылка));			   
	...
	ОстаткиРегистраПродажи.УстановитьПараметр("МоментВремени",  Новый МоментВремени(ДатаДвижений, ДокументОбъект.Ссылка));			   
	...
	Для каждого ДвижениеДокумента Из ДвиженияДокумента Цикл
		...
		СтруктураОтбора.Вставить("Сделка",			?(ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл, 
		...
		ИЛИ ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл Тогда
			// расчеты
		...
		Если  ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл
		...
		Если (НЕ ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл)
			 И (РасчОтгрВсеДок > 0 или РасчОплВсеДок  > 0) Тогда
		...
		Если   РасчОтгрВсеНач > РасчОплВсеНач И ВозвОтгрВсеДок > ВозвОплВсеДок 
			...
			Если ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл Тогда
			...
			Если ДвижениеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл
				...
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты", ДвижениеДокумента.ДоговорКонтрагента.Владелец);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Договоры"   , ДвижениеДокумента.ДоговорКонтрагента);
				...
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты", ДвижениеДокумента.ДоговорКонтрагента.Владелец);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Договоры"   , ДвижениеДокумента.ДоговорКонтрагента);
	...
	Для каждого  СтрокаПС Из ТаблицаПоПервомуСобытиюНДС Цикл
		...
		Если СтрокаПС.ИмяРегистра = "ПриобретенияНалоговыйУчет" Тогда
			Если СтрокаПС.ДоговорКонтрагента.Внешнеэкономический Тогда
...
Функция ОпределитьСделкуНалоговыйУчет(ВыборкаПоШапкеДокумента, Ссылка, Сделка = Неопределено) Экспорт
	...
	Если ВыборкаПоШапкеДокумента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
		// ведение по расчетным документам в НУ ведется паралельно с ведением из в БУ, кроме...
		Если ВыборкаПоШапкеДокумента.ВестиПоДокументамРасчетовСКонтрагентомРегл = Истина Тогда
			
			Если ВыборкаПоШапкеДокумента.СложныйНалоговыйУчет Тогда
				// ...случая, когда по договору ведется сложный налоговый учет
...
Процедура РазбитьПроводкуПоНДСНаПервоеВтороеСобытие(ТаблицаПоВторомуСобытиюНал, ДвиженияДокумента, Проводка,
											   		...
											   		ДоговорКонтрагента, 
													Сделка, РасчетныйДокументИлиМассив = Неопределено,
											   		СобытиеНДС, 
	...
	РасчетыВВалюте = ДоговорКонтрагента.Внешнеэкономический;
	...
	Если СуммаВторогоСобытия > 0 Тогда
		Если СуммаПроводки > СуммаВторогоСобытия Тогда
			...
			ПроводкаВтороеСобытие.Содержание = ПроводкаВтороеСобытие.Содержание + ", второе событие";
		...
		Иначе
			...
			Проводка.Содержание = Проводка.Содержание + ", второе событие";
	...
	Если  ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентомРегл
		И СуммаКСписанию > 0
		И НЕ РасчетныйДокументИлиМассив = Неопределено Тогда
		
		Если ТипЗнч(РасчетныйДокументИлиМассив) = Тип("Массив") Тогда
			МассивРасчетныхДокументов = РасчетныйДокументИлиМассив;	
		Иначе	
			МассивРасчетныхДокументов = Новый Массив();
			МассивРасчетныхДокументов.Добавить(РасчетныйДокументИлиМассив);	
		КонецЕсли;
		
		Для каждого РасчетныйДокумент Из МассивРасчетныхДокументов Цикл
			...
			Если СуммаВторогоСобытия > 0 Тогда
				Если СуммаПроводки > СуммаВторогоСобытия Тогда	
					...
					ПроводкаВтороеСобытие["Счет" + СуффиксСчета] = СчетВторогоСобытия;
					ОбщегоНазначения.УстановитьСубконто(ПроводкаВтороеСобытие["Счет" + СуффиксСчета],ПроводкаВтороеСобытие["Субконто" + СуффиксСчета],"ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
					...
					Если РасчетыВВалюте Тогда
						...
						Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДатаОплаты", МетаданныеРасчДокумента) Тогда
				...
				Иначе
					...
					Проводка["Счет" + СуффиксСчета] = СчетВторогоСобытия;
					ОбщегоНазначения.УстановитьСубконто(Проводка["Счет" + СуффиксСчета],Проводка["Субконто" + СуффиксСчета],"ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
	...
	Если    СуммаВторогоСобытия <=0
		...
		Проводка.Содержание = Проводка.Содержание + ", первое событие";	
	КонецЕсли;
...
Процедура РазбитьПроводкуПоНДСПоРасчетнымДокументам(ТаблицаПоПервомуСобытиюНДС, ДвиженияДокумента, Проводка,
	...
	Если СуммаВторогоСобытия > 0 Тогда
		...
		Если СуммаПроводки > СуммаВторогоСобытия Тогда	
			...
			ОбщегоНазначения.УстановитьСубконто(ПроводкаВтороеСобытие.СчетДт, ПроводкаВтороеСобытие.СубконтоДт, "ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
			ОбщегоНазначения.УстановитьСубконто(ПроводкаВтороеСобытие.СчетКт, ПроводкаВтороеСобытие.СубконтоКт, "ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
		...
		Иначе
			...
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
			ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ДокументыРасчетовСКонтрагентами", РасчетныйДокумент);
...
Функция ПодготовитьТаблицуОплатДляНалоговогоУчета(СтруктураШапкиДокумента) Экспорт
	...
	// В регистрах налогового учета суммы в валюте договора (для внешнеэкономических договоров) и гривнях в противном случае.
	...
	|			И ТаблицаПлатежей.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентомРегл = Истина
	|			И ТаблицаПлатежей.ДоговорКонтрагента.СложныйНалоговыйУчет = Ложь 
	|			ТОГДА ТаблицаПлатежей.ДокументРасчетовСКонтрагентом 
	|	ИНАЧЕ Неопределено КОНЕЦ КАК РасчетныйДокумент,
	|	ВЫРАЗИТЬ(ВЫБОР
	...
	Для каждого СтрокаПлатежа из РеестрПлатежей Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПлатежа.РасчетныйДокумент) тогда
		...
		Если СтрокаПлатежа.СложныйНалоговыйУчет = Истина Тогда
		...
		Иначе		
			...
			Если НЕ СтрокаПлатежа.ВестиПоДокументамРасчетовСКонтрагентомРегл = Истина Тогда
...
Функция ДвиженияПоРегистрамНалоговогоУчетаПлатежныхДокументов(ДокументОбъект, СтруктураШапкиДокумента, РеестрПлатежей, Отказ=Ложь) Экспорт
	...
	ЭтоПродажа =    (НаправлениеДвижения = Перечисления.НаправленияДвижений.Поступление И НЕ ЭтоВозврат)
				ИЛИ (НаправлениеДвижения = Перечисления.НаправленияДвижений.Выбытие     И    ЭтоВозврат);
	...
	РеестрПлатежейКопия.Свернуть("СчетУчетаНДСПодтвержденный, СложныйНалоговыйУчет, ВедениеВзаиморасчетов, ДоговорКонтрагента, Сделка, РасчетныйДокумент, СтавкаНДС, ВозвратнаяТара, ДляХозяйственнойДеятельности, ВидДеятельностиНДС, Амортизируется, Событие,ВестиПоДокументамРасчетовСКонтрагентомРегл, МоментОпределенияБазыНДСПоПокупкам, МоментОпределенияБазыНДСПоПродажам",  "СуммаВзаиморасчетов, СуммаНДС, СуммаНДСПропорционально");
	...
	Для каждого СтрокаПлатежа Из РеестрПлатежейКопия Цикл
		...
		Если СтрокаПлатежа.СложныйНалоговыйУчет = Ложь
		   И (РасчетыВозврат = Перечисления.РасчетыВозврат.Расчеты
			  ИЛИ НЕ  СтрокаПлатежа.ВестиПоДокументамРасчетовСКонтрагентомРегл) Тогда
			// этот реквизит не нужен
			СтрокаПлатежа.СчетУчетаНДСПодтвержденный	= ПланыСчетов.Хозрасчетный.ПустаяСсылка();
		КонецЕсли;			
	...
	Для каждого СтрокаПлатежа из РеестрПлатежей Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНДС Тогда
			...
			Если  СуммаПроводки <> 0 
				...
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Контрагенты",					 СтрокаПлатежа.Контрагент, Истина);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Договоры",						 СтрокаПлатежа.ДоговорКонтрагента);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"ДокументыРасчетовСКонтрагентами",СтрокаПлатежа.Сделка);
				
				Проводка.СчетКт                           		= ?(ЭтоПродажа, СтрокаПлатежа.СчетУчетаНДС, СтрокаПлатежа.СчетУчетаНДСПодтвержденный);
				
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Контрагенты",					 СтрокаПлатежа.Контрагент, Истина);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Договоры",						 СтрокаПлатежа.ДоговорКонтрагента);
				ОбщегоНазначения.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"ДокументыРасчетовСКонтрагентами",СтрокаПлатежа.Сделка);
				...
				Если  СтрокаПлатежа.ВестиПоДокументамРасчетовСКонтрагентомРегл 
...
Процедура ЗаполнитьДокументыРасчета(ТаблицаДвижений, ТаблицаДокументовРасчета, ВидОперации="Поступление") Экспорт

	СтруктураОтбораТара = Новый Структура;
	СтруктураОтбораТара.Вставить("ВозвратнаяТара");
	
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("СтавкаНДС");
	
	ЕстьПропорциональныйНДС = Ложь;	
    ЕстьСуммаНДСПропорционально	= Ложь;
	
	Если ВидОперации = "Поступление" Тогда
		
		СтруктураОтбора.Вставить("ДляХозяйственнойДеятельности");
		СтруктураОтбора.Вставить("ВидДеятельностиНДС");
		СтруктураОтбора.Вставить("Амортизируется");
		
		// разобъем налоговое назначение из таблицы расчетных докуменов на составляющие, как в регистре ПриобретенияНалоговыйУчет
		ТаблицаДокументовРасчета.Колонки.Добавить("ДляХозяйственнойДеятельности");
		ТаблицаДокументовРасчета.Колонки.Добавить("ВидДеятельностиНДС");
		
		НеОблагаемыйВидНалоговойДеятельности = Справочники.ВидыНалоговойДеятельности.НеОблагаемая;
		Для каждого СтрокаДокументаРасчета Из ТаблицаДокументовРасчета Цикл
			НалоговоеНазначение = СтрокаДокументаРасчета.НалоговоеНазначение;
			
			СтрокаДокументаРасчета.ДляХозяйственнойДеятельности = НЕ (НалоговоеНазначение.ВидНалоговойДеятельности = НеОблагаемыйВидНалоговойДеятельности);
			СтрокаДокументаРасчета.ВидДеятельностиНДС 			= НалоговоеНазначение.ВидДеятельностиНДС;
		КонецЦикла;	
		
	КонецЕсли;
	
	// пройдемся по таблице расчетных документов и разобъем строки движений по регистру налогового учета
	// указав закрываемый расчетный документ
	Для каждого СтрокаДокументаРасчета Из ТаблицаДокументовРасчета Цикл
		
		Если СтрокаДокументаРасчета.Сумма = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		РасчетныйДокумент   = СтрокаДокументаРасчета.ДокументРасчетовСКонтрагентом;
		СуммаКРаспределению = СтрокаДокументаРасчета.Сумма;
		
		// настроим отбор
		ЗаполнитьЗначенияСвойств(СтруктураОтбора,     СтрокаДокументаРасчета);
		ЗаполнитьЗначенияСвойств(СтруктураОтбораТара, СтрокаДокументаРасчета);
		
		СтрокиДвижений = ТаблицаДвижений.НайтиСтроки(?(СтрокаДокументаРасчета.ВозвратнаяТара, СтруктураОтбораТара, СтруктураОтбора));
		Для каждого СтрокаДвижений Из СтрокиДвижений Цикл
			...
			Если СтрокаДвижений.СуммаВзаиморасчетов <= 0 Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаДвижений.РасчетныйДокумент) Тогда
				// эта строка уже распределена
				Продолжить;
			КонецЕсли;
			
			СуммаРаспределения = Мин(СтрокаДвижений.СуммаВзаиморасчетов, СуммаКРаспределению);
			Если СуммаРаспределения = СтрокаДвижений.СуммаВзаиморасчетов Тогда
				СтрокаДвижений.РасчетныйДокумент = РасчетныйДокумент;
			Иначе
				НоваяСтрокаДвижений = ТаблицаДвижений.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаДвижений, СтрокаДвижений);
				
				НоваяСтрокаДвижений.РасчетныйДокумент   = РасчетныйДокумент;
				НоваяСтрокаДвижений.СуммаВзаиморасчетов = СуммаРаспределения;
				НоваяСтрокаДвижений.СуммаНДС 			= НоваяСтрокаДвижений.СуммаНДС * СуммаРаспределения / СтрокаДвижений.СуммаВзаиморасчетов;
				Если ВРЕГ(ВидОперации)="ПОСТУПЛЕНИЕ" Тогда
					НоваяСтрокаДвижений.СуммаНДСПропорционально = НоваяСтрокаДвижений.СуммаНДСПропорционально  * СуммаРаспределения / СтрокаДвижений.СуммаВзаиморасчетов;
					СтрокаДвижений.СуммаНДСПропорционально = СтрокаДвижений.СуммаНДСПропорционально - НоваяСтрокаДвижений.СуммаНДСПропорционально;
				КонецЕсли;
				СтрокаДвижений.СуммаВзаиморасчетов = СтрокаДвижений.СуммаВзаиморасчетов - СуммаРаспределения;
				СтрокаДвижений.СуммаНДС 		   = СтрокаДвижений.СуммаНДС - НоваяСтрокаДвижений.СуммаНДС;
				
			КонецЕсли;
			
			СуммаКРаспределению = СуммаКРаспределению - СуммаРаспределения;
...
Процедура ОпределениеАналитикиНалоговогоУчетаВПроводкахДляЗатрат(Строка, СчетЗатрат, ХарактерЗатрат = Неопределено,
																 НалоговаяАналитикаВПроводке = Неопределено, ПризнакАмортизируется = Ложь, 
																 НалоговоеНазначениеНДС = Неопределено, НалоговоеНазначениеПрибыль = Неопределено, 
																 ИмяСубконто = "Субконто", 
																 НоменклатурнаяГруппаВСтроке = Истина,
																 ОбъектСтроительстваВСтроке = Истина,
																 Дата = Неопределено,
	...
	Если ХарактерЗатрат = Неопределено Тогда                                                                   
		ХарактерЗатрат = УправлениеПроизводством.ПолучитьХарактерЗатратПоСчетуЗатрат(СчетЗатрат, Неопределено, Дата);
	КонецЕсли;
	...
	ИначеЕсли ХарактерЗатрат = "Производство" Тогда
		...
		Если НоменклатурнаяГруппаВСтроке Тогда
		
			НалоговаяАналитикаВПроводке = Строка["НоменклатурнаяГруппа"].НалоговоеНазначениеВПроизводстве;
	...
	ИначеЕсли ХарактерЗатрат = "Строительство" Тогда
		...
		Если ОбъектСтроительстваВСтроке Тогда
		
			НалоговаяАналитикаВПроводке = Строка["ОбъектСтроительства"].НалоговоеНазначение;
	...
	ИначеЕсли ХарактерЗатрат = "РБП" Тогда
		...
		Если МожноОпределитьНалоговоеНазначение Тогда
			ХарактерЗатратРБП = УправлениеПроизводством.ПолучитьХарактерЗатратПоСчетуЗатрат(Строка[ИмяСубконто +Н].СчетБУ, Неопределено, Дата);
