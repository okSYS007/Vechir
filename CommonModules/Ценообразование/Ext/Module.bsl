		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если СтруктураЗначений.ПересчитатьЦеныПоВалюте Тогда
				...
				Если ЕстьЦенаПоступления Тогда
					...
					КонецЕсли;
					Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СуммаНДСПоступления", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
						ОбработкаТабличныхЧастей.РассчитатьСуммуНДСПоступленияТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект);
					КонецЕсли;
		...
		Если СтруктураЗначений.ПересчитатьЦеныПоВалюте ИЛИ ПересчитатьЦенуПоФлагамНалогов Тогда
			...
			Для каждого СтрокаТабличнойЧасти Из ДокументОбъект[ИмяТабличнойЧасти]Цикл
				...
				Если ЕстьЦенаПоступления Тогда
					...
					Если ПересчитатьЦенуПоФлагамНалогов
					   И ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
						СтрокаТабличнойЧасти.ЦенаПоступления = ПересчитатьЦенуПриИзмененииФлаговНалогов(СтрокаТабличнойЧасти.ЦенаПоступления, 
																							 Неопределено,
																							 СтруктураЗначений.ТекущийСуммаВключаетНДС,
																							 СтруктураЗначений.НовыйУчитыватьНДС,
																							 СтруктураЗначений.НовыйСуммаВключаетНДС, 
...
Процедура УстановитьДоступностьРеквизитовФормаЦеныВалюты(ФормаЦеныВалюты, СтруктураРеквизитовДокумента,	СтруктураНеРедактируемыхРеквизитов = Неопределено)
	...
	ФормаЦеныВалюты.ЭлементыФормы.НовыйАвторасчетНДС.Доступность             	= СтруктураРеквизитовДокумента.Свойство("АвторасчетНДС");
	...
	ФормаЦеныВалюты.ЭлементыФормы.РассчитатьНДССУчетомОшибокОкругления.Видимость = СтруктураРеквизитовДокумента.Свойство("РазрешитьПересчетНДС");
...
//
Процедура ЗаполнитьПлановуюСебестоимость(СтрокаТЧ, Дата, ЕстьСуммаУпр = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТипЦенПлановойСебестоимости) Тогда
		...
		СтрокаТЧ.ПлановаяСтоимость =
			ПолучитьЦенуНоменклатуры(СтрокаТЧ.Номенклатура, ,
			...
			Если ЕстьСуммаУпр Тогда
				...
				СтрокаТЧ.ПлановаяСтоимостьУпр = ПолучитьЦенуНоменклатуры(СтрокаТЧ.Номенклатура, ,
										 ТипЦенПлановойСебестоимости, Дата, ,
										 ВалютаУправленческогоУчета, 1);
	...
	Иначе
		...
		Для каждого ТЧОбъекта Из ДокументОбъект.Метаданные().ТабличныеЧасти Цикл
			...
			Если НеУчитыватьТару и ТЧОбъекта.Имя = "ВозвратнаяТара" Тогда
				Продолжить;
			ИначеЕсли ТЧОбъекта.Имя = "ДокументыРасчетовСКонтрагентом" Тогда
				Продолжить;
			ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ЗаказПокупателя")  И (ТЧОбъекта.Имя = "Материалы") Тогда
				Продолжить;
			КонецЕсли;
	   ...
	   И УчетнаяПолитика.СпособИспользованияНакопительныхСкидок = Перечисления.СпособыИспользованияНакопительныхСкидок.СкидкиНакапливаютсяПоВладельцуДисконтнойКарты Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если Выборка.ЗначениеПорога <> ТекВидКарты Тогда
				Предупреждение(НСтр("ru='Сумма, накопленная покупателем, составила ';uk='Сума, накопичена покупцем, склала '") + (ОбщегоНазначения.ФорматСумм(Выборка.Сумма,
				глЗначениеПеременной("ВалютаУправленческогоУчета")))+"." + Символы.ПС + Символы.ПС
				   + НСтр("ru='Покупателю можно обменять дисконтную карту';uk='Покупцю можна обміняти дисконтну карту'") + Символы.ПС
				   + """" + СокрЛП(ТекВидКарты) + НСтр("ru='"" на ""';uk='"" на ""'") + СокрЛП(Выборка.ЗначениеПорога) + """.");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
