...
Функция ПолучитьПредставлениеАдреса(Запись) Экспорт
	...
	Если СокрЛП(Запись.Поле8) <> "" Тогда
		ТекПредставление = ТекПредставление + ", " + ?(ЗначениеЗаполнено(ТипКорпуса), Локализация.ПолучитьЛокализованныйСинонимОбъекта(ТипКорпуса, Локализация.КодЯзыкаИнформационнойБазы()), НСтр("ru='корп.';uk='корп.'",Локализация.КодЯзыкаИнформационнойБазы())) + " " + СокрЛП(Запись.Поле8);
	КонецЕсли;
...
Функция ПолучитьПредставлениеАдресаПочтовый(Запись) Экспорт
	...
	// Дом
	Если СокрЛП(Запись.Поле7) <> "" Тогда
		ТекПредставление = ТекПредставление + ", " + ?(ЗначениеЗаполнено(Запись.ТипДома), Локализация.ПолучитьЛокализованныйСинонимОбъекта(Запись.ТипДома, Локализация.КодЯзыкаИнформационнойБазы()), НСтр("ru='дом';uk='будинок'",Локализация.КодЯзыкаИнформационнойБазы())) + " № " + СокрЛП(Запись.Поле7);
	КонецЕсли;

	// Корпус
	Если СокрЛП(Запись.Поле8) <> "" Тогда
		ТекПредставление = ТекПредставление + ", " + ?(ЗначениеЗаполнено(Запись.ТипКорпуса), Локализация.ПолучитьЛокализованныйСинонимОбъекта(Запись.ТипКорпуса, Локализация.КодЯзыкаИнформационнойБазы()), НСтр("ru='корп.';uk='корп.'",Локализация.КодЯзыкаИнформационнойБазы())) + " " + СокрЛП(Запись.Поле8);
	КонецЕсли;

	// Квартира
	Если СокрЛП(Запись.Поле9) <> "" Тогда
		ТекПредставление = ТекПредставление + ", " + ?(ЗначениеЗаполнено(Запись.ТипКвартиры), Локализация.ПолучитьЛокализованныйСинонимОбъекта(Запись.ТипКвартиры, Локализация.КодЯзыкаИнформационнойБазы()), НСтр("ru='кв.';uk='кв.'",Локализация.КодЯзыкаИнформационнойБазы())) + СокрЛП(Запись.Поле9);
	КонецЕсли;
...
Функция ОпределитьДляОбъектаУкраинскийАдрес(ОбъектРедактирования) Экспорт
	...
	Если (НЕ ПустаяСтрока(ОбъектРедактирования.Поле1)) Тогда
		...
		// так, если есть хоть одна не цифра - это иностранный адрес, а если все цивры то Украинский
...
Процедура ЗаписатьКонтактнуюИнформацию(НаборЗаписей, Ссылка, Отказ) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, (НСтр("ru='Элемент ""';uk='Елемент ""'") + СокрЛП(Ссылка) + НСтр("ru='"" не записан. Не записана контактная информация.';uk='"" не записаний. Не записана контактна інформація.'")));
	...
	Для Каждого СтрокаДляИзменения Из ТаблицаКИ Цикл
		...
		Если СтрокаДляИзменения.Вид = ВидПоиска И НЕ ЗначениеЗаполнено(СтрокаДляИзменения.Представление) Тогда
			...
			Если ЗадаватьВопросОбИзменении Тогда
				...
				НужноМенятьАдрес = Вопрос(НСтр("ru='Изменился ';uk='Змінився '") + СтрокаСАдресом.Вид.Наименование + НСтр("ru='."
"Автоматически изменить ';uk='."
"Автоматично змінити '") + СтрокаДляИзменения.Вид.Наименование + "?", РежимДиалогаВопрос.ДаНет);
...
Процедура УдалитьЗаписьКонтактнойИнформации(Элемент, Отказ) Экспорт
	...
	Если Элемент.ТекущиеДанные <> Неопределено И (ЗначениеЗаполнено(Элемент.ТекущиеДанные.Представление) ИЛИ ТипЗнч(Элемент.ТекущиеДанные.Вид) = Тип("Строка")) Тогда
		...
		ОтветНаВопрос = Вопрос(НСтр("ru='Удалить запись?';uk='Вилучити запис?'"), РежимДиалогаВопрос.ДаНет);
...
Функция ВидОбъектаКИ(ОбъектСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектСсылка) = Тип("СправочникСсылка.СервисныеЦентры") Тогда
		Возврат Перечисления.ВидыОбъектовКонтактнойИнформации.СервисныеЦентры;
	Иначе
		Возврат Перечисления.ВидыОбъектовКонтактнойИнформации.ПустаяСсылка();
	КонецЕсли; 
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено, КодЯзыка = "ru") Экспорт
	...
	Сведения = Новый Структура("Представление, ПолноеНаименование, КодПоЕДРПОУ, КодПоДРФО, ИНН, НомерСвидетельства,"+
								"Телефоны, ЮридическийАдрес, ФактическийАдрес,"+
								"Банк, МФО, КоррСчет, НомерСчета, НомерСчетаУстаревший,"+
								"ПредставлениеБанковскогоСчета, МФО_Платеж, НомерСчетаСПереносом, АдресБанка,"+
								"ПлательщикНалогаНаПрибыль, ИнформацияОСтатусеПлательщикаНалогов, НаименованиеДСТУ");
	...
	Если Данные <> Неопределено Тогда
		...
		//Дандали В. 09.10.2025 +
		Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") 
			И ЗначениеЗаполнено(ЮрФизЛицо.АдресСтарый) 
			И ДатаПериода <= ЮрФизЛицо.АдресСтарыйДо Тогда
			
			Если ЗначениеЗаполнено(Сведения.ЮридическийАдрес) Тогда
				Сведения.ЮридическийАдрес = ЮрФизЛицо.АдресСтарый;		
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Сведения.ФактическийАдрес) Тогда
				Сведения.ФактическийАдрес = ЮрФизЛицо.АдресСтарый;		
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Сведения.НаименованиеДСТУ) Тогда
			Сведения.НаименованиеДСТУ = ВРег(Сведения.Представление);
		КонецЕсли;
	...
	//Дандали В. 27.08.2024 +
	Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		ДопИнформация = ДополнительнаяИнформацияКонтрагента(ЮрФизЛицо, ДатаПериода);
		Если ЗначениеЗаполнено(ДопИнформация.Наименование) Тогда
			Сведения.Вставить("ПолноеНаименование", ДопИнформация.Наименование);
		КонецЕсли;
	КонецЕсли;
	//Дандали В. 27.08.2024 -
...
// Определение резидент/нерезидент по контакной информации
//
// Параметры:
//  Объект - СправочникСсылка, объект контактной информации
//  ТипАдреса - тип контактной информации
//
// Возвращаемое значение
//  Булево - представление найденного адреса
//
Функция ПолучитьРезидентНерезидентИзКонтактнойИнформации(Объект, ТипАдреса = "") Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		//Адреса нет, считаем резидентом
		Возврат Истина;
	Иначе
		Страна = РезультатЗапроса.Выгрузить()[0].Поле1;
		Представление = РезультатЗапроса.Выгрузить()[0].Представление;
		Если Найти(Представление,СокрЛП(Страна))=0 Тогда
			//адрес не украинский
			Возврат Ложь;
		Иначе
			Возврат Истина;
...
Процедура КонтактнаяИнформацияРедактироватьВДиалоге(Кнопка)Экспорт
	...
	Иначе
		Предупреждение(НСтр("ru='Вам разрешено редактировать контактную информацию только в диалоге!';uk='Вам дозволено редагувати контактну інформацію тільки в діалозі!'"),5);
	КонецЕсли;
...
Функция ВернутьЖДСтанциюНазначенияПоАдресу(Знач СтрокаАдреса) Экспорт
	...
	СтруктураАдреса = ПолучитьСтруктуруАдресаИзСтроки(СтрокаАдреса);
	СтрокаЗаПределамиУкраины = Неопределено;
	Если СтруктураАдреса = Неопределено ИЛИ СтруктураАдреса.Свойство("ЗаПределамиУкраины", СтрокаЗаПределамиУкраины) Тогда
		Возврат "";
	КонецЕсли;
...
Функция ПолучитьСтруктуруАдресаИзСтроки(Знач СтроковыйАдрес) Экспорт
	...
	Иначе
		...
		// либо задана только страна, но не Украины
...
//Функция возвращает дополнительную информацию по контрагенту
//Например если у контрагента изменилось название, а документ со старой датой
//то необходимо возвращать название контрагента на старую дату
Функция ДополнительнаяИнформацияКонтрагента(Контрагент, ДатаДокумента) Экспорт
	
	стрВозврата = Новый Структура;
	стрВозврата.Вставить("Наименование");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДополнительнаяИнформацияКонтрагентовСрезПоследних.НаименованиеКонтрагента КАК Наименование
		|ИЗ
		|	РегистрСведений.ДополнительнаяИнформацияКонтрагентов.СрезПоследних(&Дата, Контрагент = &контрагент) КАК ДополнительнаяИнформацияКонтрагентовСрезПоследних";
