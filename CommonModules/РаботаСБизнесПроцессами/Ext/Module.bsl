...
Функция ПолучитьПредставлениеЗадачи(Задача)
	...
	Возврат Задача.Наименование + НСтр("ru=' от ';uk=' від '") + Задача.Дата
...
Процедура ПроверитьЗаполнениеРеквизитов(Задача, Отказ) Экспорт
	...
	Заголовок         = НСтр("ru='Выполнение задачи: ';uk='Виконання задачі: '") + ПолучитьПредставлениеЗадачи(Задача);
		...
		Если ТочкаМаршрута = ТочкиМаршрута.Согласовать Или
			...
			Если Не ПолучитьДанныеЗадачи(Задача.Ссылка) Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(
					НСтр("ru='Не заполнен реквизит ""Виза""';uk='Не заповнений реквізит ""Віза""'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТипБизнесПроцесса = Тип("БизнесПроцессСсылка.ПереоценкаТоваровАТТ") Тогда
		...
		Если ТочкаМаршрута = ТочкиМаршрута.ПровестиИнвентаризацию Тогда
			...
			Если Не ЗначениеЗаполнено(Задача.Объект) Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(
					НСтр("ru='Не заполнен реквизит ""Инвентаризация товаров""';uk='Не заповнений реквізит ""Інвентаризація товарів""'"), Отказ, Заголовок);
			КонецЕсли;
		...
		ИначеЕсли ТочкаМаршрута = ТочкиМаршрута.УстановитьНовыеЦены Тогда
			...
			Если Не ЗначениеЗаполнено(Задача.Объект) Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(
					НСтр("ru='Не заполнен реквизит ""Переоценка товаров""';uk='Не заповнений реквізит ""Переоцінка товарів""'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершитьБизнесПроцесс(ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Ответ = Вопрос(НСтр("ru='Принудительное завершение бизнес-процесса используется для прекращения процесса переоценки товаров в случае,"
...
"как «Выполненные»."
"Завершить бизнес-процесс?';uk='Примусове завершення бізнес-процесу використовується для припинення процесу переоцінки товарів у випадку,"
"коли з якихось причин подальше виконання процесу не потрібно."
"В ході операції бізнес-процесу буде встановлена ознака «Завершений», а всі невиконані задачі будуть відзначені"
"як «Виконані»."
