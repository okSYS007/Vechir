...
Функция ПолучитьОтветы(Выборка, Тип, ФлагКолонки = Ложь)
	...
	Иначе
		...
		Иначе
			...
			Иначе
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Справочник ""';uk='Довідник ""'")+Объект.Синоним+НСтр("ru='"" не имеет ни одного элемента. Рекомендуется заполнить его и сформировать анкету еще раз.';uk='"" не має жодного елемента. Рекомендується заповнити його і сформувати анкету ще раз.'"));
...
Функция СформироватьВложение(ТиповаяАнкета, ОпрашиваемоеЛицо = Неопределено, ЭлектронныйАдресОтветов = "", ДокРассылки = Неопределено, ДокОпрос = Неопределено, ИмяФайла = "", ФормироватьXSL = Ложь, КаталогАнкеты = "", КаталогОпроса = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Сообщить(НСтр("ru='Вопрос ""';uk='Питання ""'")+Выборка.Вопрос + НСтр("ru='"" встречается в разделе ""';uk='"" зустрічається у розділі ""'") + Выборка.Раздел + НСтр("ru='"" более одного раза!';uk='"" більше одного разу!'"));
	КонецЦикла;
