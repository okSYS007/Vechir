		...
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("КоличествоМест", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения)
			   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' не выбрана единица измерения цены!"
"Пересчет количества невозможен.';uk=' не обрана одиниця виміру ціни!"
"Перерахунок кількості неможливий.'"));
			Иначе
...
Процедура ОкруглитьСуммуДокументаПослеЗаполненияТЧ(ИмяТабличнойЧасти,ПорядокОкругленияСуммыВПользуПокупателя,
													ТабличнаяЧасть,СуммаВключаетНДС,АвторасчетНДС,УчитыватьНДС) Экспорт

	 
	Если (ТабличнаяЧасть.Количество()> 0) И ЗначениеЗаполнено(ПорядокОкругленияСуммыВПользуПокупателя) Тогда
		
		ТекстВопроса = НСтр("ru = 'В договоре контрагента настроено предоставление скидки покупателю,
								  |путем округления итоговой суммы в пользу покупателя, 
								  |хотите применить скидку для таб. части "+ИмяТабличнойЧасти+"?';"
		    			   +"uk = 'У договорі контрагента настроєне надання знижки покупцеві, 
						   		  |шляхом округлення підсумкової суми на користь покупця, 
								  |хочете застосувати знижку для таб. частини "+ИмяТабличнойЧасти+"?'");
		Если НЕ ОбщегоНазначения.ВопросДаНет(ТекстВопроса) Тогда
		    Возврат;
		КонецЕсли;
		
		ОкруглитьСуммуДокумента(ПорядокОкругленияСуммыВПользуПокупателя,ТабличнаяЧасть,СуммаВключаетНДС,АвторасчетНДС,УчитыватьНДС);
		
	КонецЕсли; 

КонецПроцедуры

// Округляет сумму чека по правилам установленным в договоре
Процедура ОкруглитьСуммуДокумента(ПорядокОкругленияСуммыВПользуПокупателя, ТабличнаяЧастьНоменклатуры, 
													СуммаВключаетНДС, АвторасчетНДС, ЕстьСуммаНДС=Ложь) Экспорт
	
	Если Не ЗначениеЗаполнено(ПорядокОкругленияСуммыВПользуПокупателя) ИЛИ ПорядокОкругленияСуммыВПользуПокупателя = Перечисления.ПорядкиОкругления.Окр0_01 ИЛИ ТабличнаяЧастьНоменклатуры.Количество()= 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЗначениеОкругления = Число(Строка(ПорядокОкругленияСуммыВПользуПокупателя));
	
	Если ЗначениеЗаполнено(ЗначениеОкругления) Тогда
		
		Если СуммаВключаетНДС Тогда
			СуммаБезОкругления = ТабличнаяЧастьНоменклатуры.Итог("Сумма");
		Иначе
			СуммаБезОкругления = ТабличнаяЧастьНоменклатуры.Итог("Сумма")+ТабличнаяЧастьНоменклатуры.Итог("СуммаНДС");
		КОнецЕСли;
		...
		Если Округление > 0 Тогда
			...
			Запрос.УстановитьПараметр("Товары", ТабличнаяЧастьНоменклатуры);
			
			Результат = Запрос.Выполнить();
			Выборка = Результат.Выбрать();
			
			Если Выборка.Следующий() Тогда
				СтрокаМаксимальнойСуммы = ТабличнаяЧастьНоменклатуры[Выборка.НомерСтроки - 1]
			Иначе
				Возврат;
			КонецЕсли;
			...
			Если ЕстьСуммаНДС Тогда
				ТЧСуммаНДС =  СтрокаМаксимальнойСуммы.СуммаНДС;
			Иначе
			    ТЧСуммаНДС = 0;
			КонецЕсли; 
			
			Если СуммаВключаетНДС ИЛИ (ТЧСуммаНДС = 0) Тогда
				СтрокаМаксимальнойСуммы.Сумма = СтрокаМаксимальнойСуммы.Сумма - Округление;
			ИначеЕсли НЕ АвторасчетНДС Тогда
				ОкруглениеСуммаНДС = Цел(Округление*100/6)/100;
				ОкруглениеСумма    = Округление - ОкруглениеСуммаНДС;
				СтрокаМаксимальнойСуммы.Сумма 	 = СтрокаМаксимальнойСуммы.Сумма - ОкруглениеСумма;
				СтрокаМаксимальнойСуммы.СуммаНДС = СтрокаМаксимальнойСуммы.СуммаНДС - ОкруглениеСуммаНДС;
			Иначе
				ТекстОкруглениеНеВыполнено = НСтр("ru='Округление не выполнено. Сумма не включает НДС и включен авторасчет НДС ';uk='Округлення не виконано. Сума не включає ПДВ і включений авторозрахунок ПДВ '");
				Сообщить(ТекстОкруглениеНеВыполнено);
				Возврат;
			КонецЕсли;
...
//
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, СпособРасчета = Неопределено, ЗначениеПустогоКоличества = 0) Экспорт	
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ЕдиницаИзмеренияМест", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Количество", МетаданныеДокумента, ИмяТабличнойЧасти)
		   И ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("КоличествоМест", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения)
			   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' не выбрана единица измерения!"
"Пересчет количества невозможен.';uk=' не обрана одиниця виміру!"
"Перерахунок кількості неможливий.'"));
			...
			Иначе
				Если СтрокаТабличнойЧасти.Коэффициент = 0
				   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' у единицы измерения цены ';uk=' у одиниці виміру ціни '")
					                 + СокрЛП(СтрокаТабличнойЧасти.ЕдиницаИзмерения) + НСтр("ru=' не проставлен коэффициент!"
"Пересчет количества невозможен.';uk=' не проставлений коефіцієнт!"
"Перерахунок кількості неможливий.'"));
				ИначеЕсли СтрокаТабличнойЧасти.ЕдиницаИзмеренияМест.Коэффициент = 0
				   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
					ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' у единицы измерения мест ';uk=' у одиниці виміру місць '")
					                 + СокрЛП(СтрокаТабличнойЧасти.ЕдиницаИзмеренияМест) + НСтр("ru=' не проставлен коэффициент!"
"Пересчет количества невозможен.';uk=' не проставлений коефіцієнт!"
"Перерахунок кількості неможливий.'"));
				Иначе
	...
	Иначе
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Количество", МетаданныеДокумента, ИмяТабличнойЧасти)
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков)
			   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' не выбрана единица хранения остатков!"
"Пересчет количества невозможен.';uk=' не обрана одиниця зберігання залишків!"
"Перерахунок кількості неможливий.'"));
			ИначеЕсли СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент = 0
			   И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для ';uk='Для '") + СокрЛП(СтрокаТабличнойЧасти.Номенклатура) + НСтр("ru=' у единицы измерения ';uk=' у одиниці виміру '")
				                 + (СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков) + НСтр("ru=' не проставлен коэффициент!"
"Пересчет количества невозможен.';uk=' не проставлений коефіцієнт!"
"Перерахунок кількості неможливий.'"));
			Иначе
...
"Перерахунок кількості неможливий.'"));
			ИначеЕсли СтрокаТабличнойЧасти.Коэффициент = 0 Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не проставлен коэффициент!"
"Пересчет количества мест невозможен.';uk='Не проставлений коефіцієнт!"
"Перерахунок кількості місць неможливий.'"));
			Иначе
...
Процедура ЗаполнитьПлановуюСебестоимостьНаОсновании(ДокументОбъект, ДокументОснование, ВалютаРегламентированногоУчета) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти ИЗ ДокументОбъект.Товары Цикл
		...
		Если ПлановаяСебестоимость = 0 Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В строке номер ';uk='У рядку номер '") + СокрЛП(СтрокаТабличнойЧасти.НомерСтроки) + НСтр("ru=' не определена плановая себестоимость!';uk=' не визначена планова собівартість!'"));
		КонецЕсли;
...
Процедура ПересчитатьНДСсУчетомПогрешностиОкругления(ТабличнаяЧасть, ДокументОбъект, ПогрешностиОкругления, ПредставлениеТабличнойЧасти = "", ИмяРеквизитаСумма    = "Сумма", ИмяРеквизитаСуммаНДС = "СуммаНДС", ДополнениеКТексту ="") Экспорт
	...
	#Если Клиент Тогда
		...
		Если СуммаНДСПослеОкругления <> СуммаНДСДоОкругления Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='После исправления погрешностей округления сумма НДС';uk='Після виправлення похибок округлення сума ПДВ'")+ДополнениеКТексту+НСтр("ru=' табличной части ';uk=' табличної частини '") + ПредставлениеТабличнойЧасти + НСтр("ru=' изменилась (было ';uk=' змінилася (було '")
			+ ОбщегоНазначения.ФорматСумм(СуммаНДСДоОкругления,ДокументОбъект.ВалютаДокумента) + НСтр("ru=', стало ';uk=', стало '") + ОбщегоНазначения.ФорматСумм(СуммаНДСПослеОкругления,ДокументОбъект.ВалютаДокумента) + ")");
			
			// изменилась общая сумма документа - сообщим об этом пользователю 
			Если (ДокументОбъект.УчитыватьНДС)
				И (Не ДокументОбъект.СуммаВключаетНДС) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='После исправления погрешностей округления общая сумма';uk='Після виправлення похибок округлення загальна сума'")+ДополнениеКТексту+НСтр("ru=' табличной части ';uk=' табличної частини '") + ПредставлениеТабличнойЧасти + НСтр("ru=' изменилась (было ';uk=' змінилася (було '")
				+ ОбщегоНазначения.ФорматСумм(СуммаДоОкругления,ДокументОбъект.ВалютаДокумента) + НСтр("ru=', стало ';uk=', стало '") + ОбщегоНазначения.ФорматСумм(СуммаПослеОкругления,ДокументОбъект.ВалютаДокумента) + ")");
			КонецЕсли;	
		КонецЕсли;
	#КонецЕсли	
