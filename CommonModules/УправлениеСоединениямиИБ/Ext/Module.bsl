...
Функция ОтключитьСоединенияИБ(ПараметрыАдминистрированияИБ) Экспорт
	...
	Если ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		Сообщение = ПолучитьНазванияСоединенийИБ(НСтр("ru='Не удалось завершить работу пользователей:';uk='Не вдалося завершити роботу користувачів:'"));
...
#КонецЕсли
	...
	Попытка
		...
		Для Каждого Соединение Из Соединения.Соединения Цикл
			...
			СтрСообщения = НСтр("ru='Разрывается соединение: Пользователь ';uk=""Розривається з'єднання: Користувач """) + Соединение.UserName + НСтр("ru=', компьютер ';uk="", комп'ютер """) + 
				Соединение.HostName + НСтр("ru=', установлено ';uk=', встановлено '") + Соединение.ConnectedAt + НСтр("ru=', режим ';uk=', режим '") + Соединение.AppID;
...
Функция ОбработатьПараметрыЗапуска(Знач ЗначениеПараметраЗапуска, Знач ПараметрыЗапуска) Экспорт
	...
	Если ЗначениеПараметраЗапуска = Врег("РазрешитьРаботуПользователей") Тогда
		...
		Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
			Сообщить(НСтр("ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';uk='Параметр запуску РазрешитьРаботуПользователей не відпрацьований. Немає прав на адміністрування інформаційної бази.'"), 
	...
	ИначеЕсли ЗначениеПараметраЗапуска = Врег("ЗавершитьРаботуПользователей") Тогда
		...
		Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
			Сообщить(НСтр("ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';uk='Параметр запуску ЗавершитьРаботуПользователей не відпрацьований. Немає прав на адміністрування інформаційної бази.'"), 
			         СтатусСообщения.Внимание);
			Возврат Ложь;
		КонецЕсли; 
...
Функция ПроверитьПараметрыАдминистрированияИБ(ПараметрыАдминистрированияИБ, Знач ВыдаватьСообщения = Истина) Экспорт
  ...
  Результат = Ложь;
	Попытка
		...
		Результат = Истина;
	Исключение
		Сообщение = НСтр("ru='Не удалось подключиться к кластеру серверов.';uk='Не вдалося підключитися до кластеру серверів.'");
		ЗаписьЖурналаРегистрации(НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'"), УровеньЖурналаРегистрации.Ошибка,,, 
...
Функция СфомироватьСообщениеБлокировки(Знач Сообщение, Знач КодРазрешения) Экспорт
	...
	ТекстСообщения = ТекстСообщения +
		НСтр("ru='Для администратора:';uk='Для адміністратора:'") + Символы.ПС +
		НСтр("ru='Чтобы принудительно разблокировать информационную базу, воспользуйтесь консолью кластера серверов или запустите ""1С:Предприятие"" с параметрами:';uk='Для того, аби примусово розблокувати інформаційну базу, скористуйтеся консолью кластеру серверів або запустіть ""1С:Підприємство"" з параметрами:'") + Символы.ПС +
...
Функция ИзвлечьСообщениеБлокировки(Знач Сообщение) Экспорт
	...
	ИндексМаркера = Найти(Сообщение, НСтр("ru='Для администратора:';uk='Для адміністратора:'"));
...
Функция ПолучитьАктивныеСоединенияИБ(НастройкаБлокировки, Знач ВсеКромеТекущего = Истина) 
	...
	Если ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("ru='Невозможно получить список активных соединения в Файловом варианте системы';uk='Неможливо отримати список активних з`єднань у Файловому варіанті системи'");
	КонецЕсли;	
