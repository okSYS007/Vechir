...
Функция ПроверитьУчетныеЗаписиДляОтправкиПисем(ТекущийПользователь, Отказ = Ложь, ПредлагатьСозданиеНовойУчетнойЗаписи = Истина) Экспорт
	...
	Если СписокДоступныхЗаписей.Количество() = 0 Тогда
		Если ПредлагатьСозданиеНовойУчетнойЗаписи Тогда
			ОтветНаВопрос = Вопрос(НСтр("ru='У вас нет ни одной доступной учетной записи для отправки писем из встроенного почтового клиента."
"Создать новую?';uk='У вас немає жодного доступного облікового запису для відправлення листів із вбудованого поштового клієнта."
"Створити новий?'"), РежимДиалогаВопрос.ДаНет);
			...
			СписокДоступныхЗаписей = ПолучитьДоступныеУчетныеЗаписи(ТекущийПользователь).Запись;
			Если СписокДоступныхЗаписей.Количество() = 0 Тогда
				Предупреждение(НСтр("ru='У вас нет ни одной доступной учетной записи для отправки писем из встроенного почтового клиента.';uk='У вас немає жодного доступного облікового запису для відправлення листів із вбудованого поштового клієнта.'"));
				Отказ = Истина;
			КонецЕсли;
...
Процедура ОтправитьПисьма(Письма, СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, Знач ПоказыватьОкноВыполнения = Истина, ТекстОшибок = "") Экспорт
	...
	Для каждого Письмо Из Письма Цикл
		...
		Если Письмо.Ключ.СтатусПисьма = Перечисления.СтатусыПисем.Полученное ИЛИ Письмо.Ключ.СтатусПисьма = Перечисления.СтатусыПисем.Отправленное Тогда
			Сообщить("" + СокрЛП(Письмо.Ключ) + НСтр("ru=' исключено из списка отправки, отправлять можно только сохраненные или исходящие письма.';uk='виключено зі списку відправлення, відправляти можна тільки збережені або вихідні листи.'"));
			Продолжить;;
		КонецЕсли;
		
		Если Письмо.Ключ.ПометкаУдаления Тогда
			Сообщить("" + СокрЛП(Письмо.Ключ) + НСтр("ru=' исключено из списка отправки, письмо помечено на удаление.';uk='виключено зі списку відправлення, лист позначений на вилучення.'"));
			Продолжить;
		КонецЕсли;
		
		Если Письмо.Ключ.КомуТЧ.Количество() = 0 Тогда
			Сообщить("" + СокрЛП(Письмо.Ключ) + НСтр("ru=' исключено из списка отправки, не указан ни один получатель.';uk='виключено зі списку відправлення, не вказано жодного отримувача.'"));
			Продолжить;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Письмо.Ключ.УчетнаяЗапись) Тогда
			Сообщить("" + СокрЛП(Письмо.Ключ) + НСтр("ru=' исключено из списка отправки, не указана учетная запись.';uk='виключено зі списку відпралення, не вказано обліковий запис.'"));
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗнч(Письмо.Значение) <> Тип("ДокументОбъект.ЭлектронноеПисьмо") Тогда
			...
			Исключение
				ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, (СокрЛП(Письмо) + НСтр("ru=' исключено из списка отправки.';uk=' виключено зі списку відправлення.'")));
				Продолжить;;
			КонецПопытки;
		Иначе
...
Процедура ОткрытьЭлектронныеПисьмаПоОбъекту(Ссылка) Экспорт
	...
	Если НЕ Константы.ИспользованиеВстроенногоПочтовогоКлиента.Получить() Тогда
		Предупреждение(НСтр("ru='Данная операция доступна только при использовании встроенного почтового клиента.';uk='Ця операція доступна тільки за умови використання вбудованого поштового клієнта'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьСообщениеВОтделТехническойПоддержки() Экспорт
	...
	Если НЕ Константы.ИспользованиеВстроенногоПочтовогоКлиента.Получить() Тогда
		...
		СтруктураПараметров.Вставить("Текст"        , СтрЗаменить(ОбщегоНазначения.ПолучитьТекстСообщенияВОтделТехническойПоддержки(), Символы.ПС, Символы.ВТаб));
		СтруктураПараметров.Вставить("Тема"         , НСтр("ru='Сообщение в отдел технической поддержки';uk='Повідомлення у відділ технічної підтримки'"));
	...
	Иначе
		...
		СтруктураНовогоПисьма.Вставить("Тема"     , НСтр("ru='Сообщение в отдел технической поддержки';uk='Повідомлення у відділ технічної підтримки'"));
...
Процедура ПолучениеОтправкаПисем(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, МассивУчетныхЗаписей = Неопределено, 
	...
	Если НЕ Константы.ИспользованиеВстроенногоПочтовогоКлиента.Получить() Тогда
		Сообщить(НСтр("ru='В настройках учета не установлено использование встроенного почтового клиента. Действие отправка/получение писем не выполнено.';uk='В настройках обліку не обране використання вбудованого поштового клієнта. Дія відправлення/отримання листів не виконана.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли; 
	...
	Если ПоказыватьОкноВыполнения Тогда
		...
		Для каждого ЭлементСоответствия Из СоответствиеЗаявленныхУчетныхЗаписей Цикл
			...
			Если СписокДоступныхУчетныхЗаписей.НайтиПоЗначению(ЭлементСоответствия.Ключ) = Неопределено Тогда
				...
				ТекстОшибок = Символы.ПС + ТекстОшибок + ("Заявлена " + ЭлементСоответствия.Значение.Наименование + " <" + ЭлементСоответствия.Значение.АдресЭлектроннойПочты + ">" + НСтр("ru=' - нет прав доступа для записи электронных писем.';uk=' - немає прав доступу для запису електронних листів.'"));
	...
	ФормаПрогрессора.КомментарийОбработкиДанных = НСтр("ru='Получение и отправка писем электронной почты';uk='Отримання і відправка листів електронної пошти'");
	...
	Если СоответствиеУчетныхЗаписейВсего.Количество() > 0 Тогда
		...
		Для каждого ЭлементСоответствия Из СоответствиеУчетныхЗаписейВсего Цикл
			...
			ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Подключение к ';uk='Підключення до '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
			#КонецЕсли
			...
			Если Отправить Тогда
				...
				#Если Клиент Тогда
				ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Отправка сообщений ';uk='Відправка повідомлень '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
				#КонецЕсли
					...
					Если СтрокаДереваУчетнойЗаписи <> Неопределено Тогда
						...
						Для каждого СтрокаПисьма Из СтрокаДереваУчетнойЗаписи.Строки Цикл
							...
							Если ЗначениеЗаполнено(СтрокаПисьма.Письмо.ИмяФайлаРедактированияХТМЛТекста) Тогда
								Сообщить(НСтр("ru='У письма ';uk='У листи '") + СокрЛП(СтрокаПисьма.Письмо) + " не закончено редактирование текста внешним HTML редактором. Письмо не отправлено.");
								Продолжить;
							КонецЕсли;
							#КонецЕсли
			...
			Если Получить Тогда
				...
				#Если Клиент Тогда
				ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Получение сообщений с сервера ';uk='Отримання повідомлень з сервера '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
				ФормаПрогрессора.Значение = 0;
				#КонецЕсли
				...
				#Если Клиент Тогда
				ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Выборка заголовков сообщений ';uk='Вибірка заголовків повідомлень '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
				...
				Если МассивЗаголовковПисем.Количество() > 0 Тогда
					...
					#Если Клиент Тогда
					ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Регистрация сообщений ';uk='Реєстрація повідомлень '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
				...
				Если МассивПисемДляУдаления.Количество() > 0 Тогда
					...
					#Если Клиент Тогда
					ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Удаление сообщений с сервера ';uk='Вилучення повідомлень з сервера '") + УчетнаяЗапись.Наименование + " <" + УчетнаяЗапись.АдресЭлектроннойПочты + "> ...";
					#КонецЕсли
	...
	Если ПоказыватьОкноВыполнения Тогда
		...
		Иначе
			ФормаПрогрессора.КомментарийЗначения = НСтр("ru='Задачи завершены с ошибками';uk='Задачі завершено з помилками'");
...
Функция НаписатьПисьмо(ТекущийПользователь, СтруктураНовогоПисьма = Неопределено, ПеренестиВложенияИзОснования = Ложь, Копирование = Ложь,
	...
	Если (ТипЗнч(Основание) = Тип("ДокументСсылка.ЭлектронноеПисьмо") ИЛИ ТипЗнч(Основание) = Тип("ДокументОбъект.ЭлектронноеПисьмо"))
	   И НЕ Копирование Тогда
		...
		Если Письмо.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.HTML ИЛИ Письмо.ВидТекстаПисьма = Перечисления.ВидыТекстовЭлектронныхПисем.HTMLСКартинками Тогда
			...
			СтрокаОтправителя = НСтр("ru='Отправитель: ';uk='Відправник: '");
			...
			Если НЕ ПустаяСтрока(СтрокаКому) Тогда
				СтрокаКому = НСтр("ru='Получатели: ';uk='Одержувачі: '") + СтрокаКому;
			КонецЕсли;
			...
			Если НЕ ПустаяСтрока(СтрокаКопии) Тогда
				СтрокаКопии = НСтр("ru='Копии: ';uk='Копії: '") + СтрокаКопии;
			КонецЕсли;
		...
		Иначе
			...
			Если НЕ ПустаяСтрока(СтрокаКому) Тогда
				НовыйТекстПисьма.ВставитьСтроку(ПоследняяСтрока + 1, НСтр("ru='Получатели: ';uk='Одержувачі: '") + СтрокаКому);
			...
			Если НЕ ПустаяСтрока(СтрокаКопии) Тогда
				НовыйТекстПисьма.ВставитьСтроку(ПоследняяСтрока + 1, НСтр("ru='Копии: ';uk='Копії: '") + СтрокаКопии);
				ПоследняяСтрока = ПоследняяСтрока + 1;
			КонецЕсли; 
	...
	Если НЕ ОткрыватьПисьмо Тогда
		...
		СтруктураВозврата = Новый Структура("Письмо, Форма, ПисьмоСсылка", Письмо, ФормаПисьма, Письмо.Ссылка);
		#Иначе
	    //Дандали В. 25.07.2025 +
		Если СтруктураНовогоПисьма.Свойство("ОтправленоФоном") 
			И СтруктураНовогоПисьма.ОтправленоФоном Тогда
			СтруктураВозврата = Новый Структура("Письмо, Форма, ПисьмоСсылка", Письмо, Неопределено, Письмо.Ссылка);
		Иначе
	...
	Если НЕ ПустаяСтрока(href) Тогда
		...
		Если (СтрЧислоВхождений(href, "ИмяФайлаРедактированияХТМЛТекста") > 0) И
			...
			Если ИмяКомпьютера() <> ЭлектронноеПисьмо.ИмяКомпьютераРедактированияХТМЛТекста Тогда
				Сообщить(НСтр("ru='Файл редактируется на копмьютере""';uk='Файл редагується на комп''ютері""'") + ЭлектронноеПисьмо.ИмяКомпьютераРедактированияХТМЛТекста + """" + НСтр("ru='. Редактирование на текущем комьютере невозможно.';uk="". Редагування на поточному комп'ютері неможливе."""));
			Иначе
...
Функция ОткрытьФайлДляРедактированияВнешнимХТМЛРедактором(ИмяФайла) Экспорт
	...
	Если НЕ Файл.Существует() ИЛИ Файл.ЭтоКаталог() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка открытия файла. Файл не найден на диске.';uk='Помилка вікриття файлу. Файл не знайдено на диску.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ИмяПриложения) ИЛИ ТипЗнч(ИмяПриложения) <> Тип("Строка") Тогда
		ОбщегоНазначения.СообщитьОбОшибке("У пользователя не указана командная строка для запуска внешнего HTML редактора текстов.",, НСтр("ru='Редактирование невозможно.';uk='Редагування неможливе.'"));
		Возврат Ложь;
	КонецЕсли;
	
	ИмяПриложения = Врег(ИмяПриложения);
	Если Найти(ИмяПриложения, "[FILENAME]") = 0 ИЛИ СтрЧислоВхождений(ИмяПриложения, "[FILENAME]") > 1 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В настройках пользователя некорректно задана командная строка для запуска внешнего HTML редактора текстов."
"Необходимо один раз указать место для подстановки имени файла в виде шаблона [FileName].';uk='У настройках користувача некоректно заданий командний рядок для запуску зовнішнього HTML редактора текстів."
"Необхідно один раз вказати місце для підстановки імені файлу у виді шаблону [FileName].'"),, НСтр("ru='Редактирование невозможно.';uk='Редагування неможливе.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Невозможно запустить внешнее приложение.';uk='Неможливо запустити зовнішній додаток.'"));
		Возврат Ложь;
	КонецПопытки;
...
Процедура НайтиИОткрытьПотомкаПисьма(ДокументСсылка) Экспорт
	...
	Иначе
		Предупреждение(НСтр("ru='Письмо-потомок не найдено, возможно оно было удалено.';uk='Лист-нащадок не знайдено, можливо він був вилучений.'"));
	КонецЕсли; 
...
Процедура ВыбратьОтложенноеВремяИзСписка(ЭлементУправления) Экспорт
	...
	СписокВремен.Добавить("Через30минут"          , НСтр("ru='Через 30 минут';uk='Через 30 хвилин'"));
	СписокВремен.Добавить("ЧерезЧас"              , НСтр("ru='Через 1 час';uk='Через 1 годину'"));
	СписокВремен.Добавить("Через2Часа"            , НСтр("ru='Через 2 часа';uk='Через 2 години'"));
	СписокВремен.Добавить("Завтра"                , НСтр("ru='Завтра';uk='Завтра'"));
	СписокВремен.Добавить("Послезавтра"           , НСтр("ru='Послезавтра';uk='Післязавтра'"));
	СписокВремен.Добавить("НачалоСледующейНедели" , НСтр("ru='Начало следующей недели';uk='Початок наступного тижня'"));
	СписокВремен.Добавить("ЧерезНеделю"           , НСтр("ru='Через неделю';uk='Через тиждень'"));
	СписокВремен.Добавить("Через2Недели"          , НСтр("ru='Через 2 недели';uk='Через 2 тижня'"));
	СписокВремен.Добавить("НачалоСледующегоМесяца", НСтр("ru='Начало следующего месяца';uk='Початок наступного місяця'"));
	СписокВремен.Добавить("ЧерезМесяц"            , НСтр("ru='Через месяц';uk='Через місяць'"));
