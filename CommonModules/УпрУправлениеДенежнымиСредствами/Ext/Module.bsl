...
Функция ПолучитьСвободныйОстатокДС(БанковскийСчетКасса,Дата, МассивЗаявки) Экспорт
	...
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	|	УпрДенежныеСредстваОстатки.СуммаОстаток 
	...
	|ИЗ
	|	РегистрНакопления.УпрДенежныеСредства.Остатки(&Дата, БанковскийСчетКасса = &БанковскийСчетКасса) КАК УпрДенежныеСредстваОстатки
	...
	|		ПО УпрДенежныеСредстваОстатки.БанковскийСчетКасса = ДенежныеСредстваВРезервеОстатки.БанковскийСчетКасса
	...
	|		ПО УпрДенежныеСредстваОстатки.БанковскийСчетКасса = ДенежныеСредстваКСписаниюОстатки.БанковскийСчетКасса
	...
	|		ПО УпрДенежныеСредстваОстатки.БанковскийСчетКасса = ДенежныеСредстваКПолучениюОстатки.БанковскийСчетКасса";
...
Функция ПолучитьТаблицуПлатежейУпр(ДатаДокумента,ВалютаДокумента,Документ, ВидПлатежногоДокумента) Экспорт
	...
	ВыбиратьДокументРасчетовСКонтрагентом = (	ВидПлатежногоДокумента = "ПлатежноеПоручениеВходящее"
											...
											ИЛИ ВидПлатежногоДокумента = "УпрРасходныйКассовыйОрдер"
											ИЛИ ВидПлатежногоДокумента = "УпрПриходныйКассовыйОрдер"
	...
	Если ВидПлатежногоДокумента = "УпрПриходныйКассовыйОрдер" ИЛИ ВидПлатежногоДокумента = "УпрРасходныйКассовыйОрдер" Тогда
		Запрос.Текст=Запрос.Текст+"
...
Функция ПодготовитьТаблицуОплат(СтруктураШапкиДокумента) Экспорт
	...
	Если    ВидДокумента="ПлатежноеПоручениеВходящее" 
	 	...
	 	Или ВидДокумента="УпрПриходныйКассовыйОрдер" Тогда
...
Процедура ЗаполнитьРеквизитыРасчетногоДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, 
	...
	Если МетаданныеДокумента.Имя="УпрПриходныйКассовыйОрдер" ИЛИ МетаданныеДокумента.Имя="УпрРасходныйКассовыйОрдер" Тогда
...
Функция ОграничитьТипыДокументовДляДокументовОплаты(Элемент, БазовоеОписаниеТипа,ЭтоВозврат = Ложь) Экспорт
	...
	Иначе
		...
		УдаляемыеТипыДокументовПоВидуОперации.Добавить(Тип("ДокументСсылка.УпрРасходныйКассовыйОрдер"));
		УдаляемыеТипыДокументовПоВидуОперации.Добавить(Тип("ДокументСсылка.ПлатежноеПоручениеВходящее"));
		УдаляемыеТипыДокументовПоВидуОперации.Добавить(Тип("ДокументСсылка.УпрПриходныйКассовыйОрдер"));
...
Процедура ПровестиПоступлениеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	Если Оплачено Тогда
		...
		Движения.УпрДенежныеСредства.мПериод          = ДатаДвижений;
		Движения.УпрДенежныеСредства.мТаблицаДвижений = ТаблицаДенежныеСредства;
		Движения.УпрДенежныеСредства.ВыполнитьПриход();
		...
		//		СтрокаДвиженийПолучение.ДокументПолучения = ?(НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.УпрРасходныйКассовыйОрдер), Ссылка, СтруктураШапкиДокумента.УпрРасходныйКассовыйОрдер);
		...
		Если ВидОперации = Перечисления.ВидыОперацийПКО.ПолучениеНаличныхДенежныхСредствВБанке И НЕ ПоДенежномуЧеку Тогда
			...
			Движения.УпрДенежныеСредства.мПериод          = ДатаДвижений;
			Движения.УпрДенежныеСредства.мТаблицаДвижений = ТаблицаДенежныеСредства;
			Движения.УпрДенежныеСредства.ВыполнитьРасход();
	...
	Если Оплачено Тогда  
		...
		Если СтруктураШапкиДокумента.ВедениеУчетаПоПроектам Тогда
			//Dasha. 02.08.2022 16:04 +++
			Если ВидДокумента="УпрПриходныйКассовыйОрдер" ИЛИ ВидДокумента="УпрРасходныйКассовыйОрдер" Тогда	
		...
		Движения.УпрДвиженияДенежныхСредств.мПериод          = ДатаДвижений;
		...
		Если ВидОперации = Перечисления.ВидыОперацийПКО.ПолучениеНаличныхДенежныхСредствВБанке И НЕ ПоДенежномуЧеку Тогда
			...
			Движения.УпрДвиженияДенежныхСредств.мПериод          = Дата;
			Движения.УпрДвиженияДенежныхСредств.мТаблицаДвижений = ТаблицаДвиженияДенежныхСредств;
			Движения.УпрДвиженияДенежныхСредств.ВыполнитьДвижения();
		...
		//	Запрос.Текст = "
		//	|ВЫБРАТЬ
		//	|	УпрПриходныйКассовыйОрдер.Организация              КАК Организация,
		//	|	УпрПриходныйКассовыйОрдер.Касса                    КАК БанковскийСчетКасса,
		...
		//	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УпрПриходныйКассовыйОрдер КАК УпрПриходныйКассовыйОрдер
		//	|		ПО ДенежныйЧекВыплатаЗаработнойПлаты.Ссылка = УпрПриходныйКассовыйОрдер.ДенежныйЧек
		//	|
		//	|ГДЕ
		//	|	УпрПриходныйКассовыйОрдер.Ссылка = &Ссылка";
		//		
		//	Запрос.УстановитьПараметр("Ссылка", Ссылка);
...
Процедура ПровестиСписаниеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	Если Оплачено И (ОтраженоВОперУчете ИЛИ ПоРасчетномуДокументу) Тогда  // Проводим по фактическим взаиморасчетам
		...
		Если СтруктураШапкиДокумента.ВедениеУчетаПоПроектам Тогда
			...
			Если ВидДокумента="УпрРасходныйКассовыйОрдер" ИЛИ ВидДокумента="УпрПриходныйКассовыйОрдер" Тогда
