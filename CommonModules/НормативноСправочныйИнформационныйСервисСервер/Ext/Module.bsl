...
Функция ПолучитьНастройкиЗагрузок() Экспорт
	...
	Результат = НормативноСправочныйИнформационныйСервисПереопределяемый.ПолучитьНастройкиПоУмолчанию();
...
Функция ПолучитьНастройкиВалют()
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.ПолучитьНастройкиВалют();
...
Функция ВыполнитьЗапросКСерверу(Сервис, Параметры, ВФоне) Экспорт
	...
	Если НормативноСправочныйИнформационныйСервисПереопределяемый.ПроверкаПередПодключением(ВФоне) = Неопределено Тогда
...
Функция СписокДоступныхРегистров() Экспорт
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.СписокДоступныхРегистров();
...
Функция СписокДоступныхКлассификаторов() Экспорт
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.СписокДоступныхКлассификаторов();
...
Функция СписокДоступныхНовостей() Экспорт
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.СписокДоступныхНовостей();
...
Функция СоответствиеКлассификаторовИМакетов()
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.СоответствиеКлассификаторовИМакетов();
...
Функция ПолучитьПараметрыКонфигурации()
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.ПолучитьПараметрыКонфигурации();
...
Функция ПолучитьПараметрыАвторизации()
	...
	Возврат НормативноСправочныйИнформационныйСервисПереопределяемый.ПолучитьПараметрыАвторизации();
...
Функция СформироватьЗначение(СоответствиеТипов, Ключ, Значение)
	...
	Иначе
		Если ТипЗнч(Значение) = Тип("Строка") Тогда
			Попытка
				Если Найти(Значение, "T00") > 0 Тогда
...
Функция ПрочитатьДокументИзБазы(Идентификатор) 
...
Процедура ВыполнитьОбновлениеКлассификаторов(ВыводитьПротокол = Ложь, ВФоне = Ложь) Экспорт
...
Процедура ВыполнитьОбновлениеДанных()
	...
	Настройки =  ПолучитьНастройкиЗагрузок();
	Если Настройки.ЗагружатьДанныеПриСтарте И УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ЗагружатьДанныеНСИСПриЗапускеСистемы") Тогда
		ВыполнитьОбновлениеДанныхРегламентноеЗадание();
	КонецЕсли;	
...
#Если Клиент Тогда
	...
	Если Настройки.ОткрыватьИнформациюПриСтарте Тогда
		ТМП = ВосстановитьЗначение("НСИСПоказСтартовойИнформации");
		ПоказФормы = ?(ТипЗнч(ТМП) = Тип("Неопределено"), Истина, ТМП);
		Если ПоказФормы Тогда
			Форма = Обработки.НСИСНовости.ПолучитьФорму("Форма");
			Форма.Открыть();
		КонецЕсли;	
	КонецЕсли;
#КонецЕсли	
...
#Если Клиент Тогда	
	...
	Если Настройки.ОткрыватьИнформациюПриСтарте Тогда
		...
		ПоказФормы = ?(ТипЗнч(ТМП) = Тип("Неопределено"), Истина, ТМП);
		Если ПоказФормы Тогда
			Форма = Обработки.НСИСКалендарьБухгалтера.ПолучитьФорму("Форма");
			Форма.Открыть();
		КонецЕсли;	
	КонецЕсли;
#КонецЕсли	
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если РольДоступна("ПолныеПрава") ИЛИ РольДоступна("НСИСИспользование") Тогда
