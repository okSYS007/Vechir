...
Процедура ОшибкаНетОстатка(ТекстСообщения, Остаток, Требуется, ЕИ, Отказ, Заголовок="") Экспорт
	...
	ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения + Символы.ПС + Символы.Таб +
					   НСтр("ru='Не хватает ';uk='Не вистачає '") + ?(Остаток > 0 , Требуется - Остаток, Требуется) + " " + ЕИ +
					   НСтр("ru='; Остаток ';uk='; Залишок '") + Остаток + " " + ЕИ +
					   НСтр("ru='; Требуется ';uk='; Потрібно '") + Требуется + " " + ЕИ, Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.Услуга Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится услуга. ';uk='міститься послуга. '") +
					                   НСтр("ru='Услуг здесь быть не должно!';uk='Послуг тут бути не повинно!'"), Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  Не СтрокаТаблицы.Услуга Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится номенклатура, не являющаяся услугой. ';uk='міститься номенклатура, яка не є послугою. '") +
					                   НСтр("ru='Здесь могут быть только услуги!';uk='Тут можуть бути тільки послуги!'"), Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура")
			   И  СтрокаТаблицы.Набор Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится набор-пакет. ';uk='міститься набір-пакет. '") +
					                   НСтр("ru='Наборов-пакетов здесь быть не должно!';uk='Наборів-пакетів тут бути не повинно!'"), Отказ, Заголовок);
...
Процедура ПроверитьЧтоСкладНеНТТ(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений, 
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если СтрокаТаблицы[ИмяВидаСкладаРазмещения] = Перечисления.ВидыСкладов.НТТ Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится размещение по неавтоматизированной торговой точке. ';uk='міститься розміщення по неавтоматизованій торговій точці. '") +
				                   НСтр("ru='Такого быть не должно!';uk='Такого бути не повинно!'"), Отказ, Заголовок);
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.ВестиУчетПоХарактеристикам Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится номенклатура, по которой ведется учет по характеристикам. ';uk='міститься номенклатура, по якій ведеться облік за характеристиками. '") +
					                   НСтр("ru='По возвратной таре не ведется учет по характеристикам!';uk='По зворотній тарі не ведеться облік по характеристиках!'"), Отказ, Заголовок);
...
//
Функция ПолучитьТаблицуОстатковПоСериямДляНТТ(СкладДоговор, МассивНоменклатуры)
	Запрос = Новый Запрос;
	...
	|ИЗ
	|	РегистрНакопления.ТоварыВНТТ.Остатки(, Склад В (&Склад)
...
//
Функция ПолучитьТаблицуОстатковПоСериям(СкладДоговор, Организация, МассивНоменклатуры, Комиссионер = Неопределено, НТТ = Ложь, ТаблицаРезервовПоСериям = Неопределено) Экспорт

	Если НТТ Тогда
		Возврат ПолучитьТаблицуОстатковПоСериямДляНТТ(СкладДоговор, МассивНоменклатуры);
	КонецЕсли;
	...
	МожноПревышатьОстатокПоОрганизации = НЕ ВестиУчетПоДопРазрезамРегл ИЛИ УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации();
	...
	|	NULL														   КАК ОстатокКПередаче
	|ИЗ
	|	РегистрНакопления.ТоварыОрганизаций.Остатки(,%УСЛОВИЕ_ПО_ОРГАНИЗАЦИИ% 
	|												  Номенклатура В (&Номенклатура) И Комиссионер = &Комиссионер 
	|												И Склад В (&Склад)) КАК ТоварыОрганизацийОстатки
	|СГРУППИРОВАТЬ ПО
	|	ТоварыОрганизацийОстатки.Номенклатура,
	|	%УсловиеГруппировкиСкладТоварыОрганизаций%
	...
	Если Комиссионер = Неопределено Тогда
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%ПолеСклад%", "Склад"); 
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%УсловиеГруппировкиСкладТоварыОрганизаций%", 					"ТоварыОрганизацийОстатки.Склад,");
	Иначе
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%ПолеСклад%", "NULL"); 
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%УсловиеГруппировкиСкладТоварыОрганизаций%", 					"");
	КонецЕсли;
	
	Если МожноПревышатьОстатокПоОрганизации Тогда
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%УСЛОВИЕ_ПО_ОРГАНИЗАЦИИ%", ""); 
	Иначе
	   ТекстЗапросаПоОрганизации = СтрЗаменить(ТекстЗапросаПоОрганизации, "%УСЛОВИЕ_ПО_ОРГАНИЗАЦИИ%", "Организация = &Организация И "); 
	КонецЕсли;
	...
	Если Комиссионер = Неопределено Тогда
		Если ИмяРегистра = "ТоварыНаСкладах" Тогда
			...
			|	ИЗ
			|		РегистрНакопления.ТоварыНаСкладах.Остатки(, Склад В (&Склад)
			...
			|	ИЗ
			|		РегистрНакопления.ТоварыВРознице.Остатки(, Склад В (&Склад)
		...
		Иначе
			ТекстЗапросаСклады = "
			|	РегистрНакопления.ТоварыПереданные.Остатки(, Номенклатура В (&Номенклатура)) КАК ТоварыНаСкладахОстатки
			|";
		КонецЕсли;
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если Комиссионер = Неопределено Тогда
		...
		Если Константы.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании.Получить() Тогда
			...
			|ИЗ
			|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(,Склад В (&Склад)
			|	  И Номенклатура  В (&Номенклатура)
...
Функция ВедетсяПартионныйУчет(УчетнаяПолитика) Экспорт
	...
	Если УчетнаяПолитика.Свойство("СпособОценкиМПЗ") Тогда
...
Процедура ПроверитьЧтоНетБланковСтрогогоУчета(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений = Неопределено, 
	...
	Если (НЕ ТаблицаЗначений = Неопределено) И (ТаблицаЗначений.Количество() > 0) Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.БланкСтрогогоУчета = Истина Тогда

					ОбщегоНазначения.ОшибкаПриПроведении(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке номер ""%1"" табличной части ""%2"": содержится бланк строгого учета. Бланков строгого учета здесь быть не должно!';uk='У рядку номер ""%1"" табличної частини ""%2"": міститься бланк суворого обліку. Бланків суворого обліку тут бути не повинно!'"), СокрЛП(СтрокаТаблицы.НомерСтроки), ПредставлениеТабличнойЧасти), Отказ, Заголовок);
	...
	Иначе
		...
		Для каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  СтрокаТаблицы.Номенклатура.БланкСтрогогоУчета Тогда

					ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + НСтр("ru=' содержится бланк строгого учета. Бланков строгого учета здесь быть не должно';uk=' міститься бланк суворого обліку. Бланків суворого обліку тут бути не повинно'"), Отказ, Заголовок);
...
Процедура ПроверитьЧтоНетНЕБланковСтрогогоУчета(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений = Неопределено, 
	...
	Если (НЕ ТаблицаЗначений = Неопределено) И (ТаблицаЗначений.Количество() > 0) Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  Не СтрокаТаблицы.БланкСтрогогоУчета Тогда

					ОбщегоНазначения.ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится номенклатура, не являющаяся бланком строгого учета. Здесь могут быть только бланки строгого учета!';uk='міститься номенклатура, що не є бланком суворого обліку. Тут можуть бути тільки бланки суворого обліку!'"), 
										Отказ, Заголовок);
	...
	Иначе	
		...
		Для каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура)
			   И  Не СтрокаТаблицы.Номенклатура.БланкСтрогогоУчета Тогда
...
Процедура ДобавитьСоставНабора(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	ДокументыПродажи.Вставить(МетаданныеДокументы["ОтчетКомиссионераОПродажах"], 0);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКомплектующих Цикл
		...
		Иначе
			...
			Если ТекДокументПродажи <> Неопределено Тогда
				...
				Попытка
					ДокументОбъект.ЗаполнитьСчетаУчетаВСтрокеТабЧасти(НоваяСтрока, "СоставНабора", ДокументОбъект.ОтражатьВБухгалтерскомУчете, ДокументОбъект.ОтражатьВБухгалтерскомУчете);
				Исключение
				КонецПопытки
			КонецЕсли;
		КонецЕсли;
...
Процедура КонтрольЛимитовВозвратнойТарыПоставщика(ДокументОбъект, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения = НСтр("ru='По ';uk='По '") + 
		УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, "", "") +
		НСтр("ru=' по договору ""';uk=' по договору ""'") + СокрЛП(СтруктураШапкиДокумента.ДоговорКонтрагента) + НСтр("ru='"" превышен лимит возвратной тары.';uk='"" перевищено ліміт поворотної тари.'");

		ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения + Символы.ПС + Символы.Таб +
						   НСтр("ru='Лимит ';uk='Ліміт '") + (Выборка.Лимит) + " " + Выборка.ЕдиницаХраненияОстатков +
						   НСтр("ru='; Получено ранее ';uk='; Отримано раніше '") + Выборка.ОстатокКоличество + " " + Выборка.ЕдиницаХраненияОстатков +
...
//  Заголовок         - строка, заголовок сообщения об ошибке проведения.
//
Процедура ПроверитьЧтоНетКомплектов(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений = Неопределено, 
                                                    Отказ, Заголовок) Экспорт
		...
		Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура")
			   И  СтрокаТаблицы.Комплект Тогда
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='содержится набор-комплект. ';uk='міститься набір-комплект. '") +
					                   НСтр("ru='Наборов-комплектов здесь быть не должно!';uk='Наборів-комплектів тут бути не повинно!'"), Отказ, Заголовок);
	...
	Иначе
		...
		Для каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура")
			   И  СтрокаТаблицы.Номенклатура.Комплект Тогда
...
Процедура СообщитьОНедостаткеТовара(Требуется, Свободно = 0, ВРезерве = 0, КПередаче = 0, КПолучению = 0, Отказ, Заголовок="", Склад = "", ЗаказПоставщику = "", Номенклатура = "", Характеристика = "", Серия = "", Качество = "", ЕИ = "", СтрокаСообщения = "") Экспорт
	...
	Если СтрокаСообщения = "" Тогда
		Если ВРезерве > 0 ИЛИ КПередаче > 0 Тогда
			СтрокаСообщения = НСтр("ru='Свободного остатка ';uk='Вільного залишку '");
		Иначе
	...
	Если ЗаказПоставщику <> "" И Склад <> "" Тогда
		СтрокаСообщения = СтрокаСообщения + НСтр("ru=' в заказах поставщикам на склад ';uk=' в замовленнях постачальникам на склад '") + Склад + НСтр("ru=' недостаточно для размещения.';uk=' недостатньо для розміщення.'");
	ИначеЕсли ЗаказПоставщику <> "" Тогда
		СтрокаСообщения = СтрокаСообщения + НСтр("ru=' заказанного по документу ';uk=' замовленого по документу '") + ЗаказПоставщику + НСтр("ru=' недостаточно для размещения.';uk=' недостатньо для розміщення.'");
	ИначеЕсли Склад <> "" Тогда
		СтрокаСообщения = СтрокаСообщения + НСтр("ru=' на складе ';uk=' на складі '") + Склад + НСтр("ru=' недостаточно.';uk=' недостатньо.'");
	КонецЕсли;

	ОшибкаНетОстатка(СтрокаСообщения, Свободно , Требуется, ЕИ, Отказ, Заголовок);

	Если КПередаче > 0 Тогда
		Сообщить(НСтр("ru='К передаче ';uk='До передачі '") + КПередаче + " " + ЕИ);
	КонецЕсли;
	Если ВРезерве > 0 Тогда
		Сообщить(НСтр("ru='Зарезервировано ';uk='Зарезервовано '") + ВРезерве + " " + ЕИ);
	КонецЕсли;
	Если КПолучению > 0 Тогда
		Сообщить(НСтр("ru='К получению ';uk='До отримання '")+ КПолучению + " "+ ЕИ);
	КонецЕсли;
...
Функция СформироватьТаблицуКомплектующих(ТаблицаИсточник, ДокументОбъект, СтруктураШапкиДокумента = Неопределено) Экспорт
	...
	Если НайденныеКомплекты.Количество() > 0 Тогда
		...
		ЕстьНоменклатурнаяПозиция  = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("НоменклатурнаяПозиция", МетаданныеДокумента, "СоставНабора");
		...
		|" + ?(ЕстьНоменклатурнаяПозиция, "Док.НоменклатурнаяПозиция КАК НоменклатурнаяПозиция,","") + "
		...
		Для Каждого СтрокаКомплект Из НайденныеКомплекты Цикл
			...
			Для Каждого СтрокаКомплектующая Из НайденныеКомплектующие Цикл
				...
				Для Сч = 1 По ТаблицаРезультат.Колонки.Количество() Цикл
					...
					ИначеЕсли ЕстьСчетУчетаБУ И ИмяКолонки = "СчетУчетаБУ" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.СчетУчетаБУ);
					ИначеЕсли ЕстьНоменклатурнаяПозиция И ИмяКолонки = "НоменклатурнаяПозиция" Тогда
						НоваяСтрока.Установить(Индекс, СтрокаКомплектующая.НоменклатурнаяПозиция);
					Иначе 
...
Процедура ВыборПартии(СтруктураПараметров,Элемент,ТипыДокументов) Экспорт
	...
	Если СтруктураПараметров.Свойство("Номенклатура") Тогда 
		ФормаВыбора.Номенклатура       = СтруктураПараметров.Номенклатура;
	КонецЕсли;
	Если СтруктураПараметров.Свойство("Склад") Тогда 
		ФормаВыбора.Склад = СтруктураПараметров.Склад;
	КонецЕсли;
