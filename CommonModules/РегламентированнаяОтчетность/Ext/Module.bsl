...
// Возвращает представление видаПолучитьСведенияОбОрганизации документа для отображения в журнале документов.
//
// Параметры:
...
Функция ПолучитьСведенияОбОрганизации(Организация, ДатаЗначения, СписокПоказателей) Экспорт
	...
	Для каждого ЭлементСписка Из СписокПоказателей Цикл
		...
		Если ИмяПоказателя = "ФИОРук" или ИмяПоказателя = "ИмяФАМИЛИЯРук" Тогда
		...
		ИначеЕсли ИмяПоказателя = "ФИОБух" или ИмяПоказателя = "ИмяФАМИЛИЯБух" Тогда
		...
		ИначеЕсли ИмяПоказателя = "КПР" Тогда
			...
			Если Результат <> Неопределено Тогда
				Значение = Результат;
			КонецЕсли
		ИначеЕсли ИмяПоказателя = "ОрганСоциальнойЗащитыНаселения" Тогда
			Если ЗначениеЗаполнено(Организация) Тогда 
				Данные = Новый Структура("Организация",Организация); 
				КодыОрганизации = РегистрыСведений.КодыОрганизации.СрезПоследних(ДатаЗначения, Данные);
				Если КодыОрганизации.Количество()> 0 Тогда
					Результат = КодыОрганизации[0].ОрганСоциальнойЗащитыНаселения;
				КонецЕсли;
			КонецЕсли;	
			Если Результат <> Неопределено Тогда
				Значение = Результат;
			КонецЕсли
		ИначеЕсли ИмяПоказателя = "НомерФССУ" Тогда
			Если ЗначениеЗаполнено(Организация) Тогда 
				Данные = Новый Структура("Организация",Организация); 
				КодыОрганизации = РегистрыСведений.КодыОрганизации.СрезПоследних(ДатаЗначения, Данные);
				Если КодыОрганизации.Количество()> 0 Тогда
					Результат = КодыОрганизации[0].НомерФССУ;
				КонецЕсли;
			КонецЕсли;	
			Если Результат <> Неопределено Тогда
				Значение = Результат;
	...
	Если Организация <> Неопределено тогда
		...
		Иначе
			...
			Если Исполнитель <> Неопределено тогда
				...
				ЗапросПоИсполнителям.УстановитьПараметр("Организация",         Организация);
				ЗапросПоИсполнителям.УстановитьПараметр("ДатаСреза",           ДатаСреза);
				ЗапросПоИсполнителям.УстановитьПараметр("Исполнитель",         Исполнитель.Физлицо);
				ЗапросПоИсполнителям.УстановитьПараметр("ВнутреннееСовместительство", Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство);
				ЗапросПоИсполнителям.Текст = 
				"ВЫБРАТЬ
				|	РаботникиОрганизацииСрезПоследних.Должность.Наименование КАК Должность,
				...
				|ИЗ
				|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаСреза, Организация = &Организация И Сотрудник.ФизЛицо = &Исполнитель И (Сотрудник.ВидЗанятости <> &ВнутреннееСовместительство ИЛИ (Сотрудник.ВидЗанятости) ЕСТЬ NULL )) КАК РаботникиОрганизацииСрезПоследних
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&ДатаСреза, ФизЛицо = &Исполнитель) КАК ФИОФизЛицСрезПоследних
...
Функция ПроверитьВозможностьОткрытияДочернейФормыРегламентиованногоОтчета(ЭтаФорма) Экспорт	
	...
	ДатаСтрока        = Формат(ЭтаФорма.Организация.ДатаНачалаИспользованияЗвит1С, "ДФ=dd.MM.yyyy");
	ОрганизацияСтрока = Строка(ЭтаФорма.Организация);
	
	ТекстВопорса = НСтр("ru = 'Внимание! Начиная с "+ДатаСтрока+" для формирования регламентированных отчетов по организации """+ОрганизацияСтрока+""" используется ""FREDO Звіт"". 
	                    |
	...
	Ответ = Вопрос(ТекстВопорса, РежимДиалогаВопрос.ДаНетОтмена);
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если МенеджерЗвит1С.ЗапуситьЗвит1С(Истина) Тогда
...
Процедура ВывестиНаПечатьЛистОбщая(Форма, СтруктураПараметровЛиста) Экспорт
	...
	Для Каждого Стр Из КартаСтраницРаздела Цикл
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Фомирование печатной формы %1. Лист №%2';uk='Формування друкованої форми %1. Аркуш №%2'"), ИмяЛиста, (КоличествоЛистовПоИмениЛиста(Форма.мПечатныеФормы, ИмяЛиста) + 1)));
...
Функция ПроверкаРегистрацииПриОткрытииОсновнойФормыРегламентированногоОтчета(Объект, ОткрытьОсновнуюФорму=Истина) Экспорт
	...
	НайденныйЭлемент = ПолучитьРеглОтчетПоУмолчанию(Объект.Метаданные().Имя);
	...
	Если НайденныйЭлемент.ВнешнийОтчетХранилище.Получить() <> Неопределено Тогда
		...
		Попытка
			...
			Если Сравнение.Сравнить() Тогда
                ТекстВопроса = НСтр("ru='Данный внешний регламентированный отчет уже зарегистрирован в информационной базе.';uk='Даний зовнішній регламентований звіт вже зареєстрований в інформаційній базі.'");
				Если НайденныйЭлемент.ВнешнийОтчетИспользовать Тогда
					Предупреждение(ТекстВопроса);
					Если ОткрытьОсновнуюФорму Тогда
						ТекОтчет = РеглОтчеты(Объект.Метаданные().Имя); // Помещение отчета во внутренний кэш, без него ФормаРеглОтчета(...) не рабтает
				...
				// Нужно проставить флаг ВнешнийОтчетИспользовать
				ТекстВопроса = ТекстВопроса + НСтр("ru=' Снят признак использования отчета из файла."
""
"Загрузить повторно выбранный отчет в информационную базу?';uk=' Знята ознака використання звіту з файлу."
""
"Завантажити повторно обраний звіт в інформаційну базу?'");
			Иначе
...
Функция ПродолжитьПриНесоответствииВерсийФорм(Знач ВерсияФормы, Знач мВерсияФормы) Экспорт
	...
	Если ДатамВерсияФормы < ДатаВерсияФормы Тогда
		Предупреждение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сохраненные данные не могут быть открыты текущей версией отчета!"
"Версия отчета - от %1 "
...
Функция ОбъектОтчетСУчетомКэша(РеглОтч)
	...
	Иначе
		Попытка
			...
			Кэш.Вставить(РеглОтч.ИсточникОтчета, Новый Структура("Отчет, Признак, ВерсияДанных", ОтчетВКэш, РеглОтч.ВнешнийОтчетИспользовать, РеглОтч.ВерсияДанных));
...
Функция РеглОтчеты(Наим, СоздаватьИНеЗапоминать = Ложь) Экспорт
	...
	Если МассивДублей.Количество() > 0 Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В справочнике ""Регламентированные отчеты"" обрнаружены дубли для объекта ""%1""';uk='У довіднику ""Регламентовані звіти"" виявлені дублі для об''єкта ""%1""'"), СокрП(Наим)), СтатусСообщения.Информация);
		Для каждого СсылкаДубль Из МассивДублей Цикл
			Попытка
				...
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Элемент ""%1"" (%2) помечен на удаление';uk='Елемент ""%1"" (%2) відмічений для вилучення'"), Строка(СсылкаДубль), Строка(СсылкаДубль.Код)), СтатусСообщения.БезСтатуса);
			Исключение
...
// Определяет актуален ли отчет в кеше. Кеш автоматически обновляется при вызове функций РеглОтчеты(...) или ФормаРеглОтчета(...)
// Параметры:
//		РеглОтч - строка-идентификатор регламентированного отчета.
// Возвращаемое значение:
// 		Булево - Истина - информация не кэшировалась, либо кеш актуален
// 				 Ложь - в противном случае.
Функция ВерсияРеглОтчетаАктуальна(Наим) Экспорт
	Перем ОтчетВКэше;
	...
	Попытка
		ЕстьОтчетВКэше = ПараметрыСеанса.ХранилищеПараметровВнешнихРегламентированныхОтчетов.Получить().Свойство(Наим, ОтчетВКэше);
		Если ЕстьОтчетВКэше Тогда
			Возврат ОтчетВКэше.ВерсияДанных = НайденныйЭлемент.ВерсияДанных;
		Иначе
			Возврат Истина; // Отчет не кэшировался
		КонецЕсли;
	Исключение
		Возврат Истина; // Структура кэша еще не сформирована
	КонецПопытки;	
...
Функция ПечататьРаздел(Форма, СтрокаРаздела) Экспорт
	...
	//	Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В режиме печати с двухмерным штрих-кодом не выбран обязательный для печати раздел ""%1"". Печатная форма раздела будет сформирована принудительно.';uk='У режимі друку із двомірним штрих-кодом не обраний обов''язковий для друку розділ ""%1"". Друкована форма розділу буде сформована примусово.'"), СтрокаРаздела.Представление), СтатусСообщения.Информация);
	//	Форма.мПечатаемыеРазделы.Добавить(СтрокаРаздела.ИмяСтраницы);
	//	Возврат 1;
	//КонецЕсли;
...
//
Функция ЗапуситьЗвит1С() Экспорт
	...
	Возврат глМенеджерЗвит1С.ЗапуситьЗвит1С(Истина);
...
Функция ОткрытьФормуНастроекЗвит1С() Экспорт
	...
	Возврат глМенеджерЗвит1С.ОткрытьФормуНастроекЗвит1С();
