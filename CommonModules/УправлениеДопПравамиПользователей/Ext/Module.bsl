...
//
// Параметры:
//  РезервированиеТоваров - булево. Признак того, что данное ограничение должно использоваться в системе 
//							резервирования товаров.
// Возвращаемое значение:
//  Истина - если разрешено превышать остаток, иначе Ложь.
//
Функция РазрешеноПревышениеОстаткаТоваровОрганизации(РезервированиеТоваров = Истина) Экспорт

	Если РезервированиеТоваров Тогда
		// на данном этапе нет смысла контролировать остатки по БУ,
		Возврат Истина;
	Иначе	
		// используем бухгалтерскую настройку так как этот регистр является бухгалтерским
...
Процедура ПроверитьДопустимостьЦенОтпуска(ДокументОбъект, ИмяТабличнойЧасти, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если МинимальнаяЦена > ЦенаТовара Тогда
			ТекстСообщения = НСтр("ru='Строка ';uk='Рядок '")      + Выборка.НомерСтроки + ": "
					+ НСтр("ru='Для товара ';uk='Для товару '") + Выборка.Номенклатура
					+ Символы.ПС + НСтр("ru='    цена за единицу ниже порогового значения (цена: ';uk='    ціна за одиницю нижче порогового значення (ціна: '")
					+ Формат(ЦенаТовара, "ЧДЦ=2; ЧН=0") + " " + ВалютаТовара
					+ НСтр("ru=', минимальное значение: ';uk=', мінімальне значення: '")
