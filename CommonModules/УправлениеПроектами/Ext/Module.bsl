...
Процедура РаспределитьПродажиПоПроектам(ТабДвижений, Строка, ВидРаспределения, Знач ДатаКонМесяца) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить(НСтр("ru='Не удалось выполнить распределение продаж по виду распределения ""';uk='Не вдалося виконати розподіл продажів по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если МассивСтоимость = Неопределено Тогда
		...
		Сообщить(НСтр("ru='Неверные коэффициенты (сумма) для распределения продаж по виду распределения ""';uk='Невірні коефіцієнти (сума) для розподілу продажів по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
...
Процедура РаспределитьЗакупкиПоПроектам(ТабДвижений, Строка, ВидРаспределения, Знач ДатаКонМесяца) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить(НСтр("ru='Не удалось выполнить распределение закупок по виду распределения ""';uk='Не вдалося виконати розподіл купівель по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если МассивСтоимость = Неопределено Тогда
		Сообщить(НСтр("ru='Неверные коэффициенты (сумма) для распределения закупок по виду распределения ""';uk='Невірні коефіцієнти (сума) для розподілу купівель по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
...
Процедура РаспределитьДвиженияДСПоПроектам(ТабДвижений, Строка, ВидРаспределения, Знач ДатаКонМесяца) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить(НСтр("ru='Не удалось выполнить распределение движений денежных средств по виду распределения ""';uk='Не вдалося виконати розподіл рухів грошових коштів по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	...
	Если МассивСумма = Неопределено Тогда
		Сообщить(НСтр("ru='Неверные коэффициенты (сумма) для распределения движения денежных средств по виду распределения ""';uk='Невірні коефіцієнти (сума) для розподілу руху грошових коштів по виду розподілу ""'") + ВидРаспределения + """", СтатусСообщения.Внимание);
...
Функция ВедениеУчетаПоПроектам(Дата = Неопределено, Сообщать = Ложь) Экспорт
	...
	Если НЕ ВедениеУчетаПоПроектам И Сообщать Тогда
		Сообщить(НСтр("ru='Ведение учета по проектам в данной конфигурации отключено.';uk='Ведення обліку за проектами в даній конфігурації відключено.'"), СтатусСообщения.ОченьВажное);
	КонецЕсли;
...
Функция ИспользоватьРаспределениеЗатратПоПроектам(Сообщать = Ложь) Экспорт
	...
	Если Не Флаг И Сообщать Тогда
		Сообщить(НСтр("ru='Использование механизма распределения по проектам в данной конфигурации отключено.';uk='Використання механізму розподілу за проектами в даній конфігурації відключено.'"), СтатусСообщения.ОченьВажное);
	КонецЕсли;
...
//Процедура устанавливает номенклатуру под проект
//
Процедура ДобавитьНоменклатуруПодПроект(тзДвижений) Экспорт
	
	Для Каждого стрДанные из тзДвижений Цикл    
		МенЗап = РегистрыСведений.УстановкаПроектовДляНоменклатуры.СоздатьМенеджерЗаписи();
...
Процедура КонтрольОстатковПоПроекту(СтрДанные, Отказ) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТЗ_Данные.Номенклатура,
		|	ТЗ_Данные.СерияНоменклатуры,
		|	&Проект КАК Проект
		|ПОМЕСТИТЬ ВТ_ДанныеДокумена
		|ИЗ
		|	&Данные КАК ТЗ_Данные
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	УстановкаПроектовДляНоменклатурыСрезПоследних.НоменклатураПроекта,
		|	УстановкаПроектовДляНоменклатурыСрезПоследних.СерияНоменклатуры,
		|	УстановкаПроектовДляНоменклатурыСрезПоследних.Проект
		|ПОМЕСТИТЬ ВТ_ТоварыПроекта
		|ИЗ
		|	РегистрСведений.УстановкаПроектовДляНоменклатуры.СрезПоследних(&ДатаДокумента, ) КАК УстановкаПроектовДляНоменклатурыСрезПоследних
		|ГДЕ
		|	(УстановкаПроектовДляНоменклатурыСрезПоследних.НоменклатураПроекта, УстановкаПроектовДляНоменклатурыСрезПоследних.СерияНоменклатуры, УстановкаПроектовДляНоменклатурыСрезПоследних.Проект) В
		|			(ВЫБРАТЬ
		|				ВТ_ДанныеДокумена.Номенклатура,
		|				ВТ_ДанныеДокумена.СерияНоменклатуры,
		|				ВТ_ДанныеДокумена.Проект
		|			ИЗ
		|				ВТ_ДанныеДокумена КАК ВТ_ДанныеДокумена)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_ДанныеДокумена.СерияНоменклатуры,
		|	ВТ_ДанныеДокумена.Проект,
		|	1 КАК КолвоПоДокументу,
		|	0 КАК КолвоПоРегистру
		|ПОМЕСТИТЬ ВТ_Итог
		|ИЗ
		|	ВТ_ДанныеДокумена КАК ВТ_ДанныеДокумена
		|
		...
		|
		|ВЫБРАТЬ
		|	ВТ_ТоварыПроекта.НоменклатураПроекта,
		|	ВТ_ТоварыПроекта.СерияНоменклатуры,
		|	ВТ_ТоварыПроекта.Проект,
		|	0,
		|	1
		|ИЗ
		|	ВТ_ТоварыПроекта КАК ВТ_ТоварыПроекта
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_Итог.Проект,
		|	СУММА(ВТ_Итог.КолвоПоДокументу) КАК КолвоПоДокументу,
		|	СУММА(ВТ_Итог.КолвоПоРегистру) КАК КолвоПоРегистру
		|ИЗ
		|	ВТ_Итог КАК ВТ_Итог
		|
		|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ДатаДокумента", СтрДанные.ДатаДокумента);
	Запрос.УстановитьПараметр("Данные", СтрДанные.ТзДанные);
	Запрос.УстановитьПараметр("Проект", СтрДанные.ТзДанные[0].Проект);
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КолвоПоДокументу > Выборка.КолвоПоРегистру Тогда
			Сообщить("Бракує товару на проекті " + Выборка.Номенклатура + " " + Выборка.СерияНоменклатуры);
			Отказ = Истина;	
			Возврат;
		КонецЕсли;
	КонецЦикла;
