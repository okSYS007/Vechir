...
Процедура ЗаписатьПраваДоступаПользователейКОбъекту(ПраваДоступаПользователей, Ссылка, Отказ, ПрошлыйИзмененныйРодительОбъектаДоступа = Неопределено, Интерактивно = Ложь) Экспорт
	...
	ПолныеПрава.ЗаписатьПраваДоступаПользователей(ТаблицаПравДоступа,СтруктураОтбора, Отказ, НСтр("ru='Не записаны права доступа к объекту ""';uk='Не записані права доступу до об''єкта ""'")+ Ссылка + """!", Интерактивно, ОткликИнтерактивнойЗаписи);
	
	# Если Клиент Тогда
	Если НЕ ПустаяСтрока(ОткликИнтерактивнойЗаписи) Тогда
		Сообщить(ОткликИнтерактивнойЗаписи);
		Предупреждение(НСтр("ru='Обнаружено несоответствие настроек!';uk='Виявлена невідповідність настройок!'"));
	КонецЕсли; 
	# КонецЕсли
...
Процедура ПередЗаписьюДокументаДатаЗапретаРедактированияПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если    ТипЗнч(Источник) = Тип("ДокументОбъект.ВыгрузкаРегламентированныхОтчетов")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.РегламентированныйОтчет") 
		//Дандали В. 05.06.2024 +
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеСредств")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.РеестрПлатежей")
		...
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.УстановкаЗначенийЛимитов")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ФормаВводаБюджета")
		ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.Сервис")
		//Дандали В. 27.08.2024 -
		//Дандали В. 05.06.2024 -
		Тогда
...
Процедура ПередЗаписьюРегистраДатаЗапретаРедактированияПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если Отказ ИЛИ Источник.ОбменДанными.Загрузка 
		...
		ИЛИ Источник.ДополнительныеСвойства.Свойство("БезКонтроляДатыЗапрета")
		//Дандали В. 14.08.2025 -
		Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьВерсиюДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, СоответствиеГраницЗапрета, Отказ, РежимЗаписи = Неопределено)
	...
	// t.me/konstantyn_buhantsev [11 лютого 2025 р.] --> 
	Если ПараметрыПроверкиДокумента.МетаданныеДокумента.Имя = "НевыходыИОдноразовыеВыплатыВОрганизациях" Тогда
		Возврат;
	КонецЕсли; 
	// <-- t.me/konstantyn_buhantsev [11 лютого 2025 р.] 
