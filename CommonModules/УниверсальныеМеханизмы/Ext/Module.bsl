...
Процедура ОбработатьНажатиеНаДополнительнуюКнопкуЗаполненияТЧ(СтрокаКнопки,ЭтотОбъект) Экспорт
	...
	Если ТипЗНЧ(Расшифровка) = Тип("СправочникСсылка.ВнешниеОбработки") Тогда
		...
		Если ОбъектВнешнейФормы = Неопределено Тогда
			Сообщить(НСтр("ru='Ошибка получения внешней обработки заполнения табличной части документа. Возможно обработка была удалена';uk='Помилка одержання зовнішньої обробки заповнення табличної частини документа. Можливо обробку було вилучено'"), СтатусСообщения.Важное);
			Возврат;
		КонецЕсли;
		...
		Исключение
			Сообщить(НСтр("ru='Ошибка исполнения внешней обработки табличной части документа.';uk='Помилка виконання зовнішньої обробки табличної частини документа.'")+Символы.ПС+ОписаниеОшибки(), СтатусСообщения.Важное);
			Возврат;
		КонецПопытки;
		...
		Исключение
			ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Заполнение ТЧ не произведено!';uk='Заповнення ТЧ не виконано!'"));
		КонецПопытки;
	КонецЕсли;
...
Функция ПолучитьДеревоМакетовПечати(СсылкаОбъекта, ВнутренниеПечатныеФормы, Действие = Неопределено, ДействиеУстановитьПечатьПоУмолчанию = Неопределено, ДобавлятьВДеревоРазделительИПечатьПоУмолчанию = Истина, ПредставлениеПечатнойФормы = Неопределено) Экспорт
	...
	Если ДобавлятьВДеревоРазделительИПечатьПоУмолчанию И ДеревоКнопок.Строки.Количество() > 0 Тогда
		...
		ДобавитьСтрокуВДеревоКнопок(ДеревоКнопок, "ПечатьПоУмолчанию", ТипКнопкиКоманднойПанели.Действие, НСтр("ru='По умолчанию...';uk='По умовчанню...'"), ДействиеУстановитьПечатьПоУмолчанию, "ПечатьПоУмолчанию");
...
Процедура УстановитьПечатьПоУмолчанию(ПечатьПоУмолчанию, КоманднаяПанель, ДеревоМакетов, ИмяМетаданных, ДействиеПечатьПоУмолчанию) Экспорт
	...
	Если Не СтрокаПоУмолчанию = Неопределено Тогда
		Если ПечатьПоУмолчанию = Неопределено Тогда
			...
			ПечатьПоУмолчанию.Подсказка = НСтр("ru='Печать по умолчанию';uk='Друк по умовчанню'");
	...
	ИначеЕсли Не ПечатьПоУмолчанию = Неопределено Тогда
		...
		ПечатьПоУмолчанию.Текст       = НСтр("ru='Печать по умолчанию не назначена';uk='Друк по умовчанню не призначений'"); 
...
Процедура УстановитьПодменюПечати(ПодменюПечати, КоманднаяПанель, СоздаватьПриНеобходимости) Экспорт
	...
	Если КоманднаяПанель.ВыравниваниеКнопок = ВыравниваниеКнопокКоманднойПанели.Право Тогда
		КоманднаяПанель.Кнопки.Вставить(0,,ТипКнопкиКоманднойПанели.Разделитель);
		ПодменюПечати = КоманднаяПанель.Кнопки.Вставить(0,ИмяПодменю,ТипКнопкиКоманднойПанели.Подменю,НСтр("ru='Печать';uk='Друк'"));
	Иначе
		КоманднаяПанель.Кнопки.Добавить(,ТипКнопкиКоманднойПанели.Разделитель);
		ПодменюПечати = КоманднаяПанель.Кнопки.Добавить(ИмяПодменю,ТипКнопкиКоманднойПанели.Подменю,НСтр("ru='Печать';uk='Друк'"));
	КонецЕсли;
	ПодменюПечати.Подсказка = НСтр("ru='Меню выбора печатных форм';uk='Меню вибору друкованих форм'");
		...
		Пока ВыборкаСсылок.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЭтоЗаменяемаяФорма Тогда
					...
					Подсказка = НСтр("ru='Печатная форма заменена подключаемой';uk='Друкована форма замінена що підключається'") + Подсказка;
				...
				Иначе
					...
					КонецЕсли;
					Подсказка = НСтр("ru='Подключаемая печатная форма';uk='Друкована форма, що підключається'") + Подсказка;
	...
	Если ТипЗНЧ(ВнутренниеПечатныеФормы) = Тип("ТаблицаЗначений") Тогда
		...
		Для каждого Строка Из ВнутренниеПечатныеФормы Цикл
			...
			ИначеЕсли ПредставлениеПечатнойФормы = Неопределено или ПредставлениеПечатнойФормы = Строка.Заголовок Тогда
				...
				ДобавитьСтрокуВДеревоКнопок(ДеревоКнопок, Строка.Имя, ТипКнопкиКоманднойПанели.Действие, Строка.Заголовок, Действие, СтруктураРасшифровки,,НСтр("ru='Встроенная печатная форма';uk='Вбудована друкована форма'"));
	...
	ИначеЕсли ТипЗНЧ(ВнутренниеПечатныеФормы) = Тип("Структура") Тогда
		...
		Для Каждого КлючИЗначение Из ВнутренниеПечатныеФормы Цикл
			...
			ИначеЕсли ПредставлениеПечатнойФормы = Неопределено или ПредставлениеПечатнойФормы = КлючИЗначение.Значение Тогда
				...
				ДобавитьСтрокуВДеревоКнопок(ДеревоКнопок, КлючИЗначение.Ключ, ТипКнопкиКоманднойПанели.Действие, КлючИЗначение.Значение, Действие, КлючИЗначение.Ключ,,НСтр("ru='Встроенная печатная форма';uk='Вбудована друкована форма'"));
...
Функция НапечататьДокументПоУмолчанию(Объект, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ЭтоДокумент И Объект.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить и Не УправлениеДопПравамиПользователей.РазрешитьПечатьНепроведенныхДокументов(Объект.Проведен) Тогда
		Сообщить(НСтр("ru='Недостаточно полномочий для печати непроведенного документа!';uk='Недостатньо повноважень для друку непроведенного документа!'")); 
		Возврат Ложь;
	КонецЕсли;	
	...
	Если СтрокаПоУмолчанию = Неопределено Тогда
		...
		Сообщить(Строка(Объект) + НСтр("ru=': Печать по умолчанию не определена!';uk=': Друк по умовчанню не визначений!'"), СтатусСообщения.Важное); 
		Возврат Ложь;
...
Процедура НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, ИсточникМакета = Неопределено) Экспорт
		...
		// OKP
	Если Вчасно.ОтправитьДокументыВчасно(ПечДокумент, Заголовок) Тогда
		Возврат;
	КонецЕсли;
	// OKP ..
...
Функция ВыполнитьИнтерактивныйПоискОбъектовВСписке(ТипОбъектовПоиска, СписокПоиска, ТабличноеПолеПоиска,
	...
	Если ТабличноеПолеПоиска.ИерархическийПросмотр Тогда
		ОтветНаВопрос = Вопрос(НСтр("ru='Для удобства поиска рекомендуется снять режим иерархического просмотра. Снять?';uk='Для зручності пошуку рекомендується зняти режим ієрархічного перегляду. Зняти?'"), РежимДиалогаВопрос.ДаНет);
...
Функция ПрочитатьЭлементXML(ОбъектДоступаXML, ИмяЭлемента = Неопределено) Экспорт
	...
	Если ИмяЭлемента <> Неопределено Тогда // если определили имя элемента, нужно чтобы совпадало с полученным
		Если ОбъектДоступаXML.Имя <> ИмяЭлемента Тогда
			Сообщить(НСтр("ru='Неправильное имя узла (';uk=""Неправильне ім'я вузла (""") + ИмяЭлемента + ")!"); 
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
