...
Функция СформироватьЗаголовокОсновнойФормы(ДатаНач, ДатаКон, НазваниеОтчета, РежимВводаПериода) Экспорт
	...
	ИначеЕсли РежимВводаПериода =  1 Тогда   // на дату
		Если ДатаКон = '00010101000000' Тогда
			ОписаниеПериода = НСтр("ru='на текущее время ';uk='на поточний час '") + Формат(ТекущаяДата(), "ДФ = ""дд.ММ.гггг""; ДП = ""...""");

		Иначе
			ОписаниеПериода = НСтр("ru='на конец дня ';uk='на кінець дня '") + Формат(ДатаКон, "ДФ = ""дд.ММ.гггг""; ДП = ""...""");
...
Функция ПодготовитьДанныеДляСозданияПанелиВидыУчета(ДействиеПриИзменении, Знач ВидыУчета = "Упр,Бух,Нал,МСФО") Экспорт
	...
	ГруппаЭлементов = Новый Структура("Имя, Заголовок, Ширина, СоставГруппы",
		...
		НСтр("ru='Формировать по данным:';uk='Формувати за даними:'"), // Заголовок
		136,                      // Ширина
	...
	Если Найти(ВидыУчета, ",Упр,") > 0 Тогда
		ЭлементГруппы = Новый Структура("Имя, Заголовок, Ширина, Данные, ВыбираемоеЗначение, ПервыйВГруппе, Видимость, Доступность, ДействиеПриИзменении",
			"ВидУчетаУпр",            // Имя
			НСтр("ru='Управленческого учета';uk='Управлінського обліку'"),  // Заголовок
			139,                      // Ширина
			"ВидУчета",               // Данные
	...
	Если Найти(ВидыУчета, ",Бух,") > 0 Тогда
		ЭлементГруппы  = Новый Структура("Имя, Заголовок, Ширина, Данные, ВыбираемоеЗначение, ПервыйВГруппе, Видимость, Доступность, ДействиеПриИзменении",
			"ВидУчетаБух",            // Имя
			НСтр("ru='Бухгалтерского учета';uk='Бухгалтерського обліку'"),   // Заголовок
			129,                      // Ширина
			"ВидУчета",               // Данные
	...
	Если Найти(ВидыУчета, ",Нал,") > 0 Тогда
		ЭлементГруппы  = Новый Структура("Имя, Заголовок, Ширина, Данные, ВыбираемоеЗначение, ПервыйВГруппе, Видимость, Доступность, ДействиеПриИзменении",
			"ВидУчетаНал",            // Имя
			НСтр("ru='Налогового учета';uk='Податкового обліку'"),       // Заголовок
			109,                      // Ширина
			"ВидУчета",               // Данные
	...
	Если Найти(ВидыУчета, ",МСФО,") > 0 Тогда
		ЭлементГруппы = Новый Структура("Имя, Заголовок, Ширина, Данные, ВыбираемоеЗначение, ПервыйВГруппе, Видимость, Доступность, ДействиеПриИзменении",
			"ВидУчетаМСФО",           // Имя
			НСтр("ru='Международного учета';uk='Міжнародного обліку'"),   // Заголовок
			137,                      // Ширина
			"ВидУчета",               // Данные
...
Процедура ДобавитьВТекстСвойстваИКатегории(ТаблицаПолей, Текст, СтруктураПредставлениеПолей, мСоответствиеНазначений, 
	...
	Для каждого Строка Из ТаблицаПолей Цикл
		...
		Если НЕ (Строка.НетКатегорий = Истина) Тогда
			...
			СтруктураПредставлениеПолей.Вставить("Категории" + Индекс + "Категория", НСтр("ru='Категории ';uk='Категорії '") + Строка.Представление);
...
Функция ПолеСИнтервалами(ТаблицаИнтервалов, СтруктураПараметры, СоответствиеТиповИнтервальныхПолей, ПолеИнтервала, ИсходноеПоле, ТипИсходногоПоля, СтрокаФорматирования="") Экспорт 
	...
	СтруктураПараметры.Вставить("Интервал"+ПолеИнтервала+"Прочие", Нстр("ru='Остальные';uk='Інші'"));
		...
		Для Каждого ЭлементОтбора Из ПостроительОтчета.Отбор Цикл
			...
			Если ЭлементОтбора.Использование Тогда
				...
				Если СтруктураДляОтбораПоКатегориям.Свойство(ЭлементОтбора.ПутьКДанным, ИдентификаторПараметра) Тогда
					...
					Иначе
						Предупреждение(НСтр("ru='Нельзя использовать несколько отборов по одной и той же категории!';uk='Не можна використовувати кілька відборів по одній і тій же категорії!'"));
						Возврат Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьНачальныеНастройкиПоМетаданнымРегистра(СтруктураПредставлениеПолей, МассивОтбора, Контекст, ТипОтчета) Экспорт
	...
	Для Инд = 0 По МетаданныеРегистра.Ресурсы.Количество()-1 Цикл
		...
		ИначеЕсли ТипОтчета = "ОстаткиИОбороты" Тогда
			...
			СтруктураПредставлениеПолей.Вставить(ИмяПоля+"НачальныйОстаток", ПредставлениеПоля + НСтр("ru=' нач.ост.';uk=' нач.зал.'"));
			СтруктураПредставлениеПолей.Вставить(ИмяПоля+"КонечныйОстаток", ПредставлениеПоля + НСтр("ru=' кон.ост.';uk=' кін.зал.'") );
			СтруктураПредставлениеПолей.Вставить(ИмяПоля+"Приход", ПредставлениеПоля+НСтр("ru=' приход';uk=' надходження'"));
			СтруктураПредставлениеПолей.Вставить(ИмяПоля+"Расход", ПредставлениеПоля+НСтр("ru=' расход';uk=' видаток'"));
		КонецЕсли;
...
//	ТаблицаПолей.Колонки.Добавить("ТипЗначения");  // тип значения поля, для которого добавляются свойства и категории.
	...
	Если (ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Обороты И ((ТипОтчета = "СписокКроссТаблица") ИЛИ (ТипОтчета = "Диаграмма"))) или (ТипОтчета = "ОстаткиИОбороты") Тогда
		...
		СтруктураПредставлениеПолей.Вставить("Период", НСтр("ru='Период';uk='Період'"));
		СтруктураПредставлениеПолей.Вставить("ПериодДень", НСтр("ru='По дням';uk='По днях'"));
		СтруктураПредставлениеПолей.Вставить("ПериодНеделя", НСтр("ru='По неделям';uk='По тижнях'"));
		СтруктураПредставлениеПолей.Вставить("ПериодДекада", НСтр("ru='По декадам';uk='По декадах'"));
		СтруктураПредставлениеПолей.Вставить("ПериодМесяц", НСтр("ru='По месяцам';uk='По місяцях'"));
		СтруктураПредставлениеПолей.Вставить("ПериодКвартал", НСтр("ru='По кварталам';uk='По кварталах'"));
		СтруктураПредставлениеПолей.Вставить("ПериодПолугодие", НСтр("ru='По полугодиям';uk='По півріччях'"));
		СтруктураПредставлениеПолей.Вставить("ПериодГод", НСтр("ru='По годам';uk='По роках'"));
		...
		СтруктураПредставлениеПолей.Вставить("Регистратор", НСтр("ru='Документ движения (Регистратор)';uk='Документ руху (Реєстратор)'"));
	КонецЕсли;
	...
	Если (ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Обороты И ((ТипОтчета = "СписокКроссТаблица") ИЛИ (ТипОтчета = "Диаграмма"))) Тогда
		...
		СтруктураПредставлениеПолей.Вставить("Периодичность", НСтр("ru='Периодичность разворота итогов';uk='Періодичність розвороту підсумків'"));
...
Функция СформироватьСтрокуОтборов(Отбор) Экспорт
	...
	ЯзыкФормирования = ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов();
	...
	Для а = 0 По Отбор.Количество() - 1 Цикл
		...
		Если ЭлементОтбора.ВидСравнения = ВидСравнения.Равно Тогда
			СтрокаОтборов = СтрокаОтборов + ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) + " = "+ ?(ЗначениеЗаполнено(ЭлементОтбора.Значение), СокрЛП(Строка(ЭлементОтбора.Значение)), НСтр("ru='Пусто';uk='Пусто'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВСписке Тогда
			...
			КонецЦикла; 
			СтрокаОтборов = СтрокаОтборов 
							+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) 
							+ НСтр("ru=' в списке ';uk=' у списку '", ЯзыкФормирования) 
							+ ?(ЗначениеЗаполнено(ЭлементОтбора.Значение), СтрокаСписка, НСтр("ru='Пусто';uk='Пусто'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВСпискеПоИерархии Тогда
			...
			СтрокаОтборов = СтрокаОтборов 
							+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) 
							+ НСтр("ru=' в группе из списка ';uk=' у групі зі списку '", ЯзыкФормирования) 
							+ ?(ЗначениеЗаполнено(ЭлементОтбора.Значение), СтрокаСписка, НСтр("ru='Пусто';uk='Пусто'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВСписке Тогда
			...
			СтрокаОтборов = СтрокаОтборов 
							+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) 
							+ НСтр("ru=' не в списке ';uk=' не в списку '", ЯзыкФормирования)
							+ ?(ЗначениеЗаполнено(ЭлементОтбора.Значение), СтрокаСписка, НСтр("ru='Пусто';uk='Пусто'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВСпискеПоИерархии Тогда
			...
			СтрокаОтборов = СтрокаОтборов 
							+ ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) 
							+ НСтр("ru=' не в группе из списка ';uk=' не в групі зі списку '", ЯзыкФормирования)
							+ ?(ЗначениеЗаполнено(ЭлементОтбора.Значение), СтрокаСписка, НСтр("ru='Пусто';uk='Пусто'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.Интервал Тогда
			СтрокаОтборов = СтрокаОтборов + ?(ЗначениеЗаполнено(ЭлементОтбора.ЗначениеС), СокрЛП(Строка(ЭлементОтбора.ЗначениеС)), "...") + " < " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ЭлементОтбора.Представление) + " < "+ ?(ЗначениеЗаполнено(ЭлементОтбора.ЗначениеПо), СокрЛП(Строка(ЭлементОтбора.ЗначениеПо)), "...");
...
Функция СформироватьСтрокуИзмерений(КоллекцияИзмерений) Экспорт
	...
	Для а = 0 По КоллекцияИзмерений.Количество() - 1 Цикл
		...
		КонецЕсли;
		СтрокаИзмерений = СтрокаИзмерений + ЛокализацияПовтИсп.ПолучитьПереводТекста(КоллекцияИзмерений[а].Представление);
	КонецЦикла;
...
Функция ПроверитьПовторыИзмеренийПостроителя(ПостроительОтчета, ТолькоПовторы = Ложь) Экспорт
	...
	Для каждого ИзмерениеКолонки из ПостроительОтчета.ИзмеренияКолонки Цикл
		...
		Для каждого ИзмерениеСтроки из ПостроительОтчета.ИзмеренияСтроки Цикл
			...
			Если ИзмерениеКолонки.ПутьКДанным = ИзмерениеСтроки.ПутьКДанным Тогда
				...
				Предупреждение(НСтр("ru='Повторяющаяся группировка ';uk='Повторюване групування '") + ИзмерениеКолонки.Представление + "." + Символы.ПС +
	...
	Для каждого ИзмерениеСтроки из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		Иначе
			...
			Предупреждение(НСтр("ru='Повторяющаяся группировка ';uk='Повторюване групування '") + ИзмерениеСтроки.Представление + "." + Символы.ПС +
			НСтр("ru='Нельзя использовать одинаковые поля группировки строк!';uk='Не можна використовувати однакові поля групування рядків!'"), 30);
			Возврат Ложь;
	...
	Для каждого ИзмерениеКолонки из ПостроительОтчета.ИзмеренияКолонки Цикл
		...
		Иначе
			...
			Предупреждение(НСтр("ru='Повторяющаяся группировка ';uk='Повторюване групування '") + ИзмерениеКолонки.Представление + "." + Символы.ПС +
			НСтр("ru='Нельзя использовать одинаковые поля группировки колонок!';uk='Не можна використовувати однакові поля групування колонок!'"), 30);
			Возврат Ложь;
	...
	Для НСтр = 1 По ОбластьПоказателиВысота Цикл
		...
		Если НЕ Булево(СокрЛП(ОбластьПоказатели.Область(НСтр, 4).Текст)) Тогда
			...
			Если Врег(СтдВалюта) = "УПР" Тогда
				ПредставлениеПоля = ПредставлениеПоля + " (" +глЗначениеПеременной("ВалютаУправленческогоУчета") + ")";
...
"В настоящее время в настройках параметров учета установлено использование основоного почтового клиента операционной системы.';uk='Надсилати документи електронною поштою можна тільки з вбудованого поштового клієнта."
"В даний час в настройках параметрів обліку встановлено використання основного поштового клієнта операційної системи.'"));
...
Процедура ДобавитьВТекстПостроителяДанныеОФизлице(ТекстЗапроса, СтруктураПредставлениеПолей, СтруктураФорматаПолей, ИмяОсновнойВыборки, ИмяПоляФизЛицо = "ФизЛицо") Экспорт
	...
	|	" + ПолноеИмяПоляФизЛицо +".УНЗР 				КАК УНЗР,
	...
	// дополнительные гарантии по трудоустройству
	...
	|		ПО " + ПолноеИмяПоляФизЛицо + " = СведенияОЛицахСДополнительнымиГарантиями.Физлицо}";
	...
	//данные физлица
	СтруктураПредставлениеПолей.Вставить("ДатаРождения", НСтр("ru='Дата рождения';uk='Дата народження'"));
	СтруктураПредставлениеПолей.Вставить("КодПоДРФО", НСтр("ru='Код по ДРФО';uk='Код за ДРФО'"));
	СтруктураПредставлениеПолей.Вставить("УНЗР", НСтр("ru='УНЗР';uk='УНЗР'"));
	СтруктураПредставлениеПолей.Вставить("МестоРождения", НСтр("ru='Место рождения';uk='Місце народження'"));
	СтруктураПредставлениеПолей.Вставить("ФиоПолное", НСтр("ru='ФИО (полностью)';uk='ПІБ (повністю)'"));
	СтруктураПредставлениеПолей.Вставить("ФиоКраткое", НСтр("ru='Фамилия И. О. ';uk='Прізвище І. Б. '"));
	//состав семьи
	СтруктураПредставлениеПолей.Вставить("СС_СтепеньРодства", НСтр("ru='Состав семьи: степень родства';uk=""Склад сім'ї: ступінь спорідненості"""));
	СтруктураПредставлениеПолей.Вставить("СС_ИмяРодственника", НСтр("ru='Состав семьи: имя родственника';uk=""Склад сім'ї: ім'я родича"""));
	СтруктураПредставлениеПолей.Вставить("СС_ГодРожденияРодственника", НСтр("ru='Состав семьи: год рождения родственника';uk=""Склад сім'ї: рік народження родича"""));
	СтруктураПредставлениеПолей.Вставить("СС_ВозрастРодственника", НСтр("ru='Состав семьи: возраст родственника';uk=""Склад сім'ї: вік родича"""));
	//образование
	СтруктураПредставлениеПолей.Вставить("Образование_ВидОбразования", НСтр("ru='Образование: вид образования';uk='Освіта: вид освіти'"));
	СтруктураПредставлениеПолей.Вставить("Образование_УчебноеЗаведение", НСтр("ru='Образование: учебное заведение';uk='Освіта: навчальний заклад'"));
	СтруктураПредставлениеПолей.Вставить("Образование_Специальность", НСтр("ru='Образование:  специальность';uk='Освіта: спеціальність'"));
	СтруктураПредставлениеПолей.Вставить("Образование_Диплом", НСтр("ru='Образование: диплом';uk='Освіта: диплом'"));
	СтруктураПредставлениеПолей.Вставить("Образование_ГодОкончания", НСтр("ru='Образование: год окончания учебного заведения';uk='Освіта: рік закінчення навчального закладу'"));
	СтруктураПредставлениеПолей.Вставить("Образование_Квалификация", НСтр("ru='Образование: учебное заведение';uk='Освіта: навчальний заклад'"));
	СтруктураПредставлениеПолей.Вставить("Образование_УченаяСтепень", НСтр("ru='Образование: ученая степень';uk='Освіта: науковий ступінь'"));
	СтруктураПредставлениеПолей.Вставить("Образование_ФормаОбучения", НСтр("ru='Образование: форма обучения';uk='Освіта: форма навчання'"));
	СтруктураПредставлениеПолей.Вставить("Образование_Факультет", НСтр("ru='Образование: факультет';uk='Освіта: факультет'"));
	//трудовая деятельность
	СтруктураПредставлениеПолей.Вставить("ТД_Организация", НСтр("ru='Трудовая деятельность: организация';uk='Трудова діяльність: організація'"));
	СтруктураПредставлениеПолей.Вставить("ТД_ДатаНачала", НСтр("ru='Трудовая деятельность: начало работы';uk='Трудова діяльність: початок роботи'"));
	СтруктураПредставлениеПолей.Вставить("ТД_ДатаОкончания", НСтр("ru='Трудовая деятельность: окончание работы';uk='Трудова діяльність: закінчення роботи'"));
	СтруктураПредставлениеПолей.Вставить("ТД_Должность", НСтр("ru='Трудовая деятельность: должность';uk='Трудова діяльність: посада'"));
	// паспортные данные
	СтруктураПредставлениеПолей.Вставить("ПД_ДокументВид", НСтр("ru='Паспортные данные: вид документа';uk='Паспортні дані: вид документа'"));
	СтруктураПредставлениеПолей.Вставить("ПД_ДокументСерия", НСтр("ru='Паспортные данные: серия документа';uk='Паспортні дані: серія документа'"));
	СтруктураПредставлениеПолей.Вставить("ПД_ДокументНомер", НСтр("ru='Паспортные данные: номер документа';uk='Паспортні дані: номер документа'"));
	СтруктураПредставлениеПолей.Вставить("ПД_ДокументДатаВыдачи", НСтр("ru='Паспортные данные: дата выдачи документа';uk='Паспортні дані: дата видачі документа'"));
	СтруктураПредставлениеПолей.Вставить("ПД_ДокументКемВыдан", НСтр("ru='Паспортные данные: кем выдан документ';uk='Паспортні дані: ким виданий документ'"));
	// гражданство
	СтруктураПредставлениеПолей.Вставить("НеИмеетПравоНаПенсию", НСтр("ru='Не имеет права на пенсию';uk='Не має права на пенсію'"));
	СтруктураПредставлениеПолей.Вставить("НеЯвляетсяНалоговымРезидентом", НСтр("ru='Не является налоговым резидентом';uk='Не є податковим резидентом'"));
	// воинский учет
	СтруктураПредставлениеПолей.Вставить("ВУ_ОтношениеКВоинскойОбязанности", НСтр("ru='Воинский учет: отношение к воинской обязанности';uk=""Військовий облік: відношення до військового обов'язку"""));
	СтруктураПредставлениеПолей.Вставить("ВУ_ОтношениеКВоинскомуУчету", НСтр("ru='Воинский учет: отношение к воинскому учету';uk='Військовий облік: відношення до військового обліку'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Годность", НСтр("ru='Воинский учет: годность';uk='Військовий облік: придатність'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Звание", НСтр("ru='Воинский учет: звание';uk='Військовий облік: звання'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_ЗабронированОрганизацией", НСтр("ru='Воинский учет: забронирован организацией';uk='Військовий облік: заброньований організацією'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_НаличиеМобпредписания", НСтр("ru='Воинский учет: наличие мобпредписания';uk='Військовий облік: наявність мобприпису'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Военкомат", НСтр("ru='Воинский учет: военкомат';uk='Військовий облік: військкомат'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_ВУС", НСтр("ru='Воинский учет: ВУС';uk='Військовий облік: ВОС'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_ОснованиеНегодности", НСтр("ru='Воинский учет: основание негодности';uk='Військовий облік: підстава непридатності'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_СпецУчет", НСтр("ru='Воинский учет: спец учет';uk='Військовий облік: спец облік'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Группа", НСтр("ru='Воинский учет: группа';uk='Військовий облік: група'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Категория", НСтр("ru='Воинский учет: категория';uk='Військовий облік: категорія'"));
	СтруктураПредставлениеПолей.Вставить("ВУ_Профиль", НСтр("ru='Воинский учет: профиль офицера';uk='Військовий облік: профіль офіцера'"));
	// семейное положение
	СтруктураПредставлениеПолей.Вставить("СемейноеПоложение", НСтр("ru='Семейное положение';uk='Сімейний стан'"));
	// дополнительные гарантии по трудоустройству
	СтруктураПредставлениеПолей.Вставить("НаличиеГарантий", НСтр("ru='Наличие гарантий';uk='Наявність гарантій'"));
	...
	СтруктураФорматаПолей.Вставить("ВУ_НаличиеМобпредписания", "БЛ=Нет; БИ=Есть");
	СтруктураФорматаПолей.Вставить("НеИмеетПравоНаПенсию", "БЛ=Нет; БИ=Да");
...
// Процедура облуживет интерфейс справок-расчетов:
