...
Процедура ИзменитьНастройкиВыгрузкиДляВыделенныхСтрок(ЭлементыФормы) Экспорт
	...
	ВыбранныйЭлемент = СписокЗначений.ВыбратьЭлемент(НСтр("ru='Укажите вариант выгрузки';uk='Укажіть варіант вивантаження'"), СписокЗначений.НайтиПоЗначению(ОпределитьПоданнымСтрокиНастройкиИмяВыгрузки(ВыделенныеСтроки[0])));
...
Процедура ОбработатьИзменениеПрефиксаДляОбъектов(Элемент) Экспорт
	...
	Для НомерПозиции = 1 По СтрДлина(Элемент.Значение) Цикл
		...
		Если СимволПрефикса >= "0"
			...
			Предупреждение(НСтр("ru='Не рекомендуется в префиксах использовать цифры. Это может привести к ошибочной нумерации объектов или к ошибкам при обмене данными';uk=""Не рекомендується в префіксах використовувати цифри. Це може привести до помилкової нумерації об'єктів або до помилок при обміні даними"""));
			Возврат;
...
Функция ПолучитьПоНастройкеОбменаИнформациюОПравилах(НастройкиОбмена, ЭтоВыгрузкаИзТекущейИБ = Истина) Экспорт
	...
	Попытка
	 ...
	 Исключение
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при получении информации о правилах обмена: ';uk='Помилка при одержанні інформації про правила обміну: '") + ОписаниеОшибки());
...
Процедура ПоказатьСоответствиеУзлаПланаОбменаИПравилОбмена(УзелПланаОбмена, ОбработкаОбменаДаннымиXML, ТаблицаТиповПравилВыгрузки)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТиповПравилВыгрузки Цикл
		...
		Если СтрокаПланаОбмена = Неопределено Тогда
			...
			Если НЕ ЕстьВПравилеНоНетвПлане Тогда
				...
				Сообщить(НСтр("ru='Есть правила выгрузки, но нет регистрации изменений в плане обмена: ';uk='Є правила вивантаження, але немає реєстрації змін у плані обміну: '"));
				Сообщить("=========================================================================");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаМетаданныхПланаОбмена Цикл
		...
		Если СтрокаПравил = Неопределено Тогда
			...
			Если НЕ ЕстьВПланеНоНетВПравиле Тогда
				...
				Сообщить(НСтр("ru='Есть в плане обмена, но нет в правиле выгрузки: ';uk='Є в плані обміну, але немає в правилі вивантаження: '"));
				Сообщить("=========================================================================");
	...
	Если Не ЕстьВПравилеНоНетвПлане
		...
		Сообщить(НСтр("ru='Регистрация изменений в плане обмена соответствует правилам выгрузки данных';uk='Реєстрація змін у плані обміну відповідає правилам вивантаження даних'"));
...
Процедура ВыполинтьПоискСоотвестствияПравилВыгрузкиИДанныхПланаОбмена(НастройкиОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиОбмена.УзелИнформационнойБазы) Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан узел для обмена данными.';uk='Не зазначений вузол для обміну даними.'"));
		Возврат;
	...
	Попытка
		...
		Сообщить(Символы.ПС + НСтр("ru='Проверка основного обмена';uk='Перевірка основного обміну'"));
		...
		Если ЗначениеЗаполнено(НастройкиОбмена.УзелФоновогоОбмена) Тогда
			...
			Сообщить(Символы.ПС + НСтр("ru='Проверка фонового обмена';uk='Перевірка фонового обміну'"));
    ...
    Исключение
		...
		// в режиме клиента выводим информацию об ошибке
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при тестировании соответствия плана обмена и правил выгрузки: ';uk='Помилка при тестуванні відповідності плану обміну та правил вивантаження: '") + ОписаниеОшибки());
...
Процедура ЗарегистрироватьВсеОбъектыДляОбмена(УзелОбмена) Экспорт
	...
	Если ЭтотУзелОбмена = УзелОбмена Тогда
		...
		Предупреждение(НСтр("ru='Нельзя зарегистрировать изменения для узла обмена, соответствующему текущей информационной базе.';uk='Не можна зареєструвати зміни для вузла обміну, що відповідає поточній інформаційній базі.'"));
		Возврат;
		
	КонецЕсли;
	
	ОтветПользователя = Вопрос(НСтр("ru='ВНИМАНИЕ!!!"
...
"Вы уверены что нужно выполнить регистрацию всех объектов для обмена?';uk=""УВАГА!!!"
"Реєстрація об'єктів для обміну - операція необоротна. Після реєстрації змін перший обмін даними може забрати тривалий час."
"Ви впевнені що потрібно виконати реєстрацію всіх об'єктів для обміну?"""),
		РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.НЕТ, НСтр("ru='Регистрация объектов для обмена';uk=""Реєстрація об'єктів для обміну"""));							
	...
	Состояние(НСтр("ru='Выполняется регистрация всех объектов для обмена с информационной базой ""';uk='Виконується реєстрація всіх об''єктів для обміну з інформаційною базою ""'")  + УзелОбмена + """");
		...
		Если ЗначениеЗаполнено(ДатаПоследнейЗагрузки) Тогда 
			...
			Если ЭтоOnLineОбмен Тогда
				...
				Иначе	
					...
					Оформление.Ячейки.РезультатОтложенныхДвиженийДокументов.УстановитьТекст(НСтр("ru='С ошибками';uk='З помилками'"));
				КонецЕсли;
				...
				Если НаличиеДанныхФоновойЗагрузки = Истина Тогда
					...
					Оформление.Ячейки.НаличиеДанныхФоновойЗагрузки.УстановитьТекст(НСтр("ru='Фоновый обмен';uk='Фоновий обмін'"));
				Иначе	
			...
			Иначе
				...
				Оформление.Ячейки.РезультатОтложенныхДвиженийДокументов.УстановитьТекст(НСтр("ru='Не производилась';uk='Не виконувалась'"));
				Оформление.Ячейки.НаличиеДанныхФоновойЗагрузки.УстановитьТекст(НСтр("ru='Не производилась';uk='Не виконувалась'"));	
...
Функция ПолучитьИмяФайлаПравилДляВыбораПравилОбмена() Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выбор файла правил обмена';uk='Вибір файлу правил обміну'");
	Диалог.Фильтр = НСтр("ru='Файл правил обмена (*.xml)|*.xml';uk='Файл правил обміну (*.xml)|*.xml'");
...
Функция ПолучитьИмяФайлаПравилДляСохраненияПравилОбмена() Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выбор файла для сохранения';uk='Вибір файлу для збереження'");
	Диалог.Фильтр = НСтр("ru='Файл правил обмена (*.xml)|*.xml';uk='Файл правил обміну (*.xml)|*.xml'");
...
Функция ПолучитьИмяФайлаПротоколаОбмена() Экспорт
	...
	Диалог = РаботаСФайлами.ПолучитьДиалогВыбораФайлов(Ложь);
	Диалог.Заголовок = НСтр("ru='Выбор файла протокола обмена';uk='Вибір файлу протоколу обміну'");
	Диалог.Фильтр = НСтр("ru='Файл протокола обмена (*.txt)|*.txt';uk='Файл протоколу обміну (*.txt)|*.txt'");
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(Задание, ТекстЗаголовка, ТекстРасписания, РасписаниеАктивно) Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Дополнительные настройки расписания ...';uk='Додаткові настройки розкладу ...'");
	
	Если Задание = Неопределено Тогда
		
		//ТекстЗаголовка = "Создать регламентную настройку ...";	
		ТекстРасписания = НСтр("ru='<Расписание не задано>';uk='<Розклад не заданий>'");
		
	Иначе
		
		//ТекстЗаголовка = "Дополнительные настройки расписания ...";
		Если Задание.Использование Тогда
			ПрефиксРасписания = НСтр("ru='Расписание: ';uk='Розклад: '");
			РасписаниеАктивно = Истина;
		Иначе
			ПрефиксРасписания = НСтр("ru='Расписание (НЕ АКТИВНО): ';uk='Розклад (НЕ АКТИВНО): '");
		КонецЕсли;
...
Функция ПолучитьПоРазностиВремениПредставлениеДляПользователя(РазностьВремени) Экспорт
	...
	Если КоличествоДней <> 0 тогда
		...
		ВремяСтрокой = Строка(КоличествоДней) + НСтр("ru=' дней ';uk=' днів '") + ВремяСтрокой;
...
Функция ЗаполнитьНеобходимыеПараметрыНастроекПользователя(ЭтаФорма, ПравилаОбмена, ПравилаОбменаДляПриемника)
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при получении информации о настройках правил обмена: ';uk='Помилка при одержанні інформації про настройки правил обміну: '") + ОписаниеОшибки());
		Возврат Ложь;
...
Процедура ОбновитьТаблицуНастроекПолейПоиска(ЭтаФорма)
	...
	Попытка		 			
		...
		Для Каждого СтрокаТЧ Из ЭтаФорма.НастройкаВариантовПоискаОбъектов Цикл
			...
			Если (МассивПолейВГруппировке = Неопределено
				...
				СтрокаТЧ.ОписаниеНастройкиДляПользователя = НСтр("ru='Настройка не поддерживается правилами обмена';uk='Настройка не підтримується правилами обміну'");
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.ИмяНастройкиДляАлгоритма) Тогда
				...
				Если СтрокиВПоляхПоиска = Неопределено
					...
					СтрокаТЧ.ОписаниеНастройкиДляПользователя = НСтр("ru='Выбранный вариант сопоставления не поддерживается правилами обмена';uk='Обраний варіант зіставлення не підтримується правилами обміну'");
					БылиИзмененыСтрокиТЧ = Истина;
					Продолжить;
    ...
    Исключение
		...
		// в режиме клиента выводим информацию об ошибке
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при получении информации о настройках полей поиска: ';uk='Помилка при одержанні інформації про настройки полів пошуку: '") + ОписаниеОшибки());
...
Процедура ОбновитьТаблицуНастроекВыгрузкиДанных(ЭтаФорма) Экспорт
    ...
    Исключение
		...
		// в режиме клиента выводим информацию об ошибке
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при получении информации о настройках выгрузки: ';uk='Помилка при одержанні інформації про настройки вивантаження: '") + ОписаниеОшибки());
...
Процедура НастройкаВариантовПоискаОбъектовИмяНастройкиДляПользователяНачалоВыбора(Этаформа, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если ДанныеСтроки.НастройкаНеПоддерживается Тогда
		...
		Если ДанныеСтроки.ЭтоНастройкаДляВыгрузки Тогда
			Предупреждение(НСтр("ru='Настройка выгрузки данных для правила: ';uk='Настройка вивантаження даних для правила: '") + ДанныеСтроки.НаименованиеПравилаОбмена + НСтр("ru='  не поддерживается правилами обмена.';uk='  не підтримується правилами обміну.'"));
		Иначе
			Предупреждение(НСтр("ru='Настройка загрузки данных для правила: ';uk='Настройка завантаження даних для правила: '") + ДанныеСтроки.НаименованиеПравилаОбмена + НСтр("ru='  не поддерживается правилами обмена.';uk='  не підтримується правилами обміну.'"));
		КонецЕсли;
