...
Процедура ДобавитьСтрокуВТабличнуюЧасть(ТабличнаяЧасть, ДобавляемаяСтрока, КонечныйПлан, ИндексСтроки)
	...
	Если КонечныйПлан.Сценарий.ДетализацияПланирования = Перечисления.ДетализацияПланирования.НоменклатурныеГруппы И ТипЗнч(ДобавляемаяСтрока.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Если ДобавляемаяСтрока.Номенклатура.НоменклатурнаяГруппа.Пустая() Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для номенклатуры ""';uk='Для номенклатури ""'") + ДобавляемаяСтрока.Номенклатура.Наименование + НСтр("ru='"" не определена номенклатурная группа.';uk='"" не визначена номенклатурна група.'"));
			Возврат;
...
Функция КорректностьЗаполненияДанных() Экспорт
	...
	Если КонечныеПланы.Количество() > 0 Тогда
		...
		Для каждого КонечныйПлан из КонечныеПланы Цикл
			...
			Если КонечныйПлан.ВидПлана.Пустая() Тогда
				...
				Текст = Текст + НСтр("ru='Не указан вид плана.';uk='Не зазначений вид плану.'") + Символы.ПС;
			...
			Если КонечныйПлан.Сценарий.Пустая() Тогда
				...
				Текст = Текст + НСтр("ru='Не указан сценарий.';uk='Не зазначений сценарій.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если КонечныйПлан.Сценарий.Периодичность.Пустая() Тогда
				...
				Текст = Текст + НСтр("ru='Не указана периодичность сценария.';uk='Не зазначена періодичність сценарію.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если КонечныйПлан.ДатаНач = Дата('00010101') Тогда
				...
				Текст = Текст + НСтр("ru='Не указана дата начала периода.';uk='Не вказана дата початку періоду.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если КонечныйПлан.ДатаКон = Дата('00010101') Тогда
				...
				Текст = Текст + НСтр("ru='Не указана дата окончания периода.';uk='Не зазначена дата закінчення періоду.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если КонечныйПлан.ДатаНач <> Дата('00010101') И КонечныйПлан.ДатаКон <> Дата('00010101') И КонечныйПлан.ДатаНач > КонечныйПлан.ДатаКон Тогда
				...
				Текст = Текст + НСтр("ru='Дата начала превышает дату окончания периода.';uk='Дата початку перевищує дату закінчення періоду.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если КонечныйПлан.ПрофильРаспределенияДетализацияПлана <> Справочники.ПрофилиРаспределенияПоПериодам.ПустаяСсылка() И КонечныйПлан.Сценарий.Периодичность <> КонечныйПлан.ПрофильРаспределенияДетализацияПлана.Период Тогда
				...
				Текст = Текст + НСтр("ru='Период профиля распределения (детализация плана) должен быть равен периодичности сценария.';uk='Період профілю розподілу (деталізація плану) повинен бути рівний періодичності сценарію.'") + Символы.ПС;
				ДанныеКорректны = Ложь;
			...
			Если СтрДлина(Текст) > 0 Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(Текст,, НСтр("ru='Конечные планы: Строка ';uk='Кінцеві плани: Рядок '") + КонечныйПлан.НомерСтроки + ".");
	...
	Иначе
		...
		ДанныеКорректны = Ложь;
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнены.';uk='Не заповнені.'"),, НСтр("ru='Конечные планы.';uk='Кінцеві плани.'"));
	...
	Для каждого СтратегияРасчета из СтратегииРасчета Цикл
		...
		Если СтрДлина(Текст) > 0 Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(Текст,, НСтр("ru='Стратегии расчета количества: ';uk='Стратегії розрахунку кількості: '") + СтратегияРасчета.НаименованиеСтратегии + ".");
...
Функция КорректностьЗаполненияДокумента() Экспорт
	...
	Если мДокумент.ВалютаДокумента.Пустая() Тогда
		...
		Текст = Текст + НСтр("ru='Не выбрана валюта.';uk='Не обрана валюта.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
	...
	Если мДокумент.КурсДокумента = 0 Тогда
		...
		Текст = Текст + НСтр("ru='Не указан курс валюты.';uk='Не зазначений курс валюти.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
	...
	Если мДокумент.КратностьДокумента = 0 Тогда
		...
		Текст = Текст + НСтр("ru='Не указана кратность валюты.';uk='Не вказана кратність валюти.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
	...
	Если мДокумент.Сценарий.Пустая() Тогда
		...
		Текст = Текст + НСтр("ru='Не указан сценарий планирования.';uk='Не зазначений сценарій планування.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
		
	ИначеЕсли мДокумент.Сценарий.Периодичность.Пустая() Тогда
		
		Текст = Текст + НСтр("ru='Не указана периодичность сценария планирования.';uk='Не зазначена періодичність сценарію планування.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
	...
	Если мДокумент.ДатаПланирования = Дата('00010101') Тогда
		...
		Текст = Текст + НСтр("ru='Не указана дата планирования.';uk='Не вказана дата планування.'") + Символы.ПС;
		ДанныеКорректны = Ложь;
...
Процедура ИнициализацияПомощникаПланирования() Экспорт
	...
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ОбъемЗакупок", 1, НСтр("ru='Объем закупок';uk='Обсяг купівель'"), "ЗакупкиОбороты", Ложь, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ПланыЗакупок", 2,НСтр("ru='Планы закупок';uk='Плани купівель'"), "ПланыЗакупокОбороты", Ложь, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ОбъемПродаж", 3, НСтр("ru='Объем продаж';uk=""Об'єм продажів"""), "ПродажиОбороты", Ложь, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ПланыПродаж", 4, НСтр("ru='Планы продаж';uk='Плани продажів'"), "ПланыПродажОбороты", Ложь, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ЗаказыПоставщикам", 5, НСтр("ru='Заказы поставщикам';uk='Замовлення постачальникам'"), "ЗаказыПоставщикамОстатки", Истина, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ЗаказыПокупателей", 6, НСтр("ru='Заказы покупателей';uk='Замовлення покупців'"), "ЗаказыПокупателейОстатки", Истина, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ВнутренниеЗаказы", 7, НСтр("ru='Внутренние заказы';uk='Внутрішні замовлення'"), "ВнутренниеЗаказыОстатки", Истина, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "СкладскиеОстатки", 8, НСтр("ru='Складские остатки';uk='Складські залишки'"), "ТоварыНаСкладахОстатки", Истина, Новый Структура));
	мМассивСтратегии.Добавить(Новый Структура("ВидСтратегии, Источник, Представление, ИмяТаблицы, НаДату, ПредставленияПолей, ТекстЗапроса", "ПлановыеОстатки", 9, НСтр("ru='Плановые остатки';uk='Планові залишки'"), "ПлановыеОстаткиТоваровСрезПоследних", Истина, Новый Структура));
	...
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"), "ЗакупкиОбороты.Подразделение");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Номенклатура", "Номенклатура", "ЗакупкиОбороты.Номенклатура");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ЗакупкиОбороты.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "ЗаказПоставщику", НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"), "ЗакупкиОбороты.ЗаказПоставщику");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Контрагент", "Контрагент", "ЗакупкиОбороты.Контрагент");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Договор", НСтр("ru='Договор';uk='Договір'"), "ЗакупкиОбороты.ДоговорКонтрагента");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "ДокументЗакупки", НСтр("ru='Документ закупки';uk='Документ купівлі'"), "ЗакупкиОбороты.ДокументЗакупки");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "СтатусПартии", НСтр("ru='Статус партии';uk='Статус партії'"), "ЗакупкиОбороты.СтатусПартии");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ОбъемЗакупок", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Сценарий", НСтр("ru='Сценарий';uk='Сценарій'"), "ПланыЗакупокОбороты.Сценарий");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Проект", "Проект", "ПланыЗакупокОбороты.Проект");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"), "ПланыЗакупокОбороты.Подразделение");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Номенклатура", "Номенклатура", "ПланыЗакупокОбороты.Номенклатура");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ПланыЗакупокОбороты.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Заказ", НСтр("ru='Заказ';uk='Замовлення'"), "ПланыЗакупокОбороты.Заказ");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "ДокументПланирования", НСтр("ru='Документ планирования';uk='Документ планування'"), "ПланыЗакупокОбороты.ДокументПланирования");
	...
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ПланыЗакупок", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"), "ПродажиОбороты.Подразделение");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Номенклатура", "Номенклатура", "ПродажиОбороты.Номенклатура");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ПродажиОбороты.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "ЗаказПокупателя", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"), "ПродажиОбороты.ЗаказПокупателя");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Контрагент", "Контрагент", "ПродажиОбороты.ДоговорКонтрагента.Владелец");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Договор", НСтр("ru='Договор';uk='Договір'"), "ПродажиОбороты.ДоговорКонтрагента");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "ДокументПродажи", НСтр("ru='Документ продажи';uk='Документ продажу'"), "ПродажиОбороты.ДокументПродажи");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ОбъемПродаж", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ПланыПродаж", "Сценарий", НСтр("ru='Сценарий';uk='Сценарій'"), "ПланыПродажОбороты.Сценарий");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Проект", "Проект", "ПланыПродажОбороты.Проект");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"), "ПланыПродажОбороты.Подразделение");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Номенклатура", "Номенклатура", "ПланыПродажОбороты.Номенклатура");
	ДобавитьПредставлениеПоля("ПланыПродаж", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ПланыПродажОбороты.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Заказ", НСтр("ru='Заказ';uk='Замовлення'"), "ПланыПродажОбороты.Заказ");
	...
	ДобавитьПредставлениеПоля("ПланыПродаж", "ДокументПланирования", НСтр("ru='Документ планирования';uk='Документ планування'"), "ПланыПродажОбороты.ДокументПланирования");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ПланыПродаж", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ЗаказыПоставщикамОстатки.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "ЗаказПоставщику", НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"), "ЗаказыПоставщикамОстатки.ЗаказПоставщику");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "Контрагент", "Контрагент", "ЗаказыПоставщикамОстатки.ДоговорКонтрагента.Владелец");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "Договор", НСтр("ru='Договор';uk='Договір'"), "ЗаказыПоставщикамОстатки.ДоговорКонтрагента");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "СтатусПартии", НСтр("ru='Статус партии';uk='Статус партії'"), "ЗаказыПоставщикамОстатки.СтатусПартии");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ЗаказыПоставщикам", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ЗаказыПокупателейОстатки.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "ЗаказПокупателя", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"), "ЗаказыПокупателейОстатки.ЗаказПокупателя");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "Контрагент", "Контрагент", "ЗаказыПокупателейОстатки.ДоговорКонтрагента.Владелец");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "Договор", НСтр("ru='Договор';uk='Договір'"), "ЗаказыПокупателейОстатки.ДоговорКонтрагента");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "СтатусПартии", НСтр("ru='Статус партии';uk='Статус партії'"), "ЗаказыПокупателейОстатки.СтатусПартии");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ЗаказыПокупателей", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
	...
	ДобавитьПредставлениеПоля("ВнутренниеЗаказы", "Номенклатура", НСтр("ru='Номенклатура';uk='Номенклатура'"), "ВнутренниеЗаказыОстатки.Номенклатура");
	ДобавитьПредставлениеПоля("ВнутренниеЗаказы", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ВнутренниеЗаказыОстатки.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ВнутренниеЗаказы", "ВнутреннийЗаказ", НСтр("ru='Внутренний заказ';uk='Внутрішнє замовлення'"), "ВнутренниеЗаказыОстатки.ВнутреннийЗаказ");
	ДобавитьПредставлениеПоля("ВнутренниеЗаказы", "СтатусПартии", НСтр("ru='Статус партии';uk='Статус партії'"), "ВнутренниеЗаказыОстатки.СтатусПартии");
	ДобавитьПредставлениеПоля("ВнутренниеЗаказы", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	...
	ДобавитьПредставлениеПоля("СкладскиеОстатки", "Номенклатура", НСтр("ru='Номенклатура';uk='Номенклатура'"), "ТоварыНаСкладахОстатки.Номенклатура");
	ДобавитьПредставлениеПоля("СкладскиеОстатки", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ТоварыНаСкладахОстатки.ХарактеристикаНоменклатуры");
	...
	ДобавитьПредставлениеПоля("ПлановыеОстатки", "Сценарий", НСтр("ru='Сценарий';uk='Сценарій'"), "ПлановыеОстаткиТоваровСрезПоследних.Сценарий");
	ДобавитьПредставлениеПоля("ПлановыеОстатки", "Номенклатура", "Номенклатура", "ПлановыеОстаткиТоваровСрезПоследних.Номенклатура");
	ДобавитьПредставлениеПоля("ПлановыеОстатки", "ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), "ПлановыеОстаткиТоваровСрезПоследних.ХарактеристикаНоменклатуры");
	ДобавитьПредставлениеПоля("ПлановыеОстатки", "Количество", НСтр("ru='Количество';uk='Кількість'"), "");
	ДобавитьПредставлениеПоля("ПлановыеОстатки", "Стоимость", НСтр("ru='Стоимость';uk='Вартість'"), "");
...
Процедура ДобавитьВТекстЗапросаСвойстваИКатегории(ТекстЗапроса, ТаблицаПолей, СтруктураПредставлениеПолей, СоответствиеНазначений, Параметры, Стратегия) Экспорт
	...
	Пока СвойстваОбъектов.Следующий() Цикл
		...
		Для каждого Поле из Поля Цикл
			...
			ДобавитьПредставлениеПоля(ВидСтратегии, ИмяСвойства + "Значение", Поле.Представление + НСтр("ru=' (св-во ';uk=' (вл-ть '") + СвойстваОбъектов.Наименование + ")", "");
			СоответствиеНазначений.Вставить(Поле.Представление + НСтр("ru=' (св-во ';uk=' (вл-ть '") + СвойстваОбъектов.Наименование + ")", СвойстваОбъектов.Ссылка);
