...
Процедура ЗаполнитьТаблицуЗначенийОбъекта(ПостроительОтчетаОтветственные) Экспорт
	...
	мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Обрабатывается таблица запроса ...';uk='Обробляється таблиця запиту ...'");
	...
	Для каждого СтрокаТекущегоУровня Из СтрокаПрошлогоУровня.Строки Цикл
		...
		Если ТипЗнч(ЗначениеГруппировки) = Тип("Дата") Тогда
			...
			Если ЗначениеГруппировки = '00010101000000' Тогда
				...
				СтрокаТекущегоУровняДереваПодбора.НоменклатураКонтрагентЗаказ = НСтр("ru='Не заявлено в плане потребности';uk='Не заявлено у плані потреби'");
		...
		ИначеЕсли ИмяГруппировки = "ИДОбъекта" Тогда
			...
			Если СтрокаОбъекта <> Неопределено Тогда
				...
				СтрокаТекущегоУровняДереваПодбора.НоменклатураКонтрагентЗаказ = "Заказ поставщику №" + Строка(ЗаказОбъект.Номер) + НСтр("ru=' от ';uk=' від '") + Формат(ЗаказОбъект.Дата, "ДФ=dd.MM.yyyy") + НСтр("ru=' (поставка: ';uk=' (поставка: '") + ?(ЗаказОбъект.ДатаПоступления = '00010101000000', НСтр("ru='не указана';uk='не указана'"), Формат(ЗаказОбъект.ДатаПоступления, "ДФ=dd.MM.yyyy")) + ")";
...
Процедура ПересортироватьТабличнуюЧастьИЗаполнитьДерево(ДеревоПодбора, ДеревоПодбораНаФорме = Неопределено, УровеньРазворотаДерева = 0) Экспорт
			...
			Если (ТипЗнч(ДеревоПодбора.Строки[0].Номенклатура) = Тип("СправочникСсылка.Номенклатура")
			 ...
			 ИЛИ (ТипЗнч(ДеревоПодбора.Строки[0].НоменклатураКонтрагентЗаказ) = Тип("СправочникСсылка.Контрагенты")
			   И ПорядокОтображенияДерева) Тогда
				...
				мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Запоминаются развороты строк ...';uk=""Запам'ятовуються розвороти рядків ...""");
	...
	Если ДеревоПодбораНаФорме <> Неопределено Тогда
		...
		ДеревоПодбораНаФорме.Колонки.СуммаУпрУчета.ТекстШапки = НСтр("ru='Сумма в ';uk='Сума в '") + СокрЛП(глЗначениеПеременной("ВалютаУправленческогоУчета").Наименование);
		ДеревоПодбораНаФорме.Колонки.ЦенаУпрУчета.ТекстШапки  = НСтр("ru='Цена в ';uk='Ціна у '")  + СокрЛП(глЗначениеПеременной("ВалютаУправленческогоУчета").Наименование);
	...
	Если ПорядокОтображенияДерева Тогда
		...
		Если ДеревоПодбораНаФорме <> Неопределено Тогда
			...
			ДеревоПодбораНаФорме.Колонки.НоменклатураКонтрагентЗаказ.ТекстШапки = НСтр("ru='Контрагент/Номенклатура/Дата потребности/Заказ';uk='Контрагент/Номенклатура/Дата потреби/Замовлення'");
	...
	Иначе
		...
		Если ДеревоПодбораНаФорме <> Неопределено Тогда
			...
			ДеревоПодбораНаФорме.Колонки.НоменклатураКонтрагентЗаказ.ТекстШапки = НСтр("ru='Номенклатура/Дата потребности/Контрагент/Заказ';uk='Номенклатура/Дата потреби/Контрагент/Замовлення'");
	...
	мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Анализируется и заполняется дерево ...';uk='Аналізується і заповнюється дерево ...'");
	...
	Если ДеревоПодбораНаФорме <> Неопределено Тогда
		...
		Если МассивСтарыхЗначений.Количество() = 6 Тогда
			...
			мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Определяется прошлая строка ...';uk='Визначається минулий рядок ...'");
			мФормаПрогрессора.МаксимальноеЗначение = ДеревоПодбора.Строки.Количество() - 1;
...
Процедура ПроверитьОтбор(КопияСтруктурыОтборов, ПрошлаяСтрока, СписокНайденныхОтборов)
	...
	мФормаПрогрессора.Значение = НСтр("ru='Устанавливаются отборы ...';uk='Встановлюються відбори ...'");
	...
	Для каждого ЭлементСруктуры Из КопияСтруктурыОтборов Цикл
		...
		ИначеЕсли ЭлементСруктуры.Ключ = "ОтбиратьДатыЗаказовНеЗаявленныеВПланеПотребности" Тогда
			Если ТипЗнч(СтрокаДерева.НоменклатураКонтрагентЗаказ) <> Тип("Строка") Тогда
				Продолжить;
			КонецЕсли;
			Если СтрокаДерева.НоменклатураКонтрагентЗаказ = НСтр("ru='Не заявлено в плане потребности';uk='Не заявлено у плані потреби'") Тогда
...
Процедура УстановитьРазворотСтрок(ДеревоПодбора, ДеревоПодбораНаФорме, УровеньРазворотаДерева, ТаблицаРазворотовСтрок = Неопределено) Экспорт
	...
	мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Устанавливается разворот строк ...';uk='Встановлюється розворот рядків ...'");
	мФормаПрогрессора.МаксимальноеЗначение = ДеревоПодбора.Строки.Количество() - 1;
...
ПоляОтборов.Добавить("ХарактеристикаНоменклатуры", НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'"), Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
ПоляОтборов.Добавить("Контрагент", "Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
ПоляОтборов.Добавить("ЗаказПоставщику", НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"), Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
ПоляОтборов.Добавить("ДатаПотребности", НСтр("ru='Дата потребности';uk='Дата потреби'"), Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата)));
ПоляОтборов.Добавить("НеобходимоеКоличество", НСтр("ru='Необходимое количество';uk='Необхідна кількість'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,3));
ПоляОтборов.Добавить("ЗаказанноеКоличество", НСтр("ru='Заказанное количество';uk='Замовлена кількість'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,3));
ПоляОтборов.Добавить("ЦенаУпрУчета", НСтр("ru='Цена в валюте упр.учета';uk='Ціна в валюті упр.обліку'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
ПоляОтборов.Добавить("СуммаУпрУчета", НСтр("ru='Сумма в валюте упр.учета';uk='Сума у валюті упр.обліку'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
ПоляОтборов.Добавить("СуммаВзаиморасчетов", НСтр("ru='Сумма в валюте взаиморасчетов';uk='Сума у валюті взаєморозрахунків'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
ПоляОтборов.Добавить("ОтбиратьНеЗаказанныеТовары", НСтр("ru='Отбирать не заказанные товары';uk='Відбирати не замовлені товари'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНеВыполненныеДатыПотребности", НСтр("ru='Отбирать не выполненные даты потребности';uk='Відбирати не виконані дати потреби'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьДатыЗаказовНеЗаявленныеВПланеПотребности", НСтр("ru='Отбирать даты поставки заказов не заявленные в плане потребности';uk='Відбирати дати поставки замовлень не заявлені в плані потреби'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьИзмененныеЗаказы", НСтр("ru='Отбирать измененные заказы';uk='Відбирати змінені замовлення'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНовыеЗаказы", НСтр("ru='Отбирать новые заказы';uk='Відбирати нові замовлення'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНовыеИИзмененныеЗаказы", НСтр("ru='Отбирать новые и измененные заказы';uk='Відбирати нові і змінені замовлення'"), Новый ОписаниеТипов("Булево"));
...
ПоляОтборов.Добавить("Контрагент", "Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
ПоляОтборов.Добавить("ЗаказПоставщику", НСтр("ru='Заказ поставщику';uk='Замовлення постачальнику'"), Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
ПоляОтборов.Добавить("ЗаказПокупателя", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"), Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
ПоляОтборов.Добавить("ДатаПотребности", НСтр("ru='Дата потребности';uk='Дата потреби'"), Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата)));
ПоляОтборов.Добавить("КоличествоПотребности", НСтр("ru='Количество потребности';uk='Кількість потреби'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,3));
ПоляОтборов.Добавить("КоличествоЗаказов", НСтр("ru='Количество заказов';uk='Кількість замовлень'"), ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,3));
ПоляОтборов.Добавить("ОтбиратьНеВыполненныеДатыПотребности", НСтр("ru='Отбирать не выполненные даты потребности';uk='Відбирати не виконані дати потреби'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНеВыполненныеЗаказыПокупателей", НСтр("ru='Отбирать не выполненные заказы покупателей';uk='Відбирати не виконані замовлення покупців'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьИзмененныеЗаказы", НСтр("ru='Отбирать измененные заказы поставщикам';uk='Відбирати змінені замовлення постачальникам'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНовыеЗаказы", НСтр("ru='Отбирать новые заказы поставщикам';uk='Відбирати нові замовлення постачальникам'"), Новый ОписаниеТипов("Булево"));
ПоляОтборов.Добавить("ОтбиратьНовыеИИзмененныеЗаказы", НСтр("ru='Отбирать новые и измененные заказы поставщикам';uk='Відбирати нові і змінені замовлення постачальникам'"), Новый ОписаниеТипов("Булево"));
