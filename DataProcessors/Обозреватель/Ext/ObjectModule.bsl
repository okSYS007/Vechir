...
Функция Получить(Сервер, АдресРесурсов, Ресурс)
	...
	Исключение
		Сообщить(НСтр("ru='Неудачная попытка очистить временный каталог ';uk='Невдала спроба очистити тимчасовий каталог '")+КаталогОбновлений);
		Возврат "";
	КонецПопытки;
		...
		Иначе
			...
			Если стПараметры.Version<>Метаданные.Версия Тогда
				...
				Если Найти(стПараметры.FromVersions,СокрП(Метаданные.Версия)+";")=0 Тогда
					ЭлементыФормы.РезультатПроверки.Значение = НСтр("ru='Комплект поставки на вэб-сервере не может быть использован для обновления текущей конфигурации.';uk='Комплект поставки на веб-сервері не може бути використаний для оновлення поточної конфігурації.'")+Символы.ПС+ 
					                                           НСтр("ru='Используйте диск ИТС!';uk='Використовуйте диск ІТС!'");

				Иначе
					ЭлементыФормы.РезультатПроверки.Значение = НСтр("ru='На вэб-сервере обнаружен комплект поставки для обновления конфигурации!';uk='На веб-сервері виявлений комплект поставки для оновлення конфігурації!'");

				КонецЕсли;

			Иначе
				ЭлементыФормы.РезультатПроверки.Значение = НСтр("ru='Обновление конфигурации не требуется!';uk='Оновлення конфігурації не потрібно!'");
				ПараметрВозврата = Ложь;
...
СерверИсточникДляПроверкиНаличияОбновления = "http://releases.1c.eu";
