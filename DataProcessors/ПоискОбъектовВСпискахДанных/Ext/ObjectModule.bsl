...
Процедура ПроизвестиОтборДанных(ЗначениеОтбора, Знач ПредлагатьСнятьИерархическийПросмотр = Ложь, 
	...
	Иначе
		...
		Состояние(НСтр("ru='Установка ограничения на список';uk='Встановлення обмеження на список'"));
...
Процедура ЗаполнитьСписокЗначенийОтборовДанных(СписокЗначений) Экспорт
	...
	СписокЗначений.Добавить("ОтборВсеВидимыеПоля", НСтр("ru='Все видимые поля';uk='Всі видимі поля'"), , БиблиотекаКартинок.ПоискПоВидимымПолям);
	...
	Если НЕ ПустаяСтрока(ИмяОбъектаНастройки) Тогда
		...
		СписокЗначений.Добавить("ПоследняяНастройкаОтбораДанных", НСтр("ru='Настроить новый отбор ...';uk='Настроїти новий відбір ...'"));
...
Процедура ВыполнитьОтборПоПоследнейНастройкеПоискаДляСпискаВыбора(Знач ЗначениеОтбораДанных, Знач ФормаВладелец = Неопределено) Экспорт
	...
	Если РезультатПоиска Тогда
		...
		НастройкаОтбораДанных = НСтр("ru='Отбор пользователя';uk='Відбір користувача'");
