...
Процедура Объект_ПриОткрытии(ЭлементыФормы) Экспорт
	...
	ЭлементыФормы.ЛичныеКонтактыСписок.Значение.Отбор.ПользовательДляОграниченияПравДоступа.Использование = Истина;
	ЭлементыФормы.ЛичныеКонтактыСписок.Значение.Отбор.ПользовательДляОграниченияПравДоступа.Значение      = глЗначениеПеременной("глТекущийПользователь");
...
Процедура Объект_НайтиНажатие(Элемент, РезультатыПоиска, СтрокаПоиска, ВыдаватьОкноПредупреждения = Истина) Экспорт
	...
	Если Выборка.Количество() = 0 И ВыдаватьОкноПредупреждения Тогда
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='По заданным параметрам ничего не найдено.';uk='По заданих параметрах нічого не знайдено.'"));
		#КонецЕсли
		Возврат;
	КонецЕсли; 
