...
Процедура ПечатьОтчетаОВыгруженныхПлатежныхДокументах(ПолеОтчета, Источник) Экспорт
	...
	Шапка.Параметры.ОписаниеПериода		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='по счету %1 с %2 по %3';uk='по рахунку %1 з %2 по %3'"), БанковскийСчет.НомерСчета, Формат(НачПериода,"ДЛФ=D"), Формат(КонПериода,"ДЛФ=D"));
...
Процедура ПечатьОтчетаОЗагруженныхПлатежныхДокументах(Источник) Экспорт
	...
	Для Каждого СтрокаСчета Из Импорт_РасчетныеСчета Цикл
		Если Не ПустаяСтрока(СтрокаСчета.ДатаНачала) Тогда
			...
			Шапка.Параметры.ОписаниеПериода		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='по счету %1 с %2 по %3';uk='по рахунку %1 з %2 по %3'"), СтрокаСчета.РасчСчетОрг, СтрокаСчета.ДатаНачала, СтрокаСчета.ДатаКонца);
			ПолеОтчета.Вывести(Шапка);
	...
	Если Не ИмеетсяСекцияРасчСчет Тогда
		Для Каждого СтрокаСчета Из Импорт_РасчетныеСчета Цикл
			...
			Шапка.Параметры.ИмяОтчета 			= НСтр("ru='Отчет о загруженных платежных документах';uk='Звіт про завантажені платіжні документи'");
			Шапка.Параметры.ОписаниеПериода		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='по счету %1 с %2 по %3';uk='по рахунку %1 з %2 по %3'"), СтрокаСчета.РасчСчетОрг, Импорт_Заголовок.ДатаНачала, Импорт_Заголовок.ДатаКонца);
			ПолеОтчета.Вывести(Шапка);
...
Функция СоздатьКонтрагента(СтрокаКонтрагент = Неопределено, ОткрыватьФорму=Истина) Экспорт //или СтрокаКонтрагент - строка, или ТаблицаКонтрагентов - ТЗ
	...
	Если СообщатьОСозданномКонтрагенте Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Контрагент(%1) создан.';uk='Контрагент(%1) створений.'"), СтрокаКонтрагент.Представление));
		ПредставлениеКонтрагента = ""+Символы.Таб+"- ";
	Иначе
		ПредставлениеКонтрагента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Контрагенту(%1)';uk='Контрагентові(%1)'"), СтрокаКонтрагент.Представление);
	КонецЕсли;
	
	ТаблицаКлассификаторБанков = Новый ("ТаблицаЗначений");
	ТаблицаКлассификаторБанков.Колонки.Добавить("Наименование");
	ТаблицаКлассификаторБанков.Колонки.Добавить("КодБанка"); 
	...
	Для Сч = 3 По КоличествоЗаписей Цикл
		...
		КлассификаторБанковКодМФО 		= Макет.Область(Сч, 2).Текст;
		КлассификаторБанковНаименование = Макет.Область(Сч, 3).Текст;
		
		Если НЕ ПустаяСтрока(КлассификаторБанковКодМФО) Тогда
			НоваяСтрока = ТаблицаКлассификаторБанков.Добавить();
			НоваяСтрока.КодБанка 	 = СокрЛП(КлассификаторБанковКодМФО);
			НоваяСтрока.Наименование = СокрЛП(КлассификаторБанковНаименование);	
		КонецЕсли; 
	...
	Для каждого Строка из СтрокаКонтрагент.Строки Цикл
		...
		Если Строка.Представление = НСтр("ru='Р/счет';uk='Р/рахунок'") и  Строка.Пометка Тогда
			Попытка
				...
				ЭтоIBAN = УправлениеДенежнымиСредствами.НомерБанковскогоСчетаСоответствуетСтандартуIBAN(НомерСчета);
				
				Если ЭтоIBAN Тогда
					КодБанка = УправлениеДенежнымиСредствами.ПолучитьКодБанкаПоНомеруСчетаIBAN(НомерСчета);
				Иначе
					КодБанка = РССчет[1+Смещение].Значение;
				КонецЕсли;
				
					//Банк контрагента
					НовыйБанк = НайтиБанкПоМФО(КодБанка);
					
					Если НовыйБанк = Справочники.Банки.ПустаяСсылка() Тогда
						
						НовыйБанк = Справочники.Банки.СоздатьЭлемент();
						Если НЕ ПустаяСтрока(РССчет[0+Смещение].Значение) Тогда
							НовыйБанкНаименование = РССчет[0+Смещение].Значение;
						Иначе
							НайденнаяСтрока = ТаблицаКлассификаторБанков.Найти(КодБанка, "КодБанка");
							Если НайденнаяСтрока <> Неопределено Тогда
								НовыйБанкНаименование = НайденнаяСтрока.Наименование;
							Иначе
								НовыйБанкНаименование = НСтр("ru='Новый банк, МФО: ';uk='Новий банк, МФО: '")+КодБанка;    
							КонецЕсли;	
						КонецЕсли; 
						
						НовыйБанк.Наименование = НовыйБанкНаименование;
						НовыйБанк.Код          = КодБанка;
						НовыйБанк.Записать();
					КонецЕсли;
		...
		Если Строка.Представление = НСтр("ru='Договор';uk='Договір'") и  Строка.Пометка Тогда
			...
			Попытка
				...
				Если НайденныйДоговорОбъект.ВалютаВзаиморасчетов <> мВалютаРегламентированногоУчета Тогда
...
Процедура СформироватьСтруктуруТаблицыИмпорта(Таб, ТабПоле) Экспорт
	...
	Т_СтрокаНомерСчета  = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(Метаданные.Справочники.БанковскиеСчета.Реквизиты.НомерСчета.Тип.КвалификаторыСтроки.Длина));
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "Сумма",                Т_Строка,    НСтр("ru='Сумма (из файла)';uk='Сума (з файлу)'"),           НеВидимая);
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "КодВалюты",            Т_Строка,    НСтр("ru='Валюта платежа';uk='Валюта платежу'"),             НеВидимая);
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "СтатусДокумента",      Т_Строка,    НСтр("ru='Статус документа';uk='Статус документа'"),           НеВидимая);
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ПлательщикСчет",       Т_СтрокаНомерСчета,  НСтр("ru='Расчетный счет плательщика';uk='Розрахунковий рахунок платника'"), НеВидимая);
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ПолучательСчет",       Т_СтрокаНомерСчета,  НСтр("ru='Расчетный счет получателя';uk='Розрахунковий рахунок отримувача'"),  НеВидимая);
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ВидДокумента",         Т_Строка,    НСтр("ru='Вид документа';uk='Вид документу'"),              НеВидимая);
...
Процедура СформироватьСтруктуруТаблицыРасчСчетов(Таб) Экспорт
	...
	СоздатьКолонкуВТаблице(Таб, Неопределено, "ДатаКонца",        Т_Строка,     НСтр("ru='Дата конца интервала';uk='Дата кінця інтервалу'"),       Видимая);
...
Процедура ЗаполнитьДокументыНаЭкспорт(ДокументыНаЭкспорт) Экспорт
	...
	Если НЕ ВыгружатьПлатПоручения Тогда
		Предупреждение(НСтр("ru='Необходимо отметить хотя бы один из выгружаемых видов платежных документов.';uk='Необхідно відзначити хоча б один з видів платіжних документів, що вивантажується.'"));
		Возврат;
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(БанковскийСчет) Тогда
		Предупреждение(НСтр("ru='Необходимо выбрать банковский счет.';uk='Необхідно вибрати банківський рахунок.'"));
		Возврат;
	КонецЕсли;
...
Функция ЗагрузитьСтрокуЗаголовка(ТекстСтрокиЗаголовка)
	...
	Иначе
		...
		Сообщить(НСтр("ru='Неверный реквизит заголовка, строка ';uk='Невірний реквізит заголовку, рядок '") + (Импорт_ТекущаяСтрока - 1) + " : " + ТекстСтрокиЗаголовка);
...
Функция ЗагрузитьСтрокуЗаголовкаXML(Тег,Значение)
	...
	Иначе
		// неправильный реквизит заголовка
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный реквизит заголовка: %1';uk='Невірний реквізит заголовку: %1'"), Тег));
...
Функция ЗагрузитьСекциюДокументаXML(ДокументыДляИмпорта, ФайлXML)
	...
	Пока ФайлXML.Прочитать() Цикл
		...
		Если ИмяТега="СЕКЦИЯДОКУМЕНТ" Тогда
			...
			Если ЗначениеЗаполнено(НоваяСтрокаДокументов.КодВалюты) И НоваяСтрокаДокументов.КодВалюты <> СокрЛП(БанковскийСчет.ВалютаДенежныхСредств.Код) Тогда
		...
		Иначе
			// неправильный реквизит заголовка
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный реквизит платежного документа: %1';uk='Невірний реквізит платіжного документу: %1'"), ИмяТега));
			Возврат Ложь;
...
Функция ЗагрузитьСекциюРасчСчетаXML(СтрокаРССчета, ФайлXML)
	...
	Пока ФайлXML.Прочитать() Цикл
		...
		Иначе
			// неправильный реквизит заголовка
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный реквизит в секции описания расчетного счета: %1';uk='Невірний реквізит в секції опису розрахункового рахунку: %1'"), ИмяТега));
			Возврат Ложь;
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
	...
	Если ПлатежноеПоручение и (СтрокаДокумента.ВидОплаты="" или Число(СтрокаДокумента.ВидОплаты)= 1) Тогда
		...
		ИмяДокумента = ?(Исходяший,НСтр("ru='Платежное поручение исходящее';uk='Платіжне доручення вихідне'"),НСтр("ru='Платежное поручение входящее';uk='Платіжне доручення вхідне'"));
	Иначе
		ВидДокумента = ?(Исходяший,"ПлатежныйОрдерСписаниеДенежныхСредств","ПлатежныйОрдерПоступлениеДенежныхСредств");
		ИмяДокумента = ?(Исходяший,НСтр("ru='Платежный ордер на списание';uk='Платіжний ордер на списання'"),НСтр("ru='Платежный ордер на поступление';uk='Платіжний ордер на надходження'"));
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если СтрокаДокументаВидДокумента = "ПлатежноеПоручениеВходящее" Тогда
...
Функция ПолучитьПредставлениеОбъекта(Объект)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") Тогда
		НазваниеОбъекта = НСтр("ru='документ ""Входящее платежное поручение"" № ';uk='документ ""Вхідне платіжне доручення"" № '") + СокрЛП(Объект.Номер) + НСтр("ru=' от ';uk=' від '") + Объект.Дата;
	
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПлатежноеПоручениеИсходящее") Тогда
		НазваниеОбъекта = НСтр("ru='документ ""Исходящее платежное поручение"" № ';uk='документ ""Вихідне платіжне доручення"" № '") + СокрЛП(Объект.Номер) + НСтр("ru=' от ';uk=' від '") + Объект.Дата;
			
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПлатежныйОрдерСписаниеДенежныхСредств") Тогда
		НазваниеОбъекта = НСтр("ru='документ ""Платежный ордер (списание)"" № ';uk='документ ""Платіжний ордер (списання)"" № '") + СокрЛП(Объект.Номер) + НСтр("ru=' от ';uk=' від '") + Объект.Дата;
		
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПлатежныйОрдерПоступлениеДенежныхСредств") Тогда
		НазваниеОбъекта = НСтр("ru='документ ""Платежный ордер (поступление)"" № ';uk='документ ""Платіжний ордер (надходження)"" № '") + СокрЛП(Объект.Номер) + НСтр("ru=' от ';uk=' від '") + Объект.Дата;
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи,СтрокаСекции)
	...
	Если ТипДокумента = "ПлатежноеПоручениеИсходящее" Тогда
		ИмяДокумента = НСтр("ru='Платежное поручение исходящее';uk='Платіжне доручення вихідне'");
		
	ИначеЕсли ТипДокумента = "ПлатежноеПоручениеВходящее" Тогда
		ИмяДокумента = НСтр("ru='Платежное поручение входящее';uk='Платіжне доручення вхідне'");
		
	ИначеЕсли ТипДокумента = "ПлатежныйОрдерСписаниеДенежныхСредств" Тогда
		ИмяДокумента = НСтр("ru='Платежный ордер на списание';uk='Платіжний ордер на списання'");
		
	ИначеЕсли ТипДокумента = "ПлатежныйОрдерПоступлениеДенежныхСредств" Тогда
		ИмяДокумента = НСтр("ru='Платежный ордер на поступление';uk='Платіжний ордер на надходження'");
	
	КонецЕсли;
	
	Проводить = (ПолучитьНастройкуЗаполнения(ИмяДокумента,"Проводить") = Истина);
	ОтражатьВУправленческомУчете 	= Истина;		
	УстановитьСвойство(ОбъектДляЗаписи, "ОтражатьВУправленческомУчете", ОтражатьВУправленческомУчете);
	...
	Если ОбъектМодифицирован Тогда
		Попытка
			...
			Если ТипДокумента = "ПлатежноеПоручениеИсходящее" Тогда
				...
				УстановитьСвойство(ОбъектДляЗаписи, "НомерПоручения", СтрокаСекции.Номер);
			КонецЕсли;	
			...
			Сообщить(?(ЭтоНовыйДокумент,НСтр("ru='Создан ';uk='Створено'"),НСтр("ru='Перезаписан ';uk='Перезаписан '"))+ ПолучитьПредставлениеОбъекта(ОбъектДляЗаписи) + ".");
	...
	Если Проводить И (ОбъектМодифицирован ИЛИ НЕ ОбъектПроведен) Тогда
		Попытка
			...
			Сообщить(?(ОбъектПроведен,НСтр("ru='Перепроведен ';uk='Перепроведений '"),НСтр("ru='Проведен ';uk='Проведено '"))+НазваниеОбъекта, СтатусСообщения.Обычное);
			СтрокаСекции.Проведен = ОбъектДляЗаписи.проведен;
		Исключение
...
//ВалютаДокумента,СчетБанк
	...
	Если НЕ ЗначениеЗаполнено(ОбъектДокумента.СчетБанк) Тогда
		...
		Если ОбъектДокумента.ВалютаДокумента = мВалютаРегламентированногоУчета Тогда
			УстановитьСвойство(ОбъектДокумента, "СчетБанк", ПланыСчетов.Хозрасчетный.ТекущиеСчетаВНациональнойВалюте, Истина);
		Иначе
			УстановитьСвойство(ОбъектДокумента, "СчетБанк", ПланыСчетов.Хозрасчетный.ТекущиеСчетаВИностраннойВалюте, Истина);
		КонецЕсли;
	КонецЕсли;
				...
				Если НЕ ОбъектДокумента.ЭтоНовый() Тогда
					Если НЕ ЗначениеЗаполнено(СтрокаСекции.Документ) Тогда
						...
						Если СтрокаСекции.ВидДокумента = "ПлатежноеПоручениеИсходящее"
							ИЛИ СтрокаСекции.ВидДокумента = "ПлатежныйОрдерСписаниеДенежныхСредств" 							Тогда
...
Процедура СинхронизироватьДокументыПоСчетам(ДокументыКИмпорту, ВидДокументовИсходящих, ВидДокументовВходящих, НачалоИнтервалаЗагрузки, КонецИнтервалаЗагрузки)
	...
	ДокументыНаУдаление = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
									   ...
									   |	И " + ВидДокументовИсходящих + ".ВалютаДокумента = &Валюта
	                                   |	И " + ВидДокументовИсходящих + ".СчетОрганизации.НомерСчета В(&НомераСчетов)");
	
	ДокументыНаУдаление.УстановитьПараметр("ДатаНач", НачалоДня(НачалоИнтервалаЗагрузки));
	ДокументыНаУдаление.УстановитьПараметр("ДатаКон", КонецДня(КонецИнтервалаЗагрузки));
	ДокументыНаУдаление.УстановитьПараметр("НомераСчетов", СписокСчетов);
	ДокументыНаУдаление.УстановитьПараметр("Валюта", БанковскийСчет.ВалютаДенежныхСредств);
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если ЗначениеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НачалоДня(ОбъектДокументаКУдаление.Дата) <= НачалоДня(ДатаГраницыОбработки) Тогда
						Сообщить(НСтр("ru='Платежный документ ""';uk='Платіжний документ ""'") + ОбъектДокументаКУдаление + НСтр("ru=' не помечен на удаление, так как имеет дату равной или меньшей границы обработки!';uk=' не позначений для вилучення, так як має дату рівної або меншою межі обробки!'"), СтатусСообщения.ОченьВажное);
						Продолжить;
	...
	ДокументыНаУдаление = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
									   ...
									   |	И " + ВидДокументовВходящих + ".ВалютаДокумента = &Валюта
