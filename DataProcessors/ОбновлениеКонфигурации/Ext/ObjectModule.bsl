...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение НСтр("ru='Неправильный формат строки версии: ';uk='Неправильний формат рядка версії: '") + СтрокаВерсии1;
	КонецЕсли;	 
	Версия2 = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(СтрокаВерсии2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение НСтр("ru='Неправильный формат строки версии: ';uk='Неправильний формат рядка версії: '") + СтрокаВерсии2;
	КонецЕсли;	 
...
"Обновление конфигурации доступно пользователям, входящим в роль ПолныеПрава.';uk='Недостатньо прав для виконання оновлення."
"Оновлення конфігурації доступно користувачам, що входять у роль ПолныеПрава.'");
...
Функция ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, 
	...
	Тема = ? (УспешноеОбновление, НСтр("ru='Успешное обновление конфигурации';uk='Успішне оновлення конфігурації'"), НСтр("ru='Ошибка обновления конфигурации';uk='Помилка оновлення конфігурації'"));
	Текст = НСтр("ru='Конфигурация: ';uk='Конфігурація: '") + Метаданные.КраткаяИнформация + ".
		| Версия: " + Метаданные.Версия + ".
		| " + ? (УспешноеОбновление, НСтр("ru='Обновление конфигурации завершено успешно.';uk='Оновлення конфігурації завершене успішно.'"), 
	...
	Попытка		
		...
		Возврат Истина;
	Исключение
		ЗаписатьОшибку(НСтр("ru='Ошибка при отправке письма электронной почты: ';uk='Помилка при відправці листа електронної пошти: '") + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;	
КонецФункции
...
Процедура РаспаковатьРекламу() 
	...
	ЗаписатьИнформацию(НСтр("ru='Выполняется распаковка файлов рекламы...';uk='Виконується розпакування файлів реклами...'"));
	Если ФайлСуществует(ПутьКФайлуАрхиваРекламы) Тогда
		Попытка 
			...
			ЗаписатьИнформацию(НСтр("ru='Файлы рекламы успешно обработаны.';uk='Файли реклами успішно оброблені.'"));
		Исключение
			ЗаписатьОшибку(НСтр("ru='Ошибка при распаковке файлов рекламы: ';uk='Помилка при розпакуванні файлів реклами: '") + ОписаниеОшибки());
		КонецПопытки;
...
Функция СкопироватьФайл(ИмяФайлаИсточник, ИмяФайлаНазначение, ВыдаватьСообщения = Ложь) Экспорт
	...
	Исключение
		Сообщение = НСтр("ru='Ошибка при копировании: ';uk='Помилка при копіюванні: '") + ОписаниеОшибки() + 
			НСтр("ru=' (источник: ';uk='(джерело: '") + ИмяФайлаИсточник + НСтр("ru='; приемник: ';uk='; приймач: '") + ИмяФайлаНазначение + ")";
...
Функция ПолучитьФайлССервера(Знач Адрес, Знач ЛокальныйПуть, Знач ВыдаватьСообщения = Истина, 
	...
	Если НЕ ЗначениеЗаполнено(Адрес) ИЛИ НЕ ЗначениеЗаполнено(ЛокальныйПуть) Тогда
		ТекстОшибки = НСтр("ru='Невозможно подключиться к сети Интернет: не инициализированы внутренние переменные.';uk='Неможливо підключитися до мережі Інтернет: не ініціалізовані внутрішні змінні.'");
	...
	ЗаписатьИнформацию(НСтр("ru='Файл успешно получен (адрес: http://';uk='Файл успішно отриманий (адреса: http://'") + Соединение.Сервер + "/" + Адрес + 
		НСтр("ru='; локальный файл: ';uk='; локальний файл: '") + ЛокальныйПуть + ").");
	Возврат "";
...
Функция ПолучитьФайлОписанияОбновления() Экспорт
	...
	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		...
		Исключение
			ЗаписатьОшибку(НСтр("ru='Ошибка при удалении временного файла ';uk='Помилка при вилученні тимчасового файлу '") + ИмяФайла + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
...
Функция ПолучитьФайлСпискаШаблонов(Знач ВыдаватьСообщения = Истина) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка при распаковке файла со списком доступных обновлений: ';uk='Помилка при розпаковуванні файлу зі списком доступних оновлень: '");
...
Функция НайтиДискИТС() Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ЗаписатьОшибку(НСтр("ru='Ошибка при поиске диска ИТС: ';uk='Помилка при пошуку диска ІТС: '") + ОписаниеОшибки());
		Возврат Неопределено;	
	КонецПопытки;
  Возврат Неопределено;
...
Функция ПолучитьФайлПорядкаОбновленияИТС() Экспорт
	...
	Если НЕ СкопироватьФайл(ДискИТС + КаталогОбновленияКонфигурацииИТС + ИмяФайлаПорядкаОбновления, 
		...
		Попытка
			УдалитьФайлы(ИмяФайла);
		Исключение
			ЗаписатьОшибку(НСтр("ru='Ошибка при удалении временного файла ';uk='Помилка при вилученні тимчасового файлу '") + ИмяФайла + Символы.ПС + ОписаниеОшибки());
		КонецПопытки;
		Возврат Неопределено;
	КонецЕсли;
	Возврат ИмяФайла;
...
Функция ВыполнитьЗагрузкуСпискаОбновлений(Знач ИмяФайлаЗагрузки)
	...
	СообщениеОбОшибке = НСтр("ru='Ошибка при чтении файла списка обновлений.';uk='Помилка при читанні файлу списку оновлень.'");
...
Функция РаспаковатьДистрибутивОбновления() 
	...
	ЗаписатьИнформацию(НСтр("ru='Выполняется распаковка дистрибутива обновления...';uk='Виконується розпакування дистрибутива оновлення...'"));
	Для Каждого Обновление Из ДоступныеОбновления Цикл
		Если ЭтоДистрибутивОбновления(Обновление.ПутьКЛокальномуФайлу) Тогда
			Попытка 
				...
				ЗаписатьИнформацию(НСтр("ru='Файлы дистрибутива обновления успешно распакованы: ';uk='Файли дистрибутива оновлення успішно розпаковані: '") + Обновление.ПутьКЛокальномуФайлу);
			Исключение
				ЗаписатьОшибку(НСтр("ru='Ошибка при распаковке дистрибутива обновления: ';uk='Помилка при розпаковуванні дистрибутива оновлення: '") + ОписаниеОшибки());
				Возврат Ложь;
			КонецПопытки;
		Иначе
...
#Если Клиент Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Ошибка при проверке подключения к информационной базе.';uk='Помилка при перевірці підключення до інформаційної бази.'") + 
				Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецЕсли; 
#КонецЕсли
		ЗаписатьОшибку(НСтр("ru='Ошибка при проверке подключения к информационной базе.';uk='Помилка при перевірці підключення до інформаційної бази.'") + Символы.ПС + ОписаниеОшибки);
	КонецПопытки;	
...
Функция ПроверитьДоступККластеру(Знач ВыдаватьСообщения = Истина)
	...
	Если НЕ Результат Тогда
		ЗаписатьОшибку(НСтр("ru='Ошибка при проверке соединения с кластером серверов.';uk=""Помилка при перевірці з'єднання із кластером серверів."""));
	КонецЕсли; 
...
Процедура ВыполнитьОбновлениеКонфигурации() Экспорт
	...
	ЗаписатьИнформацию(НСтр("ru='Выполняется процедура обновления конфигурации: ';uk='Виконується процедура оновлення конфігурації: '") + ИмяГлавногоФайлаСкрипта);
...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, 
	...
	ЗаписатьИнформацию(НСтр("ru='Завершение обновления';uk='Завершення оновлення'"));
...
Функция СоздатьЗадачуПланировщика(Знач ИмяФайлаПрограммы, Знач ДатаВремя) 
	...
	Попытка
		...
		Если КодОшибки <> 0 Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx
			ЗаписатьОшибку(НСтр("ru='Ошибка при создании задачи планировщика: ';uk='Помилка при створенні задачі планувальника: '") + КодОшибки);
			КодЗадачи = 0;
		Иначе	
			ЗаписатьИнформацию(НСтр("ru='Задача планировщика успешно запланирована (';uk='Задача планувальника успішно запланована ('") + 
			    "команда: " + ИмяФайлаПрограммы + "; " +
			    "дата: " + ДатаВремя + "; " +
			    НСтр("ru='код задачи: ';uk='код задачі:'") + КодЗадачи + ").");
		КонецЕсли;		
	Исключение
		ЗаписатьОшибку(НСтр("ru='Ошибка при создании задачи планировщика: ';uk='Помилка при створенні задачі планувальника: '") + ОписаниеОшибки());
		Возврат 0;	
	КонецПопытки;
	Возврат КодЗадачи;
...
Функция УдалитьЗадачуПланировщика(КодЗадачи) Экспорт
	...
	Попытка
		...
		Если НЕ Результат Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx
			ЗаписатьОшибку(НСтр("ru='Ошибка при удалении задачи планировщика: ';uk='Помилка при вилученні задачі планувальника: '") + КодОшибки);
		Иначе	
			ЗаписатьИнформацию(НСтр("ru='Задача планировщика успешно удалена (код задачи: ';uk='Задачу планувальника успішно вилучено (код задачі: '") + КодЗадачи + ").");
			КодЗадачи = 0;
		КонецЕсли;		
		
		Возврат Результат;
	Исключение
		ЗаписатьОшибку(НСтр("ru='Ошибка при удалении задачи планировщика: ';uk='Помилка при вилученні задачі планувальника: '") + ОписаниеОшибки());
		Возврат Ложь;	
	КонецПопытки;
...
Функция ПроверитьДопустимуюДатуОбновления(ДатаВремя, ВыдаватьСообщения = Истина) Экспорт
	...
	Если ДатаВремя < Сейчас Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';uk='Оновлення конфігурації може бути заплановано тільки на майбутню дату й час.'"));
		КонецЕсли;	
		Возврат Ложь;
	КонецЕсли;
	Если ДатаВремя > ДобавитьМесяц(Сейчас, 1) Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';uk='Оновлення конфігурації може бути заплановано не пізніше, ніж через місяць щодо поточної дати.'"));
		КонецЕсли;	
		Возврат Ложь;
	КонецЕсли;
...
СобытиеЖурналаРегистрации = НСтр("ru='Обновление информационной базы';uk='Оновлення інформаційної бази'");
