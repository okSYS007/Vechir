...
Функция ВыборФайла(РежимДиалога, ИмяФайлаПоУмолчанию = "") Экспорт
	...
	Если РежимДиалога = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
		ДиалогВыбора.Заголовок =НСтр("ru='Выберите каталог для выгрузки документов';uk='Виберіть каталог для вивантаження документів'");
	Иначе
		ДиалогВыбора.Заголовок =НСтр("ru='Выберите файл для выгрузки данных';uk='Виберіть файл для вивантаження даних'");
	КонецЕсли;
...
Процедура ВыгрузитьКонтрагентаДок(ОбъектXML, Контрагент, Роль = Неопределено, СтруктурнаяЕдиница = Неопределено, ИмяГруппыВыгрузки = "Контрагент")
	...
	Код = ?(ЗначениеЗаполнено(СведенияОКонтрагенте.КодПоЕДРПОУ),СведенияОКонтрагенте.КодПоЕДРПОУ,СведенияОКонтрагенте.КодПоДРФО);
	ЗаписатьТекстовойУзел(ОбъектXML, "Код", 				Код);
	ЗаписатьТекстовойУзел(ОбъектXML, "ИНН", 				СведенияОКонтрагенте.КодПоЕДРПОУ);
	ЗаписатьТекстовойУзел(ОбъектXML, "НомерСвидетельства",  СведенияОКонтрагенте.НомерСвидетельства);
...
Процедура ВыгрузитьКлассификатор(ОбъектXML, ИмяПрайса, ИдКаталога, ВладелецПрайса, Товары, МассивВыгружаемойНоменклатуры)
	...
	Состояние(НСтр("ru='Выгрузка классификатора...';uk='Вивантаження класифікатора...'"));
...
Процедура ВыгрузитьСчетФактуруВыданную(ОбъектXML, СписокДок, ВыводитьИнформационныесообщения)
	...
	|	СчетФактура.Ссылка.НаАванс КАК НаАванс
	|ИЗ
...
Функция ВыгрузитьНаборДокументовВКаталог(ТипОбработки, ИмяОсновногоКаталога, ОбщееКоличествоДокументовДляВыгрузки) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяОсновногоКаталога) Тогда
		Предупреждение(НСтр("ru='Не указано имя каталога для выгрузки данных! Операция не может быть выполнена!';uk=""Не вказано ім'я каталогу для вивантаження даних! Операція не може бути виконана!"""));
		Возврат ЛОЖЬ;
	КонецЕсли;
...
Функция ВыгрузитьВФайл(ТипОбработки, ИмяФайла, СписокДокументов = Неопределено, ИнформироватьОПроцессеВыгрузки = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяФайла) Тогда
		Предупреждение(НСтр("ru='Не указано имя файла выгрузки! Операция не может быть выполнена!';uk=""Не вказано ім'я файлу вивантаження! Операція не може бути виконана!"""));
		Возврат ЛОЖЬ;
	КонецЕсли;

	Если НЕ ОпределитьВыгружаютсяДокументы(ТипОбработки) И НЕ ЗначениеЗаполнено(Организация) Тогда
		Предупреждение(НСтр("ru='Не указана организация! Операция не может быть выполнена!';uk='Не вказана організація! Операція не може бути виконана!'"));
		Возврат ЛОЖЬ;
	КонецЕсли;
	...
	Если ТипОбработки = "Каталог" Тогда
		...
		Предупреждение(НСтр("ru='Выгрузка информации о товарах завершена успешно.';uk='Вивантаження інформації про товари завершено успішно.'"));
	...
	ИначеЕсли ТипОбработки = "Прайс" Тогда
		...
		Предупреждение(НСтр("ru='Выгрузка прайс-листа завершена успешно.';uk='Вивантаження прайс-листа завершено успішно.'"));
...
Процедура ВыгрузитьИзДокумента(Документ) Экспорт
	...
	Если Метаданные.Документы[ИмяТипаДокумента].Проведение = Метаданные.свойстваОбъектов.Проведение.Разрешить
		...
		Предупреждение(НСтр("ru='Непроведенный документ не может быть выгружен!';uk='Непроведений документ не може бути вивантажений!'"));
		Возврат;
	...
	Предупреждение(НСтр("ru='Выгрузка завершена!';uk='Вивантаження завершено!'"));
...
//
Функция СформироватьПисьмо(Тема, ПутьВложения, ИмяФайлаВложения, НаименованиеВложения, ТекстПисьма, Адрес = Неопределено, Открывать = ЛОЖЬ,ЭлАдресДоп = Неопределено,АвтоОтправка = Ложь, СписокАдресовКопия = Неопределено)
	...
	Если НЕ Адрес = Неопределено Тогда
		...
		Если ЭлАдресДоп <> Неопределено Тогда		
			СписокАдресов.Добавить(ЭлАдресДоп, ЭлАдресДоп);				
		КонецЕсли;
	...
	СтруктураНовогоПисьма                      = Новый Структура("Тело, Тема, СписокФайловВложений, Кому, Копии");
	...
	//Дандали В. 22.09.2025 +
	СтруктураНовогоПисьма.Копии				   = СписокАдресовКопия;
	//Дандали В. 22.09.2025 -
	...
	Если АвтоОтправка Тогда
		СтруктураПисьма.Форма.А_АвтоОтправка = Истина;
...
Процедура ОтправитьПоПочте() Экспорт
	...
	Если СписокДоступныхЗаписей.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Для текущего пользователя не зарегистрировано ни одной учетной записи!';uk='Для поточного користувача не зареєстровано жодного облікового запису!'"));
		Возврат;
	КонецЕсли;
		...
		Для Каждого СтрокаКонтрагент Из ДеревоДокументов.Строки Цикл
			...
			Если ВыгрузитьВФайл(ТипВыгрузки, ИмяВремФайла, СписокДок, Ложь) Тогда
				...
				Если ТипЗнч(СтруктураПисьма) <> Тип("Структура") Тогда
					Сообщить(НСтр("ru='Произошла ошибка при записи письма для контрагента ';uk='Сталася помилка при запису листа для контрагента '") + СтрокаКонтрагент.Строка, СтатусСообщения.Важное);
				Иначе
...
Функция ПолучитьОтборДляАдресаОтправкиПисемДоп(Объект = Неопределено) Экспорт
	...
	ОтборРегистра.Тип    = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
	ОтборРегистра.Вид    = Справочники.ВидыКонтактнойИнформации.АдресЭлектроннойПочтыКонтрагентаДляОбменаДокументамиДополнительный;	
	
	Возврат ОтборРегистра;
	
КонецФункции

Функция ПолучитьКопииДляОтправки(Контрагент)
	
	ОтборРегистра        = Новый Структура("Объект, Тип, Вид");
	ОтборРегистра.Объект = Контрагент;
	
	ОтборРегистра.Тип    = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
	ОтборРегистра.Вид    = Справочники.ВидыКонтактнойИнформации.НайтиПоНаименованию("Адрес электронной почты контрагента для обмена электронными документами Копии", Истина);		
	
	Ресурсы = РегистрыСведений.КонтактнаяИнформация.Получить(ОтборРегистра);

	МассСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Ресурсы.Представление, ";");
	
	СЗВозврата = Новый СписокЗначений;
	Для Каждого Адрес из МассСтрок Цикл
		ПравильныйАдрес = СтрЗаменить(Адрес, " ", "");
		СЗВозврата.Добавить(ПравильныйАдрес,ПравильныйАдрес);	
	КонецЦикла;
...
//  Документ - документ-ссылка "Заказ поставщику"
//
Процедура ОтправитьИзДокумента(Документ, Параметр = Неопределено) Экспорт
	...
	Если Параметр <> Неопределено и Параметр.Свойство("АвтоОтправка") Тогда 
		Если Параметр.АвтоОтправка = Истина Тогда 
			АвтоОтправка = Истина;
		КонецЕсли;	
	Иначе
		АвтоОтправка = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ЭлАдрес) Тогда
		Предупреждение(НСтр("ru='У контрагента не обнаружен электронный адрес для обмена документами! Выгрузка невозможна!';uk='У контрагента не виявлено електронну адресу для обміну документами! Вивантаження неможливе!'"));
		Сообщить("У контрагента не обнаружен электронный адрес для обмена документами!" + "Контрагент: " + Документ.Контрагент); 
		Возврат;
	КонецЕсли;
	...
	//+NovashIV_10.11.2022
	ОтборРегистра        = ПолучитьОтборДляАдресаОтправкиПисемДоп(Документ.Контрагент);
	
	Ресурсы = РегистрыСведений.КонтактнаяИнформация.Получить(ОтборРегистра);
	ЭлАдресДоп = Ресурсы.Представление;
	
	//Дандали В. 22.09.2025 +
	СписокАдресовКопия = Новый СписокЗначений;
	Если ЗначениеЗаполнено(Документ.Контрагент) Тогда
		СписокАдресовКопия = ПолучитьКопииДляОтправки(Документ.Контрагент);
	КонецЕсли;
	//Дандали В. 22.09.2025 -
	
	
	Если ИмяТипаДокумента = "СчетНаОплатуПокупателю" Тогда
		
		//формирование файла РахунокНаОплатуПостачальника.pdf
		Док = Документ.ПолучитьОбъект();
		ИмяМакета = "СчетСПечатью";
		ФайлСПечатью = Док.ПечатьСчетаЗаказа(ИмяМакета,Истина);
		ИмяФайла = ПолучитьИмяВременногоФайла(".pdf");
		ФайлСПечатью.КоличествоЭкземпляров = 1;

		ФайлСПечатью.Записать(ИмяФайла , ТипФайлаТабличногоДокумента.PDF);
		
		ТемаТекст = Строка("Рахунок на оплату покупцю "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));	
		Тема = Строка(ТемаТекст);
		
		ТекстПисьма =  "Лист сформовано автоматично, відповідати не потрібно." + Символы.ПС + 
		"У доданому файлі знаходяться наступні електронні документи: "+ Символы.ПС + 
		Строка("Рахунок на оплату покупцю "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));		
		СтруктураПисьма = СформироватьПисьмо(Тема, ИмяФайла, "РахунокНаОплатуПостачальника.pdf", "Документи в форматі PDF", ТекстПисьма, ЭлАдрес, ИСТИНА,ЭлАдресДоп,АвтоОтправка);
		УдалитьФайлы(ИмяФайла);
		
		Док.А_ОтправленEmail = Истина;
		Док.Записать();
		
	ИначеЕсли ИмяТипаДокумента = "РеализацияТоваровУслуг" Тогда  
		ПакетнаяПечатьСоСчетом = Ложь;
		
		Док = Документ.ПолучитьОбъект();
		ФайлСПечатью = Док.ПечатьАктаОбОказанииУслуг(Истина);
		
		//Добавляю счет
		Если Параметр <> Неопределено Тогда 
			Если Параметр.Свойство("ДокСчет") <> Ложь Тогда 
				ИмяТипаДокументаСчет = ОпределитьИмяТипаДокумента(Параметр.ДокСчет);
				
				//формирование файла РахунокНаОплатуПостачальника.pdf
				ДокСчет = Параметр.ДокСчет.ПолучитьОбъект();
				ИмяМакета = "СчетСПечатью";
				ФайлСПечатьюСчет = ДокСчет.ПечатьСчетаЗаказа(ИмяМакета,Истина);
				
				ФайлСПечатью.ВывестиГоризонтальныйРазделительСтраниц();
				ФайлСПечатью.Присоединить(ФайлСПечатьюСчет);
				
				ДокСчет.А_ОтправленEmail = Истина;
				ДокСчет.Записать();
				
				ПакетнаяПечатьСоСчетом = Истина;
			КонецЕсли;		
		КонецЕсли;
		...
		Если ПакетнаяПечатьСоСчетом = Истина Тогда 
			ТемаТекст = 
			Строка("Акт про надання послуг "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D")) + Символы.ПС + 
			Строка("Рахунок на оплату покупцю "+ ДокСчет.Номер + " від " +  Формат(ДокСчет.Дата,"ДЛФ=D"));
			...
			Строка("Рахунок на оплату покупцю "+ ДокСчет.Номер + " від " +  Формат(ДокСчет.Дата,"ДЛФ=D"));
		Иначе 
			ТемаТекст = Строка("Акт про надання послуг "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));		
			
			ТекстПисьма =  "Лист сформовано автоматично, відповідати не потрібно." + Символы.ПС +
			"У доданому файлі знаходяться наступні електронні документи: "+ Символы.ПС + 
			Строка("Акт про надання послуг "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));		
		КонецЕсли;
		Тема = Строка(ТемаТекст);

				
		СтруктураПисьма = СформироватьПисьмо(Тема, ИмяФайла, "АктПроНаданняПослуг.pdf", "Документи в форматі PDF", ТекстПисьма, ЭлАдрес, ИСТИНА,ЭлАдресДоп,АвтоОтправка, СписокАдресовКопия);
		УдалитьФайлы(ИмяФайла);
		
		Док.А_ОтправленEmail = Истина;
		Док.Записать();
		
	ИначеЕсли ИмяТипаДокумента = "АктСверкиВзаиморасчетов" Тогда  
		
		
		Док = Документ.ПолучитьОбъект();
		ФайлСПечатью = Док.ПечатьАктаСверки(Истина);
		
		мВариант = ВыборВарианта();
		
		
		Если мВариант = "PDF" Тогда 
			ИмяФайла = ПолучитьИмяВременногоФайла(".pdf");
			ФайлСПечатью.Записать(ИмяФайла , ТипФайлаТабличногоДокумента.PDF);
		Иначе
			ИмяФайла = ПолучитьИмяВременногоФайла(".XLSX");
			ФайлСПечатью.Записать(ИмяФайла , ТипФайлаТабличногоДокумента.XLSX);
		КонецЕсли;
		
		ТемаТекст = Строка("Акт звірки взаєморозрахунків "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));
		Тема = Строка(ТемаТекст);
		ТекстПисьма =  "Лист сформовано автоматично, відповідати не потрібно." + Символы.ПС +
		"У доданому файлі знаходяться наступні електронні документи: "+ Символы.ПС +
		Строка("Акт звірки взаєморозрахунків "+ Документ.Номер + " від " +  Формат(Документ.Дата,"ДЛФ=D"));
		
		СтруктураПисьма = СформироватьПисьмо(Тема, ИмяФайла, "Акт звірки взаєморозрахунків.pdf", "Документи в форматі PDF", ТекстПисьма, ЭлАдрес, ИСТИНА,ЭлАдресДоп,АвтоОтправка);
		УдалитьФайлы(ИмяФайла);
