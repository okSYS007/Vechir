...
Процедура ЗаполнитьТаблицуПолей()
	...
	мФормаПрогрессора.КомментарийЗначения = НСтр("ru='Выполняется заполнение таблицы полей ...';uk='Виконується заповнення таблиці полів ...'");
	...
	Если НаименованиеПрограммы = "OutlookExpress" Тогда
		...
		Для каждого ЭлементСписка Из СписокПолей Цикл
			...
			Если СокрЛП(ЭлементСписка.Значение) = НСтр("ru='Улица';uk='Вулиця'")
				 ИЛИ СокрЛП(ЭлементСписка.Значение) = НСтр("ru='Город';uk='Місто'") 
				 ИЛИ СокрЛП(ЭлементСписка.Значение) = НСтр("ru='Почтовый индекс';uk='Поштовий індекс'") 
				 ИЛИ СокрЛП(ЭлементСписка.Значение) = НСтр("ru='Область';uk='Область'") 
				 ИЛИ СокрЛП(ЭлементСписка.Значение) = НСтр("ru='Дом. адрес, страна';uk='Будинок. адреса, країна'")
				...
				СтрокаАдреса = НастройкаПолей.Найти(НСтр("ru='Домашний адрес';uk='Домашня адреса'"), "ИмяРеквизита");
				...
				Иначе
					...
					НоваяСтрокаТаблицы.ИмяРеквизита          = НСтр("ru='Домашний адрес';uk='Домашня адреса'");
		...
		Если СписокПолей.Количество() <> ЗначенияСтроки.Количество() Тогда
			Если ФорматФайла = "ТекстCSV" Тогда
				Если мФормаПрогрессора.Открыта() Тогда
					мФормаПрогрессора.Закрыть();
				КонецЕсли; 
				Предупреждение(НСтр("ru='Выявлено несоответствие файла и типа экспортной программы.';uk='Виявлено невідповідність файлу і типу експортної програми.'"));
				Возврат Ложь;
			Иначе
...
Процедура НачатьНастройкуЗагрузки() Экспорт
	...
	Если ТаблицаДанныхФайла.Количество() = 0 Тогда
		...
		Предупреждение(НСтр("ru='Проверьте файл на наличие данных.';uk='Перевірте файл на наявність даних.'"));
		Возврат;
...
Процедура ЗаписатьЗагружаемуюИнформацию(РодительДляЗагрузки = Неопределено) Экспорт
	...
	Для каждого СтрокаТаблицы Из НастройкаЗагрузкиЗначений Цикл
		...
		Для каждого СтрокаПолей Из НастройкаПолей Цикл
				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					ИначеЕсли ТипОбъекта = "КонтактныеЛицаКонтрагентов"
						...
						Если ПреобразованноеЗначениеФайла = Неопределено Тогда
							Сообщить(НСтр("ru='Установлено несоответствие типов полей. 1С - ';uk='Встановлена невідповідність типів полів. 1С - '") + СокрЛП(СтрокаПолей.НазначениеРеквизита1С) + НСтр("ru=', файл - ';uk=', файл - '") + СокрЛП(СтрокаПолей.ИмяРеквизита));
							Продолжить;
						КонецЕсли;
			...
			ИначеЕсли СтрокаПолей.ТипРеквизита1С = "ВидКИ" Тогда
				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					ЗаписьРегистраКИ.Комментарий   = НСтр("ru='Перенсено из ';uk='Перенесено з '") + СокрЛП(НаименованиеПрограммы);
					...
					Попытка
						...
						Сообщить(НСтр("ru='Записана контактная информация (';uk='Записана контактна інформація ('") + (ЗаписьРегистраКИ.Вид) + ") " + (Объект1С.Наименование));
					Исключение
		...
		Попытка
			...
			Сообщить(НСтр("ru='Записан элемент справочника ""';uk='Записаний елемент довідника ""'") + ТипОбъекта + """ " + (Объект1С.Наименование));
