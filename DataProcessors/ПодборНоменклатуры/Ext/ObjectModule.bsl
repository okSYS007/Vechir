...
Функция ПолучитьСтрокуЗапросаСвободныеОстатки(СтруктураРегистровОстатков, ТекстУсловияКачества, ПолучатьНоменклатуры) Экспорт
	...
	Перем СтрокаЗапроса;
	ТекстУсловияКачестваОрганизации = ?(мВестиУчетПоДопРазрезамРегл, ТекстУсловияКачества, "");
	СтрокаЗапроса = "";
	...
	СоответствиеИмяРегистраУсловие.Вставить("ТоварыОрганизаций", ЭтотОбъект.мУсловиеНоменклатураСкладОрганизация + ТекстУсловияКачестваОрганизации);
	...
	Для Каждого ИмяРегистра Из СтруктураРегистровОстатков Цикл
		...
		|	" + ?((ИмяРегистра.Ключ = "ТоварыВНТТ") ИЛИ (НЕ мВестиУчетПоДопРазрезамРегл И ИмяРегистра.Ключ = "ТоварыОрганизаций") , "ЗНАЧЕНИЕ(Справочник.Качество.Новый)", "ТаблицаОстатков.Качество")+ " КАК Качество,
...
Процедура ВременнаяТаблицаОстаткиНоменклатуры(Запрос, РегистрыОстатков)
	...
	Если РегистрыОстатков = "ТоварыВНТТ" Тогда
		...
		|	ИЗ
		|		РегистрНакопления.ТоварыОрганизаций.Остатки(&Дата, " + мУсловиеНоменклатураСкладОрганизация + ТекстУсловияКачестваОрганизации + ") КАК ОстаткиОрганизации
	...
	|		ОстаткиОрганизации.Номенклатура               КАК Номенклатура,
	|		ОстаткиОрганизации.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
	|		"+ ?(мВестиУчетПоДопРазрезамРегл, "ОстаткиОрганизации.Качество", "ЗНАЧЕНИЕ(Справочник.Качество.Новый)") + " КАК Качество,
...
Функция ЗапросРасходОстаткиИЦеныНоменклатуры(Запрос, Организация, ВидПодбора)
	...
	|		И (ЕСТЬNULL(Остатки.КоличествоСвободныйОстаток, 0.00) > 0" +?(УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации(),""," И ЕСТЬNULL(Остатки.КоличествоОстатокОрганизации, 0.00) > 0") + "))
...
Функция ЗапросОстаткиНоменклатуры(Запрос, Организация)
	...
	|	ИЛИ
	|	(Остатки.КоличествоСвободныйОстаток > 0"+?(УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации(),""," И Остатки.КоличествоОстатокОрганизации > 0")+"))
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьСтрокуУсловия(ПоСправочнику, ИмяУсловия, Склад = Неопределено, Организация = Неопределено) Экспорт
	...
	ИначеЕсли ИмяУсловия = "НоменклатураСкладОрганизация" Тогда
		мПревышатьОстаткиПоОрганизации = УправлениеДопПравамиПользователей.РазрешеноПревышениеОстаткаТоваровОрганизации(Ложь);
...
Процедура мПриОткрытии(Форма) Экспорт
	...
	Форма.Заголовок = НСтр("ru='Количество и цена ""';uk='Кількість і ціна ""'") + Форма.Номенклатура + """";
	...
	Иначе
		...
		Если СтруктураИсходныхПараметров.Свойство("ЕстьКоличество" , ЕстьКоличество) И НЕ ЕстьКоличество Тогда
			...
			Форма.ЭлементыФормы.ТаблицаХарактеристикНоменклатуры.Колонки.Количество.ТекстШапки   = НСтр("ru='Подбирать в документ';uk='Підбирати в документ'");
		КонецЕсли;
...
Процедура мТаблицаХарактеристикНоменклатурыХарактеристикаПриИзменении(Форма, Элемент) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Характеристика)
	 Или ТипЗнч(СтрокаТабличнойЧасти.Характеристика) = Тип("Строка") Тогда
		СтрокаТабличнойЧасти.Характеристика = НСтр("ru='<без характеристики>';uk='<без характеристики>'");
	КонецЕсли;
...
Процедура мСерияНачалоВыбораИзСписка(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если СписокСерий.Количество() = 0 Тогда
		...
		Для каждого Строка Из ТаблицаСерий Цикл
			...
			Иначе
				СписокСерий.Добавить(Строка.Серия, (?(ПустаяСтрока(Строка(Строка.Серия)), НСтр("ru='<без серии>';uk='<без серії>'"), Строка(Строка.Серия)) + " (" + СокрЛП(Формат(Строка.Количество, "ЧЦ=15; ЧДЦ=3")) + " " + СокрЛП(Форма.Номенклатура.ЕдиницаХраненияОстатков) + ")"));
			КонецЕсли;
		КонецЦикла;
...
Процедура мКоманднаяПанельТаблицыХарактеристикНоменклатурыЗаполнить(Форма) Экспорт
	...
	Пока СтрокаПустойХарактеристики <> Неопределено Цикл
		СтрокаПустойХарактеристики.Характеристика = НСтр("ru='<без характеристики>';uk='<без характеристики>'");
		СтрокаПустойХарактеристики = Форма.ТаблицаХарактеристикНоменклатуры.Найти(Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка(), "Характеристика");
	КонецЦикла;
...
Процедура мКоманднаяПанельДействийСлеваПолучитьВес(Форма, Кнопка) Экспорт
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		...
		КонецЕсли;
	Иначе
		Предупреждение(ПолучитьСерверТО().ПолучитьТекстОшибкиЭВТО(Ответ), 5,НСтр("ru='Ошибка ТО';uk='Помилка ТО'"));
	КонецЕсли;
...
УчетнаяПолитикаПоСкладу  = ?(БУ.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Склады, "ВидСубконто") = Неопределено, Ложь, Истина);
#КонецЕсли
