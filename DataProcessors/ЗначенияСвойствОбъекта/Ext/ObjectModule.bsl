...
Процедура ОткрытьФормуНовогоСвойства(ФормаВладелец, Элемент = Неопределено, Копирование = Ложь) Экспорт
	...
	Иначе
		...
		Если НазначениеСвойств.Количество() > 1  Тогда
			ВыбранныйЭлемент = НазначениеСвойств.ВыбратьЭлемент(НСтр("ru='Выбор назначения свойства';uk='Вибір призначення властивості'"));
...
Процедура ИнвертироватьПометкуУдаленияСвойства(ТекущаяСтрока) Экспорт
	...
	Исключение
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='Не удалось изменить пометку удаления свойства:';uk='Не вдалося змінити позначку вилучення властивості:'") + Символы.ПС + ОписаниеОшибки());
		#КонецЕсли
	КонецПопытки;
