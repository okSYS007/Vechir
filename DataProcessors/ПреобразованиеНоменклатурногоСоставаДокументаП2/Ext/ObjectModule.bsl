...
Процедура ЗаполнитьНоменклатурнымСоставом(СообщенияВПроцессеЗаполнения = Неопределено) Экспорт
	...
	|	Приложение2КНалоговойНакладной.УчитыватьНДС,
	...
	|	Приложение2КНалоговойНакладнойНомСостав.ХарактеристикаНоменклатуры,
	|	Приложение2КНалоговойНакладнойНомСостав.Коэффициент,
	|	Приложение2КНалоговойНакладнойНомСостав.Количество,
	|	Приложение2КНалоговойНакладнойНомСостав.КоличествоМест,
	...
	|	Приложение2КНалоговойНакладнойНомСостав.ПроцентАвтоматическихСкидок,
	...
	|	Приложение2КНалоговойНакладнойНомСостав.ЕдиницаИзмерения.ЕдиницаПоКлассификатору.Код КАК ЕдиницаИзмеренияКод,
	|	Приложение2КНалоговойНакладнойНомСостав.ЕдиницаИзмеренияМест,
...
Процедура ИзменениеКоличестваПриИзменении(СтрокаТЧ, ИмяТабЧасти) Экспорт
...
Процедура ИзменениеЦеныПриИзменении(СтрокаТЧ, ИмяТабЧасти) Экспорт
...
Процедура УстановитьКодУКТВЭДВстрокеТоваров(СтрокаТабличнойЧасти, ЕстьХарактеристики = Ложь) Экспорт
...
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ЗначениеПустогоКоличества = 1, ИмяТабличнойЧасти, СпособРасчета = Неопределено) Экспорт
...
Процедура РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти, ИмяТабличнойЧасти) Экспорт
...
Процедура ЗаполнитьСодержаниеТабЧасти(СтрокаТабличнойЧасти, ИмяТабличнойЧасти) Экспорт

	МетаданныеДокумента = СтрокаТабличнойЧасти.Ссылка.Метаданные();
...
Процедура ВставитьИмяТЧвСообщение(Сообщение, ИмяТЧ, Параметр) Экспорт
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.НомерСтрокиНН) Тогда
					Если НайденныеСтроки[0].НомерСтрокиНН = 0 Тогда
						СтрокаТЧ.НеНайденаСтрокаННПричина = НСтр("ru='Строка НН найдена, но в ней не указано значение номера строки.';uk='Рядок ПН знайдена, але в ній не зазначено значення номера рядка.'");
					КонецЕсли; 
	...
	Для каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.ЦенаБезНДССУчетомКорректировок) 
			...
			Если СуммаВключаетНДС Тогда
				...
				СтрокаТЧ.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(СтрокаТЧ.Цена, 
																						 Неопределено,
																						 Ложь,
																						 Истина,
																						 Истина, 
...
"Цена получена с учетом предыдущих корректировок данной строки другими документами ""Приложение 2 к налоговой накладной""!';uk='У рядку %1 табличної частини ""%2"" змінена ціна. Попереднє значення - %3, нове - %4."
"Ціна отримана з урахуванням попередніх коригувань даного рядк іаншими документами ""Додаток 2 до податкової накладної""!'"),
		...
		Если (ИмяТабЧасти = "Товары" Или ИмяТабЧасти = "Услуги") Тогда 
			РассчитатьСуммуТабЧасти(СтрокаТЧ,, ИмяТабЧасти);
		КонецЕсли;
		РассчитатьСуммуНДСТабЧасти(СтрокаТЧ, ИмяТабЧасти);	
		РассчитатьСуммыИзменений(СтрокаТЧ, ИмяТабЧасти);
...
Функция ПолучитьЦенуБезНДСРеглДокументП2(СтрокаТаблицы, ИмяТабЧасти)
	...
	Если ИмяТабЧасти = "ОС" Или ИмяТабЧасти = "НМА" Тогда
		...
		Количество = 1;
	Иначе
		ПроцентСкидкиНаценки = СтрокаТаблицы.ПроцентСкидкиНаценки + СтрокаТаблицы.ПроцентАвтоматическихСкидок;
	...
	Для каждого СтрокаТЧ Из СтрокиНомСоставаДокумента Цикл
		...
		ИначеЕсли СтрокаТЧ.ПричинаКорректировкиНовая = "Зміна кількості"
			...
			Если ОстатокУПокупателя = 0 Тогда 
				ТекстСообщения = НСтр("ru='В табличной части ""%1"" для строки №%2 указана причина ""Зміна кількості"", но выполняется полный возврат!';uk='У табличній частині ""%1"" для рядка №%2 вказана причина ""Зміна кількості"", але виконується повне повернення!'");   
...
Функция ПолучитьТаблицуНоменклатурыНалоговогоДокументаДляПечати(Шапка)
	...
	Запрос.УстановитьПараметр("НалоговаяНакладная", НалоговыйДокументОбъект.Ссылка);
	...
	Если Не РезультатЗапроса.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка 
				...
				ДокументПриложение2Объект = Выборка.Ссылка.ПолучитьОбъект();
				
				ПечатнаяФормаП2 = ДокументПриложение2Объект.ПечатьДокумента(Истина); 
...
"Необходимо предварительно зайти в налоговую накладную, записать документ, после чего повторить действие!';uk='Не встановлені номери рядків ПН (або вони не актуальні) в документі податкової накладної, який коригується поточним документом Додаток 2."
"Автоматичне встановлення  коригованих номерів рядків ПН може бути не правильним або не виконаним зовсім."
