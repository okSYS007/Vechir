...
КонецФункции // ОбщегоНазначения.ЗначениеНеЗаполнено()
...
Функция мВыбратьЗначение(Значение, ОписаниеТипов, СвязьПоТипу, ЭлементСвязиПоТипу,СвязьПоВладельцу,ВыборГруппы = Ложь)
	...
	Иначе
		...
		Если ЭлементСвязиПоТипу > ВидыСубконто.Количество() Тогда
			...
			Предупреждение(НСтр("ru='Данная ячейка не может содержать значение';uk='Дана комірка не може містити значення'"));
			Возврат ложь;
		КонецЕсли;
...
Функция мПривестиКЧислу(Представление, Знач ОписаниеТипов = Неопределено, Примечание = "")
	...
	Исключение
		Примечание = НСтр("ru='Неправильный формат числа';uk='Неправильний формат числа'");
		Возврат 0;
	КонецПопытки;
	...
	Если Не Результат1 = Результат Тогда
		Примечание = НСтр("ru='Недопустимое числовое значение';uk='Неприпустиме числове значення'");
	КонецЕсли;
...
Функция мПривестиКДате(Представление, ТипРеквизита, Примечание = "")
	...
	Если Результат = '00010101' Тогда
		...
		Если ТипРеквизита.КвалификаторыДаты.ЧастиДаты = ЧастиДаты.Время Тогда
			...
			Исключение
				Примечание = НСтр("ru='Неправильный формат даты';uk='Неправильний формат дати'");
			КонецПопытки;
...
Функция мПрочитатьТабличныйДокументИзExcel(ТабличныйДокумент, ИмяФайла, НомерЛистаExcel = 1) Экспорт
	...
	Если НЕ ВыбФайл.Существует() Тогда
		Сообщить(НСтр("ru='Файл не существует!';uk='Файл не існує!'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Попытка
		...
		Состояние(НСтр("ru='Обработка файла Microsoft Excel...';uk='Обробка файлу Microsoft Excel...'"));
		ExcelЛист = Excel.Sheets(НомерЛистаExcel);
	Исключение
		Сообщить(НСтр("ru='Ошибка. Возможно неверно указан номер листа книги Excel.';uk='Помилка. Можливо невірно вказаний номер аркуша книги Excel.'"));
		Возврат ложь;
...
Функция мПрочитатьТабличныйДокументИзТекста(ТабличныйДокумент, ИмяФайла) Экспорт
	...
	Исключение
		Сообщить(НСтр("ru='Ошибка открытия файла!';uk='Помилка відкриття файлу!'"));
		Возврат Ложь;
	КонецПопытки;
...
Процедура ВыбратьЗначениеВЯчейке(ТабличныйДокумент) Экспорт
	...
	Если ТекущаяОбласть.Верх <> ТекущаяОбласть.Низ Или ТекущаяОбласть.Лево <> ТекущаяОбласть.Право Тогда
		Предупреждение(НСтр("ru='Для непосредственного выбора значения необходимо выбирать только одну ячейку';uk='Для безпосереднього вибору значення необхідно вибирати тільки одну клітинку'"));
		Возврат;
	КонецЕсли;
	...
	Если ТекущаяКолонка = Неопределено  Тогда
		...
		Предупреждение(НСтр("ru='Значение данной колонки не выбирается';uk='Значення цієї колонки не вибирається'"));
		Возврат;
...
Процедура ОбновитьДанныеТабличногоДокумента(ТабличныйДокумент, БезВопросов = Ложь) Экспорт
	...
	Если (РежимЗагрузки = 0 или РежимЗагрузки = 2) и ТабличныйДокумент.ВысотаТаблицы > 1 И Не БезВопросов Тогда
		Результат = Вопрос(НСтр("ru='Табличный документ содержит данные. Очистить?';uk='Табличний документ містить дані. Очистити?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор) Экспорт
	...
	Если ТаблицаЗагружаемыхРеквизитов.НайтиСтроки(Новый Структура("Пометка",Истина)).Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не отмечен ни один загружаемый реквизит!';uk='Не відзначений жоден реквізит, що завантажується!'"));
		Возврат Ложь;
	КонецЕсли;
	
	КоличествоЭлементов = ТабличныйДокумент.ВысотаТаблицы - ПерваяСтрокаДанныхТабличногоДокумента + 1;
	Если КоличествоЭлементов <= 0 Тогда
		Предупреждение(НСтр("ru='Нет данных для загрузки';uk='Немає даних для завантаження'"));
		Возврат Ложь;
	КонецЕсли;
	
	Если РежимЗагрузки = 0 Тогда
		Источник = СсылкаИсточника;
		ТекстВопросаИсточника = НСтр("ru=' элементов в справочник: ""';uk=' елементів у довідник: ""'") + МетаданныеИсточника.Представление() + """";
		
	ИначеЕсли РежимЗагрузки = 1 Тогда
		
		Если СсылкаИсточника.Пустая() Тогда
			Предупреждение(НСтр("ru='Не выбрана ссылка';uk='Не вказано посилання'"));
			Возврат Ложь;
		КонецЕсли;
		 
		ОбъектИсточника = СсылкаИсточника.ПолучитьОбъект();
		Источник = ОбъектИсточника[ТабличнаяЧастьИсточника];
		ТекстВопросаИсточника = НСтр("ru=' строк в табличную часть: ""';uk=' рядків в табличну частину: ""'") + МетаданныеИсточника.Представление() + """";
		
	ИначеЕсли РежимЗагрузки = 2 Тогда
		
		ТекстВопросаИсточника = НСтр("ru=' записей в регистр сведений: ""';uk=' записів в регістр відомостей ""'") + МетаданныеИсточника.Представление() + """";
		
	КонецЕсли;
	
	Если Вопрос(НСтр("ru='Загрузить ';uk='Завантажити '")+КоличествоЭлементов  + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		ОчиститьСообщения();
		Сообщить(НСтр("ru='Выполняется загрузка';uk='Виконується завантаження'")+ ТекстВопросаИсточника, СтатусСообщения.Информация);
		Сообщить(НСтр("ru='Всего: ';uk='Всього: '") + КоличествоЭлементов, СтатусСообщения.Информация);
		...
		Для К = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
			...
			Если РежимЗагрузки =0 Тогда
				...
				Если Не Запрос = Неопределено Тогда
					...
					Если Не ПустаяСтрока(СтрокаОшибок) Тогда
						Сообщить(НСтр("ru='Строка ';uk='Рядок '") + НомерТекущейСтроки + НСтр("ru=' не может быть записана.Не указано значение ключевых реквизитов: ';uk=' не може бути записана.Не вказано значення ключових реквізитів: '") + СтрокаОшибок,СтатусСообщения.Важное);
			...
			Для каждого КлючИЗначение Из ТекущаяСтрока Цикл
				...
				Если Не ОбъектНайден или Колонки[КлючИЗначение.Ключ].Пометка  Тогда
					Попытка
						Объект[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
					Исключение
						мСообщитьОбОшибке(НСтр("ru='Ошибка при установки значения реквизита ""';uk='Помилка при встановленні значення реквізиту ""'") + КлючИЗначение.Ключ + "" + ОписаниеОшибки());
						Отказ = Истина;
						Прервать;
					КонецПопытки;
				КонецЕсли;
			...
			Если РежимЗагрузки = 0 Тогда
				Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
					Сообщить(?(ОбъектНайден,НСтр("ru='Изменен';uk='Змінений'"),НСтр("ru='Загружен';uk='Завантажений'")) + НСтр("ru=' элемент справочника: ';uk=' елемент довідника: '") + Объект.ссылка, СтатусСообщения.Информация);
					Загружено = Загружено + 1;
				Иначе
					Сообщить(НСтр("ru='Объект не ';uk=""Об'єкт не """) + ?(ОбъектНайден,"изменен","загружен") + НСтр("ru='. Элемент справочника: ';uk='. Елемент довідника: '") + Объект + ".", СтатусСообщения.Важное);
				КонецЕсли;
			ИначеЕсли РежимЗагрузки = 1 Тогда
				...
				Если Не Отказ Тогда
					Сообщить(НСтр("ru='Добавлена строка: ';uk='Доданий рядок: '") + (Загружено + 1));
				Иначе
					Сообщить(НСтр("ru='При добавлении строки ';uk='При додаванні рядка '") + (Загружено + 1) + НСтр("ru=' возникли ошибки. ';uk=' виникли помилки. '"));
			...
			ИначеЕсли РежимЗагрузки = 2 Тогда
				Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
					Сообщить(?(ОбъектНайден,НСтр("ru='Изменена';uk='Змінена'"),НСтр("ru='Добавлена';uk='Додана'")) + НСтр("ru=' запись № ';uk=' запис № '") + НомерТекущейСтроки + ".");
					Загружено = Загружено + 1;
				Иначе
					Сообщить(НСтр("ru='Запись не ';uk='Запис не '") + ?(ОбъектНайден,"изменена","загружена") + НСтр("ru='. № записи: ';uk='. № запису: '") + НомерТекущейСтроки + ".", СтатусСообщения.Важное);
				КонецЕсли;
			КонецЕсли;
		...
		Если РежимЗагрузки = 1  Тогда
			Если ЗаписыватьОбъект и ЗаписатьОбъект(ОбъектИсточника) Тогда
				...
				Сообщить(НСтр("ru='Выполнена загрузка';uk='Виконано завантаження'")+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
				Сообщить("" +Загружено +НСтр("ru=' из ';uk=' з '")+ КоличествоЭлементов + НСтр("ru=' элементов.';uk=' елементів.'"), СтатусСообщения.Информация);
				Возврат Истина;
			Иначе
				Сообщить(НСтр("ru='Объект не записан: ';uk=""Об'єкт не записаний: """) + Объект + ".", СтатусСообщения.Важное);
				Возврат Ложь;
			КонецЕсли;
		...
		ИначеЕсли РежимЗагрузки = 2 Тогда
			Сообщить(НСтр("ru='Выполнена загрузка';uk='Виконано завантаження'")+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
			Сообщить("" +Загружено +НСтр("ru=' из ';uk=' з '")+ КоличествоЭлементов + НСтр("ru=' записей.';uk=' записів.'"), СтатусСообщения.Информация);
			Возврат Истина;
		КонецЕсли;
...
Процедура КонтрольЗаполнения(ТабличныйДокумент, Индикатор) Экспорт
	...
	Для К = 0  По КоличествоЭлементов - 1 Цикл
		Состояние(НСтр("ru='Выполняется контроль заполнения строки № ';uk='Виконується контроль заповнення рядка № '") + (К + 1));
	...
	Сообщить(НСтр("ru='Контроль заполнения завершен. Проверено строк: ';uk='Контроль заповнення завершений. Перевірено рядків: '") + КоличествоЭлементов);
	Если КоличествоОшибок Тогда
		Сообщить(НСтр("ru='Выявлено ячеек, содержащих ошибки/неоднозначное представление: ';uk='Виявлено клітинок, що містять помилки/неоднозначне представлення: '") + КоличествоОшибок);
	Иначе
		Сообщить(НСтр("ru='Ячеек, содержащих ошибки не выявлено';uk='Комірок, що містять помилки не виявлено'"));
	КонецЕсли;
		...
		Если Колонка.Пометка Тогда
			...
			ИначеЕсли Колонка.РежимЗагрузки = "Вычислять" Тогда
				...
				Если Не ПустаяСтрока(Примечание) Тогда
					Сообщить(НСтр("ru='Строка [';uk='Рядок ['")+НомерСтроки+"]("+Колонка.ПредставлениеРеквизита+"): "+ Примечание);
					КоличествоОшибок = КоличествоОшибок + 1;
				КонецЕсли;
...
Функция ОбработатьОбласть(Область, Колонка, ТекущиеДанные, ТекстыЯчеек)
	...
	Иначе
		...
		Если НайденныеЗначения.Количество() = 0 Тогда
			...
			Примечание = НСтр("ru='Не найден';uk='Не знайдений'")+?(Примечание = "","", Символы.ПС+Примечание);
			Результат = Неопределено;
		...
		Иначе
			...
			Примечание = НСтр("ru='Не однозначное представление. Вариантов: ';uk='Не однозначне представлення. Варіантів: '")+НайденныеЗначения.Количество()+?(Примечание = "","", Символы.ПС+Примечание);
	...
	Если Не ПустаяСтрока(Примечание) Тогда
		Сообщить(НСтр("ru='Ячейка[';uk='Комірка['")+Область.Имя+"]("+Колонка.ПредставлениеРеквизита+"): " + Примечание);
	КонецЕсли;
	...
	Для каждого ТипРеквизита Из Тип.Типы() Цикл
		...
		Иначе
			...
			ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипРеквизита) Тогда
				...
				Иначе
					...
					Если Лев(Представление, ДлиннаСинонима) = ""+МетаданныеТипа Тогда
						...
						ПозицияОт = Найти(НомерИДата, НСтр("ru=' от ';uk=' від '"));
			...
			ИначеЕсли Не МетаданныеТипа = Неопределено Тогда
				...
				КонецЦикла;
			Иначе
				Примечание = НСтр("ru='Не описан способ поиска';uk='Не описаний спосіб пошуку'");
				Примечание = НСтр("ru='Для Колонки не определен тип значения';uk='Для Колонки не визначений тип значення'");
			КонецЕсли;
		КонецЕсли;
...
Функция мПолучитьНастройки() Экспорт
	...
	Если РежимЗагрузки = 0 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = НСтр("ru='в справочник';uk='в довідник'");
	ИначеЕсли РежимЗагрузки = 1 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = НСтр("ru='в табличную часть';uk='у табличну частину'");
	ИначеЕсли РежимЗагрузки = 2 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = НСтр("ru='в регистр сведений';uk='у регістр відомостей'");
	КонецЕсли;
	...
	Если ЗагружаемыйРеквизит.ОписаниеТипов.Типы().Количество() = 1 Тогда
		...
		Если ЭтоСправочник ИЛИ ЭтоСчет Или ЭтоВидХарактеристик Тогда
			...
			Если МетаданныеТипа.ОсновноеПредставление = ВидОсновногоПредставление.ВВидеКода Тогда
				...
				Если ЕстьИмя Тогда
					СписокВыбора.Добавить("Наименование", НСтр("ru='Наименование';uk='Найменування'"));
				КонецЕсли;
...
Функция ПолучитьСписокСвязейПоВладельцу(ЗагружаемыйРеквизит) Экспорт
	...
	Если Не ТипыВладельцев.Количество() = 0 Тогда
		СписокВыбора.Добавить(Неопределено, НСтр("ru='< пустое значение >';uk='< пусте значення >'"));
	КонецЕсли;
	...
	Если ЕстьТипСамогоОбъекта Тогда
		...
		СписокВыбора.Вставить(0,"<Создаваемый объект>", НСтр("ru='<Создаваемый объект>';uk=""<Об'єкт, що створюється>"""));
...
Процедура ЗаполнитьНастройкиКолонок(Настройки) Экспорт
	...
	Если ТипЗнч(Настройки) = Тип("ТабличныйДокумент") Тогда
		...
		Попытка
			...
			Если ТекстВосстановленногоРежимаЗагрузки = НСтр("ru='в справочник';uk='в довідник'") или ТекстВосстановленногоРежимаЗагрузки = "" Тогда
				ВосстановленныйРежимЗагрузки = 0;
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = НСтр("ru='в табличную часть';uk='у табличну частину'") или ТекстВосстановленногоРежимаЗагрузки = "Х" Тогда
				ВосстановленныйРежимЗагрузки = 1;
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = НСтр("ru='в регистр сведений';uk='у регістр відомостей'") Тогда
			...
			Если МетаданныеОбъекта = Неопределено Тогда
				ВызватьИсключение НСтр("ru='Неправильный формат файла';uk='Неправильний формат файла'");
			КонецЕсли;
			...
			Пока Не ПустаяСтрока(ИмяРеквизита) Цикл
				...
				Для к = 1 По СтрЧислоСтрок(ОписаниеТиповСтрокой)  Цикл
					...
					ИначеЕсли МассивЧастейТипа[0] = НСтр("ru='дата и время';uk='дата і час'") Тогда
						Тип = Тип("Дата");
