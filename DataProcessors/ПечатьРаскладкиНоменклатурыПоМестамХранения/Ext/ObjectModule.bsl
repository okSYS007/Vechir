...
Процедура ПроверитьКорректностьНастроек(Отказ)
	...
	Если Не ЗначениеЗаполнено(Документ) Тогда
		Предупреждение(НСтр("ru='Не выбран документ для печати!';uk='Не вибраний документ для друку!'"));
		Отказ = Истина;
	...
	Если КоличествоУровнейМестХранения = 0 Тогда
		Предупреждение(НСтр("ru='Не указано количество уровней мест хранения!';uk='Не зазначена кількість рівнів місць зберігання!'"));
		Отказ = Истина;
	...
	Если ИзмерениеНоменклатура = Неопределено Тогда
		Предупреждение(НСтр("ru='Отсутствует обязательная группировка но номенклатуре!';uk=""Відсутнє обов'язкове групування по номенклатурі!"""));
	...
	Если Индекс <> КоличествоИзмерений Тогда
		Предупреждение (НСтр("ru='Группировка: ""Номенклатура"" должна быть последней в измерениях строк.';uk='Групування: ""Номенклатура"" повинна бути останньою у вимірах рядків.'"));
	...
	Для Каждого СтрокаПостроителя Из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		Иначе
			Предупреждение(НСтр("ru='Повторяющаяся группировка ';uk='Повторюване групування '") + СтрокаПостроителя.Представление + "." + Символы.ПС +
...
Процедура Печать(НаПринтер = Ложь) Экспорт
	...
	мМакет.КодЯзыкаМакета = КодЯзыкаПечать;

	Если ВыводитьЗаголовок Тогда // выведем заголовок
		Секция = мМакет.ПолучитьОбласть("Заголовок");

		Секция.Параметры.ТекстЗаголовка = СокрЛП(Локализация.ПолучитьЛокализованноеПредставление(Документ, КодЯзыкаПечать));
		Секция.Параметры.Склад = СокрЛП(мСклад);
		мТабДок.Вывести(Секция);
	КонецЕсли;

	Секция = мМакет.ПолучитьОбласть("ВремяПечати");
	Секция.Параметры.ВремяПечати = НСтр("ru='Дата и время печати: ';uk='Дата та час друку: '",КодЯзыкаПечать) + ТекущаяДата() + НСтр("ru='. Пользователь: ';uk='. Користувач: '",КодЯзыкаПечать) + СокрЛП(глЗначениеПеременной("глТекущийПользователь"));
	мТабДок.Вывести(Секция);
	...
	Для Каждого ДопПоле Из ТаблицаНастройкиПолейПечать Цикл
		Если ДопПоле.ВыводитьНаПечать И ДопПоле.Положение = НСтр("ru='До номенклатуры';uk='До номенклатури'") Тогда
	...
	Для Каждого ДопПоле Из ТаблицаНастройкиПолейПечать Цикл
		Если ДопПоле.ВыводитьНаПечать И ДопПоле.Положение = НСтр("ru='Вместе с номенклатурой';uk='Разом з номенклатурою'") Тогда
	...
	ТекДопПоле = 0;
	Для Каждого ДопПоле Из ТаблицаНастройкиПолейПечать Цикл
		Если ДопПоле.ВыводитьНаПечать И ДопПоле.Положение = НСтр("ru='После номенклатуры';uk='Після номенклатури'") Тогда
			ТекДопПоле = ТекДопПоле + 1;
	...
	Секция.Параметры.ИтогВес = НСтр("ru='Общий вес товаров: ';uk='Загальна вага товарів: '",КодЯзыкаПечать) + мИтогВес + " " + мЕдиницаИзмеренияВеса + "."
	+ ?(мВесВведенНеДляВсехТоваров, НСтр("ru=' Вес введен не для всех товаров.';uk=' Вага введена не для всіх товарів.'",КодЯзыкаПечать), "");
	мТабДок.Вывести(Секция);

	УниверсальныеМеханизмы.НапечататьДокумент(мТабДок, , НаПринтер, НСтр("ru='Бланк товарного наполнения';uk='Бланк товарного наповнення'"));
...
Процедура ЗаполнитьПостроительОтчета(НеЗаполнятьНастройки = Ложь) Экспорт
	...
	"Номенклатура, Количество, ЕдиницаИзмерения, Остаток, МестоХранения , МестоХраненияРодитель, СкладОтгрузкиОприходования, Вес",
	НСтр("ru='Номенклатура';uk='Номенклатура'",КодЯзыкаПечать), 
	НСтр("ru='Количество';uk='Кількість'",КодЯзыкаПечать), 
	НСтр("ru='Ед. изм.';uk='Од. вим.'",КодЯзыкаПечать), 
	НСтр("ru='Свободный остаток';uk='Вільний залишок'",КодЯзыкаПечать), 
	НСтр("ru='Место хранения';uk='Місце зберігання'",КодЯзыкаПечать), 
	НСтр("ru='Группа мест хранения';uk='Група місць зберігання'",КодЯзыкаПечать), 
	НСтр("ru='Склад отгрузки (оприходования)';uk='Склад відвантаження (оприбуткування)'",КодЯзыкаПечать),
	НСтр("ru='Вес';uk='Вага'",КодЯзыкаПечать));
...
мСписокПоложенийДляПечати.Добавить(НСтр("ru='После номенклатуры';uk='Після номенклатури'"));
мСписокПоложенийДляПечати.Добавить(НСтр("ru='До номенклатуры';uk='До номенклатури'"));
мСписокПоложенийДляПечати.Добавить(НСтр("ru='Вместе с номенклатурой';uk='Разом з номенклатурою'"));
