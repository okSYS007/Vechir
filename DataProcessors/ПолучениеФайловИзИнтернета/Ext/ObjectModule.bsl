	...
	Если Не ПерваяПопытка Тогда
		...
		ФормаПараметрыДоступаНаСервер.ЭлементыФормы.Найти("НадписьУкажитеПараметры").Заголовок =
		  ?(ПустаяСтрока(СообщениеДляАвторизации),
		    НСтр("ru='Укажите логин/код и пароль для доступа к серверу ""';uk='Вкажіть логін/код та пароль для доступу до сервера ""'") + мСерверИсточник + """",
		...
		Если Найти(ОписаниеОшибкиСоединения, "407") <> 0
		 ...
		 Или  Найти(НРег(ОписаниеОшибкиСоединения), "аутентификация") <> 0) Тогда
			Если Не ПустаяСтрока(ИмяПользователя) Тогда
				Сообщить(НСтр("ru='Ошибка! Указаны неверные имя пользователя или пароль прокси-сервера';uk=""Помилка! Вказані неправильні ім'я користувача або пароль проксі-сервера"""), СтатусСообщения.Важное);
			КонецЕсли;
			...
			Иначе
				...
				Прервать;
			КонецЕсли;
		ИначеЕсли Найти(НРег(ОписаниеОшибкиСоединения), НСтр("ru='ошибка аутентификации при доступе к ресурсу';uk='помилка аутентифікації при доступі до ресурсу'")) <> 0 Тогда
			Если Не ПустаяСтрока(КодДоступаНаСервер) Тогда
				Сообщить(НСтр("ru='Ошибка! Указаны неверные логин/код или пароль доступа на сервер';uk='Помилка! Вказані неправильні логін/код або пароль доступу на сервер'"), СтатусСообщения.Важное);
			КонецЕсли;
