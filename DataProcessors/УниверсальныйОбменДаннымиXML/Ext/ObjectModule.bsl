...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			...
			Если ФлагОшибки Тогда
				Предупреждение(НСтр("ru='При выгрузке обработчиков были обнаружены ошибки.';uk='При вивантаженні обробників були виявлені помилки.'"));
				
			Иначе
				
				Предупреждение(НСтр("ru='Обработчики успешно выгружены.';uk='Обробники успішно вивантажені.'"));
			КонецЕсли;
	...
	#Если Клиент Тогда
		Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0 
			...
			Попытка
				...
				Состояние(НСтр("ru='Выгрузка объектов ""';uk='Вивантаження об''єктів ""'") + Строка(Источник) + 
					НСтр("ru='"". Выгружено: ';uk='"". Вивантажено: '") + Строка(мСчетчикВыгруженныхОбъектов) +
					НСтр("ru=' Объект: ';uk="" Об'єкт: """) + ИмяМетаданных);
					
			Исключение
				
				Состояние(НСтр("ru='Выгрузка объектов ""';uk='Вивантаження об''єктів ""'") +  
					НСтр("ru='"". Выгружено: ';uk='"". Вивантажено: '") + Строка(мСчетчикВыгруженныхОбъектов) +
					НСтр("ru=' Объект: ';uk="" Об'єкт: """) + ИмяМетаданных);
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			...
			Текст = НСтр("ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": ';uk='Обробник: ""ПослеЗагрузкиПравилОбмена"": '") + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()); 
			СообщитьПользователю(Текст);
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Исключение
		...
		РезультирующаяСтрокаСИнформациейОбОшибке = ЗаписатьВПротоколВыполнения(НСтр("ru='Ошибка при загрузке данных: ';uk='Помилка при завантаженні даних: '") + СтрокаОшибки, Неопределено, Истина, , , Истина);
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Начало загрузки:   ';uk='Початок завантаження:   '") + ТекущаяДатаСеанса(), , Ложь, , , Истина);
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Окончание загрузки: ';uk='Закінчення завантаження: '") + ТекущаяДатаСеанса(), , Ложь, , , Истина);
	...
	#Если Клиент Тогда		
		Если ЭтоИнтерактивныйРежим Тогда			
			...
			Предупреждение(НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'"));			
		КонецЕсли;
	#КонецЕсли
...
Функция РаспаковатьZipФайл(ИмяФайлаДляРаспаковки)
	...
	Попытка
		...
		#Если Клиент Тогда
		Состояние(НСтр("ru='Выполняется распаковка файла ...';uk='Виконується розпакування файлу ...'"));
		#КонецЕсли
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Начало выгрузки:   ';uk='Початок вивантаження:   '") + ТекущаяДатаСеанса(), , Ложь, , , Истина);
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='Ошибка при выгрузке данных: ';uk='Помилка при вивантаженні даних: '") + СтрокаОшибки, Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Окончание выгрузки: ';uk='Закінчення вивантаження: '") + ТекущаяДатаСеанса(), , Ложь, , ,Истина);
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
			Предупреждение(НСтр("ru='Выгрузка данных завершена.';uk='Вивантаження даних завершено.'"));		
		КонецЕсли;	
	#КонецЕсли
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных ';uk=""Помилка отримання властивості підпорядкованого об'єкта з вхідних даних """));
	...
	одСообщения.Вставить(73, НСтр("ru='Ошибка в обработчике события ПоследовательностьПолейПоиска ';uk='Помилка в оброблювачі події ПоследовательностьПолейПоиска '"));
	...
	одСообщения.Вставить(77, НСтр("ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий.';uk='Не зазначений файл зовнішньої обробки з процедурами, що підключають обробники подій.'"));
	
	одСообщения.Вставить(78, НСтр("ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий.';uk='Помилка створення зовнішньої обробки з файлу із процедурами обробників подій.'"));
