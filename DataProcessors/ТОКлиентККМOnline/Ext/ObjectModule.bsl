...
Функция ОбработатьЗапросККМ(Номенклатура, Характеристика, Серия, Качество,
	...
	Если      НЕ ЗначениеЗаполнено(мСклад) Тогда
		Сигнал();
		Сообщение = НСтр("ru='Не задан склад по умолчанию.';uk='Не заданий склад по умовчанню.'");
		Результат = Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(мТипЦен) Тогда
		Сигнал();
		Сообщение = НСтр("ru='Не задан тип цен.';uk='Не заданий тип цін.'");
		Результат = Ложь;
	Иначе
...
Процедура ЗакрытиеЧекаККМ(Товары, ПризнакВозврата, СуммаНал, СуммаБезнал,
	...
	Попытка
		...
		Сообщить(СокрЛП(РабочаяДата) + НСтр("ru=': Был создан чек ""';uk=': Був створений чек ""'") + СокрЛП(ДокЧек) + """.",
		         СтатусСообщения.Информация);
	Исключение
		Сообщить(СокрЛП(ТекущаяДата()) + НСтр("ru=': Был создан, но не был записан чек ""';uk=': Був створений, але не був записаний чек ""'") + СокрЛП(ДокЧек) + """.");
