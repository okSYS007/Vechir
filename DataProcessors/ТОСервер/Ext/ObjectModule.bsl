...
Функция ПодключитьКлиента(Клиент) Экспорт
	...
	Если ЗначениеЗаполнено(КонечныйРезультат) Тогда
		мОписаниеОшибки = НСтр("ru='Произошла ошибка при подключении торгового оборудования';uk='Виникла помилка при підключенні торговельного обладнання'");
...
Функция ВывестиСтрокуНаДисплейПокупателя(Идентификатор, Данные) Экспорт
	...
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='дисплея покупателя';uk='дисплея покупця'"));
	КонецЕсли;
...
///////////////////////////////////////////////////////////////////////////////
//// ФУНКЦИИ API МЕХАНИЗМА РАБОТЫ С ТОРГОВЫМ ОБОРУДОВАНИЕМ (ДИСПЛЕИ ПОКУПАТЕЛЯ СОВМЕСТНО С ФИСКАЛЬНЫМ РЕГИСТРАТОРОМ)
...
//
//  Сумма                          - <Число>
//                                 - Сумма, которую необходимо отобразить на
//                                   дисплее покупателя(используется только
//                                   для цифровых дисплеев).
//
// Возвращаемое значение:
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ВывестиИнформациюНаДисплейПокупателяФР(Идентификатор, СтрокаТекста, Сумма) Экспорт
	...
	Если Обработка.ПолучитьВерсиюAPI() < 3.0 Тогда
		Возврат Неопределено;
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		Результат = Обработка.ВывестиИнформациюНаДисплейПокупателя(Объект, СтрокаТекста, Сумма);
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='дисплея покупателя';uk='дисплея покупця'"));
	КонецЕсли;
...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ОчиститьДисплейПокупателяФР(Идентификатор) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		Результат = Обработка.ОчиститьДисплейПокупателя(Объект);
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='дисплея покупателя';uk='дисплея покупця'"));
	КонецЕсли;
...
Функция ТарированиеЭВ(Идентификатор, ВесТары = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='электронных весов';uk='електронних ваг'"));
	КонецЕсли;
...
Функция НачатьВыгрузкуВВесыСПечатьюЭтикеток(Идентификатор, ЧастичнаяВыгрузка = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='весов с печатью этикеток';uk='ваги з друком етикеток'"));
	КонецЕсли;
...
Функция ПечатьЧека(Идентификатор, Пароль, НомерЧека, НомерСмены,
				СтруктураПараметровЧека, СуммаНал, СуммаБезнал, ПризнакВозврата, ВходящиеПараметры = Неопределено) Экспорт

	ОписаниеЧека = СтруктураПараметровЧека.ТаблицаТоваров;
	СтруктураОбщиеПараметры = СтруктураПараметровЧека.СтруктураОбщиеПараметры;

	Обработка = Неопределено;
	Объект    = Неопределено;
	СтруктураДопРеквизитов = Новый Структура("СтавкаНДССтрокой, ЭтоПодакцизныйТовар, СтруктураДопРеквизитовТабличнойЧасти");
	Результат = ПолучитьОбъектДрайвера(Идентификатор, Обработка, Объект);
	...
	Если Объект.Параметры.Свойство("НесколькоАкцизныхМарокДляОдногоТовара")
		И  СтруктураОбщиеПараметры.Свойство("НесколькоАкцизныхМарокДляОдногоТовара") Тогда
		
		Если СтруктураОбщиеПараметры.НесколькоАкцизныхМарокДляОдногоТовара
				И НЕ Объект.Параметры.НесколькоАкцизныхМарокДляОдногоТовара Тогда
				
			мОписаниеОшибки = НСтр("ru='РРО не поддерживает печать нескольких акцизных марок для одного товара.';uk='РРО не підтримує друк кількох акцизних марок для одного товару.'");
			Возврат Перечисления.ТООшибкиОбщие.ОшибкаПараметровНастройки;;
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		//Дандали В. ++ 
		//Доработал при проведении чека, чтоб сразу отправлять на рро.
		//подключение осуществляется на прямо через обработку
		Если ВходящиеПараметры <> Неопределено Тогда
			Если ВходящиеПараметры.Свойство("НужноПодключитьРРО") Тогда
				Рез = Обработка.Подключить(Объект);
			КонецЕсли;
		КонецЕсли;
		//Дандали В. --
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			...
			Для Каждого Позиция Из ОписаниеЧека Цикл
				...
				Если ЭтоНоваяВерсия Тогда
					...
					СтруктураДопРеквизитов.СтруктураДопРеквизитовТабличнойЧасти = Позиция.СтруктураДопРеквизитовТабличнойЧасти;
			...
			Если НЕ ЗначениеЗаполнено(Результат) Тогда
				Объект.Вставить("СтруктураОбщиеПараметры",СтруктураОбщиеПараметры);
				Результат = Обработка.ЗакрытьЧек(Объект, СуммаНал, СуммаБезнал);
				Если Объект.Свойство("ФискальныйНомерСозданногоЧека") Тогда
					НомерЧека = Объект.ФискальныйНомерСозданногоЧека;
				КонецЕСли;
				Если ВходящиеПараметры <> Неопределено Тогда
					Если ВходящиеПараметры.Свойство("НужноПодключитьРРО") Тогда
...
Функция ВнестиСумму(Идентификатор, Пароль, Сумма) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		//Если Сумма > 0 Тогда
		//	Обработка.ВывестиИнформациюНаДисплейПокупателя(Объект, Символы.ПС + НСтр("ru='Внесено: ';uk='Внесено: '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов())  + Формат(Сумма, "ЧДЦ=2; ЧРГ=' '; ЧРД='.'; ЧН=0.00; ЧГ=0"), Сумма);
		//Иначе
		//	Обработка.ВывестиИнформациюНаДисплейПокупателя(Объект, Символы.ПС + НСтр("ru='Изъято: ';uk='Вилучено: '", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + Формат(-Сумма, "ЧДЦ=2; ЧРГ=' '; ЧРД='.'; ЧН=0.00; ЧГ=0"), -Сумма);
		//КонецЕсли;	
		//
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
...
// Результат – успех выполнения функции.
//
Функция ОплатитьПлатежнойКартой(Идентификатор, Сумма, КодRRN, ИдентификаторФР, СтрДопПараметры = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			Результат = Обработка.ОплатитьПлатежнойКартой(Объект, Сумма, КодRRN, КодАвторизации, КодОперации, МассивСтрокСлипЧека, ШиринаЧека, КоличествоКопий, СтрДопПараметры);
			...
			Если ЗначениеЗаполнено(Результат) И Не ЗначениеЗаполнено(РезультатФР) Тогда
				мОписаниеОшибки = НСтр("ru='Ошибка эквайринговой системы: ';uk='Помилка еквайрингової системи: '") + Объект.ОписаниеОшибки + НСтр("ru='"
"Операция по карте не выполнена';uk='"
"Операція по карті не виконана'");
			ИначеЕсли ЗначениеЗаполнено(РезультатФР) Тогда
				Результат = РезультатФР;
				мОписаниеОшибки = НСтр("ru='Ошибка фискального регистратора: ';uk='Помилка фіскального реєстратора: '") + ПолучитьТекстОшибкиТО(РезультатФР) + НСтр("ru='"
"Операция по карте отменена.';uk='"
"Операція по карті скасована.'");
			КонецЕсли;
		Иначе
			мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО("ФР");
		КонецЕсли;
...
"Операція по карті скасована.'");
		...
		КонецЕсли;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО(НСтр("ru='эквайринговой системы';uk='еквайрингової системи'"));
	КонецЕсли;
...
Функция ПодключитьОборудование(Вид, Клиент)
	...
	Если СтрокиТО.Количество() = 0 Тогда
		...
		Для Каждого СтрокаТО Из СтрокиТО Цикл
			...
			Если Объект.Параметры.Свойство("КассаККМ",текКассаККМ) Тогда
...
Функция ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Данные, СШК)
	...
	Если ЗначениеЗаполнено(мПрефиксВесовогоТовара)
		...
		//кейс, когда код товара содержит символы
		Если ОбщегоНазначения.ТолькоЦифрыВСтроке(КодТовара) Тогда
			КоличествоТовара = Сред(ШК, 3 + мДлинаКодаВесовогоТовара, 10 - мДлинаКодаВесовогоТовара);
			Запрос           = Новый Запрос(
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|    РегКВТ.Номенклатура                         КАК Номенклатура,
...
Функция ПолучитьТекстОшибкиПодключенияТО(ТО) Экспорт
	...
	Текст = "Драйвер " + ТО + НСтр("ru=' не загружен';uk=' не завантажений'");
...
Процедура ЗагрузитьИзТерминалаСбораДанных(Терминалы, ФормаКлиент) Экспорт
	...
	Иначе // Если мТерминалы.Количество() > 1
		...
		Терминал = СписокВыбора.ВыбратьЭлемент(НСтр("ru='Выберите терминал сбора данных';uk='Виберіть термінал збору даних'"));
	...
	Если ПустаяСтрока(Ошибки) Тогда
		Предупреждение(НСтр("ru='Загрузка завершена успешно.';uk='Завантаження завершено успішно.'") + Символы.ПС +НСтр("ru='Загружено ';uk='Завантажено '") + СокрЛП(ДанныеИзТСД.Количество()) + НСтр("ru=' строк.';uk=' рядків.'"));
	Иначе
		Предупреждение(НСтр("ru='Не удалось загрузить данные:';uk='Не вдалося завантажити дані:'") + Символы.ПС + Ошибки);
	КонецЕсли;
...
мАдресОбновленияОбработокОбслуживания = "http://downloads.1c.eu/tmplts/ITS/TradeWare/";
