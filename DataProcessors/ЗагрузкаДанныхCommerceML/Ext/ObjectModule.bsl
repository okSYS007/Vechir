...
Процедура ЗаполнитьСтруктурыЗагрузки() Экспорт
	...
	СтруктураЗагрузки.Вставить("МФО_БанкНЕПрямыхРасходов_РасчетныйСчет_РасчетныеСчета_Контрагент_Контрагенты_Документ_КоммерческаяИнформация_", "БанкМФО");
	...
	СтруктураЗагрузки.Вставить("МФО_БанкНЕПрямыхРасходов_РасчетныйСчет_РасчетныеСчета_Владелец_Каталог_КоммерческаяИнформация_", "БанкМФО");
	...
	СтруктураЗагрузки.Вставить("МФО_БанкНЕПрямыхРасходов_РасчетныйСчет_РасчетныеСчета_Владелец_ПакетПредложений_КоммерческаяИнформация_", "БанкМФО");
...
Функция СоздатьТипЦен(СтрокаДанных) Экспорт
	...
	Если НЕ ЗначениеНеПустаяСсылка(СтрокаДанных.Контрагент) Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не сопоставлен контрагент для типа цен: ';uk='Не зіставлений контрагент для типу цін: '") + СтрокаДанных.ТипЦены + "
			|Необходимо сопоставить контрагенту " + СтрокаДанных.Контрагент + НСтр("ru=' элемент справочника, применить соответствие и повторить операцию.';uk=' елемент довідника, застосувати відповідність і повторити операцію.'"));
...
Функция СоздатьБанковскиеСчета(СтрокаДанных) Экспорт
	...
	Если НЕ ЗначениеНеПустаяСсылка(СтрокаДанных.Контрагент) Тогда
		...
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не сопоставлен контрагент для счета: ';uk='Не зіставлений контрагент для рахунку: '") + СтрокаДанных.НомерСчета + НСтр("ru='"
"Необходимо сопоставить контрагенту ';uk='"
"Необхідно зіставити контрагенту '") + СтрокаДанных.Контрагент + НСтр("ru=' элемент справочника, применить соответствие и повторить операцию.';uk=' елемент довідника, застосувати відповідність і повторити операцію.'"));
...
Функция ВернутьТаблицуВозможныхДокументовДляЗагрузки()
	...
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Заказ товара';uk='Замовлення товару'"), "ЗаказПокупателя", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"), Истина);	
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Счет на оплату';uk='Рахунок на оплату'"), "СчетНаОплатуПоставщика", НСтр("ru='Счет на оплату поставщика';uk='Рахунок на оплату постачальника'"), Ложь);
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Отпуск товара';uk='Відпуск товару'"), "ПоступлениеТоваровУслуг", НСтр("ru='Поступление товаров и услуг';uk='Надходження товарів і послуг'"), Ложь);
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Переоценка товаров';uk='Переоцінка товарів'"), "ПереоценкаТоваровПринятыхНаКомиссию", НСтр("ru='Переоценка товаров принятых на комиссию';uk='Переоцінка товарів прийнятих на комісію'"), Ложь);
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Возврат товара';uk='Повернення товару'"), "ВозвратТоваровОтПокупателя", НСтр("ru='Возврат товаров от покупателя';uk='Повернення товарів від покупця'"), Истина);
	ДобавитьТипДляОбработкиВТаблицу(Таблица, НСтр("ru='Отчет о продажах комиссионного товара';uk='Звіт про продажі комісійного товару'"), "ОтчетКомиссионераОПродажах", НСтр("ru='Отчет комиссионера о продажах';uk='Звіт комісіонера про продажі'"), Истина);
	...
	Для Каждого СтрокаТаблицы Из СоответствияОрганизаций Цикл
		...
		Если ЗначениеНеПустаяСтрока(СтрокаТаблицы.Организация) И НЕ ЗначениеЗаполнено(СтрокаТаблицы.НовыйОрганизация) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.ИННОрганизации) Тогда
				//СтрокаТаблицы.НовыйОрганизация = СпрОрганизации.НайтиПоРеквизиту("КодПоЕДРПОУ", СтрокаТаблицы.ИННОрганизации);
...
Процедура ОбработатьФайлы(ИмяФайлаКаталога, ЗадаватьВопросОЧтенииДанных = Истина) Экспорт
	...
	Если ПустаяСтрока(ИмяФайлаКаталога) Тогда
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='Не указано имя файла данных! Чтение данных невозможно!';uk=""Не вказано ім'я файлу даних! Читання даних неможливо!"""));
		#КонецЕсли
		Возврат;
	КонецЕсли;
	...
	#Если Клиент Тогда
		Если ЗадаватьВопросОЧтенииДанных Тогда
			Ответ = Вопрос(НСтр("ru='Прочитать данные из файла?';uk='Прочитати дані з файлу?'"), РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.ОК, НСтр("ru='Загрузка данных';uk='Завантаження даних'"));
	...
	Если НЕ Источник.Существует() Тогда
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='Некорректное имя файла или каталога! Чтение данных невозможно!';uk=""Некоректне ім'я файлу або каталогу! Читання даних неможливо!"""));
		#КонецЕсли
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьФайлXML(ИмяФайла)
	...
	Исключение
		...
		ОбъектXML.Закрыть();		
		#Если клиент тогда
		Сообщить(НСтр("ru='Возникла ошибка при чтении данных для обмена: ';uk='Виникла помилка при читанні даних для обміну: '") + Символы.ПС + ОписаниеОшибкиЗаписи, СтатусСообщения.Важное);
		#КонецЕсли
		Возврат;
	КонецПопытки;
...
Процедура ОбработатьЭлемент(ОбъектXML, Ключ)
	...
	ИначеЕсли ЗначениеТипаОбработки = "БанкНЕПрямыхРасходовАдрес" Тогда	                     		
	 ...
	 ИначеЕсли ЗначениеТипаОбработки = "БанкНЕПрямыхРасходовМФО" Тогда			
...
Процедура ОбработатьЭлПисьма(НачДата, КонДата) Экспорт
	...
	#Если Клиент Тогда
	Ответ = Вопрос(НСтр("ru='Начать загрузку данных из электронных писем?';uk='Розпочати завантаження даних з електронних листів?'"), РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.ОК, НСтр("ru='Загрузка данных';uk='Завантаження даних'"));
	Если Ответ <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	#КонецЕсли
...
Функция СохранитьКаталог(ИнформацияДокумента, СтрокиДокумента)
	...
	Для Каждого СтрокаТаблицы Из СтрокиДокумента Цикл
		...
		Иначе
			НайденыДубли = ИСТИНА;
			#Если клиент тогда
			Сообщить(НСтр("ru='В каталоге обнаружены дублирующиеся значение пар Номенклатура - Характеристика номенклатуры!';uk='В каталозі виявлені значення пар Номенклатура - Характеристика номенклатури, що дублюються!'"), СтатусСообщения.Важное);
			Сообщить(НСтр("ru='Номенклатура:                ';uk='Номенклатура: '") + СтрокаТаблицы.Номенклатура, СтатусСообщения.Важное);
			Сообщить(НСтр("ru='Характеристика номенклатуры: ';uk='Характеристика номенклатури: '") + СтрокаТаблицы.ХарактеристикаНоменклатуры, СтатусСообщения.Важное);
			#КонецЕсли
		КонецЕсли;
	КонецЦикла;
	...
	Если НайденыДубли = ИСТИНА Тогда
		#Если клиент тогда
		Сообщить(НСтр("ru='В каталоге обнаружены дублирующиеся значение пар Номенклатура - Характеристика номенклатуры';uk='В каталозі виявлені значення пар Номенклатура - Характеристика номенклатури, що дублюються'"), СтатусСообщения.Важное);
		Сообщить(НСтр("ru='Запись невозможна!';uk='Запис неможливий!'"), СтатусСообщения.Важное);
		#КонецЕсли
		Возврат ЛОЖЬ;
	КонецЕсли;
	...
	Исключение
		...
		ОписаниеОшибкиЗаписи = ОписаниеОшибки();
		#Если клиент тогда
		Сообщить(НСтр("ru='При записи каталога товаров произошла ошибка: ';uk='При запису каталогу товарів сталася помилка: '") + Символы.ПС + ОписаниеОшибкиЗаписи, СтатусСообщения.Важное);
		#КонецЕсли
		Возврат ЛОЖЬ;
...
Функция ВыполнитьСтандартныеДействияДляЗаписиДокумента(ИнформацияДокумента, СтрокиДокумента, Док, 
	...
	Исключение
		...
		ОписаниеОшибкиЗаписи = ОписаниеОшибки();
		#Если клиент тогда
		Сообщить(НСтр("ru='При записи документа ';uk='При записі документа '") + СокрЛП(Док) + НСтр("ru=' произошла ошибка: ';uk=' виникла помилка: '") + Символы.ПС + ОписаниеОшибкиЗаписи, СтатусСообщения.Важное);
		#КонецЕсли
		Возврат ЛОЖЬ;
...
Процедура УстановитьЗначенияПараметровПоУмолчанию()
	...
	мОтражатьВРеглУчете = НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(мТекущийПользователь, "НеОтражатьДокументыВУправленческомУчете");
	мОтражатьВБухУчета  = НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(мТекущийПользователь, "НеОтражатьДокументыВБухгалтерскомУчете");
	мОтражатьВНалУчете  = НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(мТекущийПользователь, "НеОтражатьДокументыВНалоговомУчете");
...
Функция СохранитьДанные(ЗадаватьВопросПользователю = Истина, КоличествоОбработанныхОбъектов = 0) Экспорт
	...
	Если ЗадаватьВопросПользователю	 Тогда
		Ответ = Вопрос(НСтр("ru='Сохранить данные в информационную базу?';uk='Зберегти дані в інформаційну базу?'"), РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.ОК, НСтр("ru='Сохранение данных';uk='Збереження даних'"));
		Если Ответ <> КодВозвратаДиалога.ОК Тогда
			Возврат ЛОжь;
		КонецЕсли;
	КонецЕсли;
	
	Состояние(НСтр("ru='Сохранение объектов в информационную базу ...';uk=""Збереження об'єктів в інформаційну базу ..."""));
	#КонецЕсли
