...
Функция ПолучитьМенеджерПоТипу(ТипРеквизита, ИмяОбъектаМД)
	...
	ИначеЕсли (Найти(СтрокаТипа,НСтр("ru='План счетов';uk='План рахунків'")) = 1) или (Найти(СтрокаТипа,"ПланСчетов") = 1) Тогда	
		Менеджер = ПланыСчетов[ИмяОбъектаМД];
	ИначеЕсли (Найти(СтрокаТипа,НСтр("ru='План видов характеристик';uk='План видів характеристик'")) = 1) или (Найти(СтрокаТипа,"ПланВидовХарактеристик") = 1) Тогда	
		Менеджер = ПланыВидовХарактеристик[ИмяОбъектаМД];	
	ИначеЕсли (Найти(СтрокаТипа,НСтр("ru='План видов расчета';uk='План видів розрахунку'")) = 1) или (Найти(СтрокаТипа,"ПланВидовРасчета") = 1) Тогда	
...
Функция ПолучитьДатуПоСтроке(Знач ДатаСтрокой, ИмяОбъекта, ИмяИзмерения)
	...
	ИначеЕсли Найти(ДатаСтрокой,":") <> 0 Тогда
		...
		Если СтрДлина(ДатаСтрокой)<>19 Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при заполнении объекта %1! При заполнении измерения %2 не удалось преобразовать ""%3"" к дате!';uk='Помилка при заповненні об''єкта %1! При заповненні виміру %2 не вдалося перетворити ""%3"" до дати!'"), ИмяОбъекта, ИмяИзмерения, ДатаСтрокой));
			Возврат '00010101';
		Иначе
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			Если МетаданныеРеквизита = Неопределено Тогда
				...
				Если МетаданныеРеквизита = Неопределено Тогда
					...
					Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден реквизит %1 справочника %2!';uk='Не знайдений реквізит %1 довідника %2!'"), ИмяРеквизита, ИмяОбъекта));
					Продолжить;
...
Процедура ЗаполнитьРегистрСведений(ИмяОбъекта)
	...
	Если МетаданныеРС.Имя = "ПереводыТекстов" Тогда
		Макет.КодЯзыкаМакета = "ru";
	КонецЕсли;
	
	СостояниеЗаполнения(НСтр("ru='Заполняется регистр сведений ';uk='Заповнюється регістр відомостей '")+МетаданныеРС.Синоним);
	...
	Для Сч = 1 по ЧислоИзмерений Цикл
		...
		Если МетаданныеИзмерения = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при заполнении объекта %1! Не найдено измерение %2."
"Объект не заполнен!';uk=""Помилка при заповненні об'єкта %1! Не знайдений вимір %2."
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяИзмерения));
			Возврат;
		КонецЕсли;
	...
	Для Сч = 1 по ЧислоРесурсов Цикл
		...
		Если МетаданныеРесурса = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при заполнении объекта %1! Не найден ресурс %2."
"Объект не заполнен!';uk=""Помилка при заповненні об'єкта %1! Не знайдений ресурс %2."
"Об'єкт не заповнений!"""), ИмяОбъекта, МетаданныеРесурса));
			Возврат;
		КонецЕсли;
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Исключение
			...
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В регистре сведений %1 не удалось создать запись с параметрами %2';uk='В регістрі відомостей %1 не вдалося створити запис із параметрами %2'"), ИмяОбъекта, ПараметрыЗаписи));
			Продолжить;
		КонецПопытки;	
...
Процедура ЗаполнитьПланВидовХарактеристик(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план видов характеристик ';uk='Заповнюється план видів характеристик '")+МетаданныеПВХ.Синоним);
    ...
    Если ЕстьТЧ Тогда
		...
		Для Каждого Область из КоллекцияОбластей Цикл
			...
			Если ЕстьНужнаяТЧ = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при заполнении объекта %1! Не найдена ТЧ %2!';uk=""Помилка при заповненні об'єкта %1! Не знайдена ТЧ %2!"""), ИмяОбъекта, ИмяТч));
				Возврат;
			КонецЕсли;
...
Процедура ЗаполнитьПланСчетов(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план счетов ';uk='Заповнюється план рахунків '")+МетаданныеПС.Синоним);
...
Процедура ЗаполнитьБВВ(ВР, ИмяОбъекта, Макет, ФлагИзменений)
	...
	Для Каждого Область из КоллекцияОбластей Цикл
		...
		Попытка
			Менеджер = ПолучитьМенеджерПоТипу(НСтр("ru='План видов расчета';uk='План видів розрахунку'"), Область.Имя);
		Исключение
			Продолжить;
		КонецПопытки;	
...
//
Процедура ЗаполнитьПланВидовРасчета(ИмяОбъекта, РежимЗагрузки = Ложь, ОтборИмя = "") Экспорт
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план видов расчета ';uk='Заповнюється план видів розрахунку '")+МетаданныеПВР.Синоним);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			МетаданныеРеквизита = МетаданныеПВР.Реквизиты.Найти(ИмяРеквизита);
			Если МетаданныеРеквизита = Неопределено Тогда
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден реквизит %1 плана видов расчета %2!';uk='Не знайдений реквізит %1 плану видів розрахунку %2!'"), ИмяРеквизита, ИмяОбъекта));
				Продолжить;
			КонецЕсли;
			НовоеЗначение = СформироватьЗначение2(ТаблицаРеквизитов, ИмяРеквизита, ЗначениеРеквизита, Параметр);
			Если НовоеЗначение <> ТекущаяЗапись[ИмяРеквизита] Тогда
	...
	Если ИмяОбъекта = "Начисления" Тогда
		ЗаполнениеДоплат();	
	КонецЕсли;
	
КонецПроцедуры	

// для ПВР Начисления заполнение ТЧ Показатели для Вечерних и Ночных
Процедура 	ЗаполнениеДоплат()
...
//
Процедура ЗаполнитьОбъект(ТипОбъекта, НаименованиеОбъекта, РежимЗагрузки = Ложь) Экспорт
	...
	ИначеЕсли ТипОбъекта = "ПланВидовРасчета" Тогда
		ЗаполнитьПланВидовРасчета(НаименованиеОбъекта, РежимЗагрузки);	
...
Процедура ЗаполнитьВсеОбъекты() Экспорт
	...
	Для каждого ОбъектМД из Метаданные.Справочники Цикл
		СостояниеЗаполнения(НСтр("ru='Поиск справочников для заполнения';uk='Пошук довідників для заповнення'"));
		ЗаполнитьСправочник(ОбъектМД.Имя);
	КонецЦикла;	
	Для каждого ОбъектМД из Метаданные.ПланыВидовХарактеристик Цикл
		СостояниеЗаполнения(НСтр("ru='Поиск планов видов характеристик для заполнения';uk='Пошук планів видів характеристик для заповнення'"));
		ЗаполнитьПланВидовХарактеристик(ОбъектМД.Имя);
	КонецЦикла;	
	Для каждого ОбъектМД из Метаданные.ПланыВидовРасчета Цикл
		СостояниеЗаполнения(НСтр("ru='Поиск планов счетов для заполнения';uk='Пошук планів рахунків для заповнення'"));
		ЗаполнитьПланВидовРасчета(ОбъектМД.Имя);
	КонецЦикла;	
	Для каждого ОбъектМД из Метаданные.РегистрыСведений Цикл
		СостояниеЗаполнения(НСтр("ru='Поиск регистров сведений для заполнения';uk='Пошук регістрів відомостей для заповнення'"));
