...
Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
	...
	"ТипЦен,    Номенклатура,   Цена,  ХарактеристикаНоменклатуры ,   ЕдиницаИзмерения, Остаток,  ЦеноваяГруппа,    ШтрихКод",
	НСтр("ru='Тип цен';uk='Тип цін'",КодЯзыкаПечать),
	НСтр("ru='Номенклатура';uk='Номенклатура'",КодЯзыкаПечать),
	НСтр("ru='Цена';uk='Ціна'",КодЯзыкаПечать),
	НСтр("ru='Характеристика номенклатуры';uk='Характеристика номенклатури'",КодЯзыкаПечать),
	НСтр("ru='Ед. измерения';uk='Од. вимірювання'",КодЯзыкаПечать),
	НСтр("ru='Остаток';uk='Залишок'",КодЯзыкаПечать),
	НСтр("ru='Ценовая группа';uk='Цінова група'",КодЯзыкаПечать),
	НСтр("ru='Штрихкод';uk='Штрихкод'",КодЯзыкаПечать));
...
Процедура ПечатьПрайсЛиста(ДокументРезультат) Экспорт
	...
	Секция.Параметры.Валюта = ?(ВВалютеОтчета, НСтр("ru='В валюте: ';uk='У валюті: '",КодЯзыкаПечать) + ВалютаОтчета + " (курс " + Окр(МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаОтчета, ДатаПрайса).Курс, 2) + ").", НСтр("ru='В валютах цен.';uk='У валютах цін.'",КодЯзыкаПечать));
	Секция.Параметры.Период = НСтр("ru='Цены указаны на ';uk='Ціни вказані на '",КодЯзыкаПечать)+ Формат(ДатаПрайса, "ДФ = ""дд.ММ.гггг""");
	...
	Для Каждого ДопПоле Из ТаблицаНастройкиПолейПечать Цикл
		Если ДопПоле.ВыводитьНаПечать И ДопПоле.Положение = НСтр("ru='В отдельной колонке до номенклатуры';uk='В окремій колонці до номенклатури'") Тогда
	...
	// Заодно посчитаем их количество и запомним соответствие имен.
	Для Каждого ДопПоле Из ТаблицаНастройкиПолейПечать Цикл
		Если ДопПоле.ВыводитьНаПечать И ДопПоле.Положение = НСтр("ru='В отдельной колонке после номенклатуры';uk='В окремій колонці після номенклатури'") Тогда
