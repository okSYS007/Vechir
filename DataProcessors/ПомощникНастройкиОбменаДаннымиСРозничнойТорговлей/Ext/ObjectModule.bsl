...
Функция ЗагрузитьНастройкиИзФайла(ИмяФайла, СтрокаДляЗагрузки = "") Экспорт
	...
	Исключение
		...
		Сообщить(НСтр("ru='Ошибка при чтении настроек обмена данными: ';uk='Помилка при читанні настройок обміну даними: '") + СтрокаОшибки, СтатусСообщения.Важное);		
		#КонецЕсли
		Возврат Ложь;
	...
	#Если Клиент Тогда
	Сообщить(НСтр("ru='Настройки обмена данными первой информационной базы успешно загружены.';uk='Настройки обміну даними першої інформаційної бази успішно завантажені.'"));
	#КонецЕсли
...
Процедура ПроверитьСобственныйКодУзлаУПриемника(ПодключениеКПриемнику, ИмяПланаОбмена, КодЭлемента) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='В информационной базе приемнике уже настроен обмен данными с другой информационной базой. Проверьте номера кодов узлов плана обмена!';uk='В інформаційній базі приймачі вже настроєний обмін даними з іншою інформаційною базою. Перевірте номера кодів вузлів плану обміну!'");
...
Процедура ВыполнитьРегистрациюИзмененийПоУзлуИМетаданному(УзелДляРегистрацииИзменений, МетаданныеДляОбмена, 
	...
	#Если Клиент Тогда
	Состояние(НСтр("ru='Регистрация изменений для объектов: ';uk=""Реєстрація змін для об'єктів: """) + ИмяСсылочногоТипа);
	#КонецЕсли
...
Процедура ИнициализироватьФормуХодаОбработки(Знач ТекстСообщения = "", Знач ПроцентПрибавления = 0)
	...
	мФормаХодаВыполненияОбмена.НаименованиеОбработкиДанных = НСтр("ru='Сохранение настроек для обмена данными ...';uk='Збереження настройок для обміну даними ...'");
	мФормаХодаВыполненияОбмена.КомментарийОбработкиДанных = ТекстСообщения;	
...
Функция ВсеНастройкиЗаданыХотиимИхПрименить(НастройкаОбмена, ИнформацияОТипеОбмена, НужноРегистрироватьИзмененияДляОбмена = Истина) Экспорт
	...
	Попытка
		...
		ИнициализироватьФормуХодаОбработки(НСтр("ru='Подготовка данных для сохранения';uk='Підготовка даних для збереження'"), 0);
		
		Если НужноВнестиИзмененияВИнформационнуюБазуПриемника Тогда
			
			ОтобразитьНовоеСообщениеВФормеХодаОбработки(НСтр("ru='Подключение к информационной базе - приемнику';uk='Підключення до інформаційної бази - приймача'"), 10);
		...
		Попытка
			...
			Если НЕ ЗначениеЗаполнено(Константы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Получить()) 
				...
				#Если Клиент Тогда
					Сообщить(НСтр("ru='Для корректной работы механизма назначения префиксов этой информационной базы "
"необходимо завершить работу всех пользователей.';uk='Для коректної роботи механізму призначення префіксів цієї інформаційної бази "
"необхідно завершити роботу всіх користувачів."
"'"));	
				#КонецЕсли	
			...
			ОтобразитьНовоеСообщениеВФормеХодаОбработки(НСтр("ru='Создание объектов для регистрации изменений';uk=""Створення об'єктів для реєстрації змін"""), 10);
			...
			Если НужноВнестиИзмененияВИнформационнуюБазуПриемника Тогда
				...
				Если НЕ ЗначениеЗаполнено(ПодключениеКПриемнику.Константы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Получить()) 
					...
					#Если Клиент Тогда
						Сообщить(НСтр("ru='Для корректной работы механизма назначения префиксов информационной базы-приемника необходимо "
"завершить работу всех пользователей в информационной базе-приемнике';uk='Для коректної роботи механізму призначення префіксів інформаційної бази-приймача необхідно "
"завершити роботу всіх користувачів в інформаційної базі-приймачі"
"'"));	
					#КонецЕсли	
			...
			ОтобразитьНовоеСообщениеВФормеХодаОбработки(НСтр("ru='Подготовка к первоначальному обмену данными';uk='Підготовка до первісного обміну даними'"), 10);
			...
			ОтобразитьНовоеСообщениеВФормеХодаОбработки(НСтр("ru='Подготовка объектов к первому обмену данными';uk=""Підготовка об'єктів до першого обміну даними"""), 40);
			...
			ОтобразитьНовоеСообщениеВФормеХодаОбработки(НСтр("ru='Сохранение настроек для обмена данными';uk='Збереження настройок для обміну даними'"), 40);
			...
			НастройкаОбмена.Наименование = НСтр("ru='Обмен с ""';uk='Обмін з ""'") + НаименованиеИнформационнойБазыДляОбмена + """";
