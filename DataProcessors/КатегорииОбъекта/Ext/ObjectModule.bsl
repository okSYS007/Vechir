...
Процедура ОткрытьФормуНовойКатегории(ФормаВладелец) Экспорт
	...
	Если ТипЗнч(НазначениеКатегорий) = Тип("СписокЗначений") Тогда
		ВыбранныйЭлемент = НазначениеКатегорий.ВыбратьЭлемент(НСтр("ru='Выбор назначения категории';uk='Вибір призначення категорії'"));
...
Процедура ИнвертироватьПометкуУдаленияКатегории(ТекущаяСтрока) Экспорт
	...
	Исключение
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='Не удалось изменить пометку удаления категории:';uk='Не вдалося змінити позначку вилучення категорії:'") + Символы.ПС + ОписаниеОшибки());
		#КонецЕсли
	КонецПопытки;
...
Функция ЗаписатьКатегорииОбъекта() Экспорт
	...
	Исключение
		#Если Клиент Тогда
			Предупреждение(НСтр("ru='Не удалось записать категории объекта:';uk=""Не вдалося записати категорії об'єкта:""") + Символы.ПС + ОписаниеОшибки());
		#КонецЕсли	
		Возврат Ложь;
	КонецПопытки;
