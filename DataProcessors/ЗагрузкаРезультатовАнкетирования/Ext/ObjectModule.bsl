			...
			Для индФлага = 1 По 2 Цикл
				...
				Если индФлага = 1 И СтрокаДерева.ЗагружатьВСправочник Тогда
					...
					Если НЕ ЗначениеЗаполнено(ОбъектДляЗагрузки) Тогда
						...
						Иначе
							Сообщить(НСтр("ru='При загрузке файла ';uk='При завантаженні файлу '") + СтрокаДерева.ИмяФайла + НСтр("ru=' произошла ошибка';uk=' виникла помилка'"), СтатусСообщения.Внимание);
							Сообщить(НСтр("ru='В анкете ';uk='В анкеті '") + АнкетаИзФайла + НСтр("ru=' отсутствует информация о виде справочника, в который следует загрузить файл';uk=' відсутня інформація про вид довідника, в який слід завантажити файл'"), СтатусСообщения.ОченьВажное);
							Продолжить;
					...
					Если ПрочитатьФайл(ИмяФайла, ДокОпроса) Тогда
						...
						Сообщить(НСтр("ru='Загружена информация в объект: ';uk=""Завантажена інформація в об'єкт: """) + ДокОпроса);
					Иначе
						ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Загрузка файла прервана.';uk='Завантаження файлу перервано.'"));
					КонецЕсли;
...
Функция ПолучитьДокРассылкиИзФайла(ИсточникXMLФайл) Экспорт
	...
	Попытка
		ДокРассылки = Документы.Опрос.ПолучитьСсылку(Новый УникальныйИдентификатор(ИдРассылки));
		Возврат ДокРассылки;
...
Функция ПрочитатьФайл(ИмяФайла, ПотокВывода) Экспорт
	...
	Если НЕ ХМЛ.Прочитать() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Невозможно прочитать файл!';uk='Неможливо прочитати файл!'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Пока ((ХМЛ.ТипУзла <> ТипУзлаXML.НачалоЭлемента) ИЛИ (ХМЛ.Имя <> "Заголовок")) Цикл
		...
		Если НЕ ХМЛ.Прочитать() Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не найден заголовок опроса!';uk='Не знайдений заголовок опитування!'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если Версия <> "1.1" Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Неверная версия формата данных (';uk='Невірна версія формату даних ('")+Версия+").");
		Возврат Ложь;
	КонецЕсли;
	...
	Пока ((ХМЛ.ТипУзла <> ТипУзлаXML.НачалоЭлемента) ИЛИ (ХМЛ.Имя <> "РезультатОпроса")) Цикл
		...
		Если НЕ ХМЛ.Прочитать() Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не найдены результаты опроса!';uk='Не знайдені результати опитування!'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если Анкета.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не обнаружена анкета с идентификатором ';uk='Не виявлена анкета з ідентифікатором '") + ИдАнкеты);
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Возможно изменился идентификатор или она удалена';uk='Можливо змінився ідентифікатор або вона вилучена'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ДобавитьСтрокуСписка(Имя, Значение, РазвернутыйОтвет = "", ПотокВывода, Вопрос = Неопределено, ТабличныйВопрос = Неопределено, НомерСтрокиВтаблице = 0)
		...
		Если ПотокВывода.Колонки.Найти("НомерСтрокиВтаблице") <> Неопределено Тогда
			...
			Если (ТипЗначения.Типы().Получить(0) = Тип("Дата"))
			 ...
			 ИЛИ (ТипЗначения.Типы().Получить(0) = Тип("Число")) Тогда
				...
				Исключение
					Сообщить(НСтр("ru='Ответ на вопрос: ""';uk='Відповідь на питання: ""'")+Имя+НСтр("ru='"". Не удалось преобразовать ответ ""';uk='"". Не вдалося перетворити відповідь ""'")+ЗначениеВОтвет+НСтр("ru='"" к типу ""';uk='"" до типу ""'")+ТипЗначения.Типы().Получить(0)+ НСтр("ru='"". Ответ не загружен!';uk='"". Відповідь не завантажено!'"));
	...
	Иначе
		...
		Если ТипЗначения <> Неопределено И ((ТипЗначения = Тип("Дата")) ИЛИ (ТипЗначения = Тип("Строка")) ИЛИ (ТипЗначения = Тип("Число"))) Тогда
			...
			Исключение
				Сообщить("Ответ на вопрос: """+Строка(Вопрос)+НСтр("ru='"". Не удалось преобразовать ответ ""';uk='"". Не вдалося перетворити відповідь ""'")+ЗначениеВОтвет+НСтр("ru='"" к типу ""';uk='"" до типу ""'")+ТипЗначения+ НСтр("ru='"". Ответ не загружен!';uk='"". Відповідь не завантажено!'"));
...
Процедура ПоместитьВопросыВОбъектЗагрузки(ХМЛ, ПотокВывода, ДатаЗаписи, Анкета, ТабличныйВопрос = Неопределено, НомерСтрокиВтаблице = 0)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		ИначеЕсли ЕстьКонтрагенты И Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.ИННГоловногоКонтрагента и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.Контрагенты") тогда
			...
			Иначе
				Сообщить(НСтр("ru='Невозможно заполнить головного контрагента. Не найден контрагент, имеющий ИНН - ';uk='Неможливо заповнити головного контрагента. Не знайдений контрагент, що має ІПН - '") + Ответ);
			КонецЕсли;
		...
		ИначеЕсли	ТипВидКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес	тогда
			...
			Если СокрЛП(СтрокаКИ.Поле7КИ) <> "" Тогда
				Представление = Представление + НСтр("ru=', дом № ';uk=', будинок № '") + СокрЛП(СтрокаКИ.Поле7КИ);
			КонецЕсли;
			
			Если СокрЛП(СтрокаКИ.Поле8КИ) <> "" Тогда
				Представление = Представление + НСтр("ru=', корпус ';uk=', корпус '") + СокрЛП(СтрокаКИ.Поле8КИ);
			КонецЕсли;
			
			Если СокрЛП(СтрокаКИ.Поле9КИ) <> "" Тогда
				Представление = Представление + НСтр("ru=', кв.';uk=', кв.'") + СокрЛП(СтрокаКИ.Поле9КИ);
			КонецЕсли;
		...
		ИначеЕсли	ТипВидКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон	тогда
			...
			Представление = Представление + ?((Не ПустаяСтрока(СтрокаКИ.Поле4КИ)),(ПроверкаПустойСтроки(Представление)+НСтр("ru='доб. ';uk='дод. '") + СтрокаКИ.Поле4КИ),"");
			СтрокаКИ.ПредставлениеКИ	= Представление
