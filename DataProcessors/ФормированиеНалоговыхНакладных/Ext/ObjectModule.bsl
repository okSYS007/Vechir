...
Перем мДокументВводаНаОсновании Экспорт;
				...
				Пока ВыборкаДоговора.Следующий() Цикл  //ДоговорКонтрагента
					...
					Пока ВыборкаСделка.Следующий() Цикл  //Сделка
						...
						Если НЕ ВыборкаСделка.СложныйНалоговыйУчет Тогда
							...
							ДобавитьСтрокуВДоговора(ВыборкаСделка);
						...
						ИначеЕсли ВыборкаСделка.Схема = Перечисления.МоментыОпределенияНалоговойБазы.ПоОплате Тогда
							...
							Если Аванс > 0 Тогда
								ДобавитьСтрокуВДоговораАванс(ВыборкаСделка, Аванс);
							КонецЕсли;
...
Процедура УдалитьСтрокиТабличныхЧастейНоменклатуры(СтруктураУдаляемыхСтрок = Неопределено) Экспорт
	...
	УдалитьСтрокиТабличнойЧасти(НМА, 				СтруктураУдаляемыхСтрок);
КонецПроцедуры
...
Процедура ЗаполнитьТабличныеЧасти(СтруктураОтбора) Экспорт 
	...
	Иначе
		...
		ЗаполнитьТабличнуюЧасть("НМА",				СтруктураОтбора);
	КонецЕсли;
...
Процедура ОбновитьИсточникиНоменклатуры(СтруктураОтбораНоменклатуры) Экспорт
	...
	ВыгрузитьТабличнуюЧастьВИтоговуюТаблицу(СтруктураОтбораНоменклатуры, мИтоговаяТаблица, "НМА");
...
Процедура ЗаполнитьТабличнуюЧасть(ИмяТабличнойЧастиДокумента, СтруктураОтбора)
	...
	// сделаем дополнительный отбор: если договор по заказам, и источник содержит
	// заказ в ТЧ - будем делать фильтр по заказу, указанному параметрах отбора
	ДоговорПоЗаказам         = СтруктураОтбора.ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоЗаказам;
	...
	Для каждого СтрокаТЧДокумента Из Документ[ИмяТабличнойЧастиДокумента] Цикл 
		...
		Для каждого Колонка Из КолокниТЧОбработки Цикл
			...
			Если ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
				СтрокаТЧОбработки.СтавкаНДС 		= Перечисления.СтавкиНДС.НДС20;
				СтрокаТЧОбработки.СуммаНДС 		    = СтрокаТЧДокумента.Сумма/6;
				СтрокаТЧОбработки.СуммаНДСВДокумент = СтрокаТЧДокумента.Сумма/6;
			КонецЕсли;
		...
		Если НЕ КолокниТЧОбработки.Найти("Содержание") = Неопределено И НЕ ЗначениеЗаполнено(СтрокаТЧОбработки["Содержание"]) Тогда
			ОбработкаТабличныхЧастей.ЗаполнитьСодержаниеТабЧасти(СтрокаТЧОбработки, ЭтотОбъект);
		КонецЕсли;
...
Процедура ЗаполнитьШапкуНалоговогоДокумента(НалоговыйДокумент, ВидОперации, Данные)
	...
	НалоговыйДокумент.Сделка			= ?(НЕ ЗначениеЗаполнено(Данные.Сделка), Документы.ЗаказПокупателя.ПустаяСсылка(), Данные.Сделка);
	...
	Если ТипЗнч(ВидОперации) = Тип("ПеречислениеСсылка.ВидыОперацийНалоговаяНакладная") Тогда
		...
		Если не мДокументВводаНаОсновании = Неопределено Тогда
			НалоговыйДокумент.ДокументВводаНаОсновании = мДокументВводаНаОсновании;
		КонецЕсли;
	...
	СчетаУчета   = УправлениеВзаиморасчетами.ПолучитьСчетаРасчетовСКонтрагентом(Организация, НалоговыйДокумент.Контрагент, НалоговыйДокумент.ДоговорКонтрагента);
...
Процедура ДобавитьСтрокиВТабличнуюЧастьНалоговогоДокумента(Документ, ИмяТабличнойЧасти, СтрокиТабличнойЧастиОбработки, Данные)
	...
	Для каждого СтрокаТЧОбработки Из СтрокиТабличнойЧастиОбработки Цикл
		...
		Документ.ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(СтрокаТЧДокумента, ИмяТабличнойЧасти); 
		
		Если ИмяДокумента = "Документ.НалоговаяНакладная" Тогда
			
			// дату отгрузки/оплаты не заполняем
		...
		Если  РежимПоДатам 
			И ИмяДокумента = "Документ.НалоговаяНакладная" Тогда
...
Процедура ПолучитьДанныеДляФормированияНалоговыхДокументов(ТаблицаДанных, СтруктураОтбораДанных)
	...
	Для каждого СтавкаНДС Из Перечисления.СтавкиНДС Цикл
		...
		ТаблицаНематериальныеАктивы = ОтобратьСтрокиТабличнойЧасти("НМА", СтавкаНДС, СтруктураОтбораДанных);
		...
		Иначе
			...
			Если КоэффЧастичнойОтгрузки < 1 Тогда
				...
				Для каждого Строка Из СтрокиТоваров Цикл
					...
					Строка.СуммаНДСВДокумент =  УчетНДС.РассчитатьСуммуНДС(Строка.СуммаВДокумент, Истина, Истина, УчетНДС.ПолучитьСтавкуНДС(Строка.СтавкаНДС));
				...
				Если Не ПоследняяСтрока = Неопределено Тогда
					...
					ПоследняяСтрока.СуммаНДСВДокумент =  УчетНДС.РассчитатьСуммуНДС(ПоследняяСтрока.СуммаВДокумент, Истина, Истина, УчетНДС.ПолучитьСтавкуНДС(ПоследняяСтрока.СтавкаНДС))
				КонецЕсли;
			КонецЕсли;
	...
	Для каждого СтрокаТаблицыЧастичнойОтгрузки Из ТаблицаЧастичнойОтгрузки Цикл
		...
		Иначе
			...
			Если  (СтрокаТаблицыЧастичнойОтгрузки.ЧастичнаяОтгрузка  = Ложь 
				...
				Строки = НМА.НайтиСтроки(СтруктураОтбораСтрокНоменклатуры);
				Для каждого Строка Из Строки Цикл
					НМА.Удалить(Строка);
				КонецЦикла; 
	...
	Для каждого СтрокаТаблицыЧастичнойОтгрузки Из ТаблицаЧастичнойОтгрузки Цикл
		...
		Иначе
			...
			Если  СтрокаТаблицыЧастичнойОтгрузки.ЧастичнаяОтгрузка = Истина Тогда 
				...
				Если СтруктураОтбораСтрокНоменклатуры.ДоговорКонтрагента.Внешнеэкономический Тогда
...
// для корректной работы алгортмов пересчета
СуммаВключаетНДС = истина;
