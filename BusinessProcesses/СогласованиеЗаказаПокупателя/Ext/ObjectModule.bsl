...
Процедура ПроверитьЗаполнениеРеквизитов(Отказ,Заголовок)
	...
	Если ОбъектСогласования.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнено значение реквизита ""Заказ"".';uk='Не заповнено значення реквізиту ""Замовлення"".'"),
											Отказ, Заголовок);
	КонецЕсли;

	Если Настройка.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнено значение реквизита ""Настройка "".';uk='Не заповнено значення реквізиту ""Настройка "".'"),
											Отказ, Заголовок);
	КонецЕсли;
