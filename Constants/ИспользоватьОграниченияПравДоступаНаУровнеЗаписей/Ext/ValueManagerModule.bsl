...
Процедура ПриЗаписи(Отказ)
	...
	Если ЭтотОбъект.Значение И ЭтотОбъект.Значение <> мСтароеЗначение Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				ВызватьИсключение (НСтр("ru='Не удалось обработать запись константы использования ограничения прав доступа на уровне записей по причине: ';uk='Не вдалося обробити запис константи використання обмеження прав доступу на рівні записів з причини: '") + ОписаниеОшибки());
			КонецПопытки;
