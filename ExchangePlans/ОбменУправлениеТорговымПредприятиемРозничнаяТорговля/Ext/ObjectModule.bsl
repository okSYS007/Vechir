...
Процедура ПередЗаписью(Отказ)
	...
	Если ВыгружатьСебестоимость Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодичностьПередачиСебестоимости) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Необходимо указать периодичность передачи себестоимости!';uk='Необхідно вказати періодичність передачі собівартості!'"), Отказ);
