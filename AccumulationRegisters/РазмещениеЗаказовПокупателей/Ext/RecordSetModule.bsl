...
Процедура КонтрольОстатков_КорректировкаЗаказаПоставщику(ДокументОбъект)
	...
	Пока Выборка.Следующий() Цикл
		...
		НСтр("ru=', размещенного по документу ';uk=', розміщеного по документу '") + (Выборка.Размещение) + НСтр("ru=' недостаточно для снятия размещения.';uk=' недостатньо для зняття розміщення.'");
...
Процедура КонтрольОстатков_РазмещениеЗаказа(ДокументОбъект)
	...
	Пока Выборка.Следующий() Цикл
		...
		УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
		Выборка.ХарактеристикаНоменклатурыПредставление) +
		НСтр("ru=', заказанного по документу ';uk=', замовленого по документу '") + (Выборка.Размещение) + НСтр("ru=' недостаточно для размещения.';uk=' недостатньо для розміщення.'");
		ДокументКоличество = Выборка.ДокументКоличество;
...
Процедура КонтрольОстатков_Заказ_КорректировкаЗаказа_Резервирование(ДокументОбъект)
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		Если ИмяДокумента = "КорректировкаЗаказаПокупателя"
			ИЛИ ИмяДокумента = "КорректировкаВнутреннегоЗаказа" Тогда
			Запрос = новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
				|Номенклатура,
				|%ПОЛЕ_Док_Характеристика% КАК ХарактеристикаНоменклатуры,
...
Процедура КонтрольОстатков_СнятиеРазмещения_ЗаказПоставщикуВТабЧастиЗаказПокупателяВТабЧасти(ДокументОбъект)
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		...
		Если ИмяТабличнойЧасти = "ВозвратнаяТара" Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%ПОЛЕ_Док_Характеристика%","&ПустаяХарактеристика");
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%ПОЛЕ_Док_Характеристика%","ХарактеристикаНоменклатуры");
		КонецЕсли;
		
		Если ЗаказПоставщикуВШапке Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%Поле_Док_ЗаказПоставщику%","Док.Ссылка."+ИмяЗаказПоставщику);
			МассивПустыхСсылокЗаказПоставщику = ОбщегоНазначения.МассивПустыхЗначений(МетаданныеДокумента.Реквизиты[ИмяЗаказПоставщику].Тип);
		Иначе	
		...
		КонецЕсли;
		Если ЗаказПокупателяВШапке Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%Поле_Док_ЗаказПокупателя%","Док.Ссылка."+ИмяЗаказПокупателя);
			МассивПустыхСсылокЗаказПокупателя = ОбщегоНазначения.МассивПустыхЗначений(МетаданныеДокумента.Реквизиты[ИмяЗаказПокупателя].Тип);
		Иначе	
	...
	Пока Выборка.Следующий() Цикл
		...
		НСтр("ru=', размещенного в документе ';uk=', розміщеного в документі '")+Выборка.ЗаказПоставщику+НСтр("ru=' по документу ';uk=' по документу '") + (Выборка.ЗаказПокупателя) + НСтр("ru=' недостаточно.';uk=' недостатньо.'");
...
Процедура КонтрольОстатков_СнятиеРазмещения_ЗаказПоставщикуВТабЧастиЗаказПокупателяВШапке(ДокументОбъект)
	...
	//исключить строки где в качетсве размещения указан склад - это относится к другому регистру
	...
	Пока Выборка.Следующий() Цикл
		...
		УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
		                          Выборка.ХарактеристикаНоменклатуры) +
		НСтр("ru=', размещенного в документе ';uk=', розміщеного в документі '")+Выборка.ЗаказПоставщику+НСтр("ru=' недостаточно.';uk=' недостатньо.'");
...
Процедура КонтрольОстатков_СнятиеРазмещения_ЗаказПоставщикуВШапкеЗаказПокупателяВТабЧасти(ДокументОбъект)
	...
	Пока Выборка.Следующий() Цикл
		...
		УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
								  Выборка.ХарактеристикаНоменклатуры) +
		НСтр("ru=', размещенного по документу ';uk=', розміщеного по документу '") + (Выборка.ЗаказПокупателя) + НСтр("ru=' недостаточно.';uk=' недостатньо.'");
