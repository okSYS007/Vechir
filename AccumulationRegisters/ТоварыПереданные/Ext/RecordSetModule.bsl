...
Процедура КонтрольОстатков_ЗаказВШапке(ДокументОбъект)
	...
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитДокумента("Сделка", мДокМетаданные) Тогда
		ИмяЗаказа = "Сделка";
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитДокумента("СделкаПоРеализации", мДокМетаданные) Тогда
		ИмяЗаказа = "СделкаПоРеализации";
	Иначе
		ИмяЗаказа = "";
	КонецЕсли;
	...
	ИначеЕсли мСтруктураШапкиДокумента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом Тогда
		Если мДокМетаданные.Имя = "ОтчетКомиссионераОПродажах" Тогда
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		...
		Если НЕ РезультатЗапросаДляБлокировки.Пустой() Тогда
			...
			СтруктураЗначенийБлокировки = Новый Структура(
				...
				, мСтруктураШапкиДокумента.ДоговорКонтрагента
				, мСтатусПередачи);
				
			Если ЗначениеЗаполнено(мСтруктураШапкиДокумента[ИмяЗаказа]) Тогда
				СтруктураЗначенийБлокировки.Вставить("Сделка", мСтруктураШапкиДокумента[ИмяЗаказа]);
			КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОстатокКоличество < Выборка.ДокументКоличество Тогда
			...
			НСтр("ru=' переданного по договору ""';uk=' переданого по договору ""'") + СокрЛП(мСтруктураШапкиДокумента.ДоговорКонтрагента) + ?(НЕ ЗначениеЗаполнено(мСтруктураШапкиДокумента[ИмяЗаказа]), "",
			НСтр("ru=' и по заказу ';uk=' і по замовленню '") + СокрЛП(мСтруктураШапкиДокумента[ИмяЗаказа])) +
		...
		Если Выборка.ОстатокСумма < Выборка.ДокументСумма Тогда
			...
			СтрокаСообщения = НСтр("ru='Остатка суммы взаиморасчетов по товару ';uk='Залишку суми взаєморозрахунків по товару '") + 
			УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
			                          ?(ЕстьХарактеристика, Выборка.ХарактеристикаНоменклатуры, ""), ?(ЕстьСерия, Выборка.СерияНоменклатуры, "")) +
			НСтр("ru=' переданному по договору ""';uk=' переданому по договору ""'") + СокрЛП(мСтруктураШапкиДокумента.ДоговорКонтрагента) + ?(НЕ ЗначениеЗаполнено(мСтруктураШапкиДокумента[ИмяЗаказа]), "",
			НСтр("ru=' и по заказу ';uk=' і по замовленню '") + СокрЛП(мСтруктураШапкиДокумента[ИмяЗаказа])) +
			НСтр("ru='"" недостаточно.';uk='"" недостатньо.'");
...
Процедура КонтрольОстатков_ВозвратТоваровОтПокупателя(ДокументОбъект)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОстатокКоличество < Выборка.ДокументКоличество Тогда
			...
			УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
			                          ?(ЕстьХарактеристика, Выборка.ХарактеристикаНоменклатуры, ""), ?(ЕстьСерия, Выборка.СерияНоменклатуры, "")) +
			НСтр("ru=' переданного по договору ""';uk=' переданого по договору ""'") + СокрЛП(мСтруктураШапкиДокумента.ДоговорКонтрагента) + ?(НЕ ЗначениеЗаполнено(Выборка.Заказ), "",
			НСтр("ru=' и по заказу ';uk=' і по замовленню '") + Выборка.ЗаказПредставление) +
			НСтр("ru='"" недостаточно.';uk='"" недостатньо.'");
		...
		Если Выборка.ОстатокСумма < Выборка.ДокументСумма Тогда
			...
			УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
			                          ?(ЕстьХарактеристика, Выборка.ХарактеристикаНоменклатуры, ""), ?(ЕстьСерия, Выборка.СерияНоменклатуры, "")) +
			НСтр("ru=' переданному по договору ""';uk=' переданому по договору ""'") + СокрЛП(мСтруктураШапкиДокумента.ДоговорКонтрагента) + ?(НЕ ЗначениеЗаполнено(Выборка.Заказ), "",
			НСтр("ru=' и по заказу ';uk=' і по замовленню '") + Выборка.ЗаказПредставление) +
			НСтр("ru='"" недостаточно.';uk='"" недостатньо.'");
...
Процедура КонтрольЛимитовВозвратнойТары(ДокументОбъект, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщения + Символы.ПС + Символы.Таб +
						   НСтр("ru='Лимит ';uk='Ліміт '") + (Выборка.Лимит) + " " + Выборка.ЕдиницаХраненияОстатковПредставление +
						   НСтр("ru='; Передано ранее ';uk='; Передано раніше '") + Выборка.ОстатокКоличество + " " + Выборка.ЕдиницаХраненияОстатковПредставление +
