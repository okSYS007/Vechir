...
Процедура КонтрольОстатков_Остатки()
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		...
		Если ЕстьДокументПередачи Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "// УСЛОВИЕ_ЗаполненДокРезерва",         "");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "// УСЛОВИЕ_ЗаполненДокРезерва",         "И Док.ДокументРезерва НЕ В(&ПустойДокРезерва)");
		КонецЕсли;
		...
		Если НЕ РезультатЗапросаДляБлокировки.Пустой() Тогда
			...
			СтруктураЗначенийБлокировки = Новый Структура("Склад", мСтруктураШапкиДокумента.Склад);
			...
			КонецЕсли;
			Если ЕстьДокументПередачи Тогда
				СтруктураЗначенийБлокировки.Вставить("ДокументПередачи", мСтруктураШапкиДокумента.ДокументПередачи);
			Иначе
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения = НСтр("ru='Остатка ';uk='Залишку '") + УправлениеЗапасами.ПредставлениеНоменклатуры(
			...
			НСтр("ru=' к передаче со склада ';uk=' до передачі зі складу '") + СокрЛП(мСтруктураШапкиДокумента.Склад) +
			НСтр("ru=' по документу ';uk=' по документу '") + Выборка.ДокументПередачи +
				?(ЕстьКачество, НСтр("ru=' с качеством ';uk=' з якістю '") + Выборка.Качество, "") + НСтр("ru=' недостаточно.';uk=' недостатньо.'");
...
Процедура КонтрольСвободныхОстатков_Реализация()
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения  = НСтр("ru='Свободного остатка для резервирования ';uk='Вільного залишку для резервування '") + 
			УправлениеЗапасами.ПредставлениеНоменклатуры(
				Выборка.НоменклатураПредставление, 
				?(ЕстьХарактеристика, Выборка.ХарактеристикаНоменклатуры, ""),
				?(ЕстьСерия, Выборка.СерияНоменклатуры, "")) +
			НСтр("ru=' на складе ""';uk=' на складі ""'") + СокрЛП(Выборка.Склад) + """" +
			?(ЕстьКачество, НСтр("ru=' с качеством ';uk=' з якістю '") + Выборка.Качество, "") + НСтр("ru=' недостаточно.';uk=' недостатньо.'");
...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабЧасти, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Иначе
		Сообщить(НСтр("ru='Для документов вида ""';uk='Для документів виду ""'") + мДокМетаданные.Представление() + НСтр("ru='"" не предусмотрен вызов процедуры ''Контроль остатков'' модуля регистра  ';uk='"" не передбачений виклик процедури ''Контроль залишків'' модуля регістра '") + "'"+Метаданные().Представление() + "'", СтатусСообщения.ОченьВажное);
	КонецЕсли;
