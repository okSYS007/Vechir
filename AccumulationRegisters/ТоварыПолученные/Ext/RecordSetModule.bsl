...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабличнойЧасти, СтруктураШапкиДокумента, СтатусПолучения, Отказ, Заголовок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения = НСтр("ru='Остатка ';uk='Залишку '") + 
			...
			НСтр("ru=' полученного по договору ""';uk=' отриманого по договору ""'") + СокрЛП(СтруктураШапкиДокумента.ДоговорКонтрагента) + ?(НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.Сделка), "",
			НСтр("ru=' и по сделке ';uk=' і по угоді '") + СокрЛП(СтруктураШапкиДокумента.Сделка)) +
