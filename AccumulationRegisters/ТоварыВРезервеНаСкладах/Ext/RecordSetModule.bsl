...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабЧасти, СтруктураШапки, ОтказПроведения, ЗаголовокСообщения) Экспорт
	...
	ИначеЕсли 
		ИмяДокумента = "ВозвратТоваровПоставщику" Тогда
		//контроль достаточности остатков в резерве для снятия резерва по заказу
	...
	ИначеЕсли  //Склад и заказ расположены в шапке документа
        ИмяДокумента = "РасходныйОрдерНаТовары" или ИмяДокумента = "ПередачаТоваров" Тогда
	...
	ИЛИ ИмяДокумента = "ТребованиеНакладная" ИЛИ ИмяДокумента = "ОтчетПроизводстваЗаСмену" Тогда
		//контроль достаточности остатков в резерве для снятия резерва
	...
	ИначеЕсли 
	  ...
	  Иначе
		  Сообщить(НСтр("ru='Для документов вида ""';uk='Для документів виду ""'") + МетаданныеДокумента.Представление() + НСтр("ru='"" не предусмотрен вызов процедуры ''Контроль остатков'' модуля регистра  ';uk='"" не передбачений виклик процедури ''Контроль залишків'' модуля регістра '") + "'"+Метаданные().Представление() + "'", СтатусСообщения.ОченьВажное);
	КонецЕсли;
...
Процедура КонтрольОстатков_Реализация(ДокументОбъект)
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ Склад, Номенклатура, ЗаказПокупателя, %ПОЛЕ_Характеристика%
		...
		Если ИмяТабличнойЧасти = "ВозвратнаяТара" Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ПОЛЕ_Характеристика%","&ПустаяХарактеристика");
		Иначе
	        Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ПОЛЕ_Характеристика%","ХарактеристикаНоменклатуры");
		КонецЕсли;	
		...
		Если НЕ РезультатЗапросаДляБлокировки.Пустой() Тогда
			...
			СтруктураИсточникаДанных = Новый Структура(
				"Склад,Номенклатура,ХарактеристикаНоменклатуры,ДокументРезерва"
				,"Склад"
				,"Номенклатура"
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения = НСтр("ru='Остатка резерва ';uk='Залишку резерву '") + 
		...
		НСтр("ru=' на складе ""';uk=' на складі ""'") + СокрЛП(Выборка.Склад) + НСтр("ru='"" по документу ""';uk='"" по документу ""'") + СокрЛП(Выборка.ДокументРезерва) + 
...
Процедура КонтрольОстатков_СкладВТабЧасти_ЗаказВТабЧасти()
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		Запрос.Текст = 
			...
			|	Не Док.Номенклатура.Комплект
			|	И Док.Ссылка  =  &ДокументСсылка
			|	"+?(ИмяДокумента = "АвансовыйОтчет"," И "+ИмяДокументРезерва+".БезПраваПродажи","И НЕ Док." + ИмяДокументРезерва + " В (&МассивПустыхСсылок)");
...
Процедура КонтрольОстатков_СкладВТабЧасти_ЗаказВШапке()
	...
	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		Запрос.Текст = 
			...
			+ ?(ИмяТабличнойЧасти <> "ВозвратнаяТара", ", ХарактеристикаНоменклатуры", "") + "
			|ИЗ Документ." + ИмяТаблицы + " КАК Док
			|ГДЕ
			|	НЕ Док.Номенклатура.Комплект
			|	И Док.Ссылка  =  &ДокументСсылка
		...
		Если НЕ РезультатЗапросаДляБлокировки.Пустой() Тогда
			...
			СтруктураЗначенийБлокировки = Новый Структура("ДокументРезерва", СтруктураШапкиДокумента[ИмяДокументРезерва]);	
...
Процедура КонтрольОстатков_СкладВШапке_ЗаказВТабЧасти()
	ИмяДокументРезерва = ?((ИмяДокумента = "ТребованиеНакладная" ИЛИ ИмяДокумента = "ОтчетПроизводстваЗаСмену"), "Заказ", "ДокументРезерва");
	...
	ТекстУсловие = "И %ПОЛЕ_Док_ДокументРезерва% НЕ в (&МассивПустыхСсылок) 
		...
		+?(ТипДокументРезерва.СодержитТип(ТипЗнч("ДокументСсылка.ВнутреннийЗаказ")), " ИЛИ %ПОЛЕ_Док_ДокументРезерва% ССЫЛКА Документ.ВнутреннийЗаказ ", "") 
	...
	Иначе
		//есть характеристика, единица измерения и коэффициент. 
		//Наличие серии зависит от того, ведется ли по заказу под который резервируем обособленный учет.
		ТекстЗапроса = СтрЗаменить( ТекстЗапроса,"%ПОЛЕ_Док_Количество%", ?(ИмяТабличнойЧасти <> "МатериалыЗаказчика",
	...
	КонецЕсли;
 	...
 	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ Номенклатура, %ПОЛЕ_ДокументРезерва% КАК ДокументРезерва" + ?(ИмяТабличнойЧасти <> "ВозвратнаяТара", ", ХарактеристикаНоменклатуры", "") + "
			|ИЗ Документ." + ИмяТаблицы + " КАК Док
			|ГДЕ
			|	Не Док.Номенклатура.Комплект
			|	И Док.Ссылка  =  &ДокументСсылка
    	...
    	Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ПОЛЕ_ДокументРезерва%",ИмяДокументРезерва);
    	Запрос.Текст = СтрЗаменить(Запрос.Текст,"%ПОЛЕ_Док_ДокументРезерва%","Док."+ИмяДокументРезерва);
...
Процедура КонтрольОстатков_СкладВШапке_ЗаказВШапке()
	...
	КонецЕсли;
 	...
 	Если глЗначениеПеременной("ИспользоватьУправляемыеБлокировки")  Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ Номенклатура" + ?(ИмяТабличнойЧасти <> "ВозвратнаяТара", ", ХарактеристикаНоменклатуры", "") + "
			|ИЗ Документ." + ИмяТаблицы + " КАК Док
			|ГДЕ
			|	Не Док.Номенклатура.Комплект
			|	И Док.Ссылка  =  &ДокументСсылка
...
Процедура КонтрольСвободныхОстатков_ПТиУ(УчитыватьСерии, ТабРаспределениеОбособленныйУчет)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СвободныйОстаток < Выборка.ДокументКоличество Тогда
			...
			, НСтр("ru='Свободного остатка для резервирования ';uk='Вільного залишку для резервування '")
			);
		КонецЕсли;
	...
	Пока Выборка.Следующий()  Цикл
		...
		Если КоличествоСнимаемое>0  Тогда
			...
			УправлениеЗапасами.ОшибкаНетОстатка(
				НСтр("ru='Недостаточно товара ""';uk='Недостатньо товару ""'") + Выборка.НоменклатураПредставление + """" +
				?(ИмяТабличнойЧасти <> "ВозвратнаяТара", " """ + Выборка.ХарактеристикаНоменклатурыПредставление + """", "") 
				+ ?(УчитыватьСерии, " """ +Выборка.СерияНоменклатурыПредставление+ """", "")
				+ НСтр("ru=' в резерве на складе ""';uk=' у резерві на складі ""'") + СокрЛП(Выборка.Склад) + """",
				ВРезервеПоЗаказу,
				КоличествоСнимаемое,
...
Процедура КонтрольПревышенияОбъемаЗаказа(ДокСсылка, ТабЧасть, СтруктураПараметров, Отказ, Заголовок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КолЗаказано - Выборка.КолРезерв < Выборка.Количество Тогда
			...
			СтрокаСообщения = НСтр("ru='Резервирование по заказу превышает объем заказа: ';uk='Резервування по замовленню перевищує обсяг замовлення: '")
...
Процедура КорректировкаСерийПриРезервировании()
	...
	Иначе
		...
		// Для каждого заказа определим ведётся ли обособленый учет, если нет, очистим серию.
		Для Каждого СтрокаДвижения Из ЭтотОбъект Цикл
