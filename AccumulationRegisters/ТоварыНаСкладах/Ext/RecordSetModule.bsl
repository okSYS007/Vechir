...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабЧасти, СтруктураШапки, ОтказПроведения, ЗаголовокСообщения) Экспорт
	...
	ИначеЕсли 	ИмяДокумента = "ТребованиеНакладная"  ИЛИ
				ИмяДокумента = "ОтчетПроизводстваЗаСмену"  ИЛИ
...
Процедура КонтрольОстатков_ПеремещениеТоваров()
	...
	ЭтоБланки = (ИмяТаблицы = "ПеремещениеТоваров.БланкиСтрогогоУчета");
	Если НЕ ЭтоБланки Тогда
	...
	Если ЭтоБланки Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//СОЕДИНЕНИЕ_ДокументРезерва_Резервы",		"");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,",%ПОЛЕ_Заказ%",							"");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//СОЕДИНЕНИЕ_ДокументРезерва_ВремРезервы",	"");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И %УсловиеДокументРезерва%",				"");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//Док_УсловиеДокументРезерва",				"");
	Иначе
...
Процедура КонтрольОстатков_ЗаказВТабЧасти()
	...
	ЭтоБланки = (ИмяТаблицы = "СписаниеТоваров.БланкиСтрогогоУчета");
	...
	ИначеЕсли ИмяДокумента = "СписаниеТоваров"	Тогда
		...
		|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки(, Склад в (&СписокСкладов) И
		|		%ВыборкаПоНоменклатуре%"
		+ ?(ЭтоБланки, ") КАК ТоварыКПередачеПоДокументу", "
		...
		Если ЕстьСерия Тогда
			...
			|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки(, Склад в (&СписокСкладов) И
			|		%ВыборкаПоНоменклатуре%" 
			+ ?(ЭтоБланки, ") КАК ТоварыКПередачеПоДокументуБезСерии", "
	...
	Если ЕстьСерия Тогда
		Если ИспользоватьУказаниеСерийНоменклатурыПриРезервировании Тогда
			...
			Если ЭтоБланки Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//СОЕДИНЕНИЕ_Серия_ВремРезервы",			"И (ТЧ."+ТекстСерия+" = ВремРезервы.СерияНоменклатуры)");
			Иначе
	...
	Если ЕстьСпособСписания Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"//УСЛОВИЕ_Док_СпособСписанияНеРезерв",		"И Док.СпособСписанияОстаткаТоваров <> &ИзРезерва");
...
Процедура КонтрольОстатков_СкладВШапке_ЗаказВШапке()
	...
	КонецЕсли;
	Если ИмяДокумента = "ПередачаТоваров" Тогда
